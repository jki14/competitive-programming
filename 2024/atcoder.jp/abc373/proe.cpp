...
lld a[210000], w[210000];
pair<lld, int> b[210000];
int p[210000];
...
int main() {
  while (scanf("%d%d%lld", &n, &m, &s) != EOF) {
    ...
    // sort(b, b + n, greater<pair<lld, int>>());
    sort(b, b + n);
    w[0] = 0LL;
    for (int i = 0; i < n; ++i) {
      p[b[i].second] = i;
    ...
    for (int i = 0; i < n; ++i) {
      static auto check = [&](lld const d) -> bool {
        lld const bar = a[i] + d + 1LL;
        int x = m;

        static auto lower_bound = [&](int const i) -> bool {
          return b[i].first >= bar;
        };
        int f = joshu::binary_search(0, n - 1, lower_bound);
        if (~f) {
          x -= n - f;
        } else {
          f = n;
        ...
        static auto sufficient_ex = [&](int const i) -> bool {
          lld const tar = bar * (f - i);
          lld const sum = w[f] - w[i];
          return tar - sum <= s - d;
        };
        int const g = joshu::binary_search(0, f - 1, sufficient_ex);
        if (~g && p[i] < g) {
          x -= f - g;
          return x > 0;
        }

        static auto sufficient_in = [&](int const i) -> bool {
          lld const tar = bar * (f - i);
          lld const sum = w[f] - w[i];
          return tar - sum <= s + 1LL;
        };
        int const h = joshu::binary_search(0, f - 1, sufficient_in);
        if (~h && p[i] >= h) {
          x -= f - h - 1;
          return x > 0;
        }
      ...
      lld res = joshu::binary_search(0LL, s, check);
