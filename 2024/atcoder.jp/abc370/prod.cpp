...
using lld = long long;
using dim_t = std::vector<std::set<std::pair<int, int>>>;
...
queue<pair<dim_t*, pair<int, int>>> todrop;

int update(dim_t& r, int const i, int const j) {
  pair<int, int> block{j, j};

  auto const rit = r[i].lower_bound(make_pair(j, inf));
  auto const lit = rit != r[i].begin() ? std::prev(rit) : r[i].end();

  if (lit != r[i].end() && lit->first <= j && j <= lit->second) {
    return 0;
  }

  if (rit != r[i].end() && j + 1 == rit->first) {
    block.second = rit->second;
    r[i].erase(rit);
  }
  if (lit != r[i].end() && j - 1 == lit->second) {
    block.first = lit->first;
    r[i].erase(lit);
  }
...
void drop(dim_t const&d, int i, int j) {
  if (&d != &r) {
    swap(i, j);
  }
...
int explosion(dim_t& r, int const i, int const j) {
  auto rit = r[i].lower_bound(make_pair(j, -1));
  if (rit != r[i].begin() && (rit == r[i].end() || rit->first > j)) {
    --rit;
  }
  if (rit != r[i].end() && rit->first <= j && j <= rit->second) {
    // drop(r, i, rit->first - 1);
    todrop.emplace(&r, make_pair(i, rit->first - 1));
    // drop(r, i, rit->second + 1);
    todrop.emplace(&r, make_pair(i, rit->second + 1));
    return 1;
  } else {
    // drop(r, i, j);
    todrop.emplace(&r, make_pair(i, j));
    return 0;
  }
}
...
int main() {
  ...
  while (scanf("%d%d%d", &h, &w, &m) != EOF) {
    ...
    for (int k = 0; k < m; ++k) {
      int i, j;
      scanf("%d%d", &i, &j);
      // fprintf(stderr, "%d, %d: vvvv\n", i, j);
      explosion(r, i - 1, j - 1);
      explosion(c, j - 1, i - 1);
      for (; !todrop.empty(); todrop.pop()) {
        auto const& x = todrop.front();
        drop(*x.first, x.second.first, x.second.second);
      }
    }
    printf("%lld\n", ans);
  }
