...
set<pair<lld, int>> pmap;
map<lld, imod_t> psum;

int n;
lld k;
lld a[210000], s[210000];

imod_t f[210000], g[210000];
...
int main() {
  while (scanf("%d%lld", &n, &k) != EOF) {
    ...
    for (int i = 1; i <= n; ++i) {
      ...
      f[i] = g[i - 1] + f[i - 1];
      lld const d = s[i] - k;
      for (auto it = pmap.lower_bound(make_pair(d, -1)); it != pmap.end() && it->first == d && it->second <i; it = pmap.erase(it)) {
        auto jt = psum.emplace(d, 0).first;
        jt->second += f[it->second];
      }
      if (auto jt = psum.find(d); jt != psum.end()) {
        f[i] -= jt->second;
      }
    }
