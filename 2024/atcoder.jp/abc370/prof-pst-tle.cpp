...
class segmax_t : public joshu::btnctx_t<segmax_t> {
public:
  segmax_t() = default;

  segmax_t(segmax_t const&) = default;
  segmax_t(segmax_t&&) = default;

  segmax_t& operator=(segmax_t const&) = default;
  segmax_t& operator=(segmax_t&&) = default;

  void aggregate(segmax_t const& lhs, segmax_t const& rhs) override { max_ = max(lhs.calc(), rhs.calc()); }

  int calc() const { return max_ + modify_; }

  int modify_ = 0, max_ = 0;
};
