...
public:
  ...
  lld calc() const { return wht != inf ? (wht + wht + len - 1LL) * len >> 1 : sum; }

  lld wht = 0, sum = 0;
  int lef = 0, mid = 0, rig = 0, len = 0;
};

joshu::segtree_t<int, segsum_t> segtree(210000);
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    static auto const initializer = [&](joshu::segtree_t<int, segsum_t>::node_t& node) -> void {
      ...
      node.ctx.len = node.rig - node.lef + 1;
      if (node.lef == node.rig) {
        node.ctx.wht = x[node.lef - 1];
      } else {
        node.ctx.wht = inf;
      }
    };
    segtree.build(1, n, initializer);
