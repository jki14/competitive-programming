...
static inline void getmin(lld& x, lld const y) {
  if (x > y) x = y;
}

int n, m, req;
int a[210000], b[210000], q[210000];
int x[99000], y[99000], blk[99000];
lld w[99000];
lld c[330][330];
vector<lld> res;
...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    ...
    for (int i = 0; i < m; ++i) {
      if (blk[i]) continue;
      getmin(c[x[i]][y[i]], w[i]);
      getmin(c[y[i]][x[i]], w[i]);
    }

    for (int p = 0; p < n; ++p) {
      for (int u = 0; u < n; ++u) {
        for (int v = 0; v < n; ++v) {
          getmin(c[u][v], c[u][p] + c[p][v]);
        }
      }
    }
    ...
    for (int i = req - 1; i >= 0; --i) {
      if (q[i] == 1) {
        ...
        for (int u = 0; u < n; ++u) {
          for (int v = 0; v < n; ++v) {
            getmin(c[u][v], c[u][x[k]] + w[k] + c[y[k]][v]);
            getmin(c[u][v], c[u][y[k]] + w[k] + c[x[k]][v]);
          }
        }
      } else {
        res.push_back(c[a[i]][b[i]]);
      }
    }
