...
int nbs[210000], dst[410000], nxt[410000], eid;
lld wht[410000];

int prv[210000];
lld lhs[210000], rhs[210000];
...
inline void setmax(int& x, int const y) {
  if (x < y) x = y;
}
...
inline bool contains(int const u, int const v) {
  pii const foo(u, v);
  return crits.contains(foo);
}

inline bool erase(int const u, int const v) {
  pii const foo(min(u, v), max(u, v));
  return crits.erase(foo);
}

inline void add_edge(int const u, int const v, lld const w) {
  dst[eid] = v;
  wht[eid] = w;
  nxt[eid] = nbs[u];
  nbs[u] = eid++;
...
void resolve(int const src, lld* dist, int* prv = nullptr) {
  while (!pq.empty()) {
    pq.pop();
  }
  for (int i = 0; i < n; ++i) {
...
int main() {
  ...
  while(scanf("%d%d", &n, &m) != EOF) {
    ...
    seq.clear();
    seq.emplace_back(n - 1);
    crits.clear();
    for (int i = n - 1; i != 0; i = prv[i]) {
      int const u = min(i, prv[i]);
      int const v = max(i, prv[i]);
      seq.emplace_back(prv[i]);
      crits.emplace(u, v);
    }
    ...
    for (int i = 0; i < m; ++i) {
      if (contains(x[i], y[i])) continue;
      int const u = x[i];
      int const v = y[i];
      if (lhs[u] + w[i] + rhs[v] == lhs[n - 1] || lhs[v] + w[i] + rhs[u] == lhs[n - 1]) {
        f[find(v)] = find(u);
      }
    }
