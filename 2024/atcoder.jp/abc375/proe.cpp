...
static inline void getmin(int& x, int const y) {
  if (x > y) x = y;
}
    ...
    for (int i = 0; i <= n; ++i) {
      for (int j = 0; j <= m; ++j) {
        for (int k = 0; k <= m; ++k) {
          ...
          if (j + b[i] <= m) {
            getmin(f[i + 1][j + b[i]][k], f[i][j][k] + (a[i] != 1 ? 1: 0));
          }
          if (k + b[i] <= m) {
            getmin(f[i + 1][j][k + b[i]], f[i][j][k] + (a[i] != 2 ? 1: 0));
          }
          getmin(f[i + 1][j][k], f[i][j][k] + (a[i] != 3 ? 1: 0));
        }
      }
    }
