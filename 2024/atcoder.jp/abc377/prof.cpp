...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      row.emplace(a[i].x);
      col.emplace(a[i].y);
      add.emplace(a[i].x + a[i].y, 0);
      sub.emplace(a[i].x - a[i].y, 0);
    }
    ...
    bar -= col.size() * 1LL * n;
    // bar -= add.size() * n;
    for (auto const& ae : add) {
      int const a = ae.first;
      int const ll = max(a - n, 1);
      int const rr = min(a - 1, n);
      bar -= rr - ll + 1;
    }
    // bar -= sub.size() * n;
    for (auto const& se : sub) {
      int const s = se.first;
      int const ll = max(s + 1, 1);
      int const rr = min(s + n, n);
      bar -= rr - ll + 1;
    }

    // double
    bar += row.size() * col.size();
    m = 0;
    add.emplace(inf, 0);
    for (auto& e : add) {
      e.second = m++;
    ...
    for (int const r : row) {
      auto const lhs = add.upper_bound(r);
      auto const rhs = add.upper_bound(r + n);
      bar += rhs->second - lhs->second;
    }
    m = 0;
    sub.emplace(inf, 0);
    for (auto& e : sub) {
    ...
    for (int const r : row) {
      auto const lhs = sub.lower_bound(r - n);
      auto const rhs = sub.lower_bound(r);
      bar += rhs->second - lhs->second;
    }

    for (int const c : col) {
      auto const lhs = add.upper_bound(c);
      auto const rhs = add.upper_bound(c + n);
      bar += rhs->second - lhs->second;
    }
    for (int const c : col) {
      auto const lhs = sub.upper_bound(-c);
      auto const rhs = sub.upper_bound(n - c);
      bar += rhs->second - lhs->second;
    }

    for (auto const& ae : add) {
      lld const a = ae.first;
      if (a == inf) continue;
      for (auto const& se : sub) {
        lld const s = se.first;
        if (s == inf) continue;
        lld const r2 = a + s;
        if (r2 & 1) continue;
        lld const r = r2 >> 1;
