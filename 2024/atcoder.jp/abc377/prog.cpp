...
int get(node_t* node, char const* st, int const len) {
  if (node == root) {
    foo = len + node->wht;
  } else {
    setmin(foo, len + node->wht);
  }
  if (st[0]) {
    int const k = st[0] - 'a';
    if (node->chd[k]) {
      get(node->chd[k], st + 1, len - 1);
    }
  }
  return foo;
}

void put(node_t* node, char const* st, int const len) {
  setmin(node->wht, len);
  // fprintf(stderr, "%d\n", len);
  if (!st[0]) return;
  int const k = st[0] - 'a';
  // fprintf(stderr, "  [%d] = ", k);
  if (node->chd[k] == nullptr) {
    node->chd[k] = last++;
    clear(node->chd[k]);
  }
  put(node->chd[k], st + 1, len - 1);
}
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    for (int i = 0; i < n; ++i) {
      scanf("%s", st);
      int const bar = get(root, st, strlen(st));
      // fprintf(stderr, "[^] = ");
      put(root, st, strlen(st));
      printf("%d\n", bar);
    }
  }
  return 0;
