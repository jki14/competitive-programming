...
int n;
double p, q;
double px[8][2], py[8][2];
int ss[8];

inline double sqr(double const x) { return x * x; }

inline double dist(double const ux, double const uy, double const vx, double const vy) {
  return sqrt(sqr(vx - ux) + sqr(vy - uy));
}

int main() {
  while (scanf("%d%lf%lf", &n, &p, &q) != EOF) {
    for (int i = 0; i < n; ++i) {
      scanf("%lf%lf%lf%lf", &px[i][0], &py[i][0], &px[i][1], &py[i][1]);
      ss[i] = i;
    }
    double foo = numeric_limits<double>::max();
    do {
      for (int s = (1 << n) - 1; s >= 0; --s) {
        ...
        for (int i = 0; i < n; ++i) {
          ...
          int const u = (s >> i) & 1;
          int const v = u^1;
          bar += dist(x, y, px[k][u], py[k][u]) / p;
          bar += dist(px[k][u], py[k][u], px[k][v], py[k][v]) / q;
          x = px[k][v];
          y = py[k][v];
        }
        foo = min(bar, foo);
