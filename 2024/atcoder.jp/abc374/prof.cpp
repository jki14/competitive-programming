...
void update(int const i, lld const j, lld const w) {
  auto rig = f[i].lower_bound(j);
  while (rig != f[i].end() && rig->second >= w) {
    rig = f[i].erase(rig);
  }
  if (rig != f[i].begin()) {
    auto lef = prev(rig);
...
int main() {
  while(scanf("%d%d%lld", &n, &m, &d) != EOF) {
    ...
    for (int i = 1; i <= n; ++i) {
      for (int j = 1; j <= m; ++j) {
        if (j > i) break;
        auto it = f[i - j].lower_bound(a[i]);
        if (it != f[i - j].begin()) it = prev(it);
        for (; it != f[i - j].end(); ++it) {
          lld const day = max(it->first, a[i]);
          lld const cost = day * j - s[i] + s[i - j];
          update(i, day + d, it->second + cost);
        }
      }
    }

    auto const it = prev(f[n].end());
    printf("%lld\n", it->second);
  }
  return 0;
}
