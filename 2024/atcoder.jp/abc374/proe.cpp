...
lld constexpr inf = 0x3f3f3f3fLL;

int n;
lld m;
lld a[110], b[110], p[110], q[110];

// unordered_map<lld, lld> g[110];

/*
lld resolve(lld const y, int const k) {
  auto const it = g[k].find(y);
  if (it != g[k].end()) return it->second;
  stack<lld> s;
  s.push(y);
  while (!s.empty()) {
    lld const y = s.top();
    lld const prev_lhs = max(y - a[k], 0LL);
    auto const lit = g[k].find(prev_lhs);
    if (lit == g[k].end()) {
      s.push(prev_lhs);
      continue;
    }
    lld const prev_rhs = max(y - b[k], 0LL);
    auto const rit = g[k].find(prev_rhs);
    if (rit == g[k].end()) {
      s.push(prev_rhs);
      continue;
    }
    g[k].emplace(y, min(lit->second + p[k], rit->second + q[k]));
    s.pop();
  }
  return g[k][y];
}
...
int main() {
  auto const able = [&](lld const y_) -> bool {
    lld const y = -y_;
    lld x = m;
    for (int i = 0; i < n; ++i) {
      // x -= min(lhs, rhs);
      x -= resolve(y, i, a, b, p ,q);
      if (x < 0)
        break;
    }
  ...
  while (scanf("%d%lld", &n, &m) != EOF) {
    ...
    auto const res = -joshu::binary_search(-m * 100LL / n, 0LL, able);
    printf("%lld\n", res);
  }
  return 0;
}
