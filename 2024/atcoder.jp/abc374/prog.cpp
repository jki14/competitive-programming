...
int n;
int f[32], h[32], vst[1100];
int anc[1100];

int ids[32], low[32], id;

set<pii> nbs[32], prv[32];
unordered_set<int> lnk[1100];

vector<int> seq;

char cs[4];

using mybit_t = bitset<704>;
mybit_t b[1100];
...
inline void setmin(int& foo, int const bar) {
  foo = min(foo, bar);
}

int bar, flag;

void merge(int const u) {
  ids[u] = low[u] = id++;
  seq.emplace_back(u);
  vst[u] = 1;
  for (auto const& vk : nbs[u]) {
    int const v = vk.first;
    if (!~ids[v]) {
      merge(v);
      setmin(low[u], low[v]);
    } else if (~vst[v]) {
...
int resolve(int const u) {
  while (u < 0 || u >= n);
  for (int const v : lnk[u]) {
    while (v < 0 || v >= n);
    if (vst[v] != flag) {
      vst[v] = flag;
      if (!~anc[v] || resolve(anc[v])) {
        anc[v] = u;
        return 1;
      }
    }
...
mybit_t const& build(int const p) {
  if (~vst[p]) return b[p];
  vst[p] = 1;
  for (auto const& uk : prv[p]) {
    int const u = uk.first;
    int const k = uk.second;
    b[p].set(k);
    b[p] |= build(u);
  }
  for (int i = 0; i < n; ++i) {
    if (b[p].test(i)) {
      for (auto const& vk : nbs[p]) {
        int const k = vk.second;
        lnk[i].emplace(k);
      }
    }
  }
...
int main() {
  ...
  while(scanf("%d", &n) != EOF) {
    ...
    for (int i = 0; i < n; ++i) {
      scanf("%s", cs);
      size_t const u = cs[0] - 'A';
      size_t const v = cs[1] - 'A';
      if (u != v) {
        nbs[u].emplace(v, i);
        prv[v].emplace(u, i);
      }
    ...
    for (int i = 0; i < n; ++i) {
      int const u = find(x[i]);
      int const v = find(y[i]);
      if (u != v) {
        nbs[u].emplace(v, i);
        prv[v].emplace(u, i);
      } else {
    ...
    for (int i = 0; i < 26; ++i) {
      ...
      if (i != p) {
        h[i] = 0;
        h[p] = 1;
        for (auto const& vk : nbs[i]) {
          int const v = find(vk.first);
          if (v == p) continue;
          int const k = vk.second;
          nbs[p].emplace(v, k);
          for (auto it = prv[v].lower_bound(make_pair(i, -1)); it != prv[v].end() && it->first == i; ) {
            it = prv[v].erase(it);
          }
          prv[v].emplace(p, k);
        }
        for (auto const& uk : prv[i]) {
          int const u = find(uk.first);
          if(u == p) continue;
          int const k = uk.second;
          for (auto it = nbs[u].lower_bound(make_pair(i, -1)); it != nbs[u].end() && it->first == i; ) {
            it = nbs[u].erase(it);
          }
          nbs[u].emplace(p, k);
          prv[p].emplace(u, k);
        ...
        for (auto it = nbs[i].begin(); it != nbs[i].end(); ) {
          if (find(it->first) == i) {
            it = nbs[i].erase(it);
          } else {
            ++it;
        ...
        for (auto it = prv[i].begin(); it != prv[i].end(); ) {
          if (find(it->first) == i) {
            it = prv[i].erase(it);
          } else {
            ++it;
    ...
    */
    ...
    bar = 0;
    for (int i = 0; i < 26; ++i) {
      if (h[i] && prv[i].empty() && nbs[i].empty()) {
        ++bar;
      }
    }
    ...
    for (int i = 0; i < 26; ++i) {
      bar += nbs[i].size();
      for (auto const& lhs : prv[i]) {
        for (auto const& rhs : nbs[i]) {
          while (lhs.second < 0 || lhs.second >= n);
          while (rhs.second < 0 || rhs.second >= n);
          // fprintf(stderr, "%d => %d\n", lhs.second, rhs.second);
          lnk[lhs.second].emplace(rhs.second);
        }
      }
    }
