...
void essential(int const i, int const j) {
  printf("? %d %d\n", i, j);
  fflush(stdout);
  static int d;
  scanf("%d", &d);
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    for (int k = 1; k <= 8; k <<= 1) {
      for (int i = 1; i <= n; ++i) {
        ...
        if (s[i] != k)
          continue;
        int j = i + 1;
        while (j <= n && (find(j) != j || s[j] != k))
          ++j;
        if (j <= n && find(j) == j && s[j] == k) {
          essential(i, j);
          i = j;
        } else {
          break;
    ...
    while (true) {
      ...
      if (distance(pq.begin(), pq.upper_bound(make_pair(m + 1, -inf))) <= 1)
        break;
      int const lhs = pq.begin()->second;
      int rhs = 0;
      for (auto it = next(pq.begin()); ; ++it) {
        if (find(g[lhs]) != it->second) {
          rhs = it->second;
          break;
        }
      }
    ...
    int const k = find(pq.begin()->second);
    printf("!");
    for (int i = 1; i <= n; ++i) {
      if (find(i) == k)
        printf(" %d", i);
