...
template <typename TX, typename TY> struct tuple2d_t {
  ...
  /*
  bool operator<(tuple2d_t const& rhs) const {
    int const l = static_cast<int>(x > rhs.x) + static_cast<int>(x > rhs.y) + static_cast<int>(y > rhs.x) + static_cast<int>(y > rhs.y);
    int const r = static_cast<int>(rhs.x > x) + static_cast<int>(rhs.x > y) + static_cast<int>(rhs.y > x) + static_cast<int>(rhs.y > y);
    return l < r;
  }
  */
...
using pii = tuple3d_t<int, int, int>;
