...
lld get(int const u) {
  auto it = dist.lower_bound(u);
  if (it != dist.end()) {
    // fprintf(stderr, "get %d = %lld\n", u, it->second);
    return it->second;
  }
  // fprintf(stderr, "get %d = inf\n", u);
  return inf;
}

void put(int const u, lld const w) {
  // fprintf(stderr, "put %d -> %lld\n", u, w);
  auto it = dist.lower_bound(u);
  if (it != dist.end() && it->second <= w) {
    return;
  }
  if (it != dist.begin()) {
    for (auto jt = prev(it); ; jt = prev(jt)) {
      int flag = jt == dist.begin();
      if (jt->second > w) {
        jt = dist.erase(jt);
      } else {
        break;
  ...
  it = dist.emplace(u, w).first;
  it->second = w;
  {
    int const s = it != dist.begin() ? prev(it)->first + 1 : 0;
    for (int i = s; i <= u; ++i) {
      // fprintf(stderr, "  pq emplace %d -> %lld\n", b[i], w + a[i]);
      pq.emplace(-w - a[i], b[i]);
    }
  }
}
