...
void prepare(const int n){
	...
	for(int i=1; i<=n; i++){
		c[i][0]=1;
		for(int j=0; j<=i; j++)c[i][j]=MOD_ADD(c[i-1][j-1], c[i-1][j]);
	}
}

int dp_entry(const int n, const int m){
	if(v[n])return f[n];
	if(n==1){
		f[n]=m;
	}else{
		...
		for(int i=0; i<n; i++){
			if(i==0 || i==n-1){
				MOD_ADD(&f[n], MOD_MUL(dp_entry(n-1, m), m-1));
			}else{
				MOD_ADD(&f[n], MOD_MUL(MOD_MUL(MOD_MUL(dp_entry(i, m), dp_entry(n-1-i, m)), m-2), c[n-1][i]));
			}
		}
	}
	v[n]=1;
