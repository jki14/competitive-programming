...
class BallsSeparating {
	public:
	int f[N][S],a[N][S],col,n;
	int minOperations(vector <int> red, vector <int> green, vector <int> blue) {
		n=red.size();
		for(int i=0;i<n;i++){ a[i][0]=red[i]; a[i][1]=green[i]; a[i][2]=blue[i]; }
		memset(f,0x3f,sizeof(f));f[0][0]=col=0;int inf=f[0][1];
		for(int i=0;i<n;i++){
			for(int j=0;j<S;j++){
				...
				for(int x=0;x<C;x++)
					if(a[i][x])col|=_ni(x);
				for(int k=0;k<C;k++){
					int nexi=i+1,nexj=j|_ni(k);
					_updmin(f[nexi][nexj],f[i][j]+a[i][0]+a[i][1]+a[i][2]-a[i][k]);
				}
			}
		}
		return f[n][col];
