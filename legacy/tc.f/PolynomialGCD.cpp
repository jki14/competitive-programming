...
class PolynomialGCD {
	...
	void get_min(const int k, const int fund, const int p, int buff){
		for(int i=p;i<n;i+=fund)buff+=a[i];
		int next=fund*k;
		if(next-fund+p>=n){
			UPDMIN(powa,buff);
			return;
		}
		for(int i=p;i<=next-fund+p;i+=fund)
			get_min(k,next,i,buff);
	}
	public:
	int gcd(string s){
		n=s.length();
		for(int i=0;i<n;i++)a[i]=s[n-i-1]-'0';
		for(int i=0;i<=n;i++)is_prm[i]=(i>=2)?1:0;
		for(int i=4;i<=n;i+=2)is_prm[i]=0;
		int e=sqrt(n*1.0)+1.0;
		ans=1;
		for(int k=2;k<=n;k++){
			if(!is_prm[k])continue;
			if(k<e){
				int d=k<<1;
				for(int i=k*k;i<=n;i+=d)is_prm[i]=0;
			}
			powa=inf;
			for(int i=0;i<k;i++){
				//int buff=0;
				//for(int p=i;p<n;p+=k)buff+=a[p]*attr(k+p-i,k);
				//UPDMIN(powa,buff);
				get_min(k,k,i,0);
			}
			//printf("k=%d powa=%d mul=%d\n",k,powa,MOD_POW(k,powa));
			MOD_MUL(&ans,MOD_POW(k,powa));
		}
		return MOD_STD(ans);
	}
...
// BEGIN CUT HERE
	...
	void test_case_2() { string Arg0 = "241"; int Arg1 = 48; verify_case(2, Arg1, gcd(Arg0)); }
