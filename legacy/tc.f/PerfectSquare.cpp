	...
	int ways(vector <int> x){
		...
		for(int p=0; p<m; p++){
			for(int i=2; i*i<=x[p]; i++){
				...
				if(cnt){
					map<int, int>::iterator it=dic.find(i);
					if(it!=dic.end()){
						mat[it->second].set(p);
					}else{
						mat.push_back(bitset<M>());
			...
			if(x[p]>1){
				map<int, int>::iterator it=dic.find(x[p]);
				if(it!=dic.end()){
					mat[it->second].set(p);
				}else{
					mat.push_back(bitset<M>());
					mat[til].set(p);
					res.push_back(0);
					dic.insert(MP(x[p], til++));
				}
			}
		}
		/*for(int i=0; i<til; i++){
			for(int j=0; j<m; j++)printf("%c", mat[i][j]?'1':'0');
			printf(" %d\n", res[i]);
		}
		printf("#\n");*/
		for(int r=0, c=0; r<til && c<m; r++, c++){
			for(int i=r+1; !mat[r].test(c) && i<til; i++){
				if(mat[i].test(c)){
					swap(mat[r], mat[i]);
					swap(res[r], res[i]);
					break;
		...
		for(int i=0; i<til; i++){
			if(mat[i].none() && res[i]==1)return 0;
		}
		ans=m;
		for(int i=0; i<til && i<m; i++)
			if(mat[i].any())ans--;
		//printf("ans=%d\n", ans);
