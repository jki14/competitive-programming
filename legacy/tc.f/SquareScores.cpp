...
class SquareScores {
	...
	double calcexpectation(vector <int> P, string S){
		...
		for(int i=0; i<m; i++)p[i]=P[i]*0.01;
		memset(f, 0, sizeof(f));
		memset(g, 0, sizeof(g));
		g[0][m]=1.0;
		for(int i=0; i<n; i++){
			if(s[i]=='?'){
				for(int j=0; j<m; j++){
					for(int k=0; k<=m; k++){
						if(g[i][k]<eps)continue;
						f[i+1][j]+=f[i][k]*p[j];
						g[i+1][j]+=(1.0*((i&&s[i-1]=='?')?p[k]:1.0)+(k==j?g[i][k]:0.0))*p[j];
					}
					f[i+1][j]+=g[i+1][j];
					//printf("f[%d][%c]=%.6f g[%d][%c]=%.6f\n", i+1, 'a'+j, f[i+1][j], i+1, 'a'+j, g[i+1][j]);
				}
			}else{
				int j=s[i]-'a';
				for(int k=0; k<=m; k++){
					if(g[i][k]<eps)continue;
					f[i+1][j]+=f[i][k];
					g[i+1][j]+=1.0*((i&&s[i-1]=='?')?p[k]:1.0)+(k==j?g[i][k]:0.0);
				}
				f[i+1][j]+=g[i+1][j];
				//printf("f[%d][%c]=%.6f g[%d][%c]=%.6f\n", i+1, 'a'+j, f[i+1][j], i+1, 'a'+j, g[i+1][j]);
			}
