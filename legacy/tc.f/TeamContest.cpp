...
class TeamContest {
	...
	int worstRank(vector <int> s) {
		int top,bot,scr,ret=1;top=-inf;bot=inf;
		for(int i=0;i<3;i++){ _updmax(top,s[i]);_updmin(bot,s[i]); }
		scr=top+bot;int n=s.size();for(int i=3;i<n;i++)a[n-3]=s[i]; n-=3;
		sort(a,a+n,greater<int>() );
		int head,tail;head=0;tail=n-1;
		for(int i=0;i*3<n;i++){
			while(head<tail && a[head]==0)head++;
			while(head<tail && a[tail]==0)tail--;
			if(head==tail)break;
			top=a[head];a[head]=0;
			while(head<tail && (a[tail]==0 || top+a[tail]<=scr))tail--;
			bot=a[tail];a[tail]=0;
			while(head<tail && a[tail]==0)tail--;
			if(head==tail)break;a[tail]=0;
			if(top+bot>scr)ret++;
		}
		return ret;
	}
