...
class EllysReversals {
	public:
	/*int sm(const int &x,const int &y,const int &u,const int &v){
		for(int i=u;i<=v;i++)if(a[x][i]!=a[y][i])return 0;
		return 1;
	}
	int re(const int &x,const int &y,const int &u,const int &v){
		if((u&1)!=0 || (v&1)!=1)return 0;
		for(int i=u;i<=v;i++)if(a[x][i]!=a[y][v-i+u])return 0;
		return 1;
	}*/
	int able(const int &x,const int &y){
		if(a[x].size()!=a[y].size())return 0;
		int len=a[x].size();if(len&1){
		       if(a[x][len-1]!=a[y][len-1])return 0;
		       len--;
		}
		memset(f,0,sizeof(f));
		for(int i=0;i<len;i+=2){
			int u=a[x][i]-'a',v=a[x][i+1]-'a';
			if(u>v)swap(u,v);f[u][v]++;
		}
		for(int i=0;i<len;i+=2){
			int u=a[y][i]-'a',v=a[y][i+1]-'a';
			if(u>v)swap(u,v);f[u][v]--;
		}
		for(int i=0;i<C;i++)
			for(int j=0;j<C;j++)
		...
		for(int i=0;i<len;i++){
			for(int j=i;j<len;j++){
				if(sm(x,y,i,j) || re(x,y,i,j))g[i+1][j+1]=1;
			}
		}
		for(int i=1;i<=len;i++){
			for(int k=0;k<i;k++)if(f[k] && g[k+1][i]){ f[i]=1;break; }
		}
		return f[len];*/
	}
	int getMin(vector <string> _a) { a=_a;
		n=ans=a.size();for(int i=0;i<n;i++){
			if(a[i].size()==0)continue;
			for(int j=i+1;j<n;j++){
				if(a[j].size()==0)continue;
				if(able(i,j)){ ans-=2;a[j].clear();break; }
			}
		}
		return ans;
	}
