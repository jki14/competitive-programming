...
void core(){
	int head,tail=n;
	lld value,lT,lF,rT,rF;
	value=lT=lF=rT=rF=0LL;
	for(int i=n*2-1;i>=n;i--){
		if(a[i]){
			rT++;value+=rF;
		}else rF++;
	}
	//printf("value=%lld rT=%lld rF=%lld\n",value,rT,rF);
	for(head=n;head<=n*2;head++){
		while((head>tail) || ((tail-head<n) && ((!a[tail]) || lF-rF<=0))){
			if(a[tail]){
				value+=lF-rF;
				rT--;lT++;
			}else{ rF--;lF++; }
			tail++;
			//printf("head=%d tail=%d: value=%lld, lT=%lld rT=%lld lF=%lld rF=%lld\n",head-n,tail-n,value,lT,rT,lF,rF);
		}
		_updmin(ans,value);
		if(a[head]){
			lT--;rT++;
		...
		//printf("head=%d tail=%d: value=%lld, lT=%lld rT=%lld lF=%lld rF=%lld\n",head-n+1,tail-n,value,lT,rT,lF,rF);
	}
}
...
class LeftAndRightHandedDiv1{
	...
	long long countSwaps(string Y, int A, int B, int C, int D, int N){
		T[0]=A;for(int i=1;i<N;i++)T[i]=(int)((((lld)T[i-1])*((lld)B)+((lld)C))%((lld)D));
		n=N;m=Y.length();ans=inf;
		for(int i=0;i<n;i++){
			a[i]=(Y[T[i]%m]=='L')?1:0;
			//printf("%d",a[i]);
		}
