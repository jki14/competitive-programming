...
#define N 20
int a[N][N],q[N][N][N][N],f[N][N][N][N],n,m;
...
class EllysFigurines {
	...
	int getMoves(vector <string> board, int R, int C) {
		n=board.size();m=board[0].size();for(int i=0;i<n;i++){
			for(int j=0;j<m;j++){
				a[i][j]=board[i][j]=='X'?1:0;
			}
		}
		for(int i=0;i<n;i++)
			for(int j=0;j<m;j++)
				for(int x=i;x<n;x++)
					for(int y=j;y<m;y++){
						if(i==x && j==y)
							...
							q[i][j][x][y]=q[i][j][x][y-1]|a[x][y];
						else if(j==y)
							q[i][j][x][y]=q[i][j][x-1][y]|a[x][y];
						else q[i][j][x][y]=q[i][j][x][y-1]|q[i][j][x-1][y]|a[x][y];
						if(!q[i][j][x][y]){ f[i][j][x][y]=0;continue; }
						if(x-i+1<=R || y-j+1<=C)
							f[i][j][x][y]=1;
						else f[i][j][x][y]=inf;
					}
			...
			for(int y=1;y<m;y++)
				for(int i=0;i+x<n;i++)
					for(int j=0;j+y<m;j++){
						int ii=i+x,jj=j+y;
						for(int k=i;k<ii;k++)
							_updmin(f[i][j][ii][jj],f[i][j][k][jj]+f[k+1][j][ii][jj]);
						for(int k=j;k<jj;k++)
							_updmin(f[i][j][ii][jj],f[i][j][ii][k]+f[i][k+1][ii][jj]);
					}
