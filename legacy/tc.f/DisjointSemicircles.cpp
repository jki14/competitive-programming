...
You are given a vector <int> labels consisting of 2N elements. 
...
Returns:string
Method signature:string getPossibility(vector <int> labels)
...
5)
{ 4, -1, 2, 4, -1, 3, 3, 12, 2, 5, -1, 0, 9, 9, 8, -1, 12, 8, -1, 6, 0, -1, -1, -1, 5, 6, 10, -1, -1, 10 }
...
// END CUT HERE
#line 96 "DisjointSemicircles.cpp"
...
class DisjointSemicircles {
	...
	int n,s[N],r[N];
	int top[N],bot[N],tp,bp;
	int sta[N],h;
	int ntv[N],nbv[N];

	int linkable(const int &des){
		if(!ntv[des] && (!tp || des>top[tp]))return 1;
		if(!nbv[des] && (!bp || des>bot[bp]))return 1;
		return 0;
	}

	string getPossibility(vector <int> a ) {
		string ans0="IMPOSSIBLE",ans1="POSSIBLE";
		memset(s,0xff,sizeof(s));
		memset(ntv,0,sizeof(ntv));
		memset(nbv,0,sizeof(nbv));
		n=a.size();for(int i=0;i<n;i++){
			if(a[i]==-1)continue;
			if(s[a[i]]<0)
				s[a[i]]=i;
			else r[s[a[i]]]=i;
		...
		for(int i=0;i<n;i++){
			if(a[i]==-1){
				...
				if(tp && top[tp]==s[a[i]]){
					for(int j=h;j && sta[j]>top[tp];j--)ntv[sta[j]]=1;
					tp--;
				}else if(bp && bot[bp]==s[a[i]]){
					for(int j=h;j && sta[j]>bot[bp];j--)nbv[sta[j]]=1;
					bp--;
				}else if(!tp || (r[top[tp]]>r[i]))
					top[++tp]=i;
				else if(!bp || (r[bot[bp]]>r[i]))
					bot[++bp]=i;
				else return ans0;
			}
		}

		char result[110];sprintf(result,"%d",h);
		string res=result;return res;
