#line 81 "LittleElephantAndIntervalsDiv1.cpp"
...
int check(const int &x){
	for(int i=u[x];i<=v[x];i++)if(a[i])return 1;
	return 0;
}
...
class LittleElephantAndIntervalsDiv1 {
	...
	long long getNumber(int M, vector <int> L, vector <int> R) {
		n=_size(L);for(int i=0;i<n;i++){ u[i]=L[i];v[i]=R[i]; }
		memset(a,0,sizeof(a));ans=0;
		for(int i=0;i<n;i++){
			for(int k=u[i];k<=v[i];k++)a[k]=1;
			for(int j=i+1;j<n;j++){
				for(int k=u[j];k<=v[j];k++)a[k]=0;
			}
			if(check(i))ans++;
		}
		return 1LL<<ans;
