...
int nbs[V],dst[E],nxt[E],num,n;
int heap[V],len,ps[V],mk[V];
...
class SkiResorts {
	...
	void dijkstra(const int &src){
		//for(int i=0;i<n;i++){ d[i]=inf;mk[i]=ps[i]=0; }
		for(int i=0;i<V;i++)d[i]=inf;
		memset(mk,0,sizeof(mk));
		memset(ps,0,sizeof(ps));
		d[src]=0LL;heap[len=1]=src;ps[src]=1;
		for(int vq=0;vq<n;vq++){
			int v=vq;lld w=abs(h[vq]-h[0]);
			if(!mk[v] && d[0]+w<d[v]){
		...
		while(true){
			...
			int u=getmin();mk[u]=1;
			int up=u/n,uq=u%n;
			for(int i=nbs[up];i;i=nxt[i]){int vp=dst[i];
				for(int vq=0;vq<n;vq++){
					if(h[vq]>h[uq])continue;
					int v=vp*n+vq;lld w=abs(h[vq]-h[vp]);
					if(!mk[v] && d[u]+w<d[v]){
						if(!ps[v]){ heap[++len]=v;ps[v]=len; }
						d[v]=d[u]+w;update(v);					
						if(vp==n-1)_updmin(ans,d[v]);
					}
				}
			}
		}
	}
	long long minCost(vector <string> themap, vector <int> high) {ans=inf;
		n=high.size();for(int i=0;i<n;i++)h[i]=(lld)high[i];
		memset(nbs,0,sizeof(nbs));num=0;
		for(int u=0;u<n;u++)for(int v=0;v<n;v++)if(themap[u][v]=='Y'){
			/*for(int i=0;i<n;i++)
				for(int j=0;j<n;j++)
					if(h[i]>=h[j])addEdge(u*K+i,v*K+j,abs(h[j]-h[v]));*/
			addEdge(u,v);
		}
		/*for(int i=0;i<n;i++)addEdge(S,i,abs(h[i]-h[0]));
		for(int j=0;j<n;j++)addEdge((n-1)*K+j,T,0LL);*/
		//addEdge(0,0);
		dijkstra(0);
		if(ans==inf)ans=-1LL;
		return ans;
