...
struct state_t{
	int x, y;
	state_t(const int lhs=0, const int rhs=0){
		this->x=lhs;
		this->y=rhs;
	}
...
class GCDLCM2 {
	...
	int getMaximalSum(vector <int> start, vector <int> d, vector <int> cnt){
		m=SIZE(start);n=ans=0;tab.clear();
		for(int i=0; i<m; i++){
			for(int j=0; j<cnt[i]; j++){
				...
				while(x>1){
					...
					tab.push_back(state_t(p,q));
				}
			}
		}
		sort(tab.begin(), tab.end());
		int pot=1, cur=0;
		for(vector<state_t>::const_iterator it=tab.begin(); it!=tab.end(); it++){
			if(it->x!=pot){
				pot=it->x;
				cur=0;
			}
			MOD_MUL(&buf[cur++], MOD_POW(pot, it->y));
		}
		for(int i=0; i<n; i++){
			MOD_ADD(&ans, buf[i]);
		}
