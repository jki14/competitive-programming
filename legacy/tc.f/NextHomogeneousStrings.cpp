// BEGIN CUT HERE
// PROBLEM STATEMENT
// A string is homogeneous if every contiguous substring of length n contains at most d different characters.  For a string seed and a long long k, the k-th homogeneous string is defined as the k-th element (0-indexed) in the list of all homogeneous strings which have the same length as seed and are lexicographically greater than or equal to seed.  Only strings containing all lowercase letters ('a'-'z') should be considered.  Return the k-th homogeneous string.  If there are less than k+1 strings in this list, return "" instead (quotes for clarity).

DEFINITION
Class:NextHomogeneousStrings
Method:getNext
Parameters:int, int, string, long long
Returns:string
Method signature:string getNext(int d, int n, string seed, long long k)
...
// END CUT HERE
#line 81 "NextHomogeneousStrings.cpp"
