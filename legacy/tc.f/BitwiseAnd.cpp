...
int core(lld &x,int k){

	//printf("x=%lld k=%d\n",x,k);

	if(k==_size(a)){
		for(int i=0;i<S && k<m-1;i++){
			if(c[i])continue;
			x|=bit[i];c[i]++;k++;
		}
		return (k<n-1)?0:1;
...
class BitwiseAnd {
	public:
	vector<long long> lexSmallest(vector<long long> _a, int _m) {
		a=_a;n=_size(a);m=_m;ret.clear();
		for(int i=0;i<S;i++)bit[i]=_niL(i);
		memset(c,0,sizeof(c));
		for(int i=0;i<n;i++){;
			for(int j=0;j<S;j++){
				if(a[i]&bit[j]){
...
// BEGIN CUT HERE
	...
	/*void test_case_3() { long long Arr0[] = {1152921504606846975}; vector<long long> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 3; long long Arr2[] = { }; vector<long long> Arg2(Arr2, Arr2 + (sizeof(Arr2) / sizeof(Arr2[0]))); verify_case(3, Arg2, lexSmallest(Arg0, Arg1)); }*/
