...
class TriangleTriples {
	...
	int count(int A, int B, int C){
		MATX<int> *matx=new MATX<int>(4);
		MATX<int> *maty=new MATX<int>(4);
		a[0]=A;a[1]=B;a[2]=C;
		sort(a, a+3);
		ans=0;
		for(int p=2; ~p; p--){
			...
			(*matx)(0, 3)=1;
			nxt=(a[0]+a[1]+2)/2-1;
			UPDMIN(nxt, MIN(a[0], a[1]));
			UPDMIN(nxt, a[2]);
			*matx=matx->mul(maty->pow(nxt-cur, MOD), MOD);
			cur=nxt;
			//printf("point a: %d\n", cur);
			//matx->show();
			...
			(*matx)(0, 3)=0;
			nxt=(a[0]+a[1]+3)/2-1;
			UPDMAX(nxt, a[0]+a[1]-MIN(a[0], a[1]));
			UPDMIN(nxt, a[2]);
			*matx=matx->mul(maty->pow(nxt-cur, MOD), MOD);
			cur=nxt;
			//printf("point b: %d\n", cur);
			...
			*matx=matx->mul(maty->pow(nxt-cur, MOD), MOD);
			...
			//printf("+%d, %d, %d, %d\n", a[0], a[1], a[2], MOD_STD(matx->at(0, 0)));
			ans=MOD_ADD(ans, matx->at(0, 0));
		}
		delete matx;
		delete maty;
		ans=MOD_ADD(MOD_MUL(MOD_MUL(A, B), C), -ans);
		return MOD_STD(ans);
	}
