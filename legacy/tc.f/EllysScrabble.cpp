...
struct El{
	int x,p;
	El(){}
	El(const int &_x, const int &_p){ x=_x;p=_p; }
	bool operator < (const El &_op) const{
		if(x != _op.x)return x<_op.x;
		return p<_op.p;
	}
};
...
class EllysScrabble{
	public:
	string getMin(string letters, int maxDistance){ans.clear();
		n=letters.length();m=maxDistance;
		for(int i=0;i<n;i++)a[i]=letters[i];

		s.clear();for(int i=0;i<m && i<n;i++)s.insert(El((int)(a[i]-'A'),i));
		for(int i=0;i<n;i++){
			if(i+m<n)s.insert(El((int)(a[i+m]-'A'),i+m));
			int flag=0;for(st=s.begin();st!=s.end();st++){
				if((*st).p==i-m){
					flag=1;
					ans.push_back('A'+(*st).x);
					s.erase(st);
					break;
				}
			}
