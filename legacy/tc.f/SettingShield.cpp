...
static const long long inf=1000000000000000003LL;
...
int n, h, t;
int pro[N], tmp[N];
int lef[N], rig[N], idx[N], til[N];

lld calc(const int sp){
	for(int i=0; i<n; i++)tmp[i]=MAX(0, pro[i]-sp);
	lld res=sp*1LL*t;
	lld buf=0LL;
	deque<pii> dq;
	...
	for(int i=0; i<n; i++){
		while(!dq.empty() && dq.front().first<i){
			buf-=dq.front().second*1LL;
			dq.pop_front();
		}
		int cur=MAX(0LL, tmp[i]*1LL-buf);
		if(!cur)continue;
		if(til[i]<i){
			//printf("sp=%d cur=%d at til[%d]=%d\n", sp, cur, i, til[i]);
			return inf;
		}
		//while(!dq.empty() && dq.back().second<=cur)dq.pop_back();
		dq.push_back(MP(til[i], cur));
		buf+=cur*1LL;
		res+=cur*1LL;
	}
	return res;
...
lld core_entry(){
	int low=0, hig=0;
	for(int i=0; i<n; i++)UPDMAX(hig, pro[i]);
	while(low<=hig){
		int mid=(low+hig)>>1;
		lld cur=calc(mid);
		if(cur!=inf && cur<=calc(mid+1)){
			if(mid==low || cur<=calc(mid-1)){
				printf("mid=%d\n", mid);
				return cur;
			}
			hig=mid-1;
...
void tail(){
	...
	int cur=-1, j=0;
	for(int i=0; i<n; i++){
		while(j<h && lef[idx[j]]<=i){
			UPDMAX(cur, rig[idx[j]]);
			j++;
		}
		til[i]=cur;
	}
...
class SettingShield {
	...
	long long getOptimalCost(int _n, int _h, int _t, vector <int> val0, vector <int> a, vector <int> b, vector <int> m){
		...
		//printf("pro[%d]=%d\n", 0, pro[0]);
		for(int i=1; i<n; i++){
			pro[i]=(int)((a[0]*1LL*pro[i-1]+b[0]*1LL)%(m[0]*1LL));
			//printf("pro[%d]=%d\n", i, pro[i]);
		}
		lef[0]=val0[1];
		rig[0]=val0[2];
		//printf("%d: %d->%d\n", 0, lef[0], rig[0]);
		for(int i=1; i<h; i++){
			lef[i]=MIN(n-1, (int)((a[1]*1LL*lef[i-1]+b[1]*1LL)%(m[1]*1LL)));
			int dist=rig[i-1]-lef[i-1];
			//printf("dist=%d\n", dist);
			rig[i]=MIN(n-1, lef[i]+(int)((a[2]*1LL*dist+b[2]*1LL)%(m[2]*1LL)));
			//printf("%d: %d->%d\n", i, lef[i], rig[i]);
		}
		tail();
		return core_entry();
...
// BEGIN CUT HERE
	...
	void test_case_6() { int Arg0 = 100000; int Arg1 = 99901; int Arg2 = 900; int Arr3[] = {712, 0, 99}; vector <int> Arg3(Arr3, Arr3 + (sizeof(Arr3) / sizeof(Arr3[0]))); int Arr4[] = {37, 1, 0}; vector <int> Arg4(Arr4, Arr4 + (sizeof(Arr4) / sizeof(Arr4[0]))); int Arr5[] = {13, 1, 99}; vector <int> Arg5(Arr5, Arr5 + (sizeof(Arr5) / sizeof(Arr5[0]))); int Arr6[] = {1000, 1000000, 1000000}; vector <int> Arg6(Arr6, Arr6 + (sizeof(Arr6) / sizeof(Arr6[0]))); long long Arg7 = 848460LL; verify_case(6, Arg7, getOptimalCost(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5, Arg6)); }
