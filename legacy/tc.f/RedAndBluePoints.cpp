...
class pnt_t:public point_t<int>{
	public:
	int idx;
};
...
static int compare_pas(const void *x, const void *y, void *z){
	const pnt_t *p1 = (pnt_t*)x;
	const pnt_t *p2 = (pnt_t*)y;
	const pnt_t *p0 = (pnt_t*)z;
	int sgn = fsign(((*p1)-(*p0))^((*p2)-(*p0)));
	if(sgn!=0)return -sgn;
	return fsign(p0->dist2(*p1)-p0->dist2(*p2));
}
void polar_angle_sort(pnt_t *pnts, const int n){
	int p=0;
	for(int i=1; i<n; i++){
		if(pnts[p]<pnts[i])p=i;
	}
	swap(pnts[0], pnts[p]);
	qsort_r(pnts+1, n-1, sizeof(pnt_t), compare_pas, pnts);
}

int intri(const pnt_t &p, const pnt_t &x, const pnt_t &y, const pnt_t &z){
	int flag=fsign(x.cross(y, p));
	if(!flag)return 0;
	if(fsign(y.cross(z, p))!=flag)return 0;
	if(fsign(z.cross(x, p))!=flag)return 0;
	return 1;
}
...
class RedAndBluePoints {
	...
	int find(vector <int> blueX, vector <int> blueY, vector <int> redX, vector <int> redY){
		...
		for(int x=0; x<n; x++){
			for(int y=0; y<n; y++){
				for(int z=0; z<n; z++){
					cnt[x][y][z]=0;
					for(int i=0; i<n; i++){
						if(intri(a[i], a[x], a[y], a[z]))
							cnt[x][y][z]++;
					}
					for(int i=0; i<m; i++){
						if(intri(b[i], a[x], a[y], a[z])){
							cnt[x][y][z]=-inf;
							break;
						}
					}
		...
		for(int k=0; k<n; k++){
			polar_angle_sort(a+k, n-k);
			UPDMAX(buf[k][k][k], 1);
			for(int i=k; i<n; i++){
				for(int j=(i==k)?i:(i+1); j<n; j++){
					if(buf[k][i][j]<0)continue;
					for(int p=j+1; p<n; p++){
						if(i!=j && fsign(a[i].cross(a[j], a[p]))<=0)continue;
						if(cnt[a[k].idx][a[j].idx][a[p].idx]<0)continue;
						UPDMAX(buf[k][j][p], buf[k][i][j]+cnt[a[k].idx][a[j].idx][a[p].idx]+1);
					}
				}
			}
		}
