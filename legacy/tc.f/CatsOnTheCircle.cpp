...
class CatsOnTheCircle {
	private:
	double gamblers_ruin(const int n1, const int n2, const double p){
		double q=1.0-p;
		if(fabs(p-q)<eps){
			return n2*1.0/(n1+n2);
		}
		double buf=(1.0-pow(p/q, n2))/(1.0-pow(p/q, n1+n2));
		if(std::isnan(buf)){
			buf=1.0/pow(p/q, n1);
		}
		return buf;
	...
	double getProb(int N, int K, int P){
		double p=P*1.0/1e9;
		double q=1.0-p;
		return gamblers_ruin(N-K-1, K-1, p)*gamblers_ruin(N-2, 1, q)+gamblers_ruin(K-1, N-K-1, q)*gamblers_ruin(N-2, 1, p);
	}
