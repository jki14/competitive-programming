...
class DiameterOfRandomTree {
	private:
	int dp_entry(const int u, const int p){
		map<pii, double> g[B];
		int cur=0;
		g[cur].clear();
		g[cur].insert(MP(MP(0, 0), 1.0));
		for(int i=nbs[u]; i; i=nxt[i]){
			...
			cur=pre;
			g[cur].clear();
			for(map<pii, double>::iterator st=g[pre].begin(); st!=g[pre].end(); st++){
				for(map<pii, double>::iterator it=f[v].begin(); it!=f[v].end(); it++){
					double value=st->second*it->second*0.5;
					for(int len=it->first.second+1; len<=it->first.second+2; len++){
						pii status=st->first;
						UPDMAX(status.first, it->first.first);
						UPDMAX(status.first, status.second+len);
						UPDMAX(status.second, len);
						map<pii, double>::iterator pt=g[cur].find(status);
						if(pt!=g[cur].end()){
							pt->second+=value;
						}else g[cur].insert(MP(status, value));
					}
				}
			}
		}
		f[u]=g[cur];
		for(map<pii, double>::iterator it=f[u].begin(); it!=f[u].end(); it++){
			printf("f[%d][%d][%d]=%.6f\n", u, it->first.first, it->first.second, it->second);
		}
		return 0;
	}
	public:
	double getExpectation(vector <int> a, vector <int> b){
		...
		for(map<pii, double>::iterator it=f[0].begin(); it!=f[0].end(); it++){
			sum+=it->first.first*it->second;
			cnt+=it->second;
		}
		return sum/cnt;
	}
