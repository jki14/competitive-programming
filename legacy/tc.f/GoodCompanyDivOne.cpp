...
int cmp(const int &x, const int &y){ return cnt[x]>cnt[y]; }

void core(const int &u, const int &flag){
	int k=cnt[u];
	if(flag){
		if(cnt[u]-1>m)ans=def;
		ans+=s[cnt[u]-1]+c[0];
		k--;
	}else{
		if(cnt[u]>m)ans=def;
	...
	for(int i=0;i<_size(sub[u]);i++){int v=sub[u][i];
		if(cnt[v]<=k){
			core(v,1);
			k--;
		}else core(v,0);
...
class GoodCompanyDivOne{
	...
	int minimumCost(vector <int> superior, vector <int> training){
		...
		n=_size(superior);m=_size(training);
		for(int i=0;i<n;i++)f[i]=superior[i];
		for(int i=0;i<m;i++)c[i]=training[i];
		sort(c,c+m);
		s[0]=0;
		for(int i=0;i<m;i++)s[i+1]=s[i]+c[i];
				
		for(int i=0;i<n;i++){ sub[i].clear();cnt[i]=1; }
		for(int i=1;i<n;i++){ 
			sub[f[i]].push_back(i);
			cnt[f[i]]++;
		}
		for(int i=0;i<n;i++)sort(sub[i].begin(),sub[i].end(),cmp);
