...
int next(const int &k){
	if(cstr[k]!='9'){ cstr[k]++;return 1; }
	return 0;
}

void findnext(){int i=0,num;
	while(cstr[i]!='$')i++;cstr[i]='0';
	sscanf(cstr,"%d$",&num);while(num>n){
		cstr[i]='$';i--;if(next(i))sscanf(cstr,"%d$",&num);	
	}
}

class FoxAndMp3 {
	public:
	vector <string> playList(int _n) {n=_n;
		vector<string> ret;ret.clear();
		for(int i=1;i<L;i++)cstr[i]='$';cstr[0]='1';
		for(int i=0;i<n && i<N;i++){
			string s;s.clear();
			if(i)findnext();
			for(int j=0;cstr[j]!='$';j++)s.push_back(cstr[j]);
			s=s+".mp3";
			ret.push_back(s);
		}
		return ret;
