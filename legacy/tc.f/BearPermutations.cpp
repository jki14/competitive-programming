...
int c[N][N], p[N];
int val[N][N], lef[N][N], rig[N][N];

class BearPermutations {
	public:
	int countPermutations(int n, int m, int mod){
		for(int i=0; i<=n; i++){
			c[i][0]=MOD_STD(1, mod);
			for(int j=1; j<=i; j++)c[i][j]=MOD_ADD(c[i-1][j-1], c[i-1][j], mod);
		}
		p[0]=MOD_STD(1, mod);
		for(int i=1; i<=n; i++)p[i]=MOD_MUL(p[i-1], i, mod);
		memset(val, 0, sizeof(val));val[0][0]=MOD_STD(1, mod);
		memset(lef, 0, sizeof(lef));lef[0][0]=MOD_STD(1, mod);
		memset(rig, 0, sizeof(rig));rig[0][0]=MOD_STD(1, mod);
		for(int len=1; len<=n; len++){
			for(int key=0; key<=m; key++){
				for(int mid=1; mid<=len; mid++){
					if(mid==1){
						MOD_ADD(&val[len][key], val[len-1][key], mod);
						if(key-len>=0){
							MOD_ADD(&lef[len][key], val[len-1][key-len], mod);
						}
						if(key-1>=0){
							MOD_ADD(&rig[len][key], val[len-1][key-1], mod);
						}
					}else if(mid==len){
						MOD_ADD(&val[len][key], val[len-1][key], mod);
						if(key-1>=0){
							MOD_ADD(&lef[len][key], val[len-1][key-1], mod);
						}
						if(key-len>=0){
							MOD_ADD(&rig[len][key], val[len-1][key-len], mod);
						}
					}else{
						for(int pre=0; pre<=key; pre++){
							MOD_ADD(&val[len][key], MOD_MUL(c[len-1][mid-1], MOD_MUL(lef[mid-1][pre], rig[len-mid][key-pre], mod), mod), mod);
							if(key-(len-mid+1)-pre>=0){
								MOD_ADD(&lef[len][key], MOD_MUL(c[len-1][mid-1], MOD_MUL(lef[mid-1][pre], rig[len-mid][key-(len-mid+1)-pre], mod), mod), mod);
							}
							if(key-mid-pre>=0){
								MOD_ADD(&rig[len][key], MOD_MUL(c[len-1][mid-1], MOD_MUL(lef[mid-1][pre], rig[len-mid][key-mid-pre], mod), mod), mod);
							}
						}
					}
				}
		...
		int ans=0;
		for(int key=0; key<=m; key++){
			MOD_ADD(&ans, val[n][key], mod);
		}
		return MOD_STD(ans, mod);
	}
