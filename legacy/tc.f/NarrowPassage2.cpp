...
public:
	int count(vector <int> size, int maxSizeSum) {
		int n=size.size(), cap=maxSizeSum;
		multimap<int, int, greater<int> > pool;
		for(int i=0; i<n; i++)
			pool.insert(make_pair(size[i], i));
		map<int, int> row;
		int64_t ans = 1LL;
		for(multimap<int, int, greater<int> >::iterator it=pool.begin(); it!=pool.end(); it++){
			int val = it->first, pos = it->second;
			row.insert(make_pair(pos, val));
			int64_t count = 1LL;
			map<int, int>::iterator prev = row.find(pos);
			if(prev==row.begin())
				prev = row.end();
			else prev--;
			for(;prev!=row.end();prev--){
				if(prev->second + val > cap)break;
				count++;
				if(prev == row.begin())break;
			}
			map<int, int>::iterator next = row.find(pos);
			next++;
			for(; next!=row.end(); next++){
				if(next->second + val > cap)break;
				count++;
			}
			ans = ans * count % mod;
		}
