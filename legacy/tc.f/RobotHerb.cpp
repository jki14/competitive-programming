...
int n;
const lld dx[K]={ 0,1,0,-1 };
const lld dy[K]={ 1,0,-1,0 };

lld px(const lld &x,const lld &y,const int &k){
	switch(k){
		case 0: return x;
		case 1: return y;
		case 2: return -x;
...
lld py(const lld &x,const lld &y,const int &k){
	switch(k){
		case 0: return y;
		case 1: return -x;
		case 2: return -y;
...
class RobotHerb {
	...
	long long getdist(int T, vector <int> a) {
		lld x,y;x=y=0LL;int k=0;n=a.size();
		for(int i=0;i<n;i++){
			x+=dx[k]*((lld)a[i]);
			y+=dy[k]*((lld)a[i]);
			k=(k+a[i])%K;
		}
		//printf("x=%lld y=%lld k=%d\n",x,y,k);
		lld fx,fy;fx=fy=0LL;int fk=0;
		for(int i=1;i<=T;i<<=1){
			if(T&i){
				fx+=px(x,y,fk);
				fy+=py(x,y,fk);
			...
			y+=py(_x,_y,k);
			k=(k+k)%K;
			//printf("fx=%lld fy=%lld fk=%d x=%lld y=%lld k=%d\n",fx,fy,fk,x,y,k);
		}
		return (fx>0LL?fx:-fx)+(fy>0LL?fy:-fy);
	}
};
