...
class KitayutaMart {
	...
	lld count(const int bas, const int pow){
		return n*1LL*pow+delta(bas);
	}
	int find_pow(){
		int lef=0, rig=m-1;
		while(lef<=rig){
			int mid=(lef+rig)>>1;
			if(count(n, mid)>=m*1LL){
				if(!mid || count(n, mid-1)<m*1LL)return mid;
				rig=mid-1;
			}else lef=mid+1;
		}
		return -1;
	...
	int find_bas(){
		...
		while(lef<=rig){
			int mid=(lef+rig)>>1;
			if(count(mid, pow)>=m*1LL){
				if(mid==1 || count(mid-1, pow)<m*1LL)return mid;
				rig=mid-1;
			}else lef=mid+1;
		}
		return -1;
	...
	int lastPrice(int N, int K){
		...
		pow=find_pow();
		bas=find_bas();
		return MOD_STD(bas*1LL*MOD_POW(2, pow));
	}
