...
#define val(x, y) ((stt[x]&bit[y])?1:0)
#define count(x) (((x&1LL)?1:0)+((x&2LL)?1:0))

#define N 52
#define M 52

int n, m, ans, f[N];
lld stt[N], msk[N], bit[N];

class SwitchingGame {
	public:
	int timeToWin(vector <string> states){
		n=SIZE(states); m=LENGTH(states[0]);
		for(int i=0;i<M;i++){ bit[i]=1LL<<i; }
		stt[0]=msk[0]=0LL;
		for(int i=1;i<=n;i++){
			stt[i]=msk[i]=0LL;
			for(int j=0;j<m;j++){
		...
		for(int i=1;i<=n;i++){
			f[i]=0;
			for(int j=0;j<m;j++){
				if(msk[i]&bit[j])continue;
				int k=i-1, trg=0;
				for(;msk[k]&bit[j];k--){
					trg|=f[k];
				}
				if(val(k,j)!=val(i,j) && !(bit[val(k,j)]&trg)){
					f[i]|=bit[val(k,j)];
				}
			}
			ans+=count(f[i]);
