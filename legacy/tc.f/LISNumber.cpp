...
void amod(lld &x, const lld &y){ x=(x+y)%mod; }

/*lld _com(const lld &x,const lld &y){
	lld ret=1LL;
	for(lld i=1LL;i<=y;i++){
		ret=_mmod(ret,(x-i+1LL),mod);
		ret=_mmod(ret,_inv(i,mod),mod);
	}
	//printf("com(%lld,%lld)=%lld\n",x,y,ret);
	return _cmod(ret,mod);
}*/

lld com(const lld &x,const lld &y){
	if(x<y)return 0LL;
	if(!y)g[x][y]=1LL;
	if(g[x][y]==-1LL)
		g[x][y]=(com(x-1,y)+com(x-1,y-1))%mod;
	return _cmod(g[x][y],mod);
}

lld dfs(const int &i,const int &j){
	...
	if(!i){
		...
		for(int k=max(0,j-a[i]);k<=j;k++){
			int d=j-k,p=a[i]-d;
			lld inc=dfs(i-1,k);if(!inc)continue;
			inc=_mmod(inc,com((lld)k,(lld)p),mod);
			inc=_mmod(inc,com((lld)(sum[i-1]+p-k+d-1),(lld)d),mod);
			//if(inc)printf("update f[%d][%d] with {%d,%d}(%lld)->%lld\n",i,j,i-1,k,dfs(i-1,k),inc);
			amod(f[i][j],inc);
		}
	}
	f[i][j]=_cmod(f[i][j],mod);
	//printf("f[%d][%d]=%lld\n",i,j,f[i][j]);
	return f[i][j];
}
...
class LISNumber {
	public:
	int count(vector <int> _a, int _m) {
		n=_size(_a);m=_m;sum[0]=1;
		for(int i=0;i<n;i++){
			a[i+1]=_a[i];
			sum[i+1]=(sum[i]+a[i+1])%_mod;
		}
		memset(f,0xff,sizeof(f));f[0][0]=1LL;
		memset(g,0xff,sizeof(g));//g[0][0]=1LL;
		return dfs(n,m);
	}
};
