...
struct stat{
	int x[N];
	stat(){ }
	stat(const int v, const int w, const int x, const int y, const int z){
		this->x[0]=v;
		this->x[1]=w;
		this->x[2]=x;
		this->x[3]=y;
...
class Nine {
	...
	int count(int n, vector <int> g){
		...
		f[0][0][0][0][0][cur]=1;
		v[0][0][0][0][0]=0;
		que[cur].push(stat(0, 0, 0, 0, 0));
		for(int k=0; k<m; k++){
			cur=pre;
			for(;!que[pre].empty();que[pre].pop()){
				for(int i=0; i<=9; i++){
					stat s=que[pre].front();
					int buf=f[s.x[0]][s.x[1]][s.x[2]][s.x[3]][s.x[4]][pre];
					for(int j=0; j<N; j++){
						if(j>=n || !(g[k]&bit[j]))continue;
						s.x[j]=(s.x[j]+i)%9;
					}
					if(v[s.x[0]][s.x[1]][s.x[2]][s.x[3]][s.x[4]]<k+1){
						v[s.x[0]][s.x[1]][s.x[2]][s.x[3]][s.x[4]]=k+1;
						que[cur].push(s);
						f[s.x[0]][s.x[1]][s.x[2]][s.x[3]][s.x[4]][cur]=0;
					}
					MOD_ADD(&f[s.x[0]][s.x[1]][s.x[2]][s.x[3]][s.x[4]][cur], buf);
				}
			}
		}
		return MOD_STD(f[0][0][0][0][0][cur]);
	}
