...
void insert(const int i, const lld key, const lld cnt){
	map<lld, lld>::iterator it=f[i].find(key);
	if(it!=f[i].end())
		it->second += cnt;
	else
		f[i].insert(_mkpr(key, cnt));
}

class TaroCoins {
public:
	long long getNumber(long long N) {
		for(int i=0;i<64;i++)f[i].clear();f[0].insert(_mkpr(0LL,1LL));
			
		int flag=0;
		for(int i=0;i<61;i++){lld key=_niL(i);
			if(key>N){ flag=i;break; }
			for(map<lld, lld>::iterator it=f[i].begin();it!=f[i].end();it++){
				 lld tar=(N-(*it).first)&key;
				 if(tar){
					 lld mor=(*it).first+key;
					 insert(i+1, (mor&key)?(mor^key):mor, (*it).second);
				 }else{
					 lld mor=(*it).first;
					 insert(i+1, (mor&key)?(mor^key):mor, (*it).second);
					 mor+=key<<1;
		...
		map<lld, lld>::iterator it=f[flag].find(0);
		return lld((*it).second);
	}
