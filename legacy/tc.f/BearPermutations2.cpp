...
class BearPermutations2 {
	...
	int getSum(int n, int mod){
		...
		for(int i=0; i<=n; i++){
			c[i][0]=MOD_STD(1, mod);
			for(int j=1; j<=i; j++){
				c[i][j]=MOD_ADD(c[i-1][j-1], c[i-1][j], mod);
			}
		}
		p[0]=1;
		...
		for(int len=1; len<=n; len++){
			for(int mid=2; mid<len; mid++){
				int lef=mid-1, rig=len-mid;
				for(int x=0; x<lef; x++){
					for(int y=0; y<rig; y++){
						int buf=MOD_MUL(MOD_MUL(c[len-1][lef], MOD_MUL(p[lef-1], p[rig-1], mod), mod), MOD_ADD(MOD_ADD(x, y, mod), 2, mod), mod);
						MOD_ADD(&f[len], buf, mod);
					}
				}
			}
			//printf("+f[%d]=%d\n", len, f[len]);
		...
		for(int len=1; len<=n; len++){
			for(int mid=1; mid<=len; mid++){
				//MOD_ADD(&f[len], MOD_MUL(c[len-1][mid-1], MOD_ADD(f[mid-1], f[len-mid], mod), mod), mod);
				MOD_ADD(&f[len], MOD_MUL(c[len-1][mid-1], MOD_MUL(f[mid-1], p[len-mid], mod), mod), mod);
				MOD_ADD(&f[len], MOD_MUL(c[len-1][mid-1], MOD_MUL(f[len-mid], p[mid-1], mod), mod), mod);
			}
			//printf("f[%d]=%d\n", len, f[len]);
		}
		return MOD_STD(f[n], mod);
	}
