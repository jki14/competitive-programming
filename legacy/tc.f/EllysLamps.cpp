...
class EllysLamps{
	...
	int getMin(string lamps){
		n=lamps.length();ans=0;
		for(int i=0;i<n;i++){
			if(lamps[i]=='N' && i-1>=0 && lamps[i-1]=='Y'){ ans++;lamps[i]='X'; }
			if(lamps[i]=='Y' && i-1>=0 && lamps[i-1]=='N'){ ans++;lamps[i]='X'; }
			if(lamps[i]=='Y' && i-1>=0 && lamps[i-1]=='Y' && i-2>=0 && lamps[i-2]=='Y'){
				ans++;lamps[i]='X';
			}
		}
		return ans;
