...
class BidirectionalQueue{
	public:
	int extractElements(int n, vector <int> c){int ans=0;
		for(int i=0;i<n;i++)a[i+1]=i;
		for(int i=0;i<_size(c);i++){
			int k=a[c[i]];
			if(k>n-i-a[c[i]]){
				k=a[c[i]]+i-n;
			}
			ans+=abs(k);
			for(int j=1;j<=n;j++)a[j]=_cmod(a[j]-k,n-i);
			if(i+1<_size(c))for(int j=1;j<=n;j++)a[j]=_cmod(a[j]-1,n-i-1);
		}
		return ans;
	}
