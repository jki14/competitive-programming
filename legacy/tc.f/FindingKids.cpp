...
class FindingKids {
	private:
	vector<int> pos, rdu, lef, rig;
	vector<pii> buf;
	set<int> dis;
	int count(const int pos, const int len){
		int low, hig, mid, cnt=0;
		low=0, hig=SIZE(lef)-1;
		while(low<=hig){
			mid=(low+hig)>>1;
			if(lef[mid]-len<=pos){
				if(mid==hig || lef[mid+1]-len>pos){
					cnt+=mid+1;
					break;
				}
				low=mid+1;
		...
		low=0, hig=SIZE(rig)-1;
		while(low<=hig){
			mid=(low+hig)>>1;
			if(rig[mid]+len<=pos){
				if(mid==hig || rig[mid+1]+len>pos){
					cnt+=mid+1;
					break;
				}
				low=mid+1;
	...
	int query(const int idx, const int len){
		int low, hig, mid;
		low=0, hig=SIZE(lef)-1;
		while(low<=hig){
			mid=(low+hig)>>1;
			if(count(lef[mid]-len, len)>=idx){
				if(count(lef[mid]-len-1, len)<idx)return lef[mid]-len;
				hig=mid-1;
			}else low=mid+1;
		}
		low=0, hig=SIZE(rig)-1;
		while(low<=hig){
			mid=(low+hig)>>1;
			if(count(rig[mid]+len, len)>=idx){
				if(count(rig[mid]+len-1, len)<idx)return rig[mid]+len;
				hig=mid-1;
			}else low=mid+1;
		}
		return -1;
	...
	public:
	long long getSum(int n, int m, int A, int B, int C){
		pos.resize(n);
		buf.resize(n);
		rdu.resize(n);
		dis.clear();
		...
		for(int i=0; i<n; i++){
			...
			if(pos[i]&1){
				//lef <--
				lef.push_back(pos[i]);
			}else rig.push_back(pos[i]);
		}
		sort(buf.begin(), buf.end());
		sort(lef.begin(), lef.end());
		sort(rig.begin(), rig.end());
		for(int i=0; i<n; i++)rdu[buf[i].second]=i;
		lld ans=0LL;
		for(int i=0; i<m; i++){
			int idx, len;
			A=MOD_ADD(MOD_MUL(A, B), C);
