...
const int bit[15]={ 1,2,4,8,16,32,64,128,256,512,1024,2048,4096,8192,16384 };
int n,m,ans,x,y;
int a[N],c[N],b[K][S];
int s[S];

void init(){
	//for(int i=0;i<=max(n,m);i++)bit[i]=_ni(i);
	for(int x=0;x<bit[m];x++){
		b[0][x]=b[1][x]=0;s[x]=0;
		for(int i=1;i<=x;i<<=1){
			if(!(x&i))continue;
			s[x]++;b[0][x]^=i;
...
void update(){
	int fir=y?((bit[m]-1)^a[0]):a[0];
	int tmp=s[fir];
	a[1]^=b[y][fir];a[2]^=fir;
	//if(x==15 && y==1 && a[0]==4)printf("a[1]=%d fir=%d\n",a[1],fir);
	for(int i=2;i<=n;i++){int k;
		if(a[i-1]==(a[i-1]&x))
			k=y;
		else if((a[i-1]&x)==0)
			k=y^1;
		else return;
		//if(x==15 && y==1 && a[0]==4)printf("k=%d ori=%d ",k,a[i]);
		tmp+=s[a[i-1]];
		a[i]^=b[k][a[i-1]];
		a[i+1]^=a[i-1];
		//if(x==15 && y==1 && a[0]==4)printf("a[i]=%d a[i-1]=%d\n",a[i],a[i-1]);
	}
	if(!a[n]){
		//printf("tmp=%d",tmp);
		_updmin(ans,tmp);
...
class YetAnotherBoardGame {
	...
	int minimumMoves(vector <string> board) {
		n=_size(board);m=_size(board[0]);init();ans=inf;
		for(int i=1;i<=n;i++){a[i]=0;
			for(int j=0;j<m;j++){
				c[i]<<=1;
				c[i]|=(board[i-1][j]=='W')?1:0;
			}
		}
		for(x=0;x<bit[m];x++){
			for(a[0]=x;;a[0]=(x&(a[0]-1))){
				for(y=0;y<2;y++){
					//printf("x=%d ",x);
					//printf("a[0]=%d ",a[0]);
					//printf("y=%d ", y);
