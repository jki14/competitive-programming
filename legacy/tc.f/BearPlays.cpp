...
typedef struct mykey_s{
	int x, y;
	mykey_s(){ this->x=this->y=0; }
	mykey_s(const int _x, const int _y){
		this->x=_x;
		this->y=_y;
		if(this->x>this->y)
		swap(this->x, this->y);
	}
	bool operator < (const mykey_s &opp) const{
		return this->x!=opp.x?(opp.x<this->x):(opp.y<this->y);
	}
} mykey_t;

class BearPlays {
	public:
	int pileSize(int x, int y, int n){
		map<mykey_t, int> tab;
		vector<mykey_t> buf;
		mykey_t cur=mykey_t(x, y);
		for(int til=0;til<n;til++){
			cur=mykey_t(cur.x<<1, cur.y-cur.x);
			map<mykey_t, int>::iterator it=tab.find(cur);
			if(it!=tab.end()){
				cur=buf[it->second+(n-1-til)%(til-it->second)];
				break;
			}else{
				buf.push_back(cur);
				tab.insert(MP(cur, til));
			}
		}
		return cur.x;
	}
...
// BEGIN CUT HERE
	...
	void test_case_4() { int Arg0 = 864000001; int Arg1 = 432000010; int Arg2 = 1994199464; int Arg3 = 1994199464; verify_case(4, Arg3, pileSize(Arg0, Arg1, Arg2)); }
