...
class EnclosingTriangle {
	private:
	int n, m, til;
	vector<point_t<double> > a, b;
	public:
	long long getNumber(int m, vector <int> x, vector <int> y){
		a.clear();b.clear();
		for(int i=0; i<m; i++)a.push_back(point_t<double>(i*1.0, 0*1.0));
		for(int i=0; i<m; i++)a.push_back(point_t<double>(m*1.0, i*1.0));
		for(int i=m; i>0; i--)a.push_back(point_t<double>(i*1.0, m*1.0));
		for(int i=m; i>0; i--)a.push_back(point_t<double>(0*1.0, i*1.0));
		for(int i=0; i<SIZE(x); i++)b.push_back(point_t<double>(x[i]*1.0, y[i]*1.0));
		n=SIZE(a);m=SIZE(b);
		...
		for(int i=0; i<n; i++){
			int hig=0;
			for(int j=1; j<m; j++){
				if(fsign(a[i].cross(b[hig], b[j]))<0)hig=j;
			}
			while(fsign(a[i].cross(b[hig], a[(til+1)%n]))<=0)til++;
			f[i]=til;
			if(til>=n)UPDMIN(g[til%n], i);
			//printf("f[%d]=%d\n", i, f[i]);
		}
		for(int i=n-2; ~i; i--)UPDMIN(g[i], g[i+1]);
		//for(int i=0; i<n; i++)printf("g[%d]=%d\n", i, g[i]);
		for(int i=0; i<n; i++)UPDMIN(f[i], n-1);
		...
		for(int i=0; i<n-2; i++){
			...
			while(!que.empty() && f[que.front()]<g[i]){
				cur-=f[que.front()]*1LL;
				que.pop();
			}
			lld tmp=cur-(g[i]*1LL-1LL)*SIZE(que);
			if(tmp>0LL && g[i]==f[i]){
				tmp--;
			}
			ans+=tmp;
			//printf("%lld at %d\n", cur-(g[i]*1LL-1LL)*SIZE(que), i);
		}
...
// BEGIN CUT HERE
	...
	void test_case_1() { int Arg0 = 58585; int Arr1[] = { 58584 }; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arr2[] = { 58584 }; vector <int> Arg2(Arr2, Arr2 + (sizeof(Arr2) / sizeof(Arr2[0]))); long long Arg3 = 0LL; verify_case(1, Arg3, getNumber(Arg0, Arg1, Arg2)); }
