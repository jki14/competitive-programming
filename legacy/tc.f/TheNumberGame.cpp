...
string answ="Manao wins";
string ansl="Manao loses";
...
class TheNumberGame {
	...
	string determineOutcome(int A, int B) {
		...
		for(int i=0;i+m-1<n;i++){int p=0;
			for(;p<m;p++)if(a[i+p]!=b[p])break;
			if(p==m)return answ;
		}
		for(int i=n-1;i-m+1>=0;i++){int p=0;
			for(;p<m;p++)if(a[i-p]!=b[p])break;
			if(p==m)return answ;
		}
		return ansl;
