...
void update(const int &mes, const int &cpb, const int &dis){
	//if(mes>=N || cpb>=N)return;
	if(f[mes][cpb]>dis){
		f[mes][cpb]=dis;
		que.push(pii(mes,cpb));
	}
}
...
class EmoticonsDiv1{
	...
	int printSmiles(int n){
		int ans=inf;memset(f,0x3f,sizeof(f));
		while(!que.empty())que.pop();
		f[1][0]=0;que.push(pii(1,0));
		for(;!que.empty();que.pop()){
			int mes=que.front().first,cpb=que.front().second;
			int dis=f[mes][cpb];
			//printf("f[%d][%d]=%d\n",mes,cpb,dis);
			if(mes==n){ ans=dis;break; }
			//copy
			if(mes<n)update(mes,mes,dis+1);
			//paste
			if(mes<n)update(mes+cpb,cpb,dis+1);
			//delete
			if(mes)update(mes-1,cpb,dis+1);
		}
		return ans;
	}
...
// BEGIN CUT HERE
	...
	void test_case_1() { int Arg0 = 997; int Arg1 = 4; verify_case(1, Arg1, printSmiles(Arg0)); }
	void test_case_2() { int Arg0 = 998; int Arg1 = 5; verify_case(2, Arg1, printSmiles(Arg0)); }
	void test_case_3() { int Arg0 = 999; int Arg1 = 8; verify_case(3, Arg1, printSmiles(Arg0)); }
	void test_case_4() { int Arg0 = 1000; int Arg1 = 8; verify_case(4, Arg1, printSmiles(Arg0)); }
