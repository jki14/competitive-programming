...
public:
	int call(const int x0, const int y0, const int x1, const int y1){
		int ret=a[x1][y1];
		if(x0>0)ret-=a[x0-1][y1];
		if(y0>0)ret-=a[x1][y0-1];
		if(x0>0 && y0>0)ret+=a[x0-1][y0-1];
		return ret;
	}

	int findBest(vector <string> chocolate) {
		int n=_size(chocolate), m=chocolate[0].length();
		for(int i=0;i<n;i++)for(int j=0;j<m;j++)a[i][j]=chocolate[i][j]-'0';
		...
		int ans=0;
		for(int h0=0;h0<n-1;h0++)for(int h1=h0+1;h1<n-1;h1++){
			for(int v0=0;v0<m-1;v0++)for(int v1=v0+1;v1<m-1;v1++){
				int cur=0x3f3f3f3f;
				_updmin(cur, call(0,	0,	h0,	v0));
				_updmin(cur, call(h0+1,	0,	h1,	v0));
				_updmin(cur, call(h1+1,	0,	n-1,	v0));
				_updmin(cur, call(0,	v0+1,	h0,	v1));
				_updmin(cur, call(h0+1,	v0+1,	h1,	v1));
				_updmin(cur, call(h1+1,	v0+1,	n-1,	v1));
				_updmin(cur, call(0,	v1+1,	h0,	m-1));
				_updmin(cur, call(h0+1,	v1+1,	h1,	m-1));
				_updmin(cur, call(h1+1,	v1+1,	n-1,	m-1));
				_updmax(ans,cur);
			}
		}
		return int(ans);
