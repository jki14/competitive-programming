...
=======
>>>>>>> 42f6cb1b1b1d8cf9c65ef137c2f26b9655d823ec
#include<iostream>
#include<sstream>
#include<cstdio>
#include<cstdlib>
...
struct state{
	...
	bool operator < (const state &other) const{
		return (sta==other.sta)?(pos<other.pos):(sta<other.sta);
	}
}now,nxt;

map<state, lld> f;
map<state, lld>::iterator it;
//set<state> mrk;
queue<state> que;
int n,m,full,bit[POS],a[POS];
lld ans;

class PolygonTraversal {
	public:
	int check(const int &s,int x,int y){
		for(int i=x,l=0;l<n;l++){
			i=(i+1)%n;if(i==y)return 0;
			if(s&bit[i])break;
		}
		for(int i=x,l=0;l<n;l++){
			i=((i-1)%n+n)%n;if(i==y)return 0;
			if(s&bit[i])break;
		}
		return 1;
	}
	lld count(int N, vector <int> points) {ans=0LL;
		n=N;m=points.size();
		for(int i=0;i<m;i++)a[i]=points[i]-1;
		for(int i=0;i<=n;i++)bit[i]=_ni(i);full=bit[n]-1;
		int src=0;for(int i=0;i<m;i++)src|=bit[a[i]];
		//memset(f,0,sizeof(f));f[src][a[m-1]]=1LL;
		f.clear();now.sta=src;now.pos=a[m-1];f.insert(_mkpr(now,1LL));
		while(!que.empty())que.pop();que.push(now);
		//memset(mrk,0,sizeof(mrk));mrk[src][a[m-1]]=1;
		for(;!que.empty();que.pop()){
			now=que.front();int sta=now.sta,pos=now.pos;
			lld val=f[now];
			
			//if(n<10)printf("f[%d][%d]=%lld\n",sta,pos,val);
			
			for(int nxtpos=0;nxtpos<n;nxtpos++){
				if(sta==full && nxtpos==a[0] && check(sta,nxtpos,pos))ans+=val;
				if(sta&bit[nxtpos])continue;
				if(!check(sta,nxtpos,pos))continue;
				int nxtsta=sta|bit[nxtpos];
				nxt.sta=nxtsta;nxt.pos=nxtpos;
				
				//if(n<10)printf("    update[%d][%d]\n",nxtsta,nxtpos);
				
				it=f.find(nxt);
				if(it==f.end()){
					f.insert(_mkpr(nxt,val));
					que.push(nxt);
				}else{
					(*it).second+=val;
				}	
		...
		/*now.sta=full;now.pos=a[0];
		it=f.find(now);
		return it==f.end()?0LL:(*it).second;*/
		return ans;
	}
};
