...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    auto const comp = [](vector<int> const& lhs, vector<int> const& rhs) -> int {
      int const n = lhs.size();
      int const m = rhs.size();
      if (n != m) return n - m;
      for (int i = 0; i < n; ++i) {
        if (lhs[i] != rhs[i]) {
          return lhs[i] - rhs[i];
    ...
    auto const less = [&comp](vector<int> const& lhs, vector<int> const& rhs) -> int {
    ...
    while (i < n) {
      int j = 0;
      while (i + j + 1 < n && comp(a[i], a[i + j + 1]) == 0) {
        ++j;
      }
      i += j + 1;
      ++r;
