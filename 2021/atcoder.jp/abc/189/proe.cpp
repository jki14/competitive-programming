...
int n, m, q;
int opt[210000], tar[210000];
pll a[210000], bar[210000];

queue<pii> foo[210000];
    ...
    for (int i = 0; i < m; ++i) {
      if (opt[i] == 1) {
        ...
        if (opt[i] == 3) {
          ...
          // fprintf(stderr, "symmetric %d %d\n", opt[i], tar[i]);
          acc.first = tar[i] * 2LL - acc.first;
          mul.first *= -1;
        } else {
          if (flip.second) {
            tar[i] *= -1;
          ...
          // fprintf(stderr, "symmetric %d %d\n", opt[i], tar[i]);
          acc.second = tar[i] * 2LL - acc.second;
          mul.second *= -1;
        }
      }
      for (; !foo[i].empty(); foo[i].pop()) {
        int const k = foo[i].front().first;
        int const p = foo[i].front().second;
        // fprintf(stderr, "req ? %d at %d\n", k, p);
        bar[p].first = a[k].first * mul.first + acc.first;
        bar[p].second = a[k].second * mul.second + acc.second;
        if (flip.first) {
          bar[p].first *= -1;
        }
        if (flip.second) {
        ...
        if (swap) {
          std::swap(bar[p].first, bar[p].second);
        }
      }
    }

    for (int i = 0; i < q; ++i) {
      printf("%lld %lld\n", bar[i].first, bar[i].second);
    }
  }
  return 0;
}
