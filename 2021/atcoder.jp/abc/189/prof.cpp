...
int sufficient(llf const f0) {
  llf ss = 0;
  llf const rate = static_cast<llf>(1.0) / m;
  for (int i = n - 1; i >= 0; --i) {
    if (a[i]) {
      f[i] = f0;
    } else {
  ...
  // fprintf(stderr, "f[0] = %.6Lf for f0 = %.6Lf\n", f[0], f0);
  return f[0] < f0 + 1e-9;
}

llf solution() {
  llf low = 1.0, hig = 1e14;
  static auto const related_error =
      [](llf const expect, llf const actual) -> llf {
      return abs(expect - actual) / expect;
  };
  while (related_error(hig, low) >= 1e-6) {
    llf const mid = sqrt(low * hig);
    if (sufficient(mid)) {
      hig = mid;
    } else {
      low = mid;
