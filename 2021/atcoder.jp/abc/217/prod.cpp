...
class segminmax_t : public joshu::btnctx_t<segminmax_t> {
public:
  segminmax_t() = default;

  segminmax_t(int const minimum, int const maximum) : minimum_(minimum),
                                                      maximum_(maximum) { }

  segminmax_t(segminmax_t const&) = default;
  segminmax_t(segminmax_t&&) = default;

  segminmax_t& operator=(segminmax_t const&) = default;
  segminmax_t& operator=(segminmax_t&&) = default;

  void aggregate(segminmax_t const& lhs,
                 segminmax_t const& rhs) override {
    if (lhs.minimum_ == -inf) {
      minimum_ = max(minimum_, rhs.minimum_);
      maximum_ = min(maximum_, rhs.maximum_);
    } else if (rhs.minimum_ == -inf) {
      minimum_ = max(minimum_, lhs.minimum_);
      maximum_ = min(maximum_, lhs.maximum_);
    }
  }
  void flush(segminmax_t& lhs, segminmax_t&rhs) override {
    lhs.minimum_ = max(lhs.minimum_, minimum_);
    rhs.minimum_ = max(rhs.minimum_, minimum_);
    lhs.maximum_ = min(lhs.maximum_, maximum_);
    rhs.maximum_ = min(rhs.maximum_, maximum_);
  }
...
using ds_t = joshu::segtree_t<int, segminmax_t>;

int main() {
  int l, q;
  int lhs, rhs;

  auto const initializer = [&l](ds_t::node_t const& node) -> segminmax_t {
    return {0, l};
  };
  auto const updater = [&lhs, &rhs](ds_t::node_t const& node) -> segminmax_t {
    auto bar = node.ctx;
    bar.minimum_ = max(bar.minimum_, lhs);
    bar.maximum_ = min(bar.maximum_, rhs);
    return bar;
  };
  auto const querier = [](ds_t::node_t const& node) -> segminmax_t {
    return node.ctx;
  };
