...
int main() {
  ...
  while (scanf("%lu%lu", &n, &m) != EOF) {
    std::vector<pii_t> bar;
    bar.reserve(n);
    for (size_t i = 0; i < n; ++i) {
      bar.push_back(make_pair(0, -i));
    }

    std::vector<std::list<int>> arrows(n);
    for (size_t i = 0; i < m; ++i) {
      int u, v;
      scanf("%d%d", &u, &v);
      --u;
      --v;
      arrows[u].push_back(v);
      --bar[v].first;
    }

    joshu::heap_t<pii_t> foo(262144);
    ...
    while (res.size() < n) {
      ...
      int const u = -top.second;
      for (auto const v : arrows[u]) {
        (*entries[v])->set(make_pair(++bar[v].first, -v));
      }
      res.push_back(u);
    }
    if (res.size() == n) {
