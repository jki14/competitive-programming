...
int main() {
  ...
  while (scanf("%d%d", &n, &m) != EOF) {
    vector<int> d(n, 0);
    set<pair<int, int>> budget;
    for (unsigned i = 0; i < n; ++i) {
      d[i] = 0;
      budget.insert(make_pair(0, i));
    }
    vector<list<int>> arrows(n);
    for (unsigned i = 0; i < m; ++i) {
      ...
      budget.erase(make_pair(d[v], v));
      ++d[v];
      budget.insert(make_pair(d[v], v));
    }
    vector<int> res;
    res.reserve(n);
    while (res.size() < n) {
