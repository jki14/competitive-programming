...
void expand(int const x, int const y, int const flag) {
  v[x][y] = flag;
  f[x][y] = 1;
  for (int i = 0; i < 4; ++i) {
    int const p = x + dx[i];
      ...
      for (int i = 0; i < h; ++i) {
        ...
        for (int j = 0; j < w; ++j) {
          if (g[i][j]) {
            ...
            if (j - 1 >= 0) row.set(j - 1);
            if (j + 1 < w) row.set(j + 1);
            if (i - 2 >= 0) f[i - 2] |= row;
            if (i + 2 < h) f[i + 2] |= row;
            
            if (j - 2 >= 0) row.set(j - 2);
            if (j + 2 < w) row.set(j + 2);
            if (i - 1 >= 0) f[i - 1] |= row;
            if (i + 1 < h) f[i + 1] |= row;
            f[i] |= row;
          }
