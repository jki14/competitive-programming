...
using heap_t = joshu::heap_t<pii>;

static int constexpr inf = 0x3f3f3f3f;

int n, m;
int nbs0[2100], nxt0[2100], dst0[2100], wht0[2100], num0;
int nbs1[2100], nxt1[2100], dst1[2100], wht1[2100], num1;

int foo[2100], bar0[2100], bar1[2100];

heap_t heap;

void add_edge(int const u, int const v, int const w,
              int* nbs, int* nxt, int* dst, int* wht, int& num) {
  nxt[num] = nbs[u];
  dst[num] = v;
  wht[num] = w;
  nbs[u] = num++;
...
void spfa(int const s, int* nbs, int* nxt, int* dst, int* wht, int* bar) {
  for (int i = 0; i < n; ++i) {
    bar[i] = inf;
  }
  bar[s] = 0;

  vector<heap_t::iter_t*> hooks;
  hooks.reserve(n);
  for (int i = 0; i < n; ++i) {
    hooks.push_back(heap.push(make_pair(-bar[i], i)));
  }
  ...
  while (!heap.empty()) {
    ...
    int const base = -cur.first;
    if (base == inf) continue;
    int const u = cur.second;
    for (int i = nbs[u]; ~i; i = nxt[i]) {
      int const v = dst[i];
      int const w = wht[i];
      if (bar[v] > base + w) {
        bar[v] = base + w;
        (*hooks[v])->set(make_pair(-bar[v], v));
      }
    }
  }
}
  ...
  while (scanf("%d%d", &n ,&m) != EOF) {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      if (u == v) {
        ...
        add_edge(u, v, w, nbs0, nxt0, dst0, wht0, num0);
        add_edge(v, u, w, nbs1, nxt1, dst1, wht1, num1);
      }
    }

    for (int i = 0; i < n; ++i) {
      spfa(i, nbs0, nxt0, dst0, wht0, bar0);
      spfa(i, nbs1, nxt1, dst1, wht1, bar1);
      for (int v = 0; v < n; ++v) {
        if (v == i) continue;
        foo[i] = min(bar0[v] + bar1[v], foo[i]);
      }
    }
