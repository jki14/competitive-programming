...
static llf constexpr eps = 1e-9;

int main(){
  llf x, y, r;
  while (scanf("%Lf%Lf%Lf", &x, &y, &r) != EOF) {
    lld foo = 0;
    int const p = static_cast<int>(ceil(y - r));
    int const q = static_cast<int>(floor(y + r));
    for (int i = p; i <= q; ++i) {
      // fprintf(stderr, "i = %d\n", i);

      auto sufficient_l = [x, y, i, r](int const v) -> bool {
        llf const dx = static_cast<llf>(v) - x;
        llf const dy = static_cast<llf>(i) - y;
        return (dx * dx + dy * dy) < (r * r + eps);
      };
      int lef = static_cast<int>(ceil(x - r));
      int rig = static_cast<int>(floor(x - eps));
      int bar = joshu::binary_search(lef, rig, sufficient_l);
      if (sufficient_l(bar)) {
        foo += rig - bar + 1;
      }

      auto sufficient_r = [x, y, i, r](int const v) -> bool {
        llf const dx = static_cast<llf>(-v) - x;
        llf const dy = static_cast<llf>(i) - y;
        return (dx * dx + dy * dy) < (r * r + eps);
      };
      lef = static_cast<int>(ceil(x + eps));
      rig = static_cast<int>(floor(x + r));
      bar = joshu::binary_search(-rig, -lef, sufficient_r);
      if (sufficient_r(bar)) {
        foo += - lef - bar + 1;
      }

      if (static_cast<int>(floor(x - eps)) + 1 <
              static_cast<int>(ceil(x + eps))) {
        ++foo;
      }
    }
    printf("%lld\n", foo);
