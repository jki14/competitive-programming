...
vector<int> lef(64, 0), rig(64, 0);

int median2(int const x, int const y, int const z) {
  if (f[x][y] * f[x][z] == -1) return x;
  if (f[y][x] * f[y][z] == -1) return y;
  if (f[z][x] * f[z][y] == -1) return z;
...
int median(int const x, int const y, int const z) {
  int const v = median2(a[x], a[y], a[z]);
  if (v == a[x]) {
    return x;
  } else if (v == a[y]) {
    return y;
...
void less3(int const x, int const y) {
  f[x][y] = -1;
  f[y][x] = 1;
}

void less2(int const x, int const y) {
  lef.clear();
  lef.push_back(x);
  ...
  for (auto const p : lef) {
    for (auto const q : rig) {
      less3(p, q);
    }
  }
}
...
void solution() {
  ...
  for (int i = 0; i < n; ++i) {
    int const p = joshu::randllu() % (n - i) + i;
    swap(a[i], a[p]);
    // fprintf(stderr, "%d%c", a[i] + 1, (i == n - 1) ? '\n' : ' ');
  }
  ...
  */
  ...
  auto const cmp = [&](int const lhs, int const rhs) -> bool {
    if (lhs != rhs && f[lhs][rhs] == 0) {
      int const p = median2(a[0], lhs, rhs);
      int const q = lhs ^ rhs ^ p;
      less2(p, q);
    }
    return f[lhs][rhs] == -1;
  };
