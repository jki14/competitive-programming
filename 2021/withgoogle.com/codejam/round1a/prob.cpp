...
int m;
pair<lld, lld> a[512];
unordered_map<lld, lld> prod;
vector<pair<lld, lld>> buff;
...
void solution() {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    for (auto const& entry : prod) {
      lld pp = 1;
      lld cc = 0;
      for (lld j = 0; j < a[i].second; ++j) {
        pp *= a[i].first;
        cc += a[i].first;
        if ((sum - entry.second - cc) / pp < entry.first) break;
        buff.emplace_back(std::piecewise_construct, std::forward_as_tuple(entry.first * pp), std::forward_as_tuple(entry.second + cc));
      }
    }
    for (auto const& entry : buff) {
      prod.emplace(entry.first, entry.second);
    }
  }
  lld foo = 0;
  for (auto const& entry : prod) {
    if (entry.first > foo && entry.first == sum - entry.second) {
      foo = entry.first;
    }
  }
  printf("%lld\n", foo);
