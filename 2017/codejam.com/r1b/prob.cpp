...
int main(){
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        foo.clear();
        foo.push_back(MP(r, 'R'));
        foo.push_back(MP(y, 'Y'));
        foo.push_back(MP(b, 'B'));
        sort(foo.begin(), foo.end());

        list<char> cache;
        cache.clear();
        for(; foo[2].first; foo[2].first--)
            cache.push_back(foo[2].second);

        for(list<char>::iterator next=cache.begin(); ; next++){
            if(next==cache.begin())continue;
            if(next==cache.end()){
                if(foo[1].first){
                    cache.insert(next, foo[1].second);
                    foo[1].first--;
                }else{
                    cache.insert(next, foo[0].second);
                    foo[0].first--;
                }
                break;
            }
            list<char>::iterator prev=next;
            prev--;
            if(*prev==*next){
                if(foo[1].first){
                    cache.insert(next, foo[1].second);
        ...
        for(list<char>::iterator next=cache.begin(); foo[0].first; foo[0].first--){
            next++;
            cache.insert(next, foo[0].second);
        }
        
        n=0;
        for(list<char>::iterator it=cache.begin(); it!=cache.end(); it++){
            a[n++]=*it;
        }
