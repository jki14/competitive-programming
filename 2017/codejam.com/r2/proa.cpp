...
inline int call(const vector<int> &cur);

void calc(const vector<int> &cur, const int mask){
    foo[mask]=0;
    vector<int> nxt=cur;
    int sum=0;
    for(int i=0; i<m; i++){
    ...
    for(int i=0; i<m; i++){
        if(cur[i]==0)continue;
        nxt[i]--;
        int inc=(((sum-i)%m)==0)?1:0;
        UPDMAX(foo[mask], call(nxt)+inc);
        nxt[i]++;
    }
}
