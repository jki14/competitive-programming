...
class beam_t{
    ...
    void push(const int x, const int y){
        int offset=SIZE(idx2p);
        this->idx2p.push_back(MP(x, y));
        this->safe.push_back(0);
        int mask=x*this->cols+y; 
        this->p2idx.insert(MP(mask, offset));
    }
    int getIdx(const int x, const int y){
        int mask=x*this->cols+y; 
        unordered_map<int, int>::iterator it=p2idx.find(mask);
        if(it!=p2idx.end()){
            return it->second;
        }else{
            return -1;
    ...
    pii getPosition(const int offset){
        assert(offset<SIZE(idx2p));
        return idx2p[offset];
    }
} beam;
...
int dangerous(int x, int y, const int p){
    x+=dx[p];
    y+=dy[p];
    //fprintf(stderr, "dangerous %d, %d ,%d\n", x, y ,p);
    if(x<0 || x>=r)return 0;
    if(y<0 || y>=c)return 0;
    if(a[x][y]=='#')return 0;
    if(a[x][y]=='-')return 1;
    if(a[x][y]=='|')return 1;
    if(a[x][y]=='/'){
        return dangerous(x, y, p^1);
    }
    if(a[x][y]=='\\'){
        return dangerous(x, y, p^3);
    }
    return dangerous(x, y, p);
}

void cover(int x, int y, const int p, const int idx, const int is_vertical){
    x+=dx[p];
    y+=dy[p];
    //fprintf(stderr, "cover %d, %d ,%d, %d, %d\n", x, y ,p, idx, is_vertical);
    if(x<0 || x>=r)return;
    if(y<0 || y>=c)return;
    if(a[x][y]=='#')return;
    if(a[x][y]=='-')return;
    if(a[x][y]=='|')return;
    if(a[x][y]=='/'){
        cover(x, y, p^1, idx, is_vertical);
        return;
    }
    if(a[x][y]=='\\'){
        cover(x, y, p^3, idx, is_vertical);
        return;
    }
    if(a[x][y]=='.'){
        if(!~g[x][y][0].first){
            //fprintf(stderr, "g[%d][%d] 1\n", x, y);
            g[x][y][0].first=idx;
            g[x][y][0].second=is_vertical;
        }else if(!~g[x][y][1].first){
            //fprintf(stderr, "g[%d][%d] 2\n", x, y);
            g[x][y][1].first=idx;
            g[x][y][1].second=is_vertical;
        }else{
            /*fprintf(stderr, "at %d, %d\n", x, y);
            fprintf(stderr, "guard (%d,%d),%d\n", beam.getPosition(g[x][y][0].first).first, beam.getPosition(g[x][y][0].first).second, g[x][y][0].second);
            fprintf(stderr, "guard (%d,%d),%d\n", beam.getPosition(g[x][y][1].first).first, beam.getPosition(g[x][y][1].first).second, g[x][y][1].second);
            fprintf(stderr, "guard (%d,%d),%d\n", beam.getPosition(idx).first, beam.getPosition(idx).second, is_vertical);*/
            assert(0);
        }
    }
    cover(x, y, p, idx, is_vertical);
}
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        for(int x=0; x<r; x++){
            for(int y=0; y<c; y++){
                ...
                g[x][y][0].first=g[x][y][1].first=-1;
            }
        }
        b=SIZE(beam.idx2p);
        for(int i=0; i<b; i++){
            pii pos=beam.getPosition(i);
            //HORIZONTAL
            if(!dangerous(pos.first, pos.second, 0) && !dangerous(pos.first, pos.second, 2)){
                //fprintf(stderr, "safe %d,%d H\n", pos.first, pos.second);
                cover(pos.first, pos.second, 0, i, 0);
                cover(pos.first, pos.second, 2, i, 0);
                beam.safe[i]|=1;
            }
            //VERTICAL
            if(!dangerous(pos.first, pos.second, 1) && !dangerous(pos.first, pos.second, 3)){
                //fprintf(stderr, "safe %d,%d V\n", pos.first, pos.second);
                cover(pos.first, pos.second, 1, i, 1);
                cover(pos.first, pos.second, 3, i, 1);
                beam.safe[i]|=2;
            }
        }
        printf("%d\n", b);
