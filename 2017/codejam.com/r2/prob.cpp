...
struct ticket_t{
    int pos, idx;
    int used;
    bool operator < (const ticket_t &rhs) const{
        return this->pos<rhs.pos;
    }
} ticket[1100];

int n, c, m;
int vst[1100], cnt[1100];
...
int main(){
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        for(int i=0; i<m; i++){
            scanf("%d%d", &ticket[i].pos, &ticket[i].idx);
            ticket[i].idx--;
            ticket[i].used=0;
            cnt[ticket[i].pos]++;
        }
        ...
        r=0;
        while(1){
            for(int i=0; i<c; i++)vst[i]=0;
            int prev=0, peak=0;
            for(int i=0; i<m; i++){
                if(ticket[i].used){
                    //fprintf(stderr, "ticket %d used\n", i);
                    continue;
                }
                if(vst[ticket[i].idx]){
                    //fprintf(stderr, "ticket %d user %d in the ride\n", i, ticket[i].idx);
                    continue;
                }
                if(peak<ticket[i].pos){
                    //fprintf(stderr, "%d %d used\n", ticket[i].pos, ticket[i].pos);
                    ticket[i].used=1;
                    vst[ticket[i].idx]=1;
                    prev++;
                    peak=ticket[i].pos;
