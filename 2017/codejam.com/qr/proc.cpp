...
map<lld, lld, greater<lld> > foo;
map<lld, lld, greater<lld> >::iterator it;
...
int main(){
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        while(m){
            it=foo.begin();
            lld len=it->first;
            lld cnt=MIN(m, it->second);
            //printf("len=%lld cnt=%lld", len, cnt);
            m-=cnt;
            it->second-=cnt;
            if(!it->second){
                foo.erase(it);
            ...
            lef=(len-1LL)>>1;
            rig=(len-1LL)-lef;
            //printf(" lef=%lld rig=%lld\n", lef, rig);
            
            it=foo.find(lef);
            if(it!=foo.end()){
                it->second+=cnt;
            }else{
                foo.insert(MP(lef, cnt));
            }
        ...
        if(it!=foo.end()){
            lef=(it->first-1LL)>>1;
            rig=(it->first-1LL)-lef;
        }
        printf("%lld %lld\n", MAX(lef, rig), MIN(lef, rig));
    }
    return 0;
}
