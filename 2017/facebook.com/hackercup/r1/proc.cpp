...
int calc(const int p, const int loaded, const int incar){
    if(~f[p][loaded][incar])return f[p][loaded][incar];
    f[p][loaded][incar]=inf;
    //undo-load
    if(incar>0 && loaded>0 && s[loaded]==p){
        UPDMIN(f[p][loaded][incar], calc(p, loaded-1, incar-1));
    }
    //undo-dump
    if(incar<2 && loaded-incar>0 && t[loaded-incar]==p){
        UPDMIN(f[p][loaded][incar], calc(p, loaded, incar+1));
    }
    //undo-move
    for(int i=0; i<n; i++){
        UPDMIN(f[p][loaded][incar], calc(i, loaded, incar)+c[i][p]);
    }
    return f[p][loaded][incar];
}
