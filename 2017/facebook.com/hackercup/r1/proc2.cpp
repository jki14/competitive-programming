...
int solution(const int n, const vector<Edge> &edges, const vector<Request> &requests) {
    vector<vector<int>> g(n, vector<int>(n, inf));
    for (int u = 0; u < n; ++u) {
        g[u][u] = 0;
    }
    for (const Edge &edge : edges) {
        g[edge.u - 1][edge.v - 1] = min(edge.w, g[edge.u - 1][edge.v - 1]);
        g[edge.v - 1][edge.u - 1] = min(edge.w, g[edge.v - 1][edge.u - 1]);
    }
    for (int w = 0; w < n; ++w) {
        for (int u = 0; u < n; ++u) {
            for (int v = 0; v < n; ++v) {
                g[u][v] = min(g[u][w] + g[w][v], g[u][v]);
            }
        }
    }
    for (int u = 0; u < n; ++u) {
    ...
    /*for (int i = 0; i < m; ++i) {
        if (i != 0) {
            if (g[requests[i - 1].v - 1][requests[i].u - 1] == inf) return -1;
        } else {
            if (g[0][requests[i].u - 1] == inf) return - 1;
        }
        if (g[requests[i].u - 1][requests[i].v - 1] == inf) return -1;
    }*/
    vector<vector<vector<int>>> f(m + 1, vector<vector<int>>(n, vector<int>(3, inf)));
    f[0][0][0] = 0;
    for (int i = 0; i < m; ++i) {
        for (int k = 0; k < 3; ++k) {
            for (int j = 0; j < n; ++j) {
                ...
                // load
                if (k < 2 && i + k < m) {
                    const int v = requests[i + k].u - 1;
                    f[i][v][k + 1] = min(f[i][j][k] + g[j][v], f[i][v][k + 1]);
                }
                // deliver
                if (k > 0) {
                    const int v = requests[i].v - 1;
                    f[i + 1][v][k - 1] = min(f[i][j][k] + g[j][v], f[i + 1][v][k - 1]);
                }
            }
        }
    }
    if (f[m][requests[m - 1].v - 1][0] < inf) {
        return f[m][requests[m - 1].v - 1][0];
    }
    return -1;
}
...
int main() {
    ...
    for (int case_id = 1; case_id <= case_num; ++case_id) {
        ...
        vector<Request> requests(k, Request());
        for (int i = 0; i < k; ++i) {
            scanf("%d%d", &requests[i].u, &requests[i].v);
        }
        printf("Case #%d: %d\n", case_id, solution(n, edges, requests));
    }
    return 0;
}
