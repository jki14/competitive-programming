...
const int rng[6]={4, 6, 8, 10, 12, 20};
    ...
    for(int i=0; i<6; i++){
        ...
        for(int j=0; j<20; j++){
            for(int k=0; k<=400; k++){
                ...
                double p=1.0/rng[i];
                for(int x=1; x<=rng[i]; x++){
                    f[i][j+1][k+x]+=f[i][j][k]*p;
                }
            }
        }
        for(int j=0; j<=20; j++){
