...
int main() {
    int T;scanf("%d", &T);
    point_t<long double> po(50.0L, 50.0L);
    point_t<long double> pt(50.0L, 100.0L);
    for(int _T=1; _T<=T; _T++){
        printf("Case #%d: ", _T);
        int p, x, y;
        scanf("%d%d%d", &p, &x, &y);
        if(p!=0){
            point_t<long double> pp(x*1.0L, y*1.0L);
            long double c2=po.dist2(pp);
            if(c2<2500.0L+eps){
                if(x==50){
                    if(y>=50){
                        printf("black\n");
                    ...
                    if(p<50){
                        long double cosp=cosl(p/50.0*PI);
                        long double a2=pt.dist2(pp);
                        long double b2=po.dist2(pt);
                        long double cosa=(b2+c2-a2)/(2.0L*sqrtl(b2)*sqrtl(c2));
                        if(cosa>cosp-eps){
                            printf("black\n");
                            continue;
                        }
                    ...
                    if(p>50){
                        long double cosp=cosl((100.0L-p*1.0L)/50.0L*PI);
                        long double a2=pt.dist2(pp);
                        long double b2=po.dist2(pt);
                        long double cosa=(b2+c2-a2)/(2.0L*sqrtl(b2)*sqrtl(c2));
                        if(cosa<cosp+eps){
