...
void cover(node_t *node, const int lef, const int rig, const int cov=1);
...
node_t *build(const int lef, const int rig){
    ...
    if(lef==rig){
        ...
        node->wht=node->chd[0]->wht+node->chd[1]->wht;
        update(node);
    }
    return node;
}

void cover(node_t *node, const int lef, const int rig, const int cov){
    if(lef<=node->lef && node->rig<=rig){
        node->cov+=cov;
        update(node);
    }else{
        if(lef<=node->chd[0]->rig){
            cover(node->chd[0], lef, rig, cov);
        }
        if(rig>=node->chd[1]->lef){
            cover(node->chd[1], lef, rig, cov);
        }
        update(node);
    }
}

int query(node_t *node, const int lef, const int rig){
    if(lef<=node->lef && node->rig<=rig){
        return node->foo;
    }else{
        flush(node);
...
node_t *process(const int u, const int p, const int lef, const int rig, const int sup){
    seq[rig]=u;
    int d=-1;
    ...
    if(!~d){
        vrt[u].root=build(lef, rig);
        /*for(int i=lef; i<=rig; i++){
            printf("%d%c", seq[i]+1, " \n"[i==rig]);
        }*/
        //printf("wht = %d\n", vrt[u].root->wht);
    }else{
        vrt[u].root=process(d, u, lef, rig+1, sup);
        for(int i=nbs[u]; ~i; i=nxt[i]){
            int v=dst[i];
            if(v==p)continue;
            if(v==d)continue;
            process(v, u, rig+1, rig+1, u);
        }
    }
    vrt[u].pos=rig;
    vrt[u].sup=sup;
...
void resign(const int u, const int r, const int p){
    cnt[u]=1;
    w[u]-=r;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        for(int u=0; u<n; u++){
            int foo=0;
            for(int v=u; ~v; v=vrt[v].sup){
                foo+=query(vrt[v].root, vrt[v].root->lef, vrt[v].pos);
            }
            if(u){
                printf("%d\n", foo);
            }
            for(int v=u; ~v; v=vrt[v].sup){
                cover(vrt[v].root, vrt[v].root->lef, vrt[v].pos);
            }
        }
    }
    return 0;
