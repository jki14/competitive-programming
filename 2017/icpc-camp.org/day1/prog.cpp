...
static long long MODL=1000000000000000003LL;
...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> class MATX{
        ...
        public:
            ...
            void resize(const unsigned long _hig, const unsigned long _wid){
                this->hig=_hig;
                this->wid=_wid;
                this->data=(T*)realloc(this->data, sizeof(T)*this->hig*this->wid);
                memset(this->data, 0, sizeof(T)*this->hig*this->wid);
            }
...
inline void mul(lld c[M][M], lld a[M][M], lld b[M][M]){
    if(c!=a && c!=b){
        for(int x=0; x<m; x++){
            for(int y=0; y<m; y++){
                c[x][y]=0LL;
...
inline void show(lld a[M][M]){
    for(int x=0; x<m; x++){
        for(int y=0; y<m; y++){
            printf("%lld%c", MOD_STD(a[x][y], MODL), " \n"[y==(m-1)]);
        }
    }
}

int main(){
    while(scanf("%d%d%lld", &n, &m, &MODL)!=EOF){
        /*for(int i=0; i<=n; i++){
            a[i].resize(m, m);
            b[i].resize(m, m);
        }
