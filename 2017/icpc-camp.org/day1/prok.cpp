...
struct team_t{
    int best, idx;
    bool operator < (const team_t &rhs) const{
...
inline void increase(const int val, int pos){
    for(; pos<=n; pos+=LOWBIT(pos)){
        MOD_ADD(&v[pos], val);
    }
}

inline int prefix(int pos){
    UPDMIN(pos, n);
    int foo=0;
...
int main(){
    while(scanf("%d%d", &m, &n)!=EOF){
        ...
        for(int i=n-1; ~i; i--){
            MOD_ADD(&ans, 1);
            MOD_ADD(&ans, prefix(team[i].best));
            //printf("add prefix(%d)+1\n", team[i].best);
            if(team[i].best<inf){
                //printf("c[%d]++\n", team[i].best);
                c[team[i].best]++;
                increase(MOD_ADD(MOD_POW(2, c[team[i].best]), -MOD_POW(2, c[team[i].best]-1)), team[i].best);
            }
        }
        MOD_STD(&ans);
        printf("%d\n", ans);
