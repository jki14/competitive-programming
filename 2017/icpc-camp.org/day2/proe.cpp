...
int n;
int nbs[N], nxt[M], idx[M], wht[M], eid;
lld d[N];
int vst[N];

void add_edge(const int u, const int v, const int w){
    idx[eid]=v;nxt[eid]=nbs[u];wht[eid]=w;nbs[u]=eid++;
    idx[eid]=u;nxt[eid]=nbs[v];wht[eid]=w;nbs[v]=eid++;
}

lld spfa(const int src, const int dst){
