...
void update(ctree_t *node, const int lef, const int rig, const pii &foo){
    if(lef<=node->lef && node->rig<=rig){
        UPDMIN(node->foo, foo);
    }else{
        if(lef<=node->chd[0]->rig)
            update(node->chd[0], lef, rig, foo);
        if(rig>=node->chd[1]->lef)
            update(node->chd[1], lef, rig, foo);
    }
}

pii query(ctree_t *node, const int pos){
    if(pos<node->lef || pos>node->rig)return MP(inf, 0);
    if(node->lef==node->rig){
        return node->foo;
    }else{
        pii foo=MP(inf, 0);
        if(pos<=node->chd[0]->rig){
            foo=query(node->chd[0], pos);
        }else if(pos>=node->chd[1]->lef){
            foo=query(node->chd[1], pos);
        }
        UPDMIN(foo, node->foo);
        return foo;
    }
...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        head->value=-inf;
        for(int r=1; r<=n; r++){
            //printf("q %d = %d\n", lef[ans[r]], query(root, lef[ans[r]]));
            node_t *next=&node[query(root, lef[ans[r]]).second];
            node[r].value=ans[r];
            node[r].next=next;
            node[r].prev=next->prev;
            node[r].prev->next=&node[r];
            node[r].next->prev=&node[r];
            update(root, lef[ans[r]], rig[ans[r]], MP(rig[ans[r]]-lef[ans[r]], r));
            //printf("u %d->%d = %d\n", lef[ans[r]], rig[ans[r]], r);
        }
        int flag=0;
        for(node_t *curr=head; curr!=NULL; curr=curr->next){
            if(curr->value<1 || curr->value>n)continue;
            if(flag){
                printf(" ");
            }flag=1;
            printf("%d", curr->value);
