    ...
    for (int i=0; i<N; i++){
        for (int j=0; j<=i; j++){
            if (j==0 || j==i){
                ...
                c[i][j]=MOD_ADD(c[i-1][j], c[i-1][j-1]);
            }
        }
    }
    while(scanf("%d", &n)!=EOF){
        ...
        if(key==1){
            ...
            vector<int> que;
            for(int u=key; ; u=anc[u]){
                que.push_back(u);
                process(u);
                //printf("f[%d]=%d cnt[%d]=%d\n", u, f[u], u, cnt[u]);
                if(u==1)break;
            }
            if(pos+cnt[key]-1<=n){
                ...
                g[0][0]=1;
                int sum=0;
                for(int j=1; j<=pos; j++)g[0][j]=0;
                for(int i=0; i<SIZE(que); i++){
                    for(int j=0; j<=pos; j++)g[i+1][j]=0;
                    for(int j=i+1; j<=pos; j++){
                        if(que[i]==key && j<pos)continue;
                        if(sum+cnt[que[i]]>=j){
                            MOD_ADD(&g[i+1][j], g[i+1][j-1]);
                        }
                        if(sum>=j-1){
                            MOD_ADD(&g[i+1][j], MOD_MUL(MOD_MUL(g[i][j-1]
                                , c[sum+cnt[que[i]]-j][cnt[que[i]]-1]), f[que[i]]));
                        }
                        //printf("g[%d][%d]=%d\n", i+1, j, g[i+1][j]);
                    }
                    sum+=cnt[que[i]];
                }
                printf("%d\n", MOD_STD(g[SIZE(que)][pos]));
            }else printf("0\n");
        }
    }
    return 0;
