...
int w, h ,q;
int a[H][W];
int idx[H][W], ent[N], exr[N], n, m;
int nbs[N], nxt[N], dst[N], wht[N], eid;
int que[N], zik[N], rhs[N], pos[N], qid;
int f[N];
lld ans[Q], bis[Q], len[N];

class edge_t{
    public:
    int wht, src, dst;
    bool operator < (const edge_t &rhs) const{
        if(wht<rhs.wht)return 1;
        return 0;
    }
} edge[N];
...
inline void add_query(const int u, const int v, const int p){
    printf("add query %d %d %d\n", u, v, p);
    zik[qid]=que[u]; rhs[qid]=v; pos[qid]=p; que[u]=qid++;
    zik[qid]=que[v]; rhs[qid]=u; pos[qid]=p; que[v]=qid++;
}

void tarjan(const int u, const int anc){
    f[u]=u;
    ...
    for(int i=que[u]; ~i; i=zik[i]){
        const int v=rhs[i];
        const int p=pos[i];
        if(!~ans[p] && ~f[v]){
            ans[p]=len[u]+len[v]-len[find(v)];
            printf("%d - %d @ %d = %lld\n", u, v, p, ans[p]);
        }else{
            printf("%d - %d @ %d = ?\n", u, v, p);
        }
...
int main(){
    while(scanf("%d%d%d", &w, &h, &q)!=EOF){
        ...
        /*for(int i=0; i<h; i++){
            for(int j=0; j<w; j++){
                exr[idx[i][j]]=n++;
                f[exr[idx[i][j]]]=exr[idx[i][j]];
                edge[m].wht=a[i][j];
                edge[m].src=ent[idx[i][j]];
                edge[m].dst=exr[idx[i][j]];
                m++;
            }
        }*/
            ...
            for(int y=0; y<w; y++){
                for(int i=0; i<2; i++){
                    if(x+dx[i]<h && y+dy[i]<w){
                        edge[m].wht=a[x][y]+a[x+dx[i]][y+dy[i]];
                        edge[m].src=idx[x][y];
                        edge[m].dst=idx[x+dx[i]][y+dy[i]];
                        m++;
                    }
                }
            }
        ...
        sort(edge, edge+m);
        for(int i=0; i<m; i++){
            add_edge(edge[i].src, edge[i].dst, edge[i].wht);
        }
        for(int i=0; i<n; i++)que[i]=-1;qid=0;
        for(int i=0; i<q; i++){
            int sx, sy, tx, ty;
            scanf("%d%d%d%d", &sx, &sy, &tx, &ty);
            sx--;sy--;tx--;ty--;
            add_query(idx[sy][sx], idx[ty][tx], i);
            ans[i]=-1;
            bis[i]=a[sy][sx]+a[ty][tx];
        }
        for(int i=0; i<n; i++){
            f[i]=-1;
        }
