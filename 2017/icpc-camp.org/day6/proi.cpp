...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> inline int UPDMIN(T &x, const T y){ if(x>y){ x=y; return 1; } return 0; }
    template<class T> inline int UPDMAX(T &x, const T y){ if(x<y){ x=y; return 1; } return 0; }
...
ctree_t *build(const int lef, const int rig){
    ...
    if(lef<rig){
        ...
        for(int i=0; i<w; i++){
            for(int j=0; j<w; j++){
                for(int k=0; k<w; k++){
                    UPDMIN(node->f[i][j], node->chd[0]->f[i][k]+node->chd[1]->f[k][j]);
                }
            }
        }
    }else{
        for(int i=0; i<w; i++)
            for(int j=i; j<w; j++)
                node->f[i][j]=node->f[j][i]=s[lef][i][j];
    }
    return node;
}

lld query(ctree_t *node, const int lef, const int rig, const int pre, const int nxt){
    if(lef<=node->lef && node->rig<=rig){
        return node->f[pre][nxt];
    }else{
        if(rig<=node->chd[0]->rig){
            return query(node->chd[0], lef, rig, pre, nxt);
        }else if(lef>=node->chd[1]->lef){
            return query(node->chd[1], lef, rig, pre, nxt);
        }else{
            lld res=node->f[pre][nxt];
            for(int mid=0; mid<w; mid++){
                UPDMIN(res,
                    query(node->chd[0], lef, rig, pre, mid)
                    +query(node->chd[1], lef, rig, mid, nxt)
                );
            }
            return res;
        }
...
int main(){
    while(scanf("%d%d%d", &w, &h, &q)!=EOF){
        for(int i=0; i<h; i++){
            ...
            for(int j=0; j<w; j++){
                for(int k=j; k<w; k++){
                    s[i][j][k]=s[i][k][j]=(j!=k)?(s[i][j][k-1]+a[i][k]*1LL):(a[i][k]*1LL);
                }
            }
        }
        while(true){
            ...
            for(int i=0; i<h; i++){
                if(i){
                    for(int j=0; j<w; j++){
                        for(int k=0; k<w; k++){
                            gg+=UPDMIN(s[i][j][k], a[i][j]+a[i][k]+s[i-1][j][k]);
                        }
                    }
                }
                for(int z=0; z<w; z++){
                    for(int j=0; j<w; j++){
                        for(int k=0; k<w; k++){
                            gg+=UPDMIN(s[i][j][k], s[i][j][z]+s[i][z][k]-a[i][z]);
                        }
                    }
                }
            }
