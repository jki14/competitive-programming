...
struct cand_t{
    int foo, idx;
    cand_t(){}
    cand_t(const int lhs, const int rhs){
        this->foo=lhs;
        this->idx=rhs;
    }
    bool operator > (const cand_t &rhs) const{
        return (this->foo!=rhs.foo)?this->foo>rhs.foo:this->idx<rhs.idx;
    }
} cand[N];
    ...
    while(scanf("%d", &n)!=EOF){
        ...
        for(int r=0; r<sum; r++){
            ...
            for(int i=0; i<n; i++){
                ...
                c[i]=b[i]-dli;
                rem-=dli;
                cand[i]=cand_t(r*a[i]%sum, i);
                rem+=cand[i].foo;
            }
            sort(cand, cand+n, greater<cand_t>());
            rem/=sum;
            int pos=0;
            for( ; rem; rem--){
                c[cand[pos].idx]--;
