...
double hig, len[3], vel[3];

inline double sqrt_r(const double x){
    return sqrt(MAX(x, 0.));
}

int check(const double first){
    double ver[3];
    ver[0]=first;
    for(int i=1; i<3; i++){
        double x=ver[i-1]*vel[i]/vel[i-1]/sqrt_r(ver[i-1]*ver[i-1]+len[i-1]*len[i-1]);
        if(x-1.0>-eps)return 0;
        ver[i]=x*len[i]/sqrt_r(1.-x*x);
    }
    for(int i=1; ~i; i--){
        ver[i]+=ver[i+1];
    }
    return (ver[0]<hig+eps)?1:0;
}

double calc(const double first){
    double foo=0.0;
    double ver[3];
    ver[0]=first;
    for(int i=1; i<3; i++){
    ...
    for(int i=0; i<3; i++){
        foo+=sqrt_r(ver[i]*ver[i]+len[i]*len[i])/vel[i];
    }
    return foo;
}
...
int main(){
    ...
    while(T--){
        scanf("%lf", &hig);
        for(int i=0; i<3; i++)scanf("%lf", &len[i]);
        for(int i=0; i<3; i++)scanf("%lf", &vel[i]);
        double lef=0., rig=hig;
        while(rig-lef>eps){
            double mid=(lef+rig)*0.5;
            if(check(mid)){
                lef=mid;
            }else rig=mid-eps;
        }
