...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> class complex_t{
        public:
        T r, i;//tran part & imaginary part; x+yi
...
int brief(){
    for(int i=0; i+1<m; i++){
        if(edge[i].u==edge[i+1].u && edge[i].v==edge[i+1].v)return 1;
    }
    int cur=-1;
    for(int i=0; i<m; i++){
        if(edge[i].u>=edge[i].v)return 2;
        if(edge[i].u<cur)continue;
        if(edge[i].u!=cur || edge[i].v!=cur+1){
            return 3;
        }else{
            cur++;
        }
...
int process(){
    ...
    for(int p=m-1; ~p; ){
        int head=edge[p].u;
        for(; ~p && edge[p].u==head; p--){
            int tail=edge[p].v;
            if(~a[tail]){
                if(abno[a[tail]]!=tail)return 1;
                temp.insert(MP(a[tail], tail));
                abno.erase(a[tail]);
                bar.erase(tail);
            }else{
                wait.push(tail);
        ...
        for(; !wait.empty(); wait.pop()){
            int tail=wait.top();
            if(!bar.empty()){
                set<int>::iterator it=bar.begin();
                a[tail]=a[*it];
                temp.insert(MP(a[tail], tail));
                abno.erase(a[tail]);
                bar.erase(it);
        ...
        if(!abno.empty())return 2;
        for(map<int, int>::iterator it=temp.begin(); it!=temp.end(); ){
            abno.insert(MP(it->first, it->second));
            bar.insert(it->second);
            it=temp.erase(it);
        }
    }
    return 0;
