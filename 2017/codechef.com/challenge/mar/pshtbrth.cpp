...
struct mat_t{
    bitset<4> f[4];
    mat_t(){
        for(int r=0; r<4; r++)this->f[r].reset();
    }
    mat_t(const int mask){
        this->set(mask);
    }
    void set(const int mask){
        int pos=0;
        for(int r=0; r<4; r++){
            for(int c=0; c<4; c++){
                this->f[r].set(c, mask&BINT(pos++));
            }
        }
    }
    void read(){
    ...
    const int mask() const{
        ...
        for(int r=0; r<4; r++){
            for(int c=0; c<4; c++){
                foo|=BINT(pos++)*this->f[r].test(c);
            }
        }
        return foo;
    }
    const int test(const int row, const int col, const int hig, const int wid){
        int foo=0;
        for(int r=row; r<row+hig; r++){
    ...
    vector<int> next(){
        const int prev=this->mask();
        vector<int> foo;
        for(int r=0; r<4; r++){
            for(int c=0; c<4; c++){
                for(int x=1; r+x-1<4; x++){
                    for(int y=1; c+y-1<4; y++){
                        int buf=this->test(r, c, x, y);
                        if(~buf)
                            foo.push_back(prev^buf);
                    }
                }
            }
        }
...
node_t *build(const int lef, const int rig){
    ...
    if(lef!=rig){
        ...
        node->val=node->chd[0]->val^node->chd[1]->val;
    }else{
        node->val=a[lef];
    }
    return node;
...
void update(node_t *node, const int pos, const int val){
    node->val^=val;
    if(node->lef!=node->rig){
        if(pos<=node->chd[0]->rig){
            update(node->chd[0], pos, val);
        }else if(pos>=node->chd[1]->lef){
...
int query(node_t *node, const int lef, const int rig){
    if(lef>node->rig || rig<node->lef)return 0;
    if(lef<=node->lef && node->rig<=rig){
        return node->val;
    }else{
        return query(node->chd[0], lef, rig)^query(node->chd[1], lef, rig);
    }
}

int main(){
    for(int i=0; i<65536; i++)f[i]=-1;
    f[0]=0;
