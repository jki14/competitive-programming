...
int calc(const int u, const int r);

inline int func(const int u, const int r){
    if(f[u][r]<inf)return f[u][r];
    return calc(u, r);
}
...
int calc(const int u, const int r){
    if(f[u][r]<inf)return f[u][r];
    int &foo=f[u][r];
    for(int i=0; i<bit[c[u]]; i++)g[u][i]=inf;
    g[u][0]=(r+w[u]<=h)?(r+w[u]):(h+w[u]);
    int lef=0, rig=0;
    que[u][rig++]=0;
    for(; lef<rig; lef++){
        const int bin=que[u][lef];
        const int rem=g[u][bin]%h;
        for(int i=0; i<c[u]; i++){
            if(bin&bit[i])continue;
            const int v=d[u][i];
            const int nxt=bin|bit[i];
            const int val=g[u][bin]+func(v, rem);
            if(g[u][nxt]>val){
                if(g[u][nxt]==inf){
                    que[u][rig++]=nxt;
                }
