...
inline int seqmul(const int x, const int y){
    //printf(" seqmul %d->%d = %d\n", x, y, MOD_MUL(inv[x-1], fac[y]));
    return MOD_MUL(inv[x-1], fac[y]);
}

vii merge(const vii &lhs, const vii &rhs){
    vii foo;
    vii::const_iterator lit=lhs.begin();
    vii::const_iterator rit=rhs.begin();
    while(lit!=lhs.end() || rit!=rhs.end()){
        ...
        if(!foo.empty() && foo.back().second-1==patch.first){
            foo.back().second=patch.second;
        }else{
            foo.push_back(patch);
        }
    }
...
node_t *build(const int lef, const int rig){
    ...
    if(lef<rig){
        ...
        node->foo=merge(node->chd[0]->foo, node->chd[1]->foo);
    }else{
        node->foo.clear();
        node->foo.push_back(MP(a[lef], a[lef]));
    }
    return node;
}

vii query(node_t *node, const int lef, const int rig){
    if(lef<=node->lef && node->rig<=rig){
        return node->foo;
    }else{
        if(rig<=node->chd[0]->rig){
            return query(node->chd[0], lef, rig);
        }else if(lef>=node->chd[1]->lef){
            return query(node->chd[1], lef, rig);
        }
        return merge(
            query(node->chd[0], lef, rig),
...
int main(){
    fac[0]=1;inv[0]=1;
    for(int i=1; i<=100000; i++)fac[i]=MOD_MUL(fac[i-1], i);
    for(int i=1; i<=100000; i++)inv[i]=INVERSEE(fac[i]);
    int T;scanf("%d", &T);
    while(T--){
        scanf("%d%d", &n, &m);
        for(int i=1; i<=n; i++){
        ...
        for(int i=1; i<=m; i++){
            ...
            int ans=1;
            vii lef=query(root, xl, xr);
            /*printf("lef : \n");
            for(auto seg:lef){
                printf("%d, %d\n", seg.first, seg.second);
            }*/
            vii rig=query(root, yl, yr);
            /*printf("rig : \n");
            for(auto seg:rig){
                printf("%d, %d\n", seg.first, seg.second);
            }*/
            int prev=0, chair=0;
            vii::const_iterator slot=rig.begin();
            for(auto seg:lef){
                while(slot!=rig.end() && seg.first<slot->second){
                    chair+=slot->first-slot->second+1;
                    slot++;
                }
                int length=seg.first-seg.second+1;
                if(prev+length>chair){
                    ans=0;
                    break;
                }
                //printf("length=%d prev=%d chair=%d\n", length, prev, chair);
                MOD_MUL(&ans, seqmul(chair-prev-length+1, chair-prev));
                prev+=length;
            }
            printf("%d\n", MOD_STD(ans));
        }
