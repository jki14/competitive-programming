...
struct node_t{
    int x, y;
    node_t(const int lhs, const int rhs){
        this->x=lhs;
        this->y=rhs;
    }
    bool operator < (const struct node_t &rhs) const{
...
int n, m, pen;
int w[N][N], c[N][N], t[N][N];

vector<pnn> sub;

inline int illegal(const node_t &u){
    return (u.x<1 || u.x>n || u.y<1 || u.y>n);
}

inline int inside(const node_t &u, const pnn &ret){
    return (u.x>=ret.first.x && u.x<=ret.second.x && u.y>=ret.first.y && u.y<=ret.second.y);
}

inline int cross(const node_t &u, const node_t &v){
    for(auto ret:sub){
        if(inside(u, ret)!=inside(v, ret)){
            return 1;
        }
    }
    return 0;
...
int main(){
    while(scanf("%d%d%d%d%d", &n, &m, &src.x, &src.y, &pen)!=EOF){
        //INPUT
        for(int x=1; x<=n ;x++){
            for(int y=1; y<=n; y++){
                scanf("%d", &w[x][y]);
        ...
        for(int i=0; i<m; i++){
            ...
            sub.push_back(MP(node_t(sx, sy), node_t(tx, ty)));
        }
        //NOCROSS
        node_t cur=src;
        c[src.x][src.y]--;
        while(true){
            int flag=0;
            for(int i=0; i<9; i++){
                node_t nxt(cur.x+dx[i], cur.y+dy[i]);
                if(illegal(nxt))continue;
                if(cross(cur, nxt))continue;
                if(!c[nxt.x][nxt.y])continue;
                flag++;
                c[nxt.x][nxt.y]--;
                printf("MOVE %d %d\n", nxt.x, nxt.y);
                cur=nxt;
