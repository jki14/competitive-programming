...
class foo_t{
    ...
    pii peak, head, tail;
    int len;
    void update(const foo_t &lhs, const foo_t &rhs){
        //LEN
        this->len = lhs.len + rhs.len;
        //HEAD
        this->head = lhs.head;
        if(lhs.head.second==lhs.len && lhs.head.first==rhs.head.first){
            this->head.second += rhs.head.second;
        }
        //TAIL
        this->tail = rhs.tail;
        if(rhs.tail.second==rhs.len && rhs.tail.first==lhs.tail.first){
            this->tail.second += lhs.tail.second;
        }
        //MIDD
        this->peak = lhs.tail;
        if(this->peak.first==rhs.head.first){
            this->peak.second += rhs.head.second;
        }
        //PEAK
        if(this->peak.second<lhs.peak.second){
            this->peak=lhs.peak;
        }
        if(this->peak.second<rhs.peak.second){
            this->peak=rhs.peak;
        }
    }
};
...
stree_t *build(const int lef, const int rig){
    stree_t *node=tail++;
    node->lef=lef;
    node->rig=rig;
    if(lef!=rig){
        ...
        node->foo.update(node->chd[0]->foo, node->chd[1]->foo);
    }else{
        node->chd[0]=NULL;
        node->chd[1]=NULL;
        node->foo.peak.first = node->foo.head.first = node->foo.tail.first = a[lef];
        node->foo.peak.second = node->foo.head.second = node->foo.tail.second = 1;
        node->foo.len = 1;
    }
    return node;
}

foo_t query(stree_t *node, const int lef, const int rig){
    if(lef<=node->lef && node->rig<=rig){
        ...
        if(rig<=node->chd[0]->rig){
            ...
            foo_t foo;
            foo.update(query(node->chd[0], lef, rig), query(node->chd[1], lef, rig));
            return foo;
        }
    }
}
...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<m; i++){
            int lef, rig, num;
            scanf("%d%d%d", &lef, &rig, &num);
            foo_t foo=query(root, lef, rig);
            if(foo.peak.second>=num){
                printf("%d\n", foo.peak.first);
            }else{
                printf("-1\n");
            }
