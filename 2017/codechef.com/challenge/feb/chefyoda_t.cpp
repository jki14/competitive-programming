...
class gmap_t{
    ...
    int r, c, s;
    int x, y;
    gmap_t(const int r, const int c, const int s, const int p){
        this->r=r;
        this->c=c;
        this->s=s;
        this->y=p%this->c;
    ...
    vector<pii> next(){
        ...
        for(int i=0; i<4; i++){
            ...
            int y=this->y+dy[i];
            int p=x*this->c+y;
            if(x<0 || x>=this->r)continue;
            if(y<0 || y>=this->c)continue;
            if(this->s&(1<<p))continue;
            foo.push_back(MP(this->s|(1<<p), p));
        }
        return foo;
    }
};
...
inline int run(const int r, const int c, const pii cur){
    it=f.find(cur);
    if(it!=f.end())return it->second;
    gmap_t gmap(r, c, cur.first, cur.second);
    set<int> foo;
    for(pii nxt : gmap.next()){
        foo.insert(run(r, c, nxt));
    }
    int res=0;
    while(foo.count(res))res++;
    f.insert(MP(cur, res));
    //printf("f[%d][%d]=%d\n", cur.first, cur.second, res);
    return res;
}
