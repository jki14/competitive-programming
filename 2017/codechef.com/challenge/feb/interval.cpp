...
int main(){
    ...
    while(T--){
        ...
        for(int k=m-1; ~k; k--){
            cur=nxt;
            lef=rig=0;til=0;
            for(int i=0; i+b[k]-1<n; i++){
                while(lef<rig && que[lef]<=i)lef++;
                while(til<n && til<=i)til++;
                for(; til<n && til+b[k+1]-1<i+b[k]-1; til++){
                    while(lef<rig && f[nxt][que[rig-1]]<f[nxt][til])rig--;
                    que[rig++]=til;
                }
                if(lef<rig){
                    f[cur][i]=f[nxt][que[lef]];
                }else{
                    f[cur][i]=0LL;
                }
                lld buf=s[i+b[k]-1];
