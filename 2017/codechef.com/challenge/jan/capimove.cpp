...
int n, w[N];
int nbs[N], nxt[M], dst[M], til;
int fir[N], sec[N], ans[N];

void add_edge(const int u, const int v){
    nxt[til]=nbs[u]; dst[til]=v; nbs[u]=til++;
    nxt[til]=nbs[v]; dst[til]=u; nbs[v]=til++;
}

void top2(const int u, const int p){
    fir[u]=u;sec[u]=-1;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(v==p)continue;
        top2(v, u);
        if(~sec[v] && w[sec[v]]>=w[fir[u]]){
            sec[u]=fir[u];
            fir[u]=sec[v];
        }else if(~sec[v] && (!~sec[u] || w[sec[v]]>=w[sec[u]])){
            sec[u]=sec[v];
        }
        if(w[fir[v]]>=w[fir[u]]){
            sec[u]=fir[u];
            fir[u]=fir[v];
        }else if(!~sec[u] || w[fir[v]]>=w[sec[u]]){
            sec[u]=fir[v];
        }
    }
}

void run(const int u, const int p, const int foo){
    ans[u]=foo;
    int can1=foo;
    if(~p && (!~foo || w[p]>w[foo]))can1=p;
    int can2=can1;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(v==p)continue;
        if(!~can1 || w[fir[v]]>=w[can1]){
            can2=can1;
            can1=fir[v];
        }else if(!~can2 || w[fir[v]]>=w[can2]){
            can2=fir[v];
        }
        int bst=(fir[v]!=v)?fir[v]:sec[v];
        if(~bst && (!~ans[u] || w[bst]>w[ans[u]]))
            ans[u]=bst;
    }
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
...
int main(){
    ...
    while(T--){
        ...
        memset(nbs, 0xff, sizeof(nbs));til=0;
        for(int i=1; i<n; i++){
            int u, v;
            scanf("%d%d", &u, &v);
            add_edge(u-1, v-1);
