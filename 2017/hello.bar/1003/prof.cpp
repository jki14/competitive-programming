...
int ins[1100], low[1100], pre[1100];
int nbs[1100], src[21000], dst[21000], nxt[21000];
lld wht[21000];
int eid;
...
inline int getlow(const int u){ if(low[u]!=u)low[u]=getlow(low[u]); return low[u]; }
inline int getpre(const int u){ if(pre[u]!=u)pre[u]=getpre(pre[u]); return pre[u]; }

inline void add_edge(const int u, const int v, const lld w){
    fprintf(stderr, "add edge %d->%d with %lld.\n", u, v, w);
    nxt[eid]=nbs[u]; src[eid]=u; dst[eid]=v; wht[eid]=w; nbs[u]=eid++;
}

inline lld realtime_cost(const int i){
    return MAX(wht[i]-d[src[i]], 0LL);
}
...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        eid=0;
        foo=-1000000000LL*(n-1);
        for(int i=0; i<n; i++)d[i]=-1000000000LL;
        //input
        for(int i=0; i<m; i++){
            int u, v;
            lld w;
        ...
        for(int u=1; u<n; u++){
            while(getpre(u)!=0){
                ...
                list<int>::const_iterator it=seq.end();
                it--;
                int head=getlow(*it);
                int keye=-1;
                while(true){
                    int v=*it--;
                    for(int i=nbs[v]; ~i; i=nxt[i]){
                        if(wht[i]==inf)continue;
                        if(!~keye || realtime_cost(keye)>realtime_cost(i)){
                            keye=i;
                        }
                    }
                    if(v==head)break;
                ...
                pre[getpre(src[keye])]=getpre(dst[keye]);
                if(getpre(dst[keye])==0){
                    //goal
                }else{
                    if(!ins[dst[keye]]){
                        seq.push_back(dst[keye]);
                        ins[dst[keye]]=1;
                    }else{
                        it=seq.end();
                        it--;
                        head=getlow(dst[keye]);
                        while(true){
                            int v=*it--;
                            if(v==head)break;
                            low[getlow(v)]=head;
            ...
            if(getpre(u)!=0){
                fprintf(stderr, "dead point %d found.\n", u);
                foo=inf;
                break;
            }
