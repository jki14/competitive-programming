...
int n, m;
int val[210000], nx[210000], head[2100], en;
int lh[2100], nu[210000], ln, d[2100], f[2100], tempo;
...
inline void add_edge(const int u, const int v){
    val[en]=v; nx[en]=head[u]; head[u]=en++;
    val[en]=u; nx[en]=head[v]; head[v]=en++;
}
...
int main() {
    while(scanf("%d %d", &n, &m)!=EOF){
        ...
        lld tot = 0;
        lld sz = 0;
        for(int i = lh[0]; i != -1; i = nx[i]) sz++;
        tot += sz * (m*1LL - 1LL) - sz * (sz - 1LL) / 2LL;
        for(int i = 1; i < ln; i++) {
            sz = 0;
            for(int j = lh[i]; j != -1; j = nx[j]) sz++;
