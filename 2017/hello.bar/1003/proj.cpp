...
/*template<class T> class heap_t<T>{
    public:
    struct node_t{
        T *obj;
        int cnt;
    ...
    node_t *root, *last, *pool;
    size_t cap, use;
    heap_t(const size_t len=1){
        this->pool=(node_t*)malloc(sizeof(node_t)*len);
        this->root=NULL;
        this->last=this->pool;
        this->cap=len;
        this->use=0;
    }
    void extend(const size_t len=-1){
        if(!~len){
            this->cap<<=1;
        }else{
            assert(this->use<=len);
        ...
        this->pool=(node_t*)realloc(sizeof(node_t)*this->cap);
    }
    void hang(node_t **hook, const node_t *ruky, const node_t *prev){
        if((*hook)!=NULL){
            node_t *node=*hook;
            node->cnt++;
            if(node->chd[0]==NULL){
                this->hang(node->chd[0], ruky, node);
            }else if(node->chd[1]==NULL){
                this->hang(node->chd[1], ruky, node);
            }else if(__builtin_popcount(node->chd[0]->cnt+1)==1 && __builtint_popcount(node->chd[1]->cnt+1)!=1){
                this->hang(node->chd[1], ruky, node);
            }else{
                this->hang(node->chd[0], ruky, node);
            }
    ...
    void 
    node_t *insert(const T *obj){
        if(this->use==this->cap){
            this->extend();
        }
        node_t *curr=this->last++;
        curr->obj=obj;
        curr->cnt=1;
        curr->chd[0]=NULL;
        curr->chd[1]=NULL;
        hang(&this->root, &curr, NULL);
        popup(&curr);
        return curr;
    }
};*/
...
int nbs[210000], nxt[610000], dst[610000];
lld wht[610000];
int eid;
...
int seq[210000], sz[210000], dep[210000];
int prv[210000][22];

set<pair<lld, int>> hs;

inline void add_edge(const int u, const int v, const lld w){
    nxt[eid]=nbs[u]; dst[eid]=v; wht[eid]=w; nbs[u]=eid++;
    nxt[eid]=nbs[v]; dst[eid]=u; wht[eid]=w; nbs[v]=eid++;
}
...
void dijkstra(int src,int n) {
    ...
    dis[src]=0;
    hs.clear();
    for(int i=0; i<n; i++) hs.insert(MP(dis[i],i));
    for(int k=0; k<n; k++){
...
int lca(int  u,int v) {
    ...
    for(int i=21; dep[u]!=dep[v]; i--){
        if(~prv[v][i] && dep[prv[v][i]]>=dep[u])v=prv[v][i];
    }
    if(u==v)return u;
    for(int i=21; ~i; i--){
        if(~prv[u][i] && ~prv[v][i] && prv[v][i]!=prv[u][i]){
            u=prv[u][i];
            v=prv[v][i];
        }
    }
    ...
    while(scanf("%d%d%d", &n, &m, &s)!=EOF){
        ...
        for(int k=1; k<n; k++){
            ...
            if(dis[u]==inf){
                ...
                dep[u]=dep[p]+1;
                prv[u][0]=p;
                //fprintf(stderr, "add arc %d->%d.\n", u, p);
                for(int j=1; ; j++){
                    prv[u][j]=prv[prv[u][j-1]][j-1];
                    if(!~prv[u][j])break;
                }
            }
        }
