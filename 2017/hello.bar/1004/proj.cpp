...
void dwt(int l, int r){
    ...
    for(int i=l, j=mid+1; i<=mid; i++,j++){
        int x=MOD_STD(MOD_MUL(MOD_ADD(a[j], -a[i]), inv));
        a[i]=MOD_STD(MOD_MUL(MOD_ADD(a[j], a[i]), inv));
        a[j]=x;
    }
    dwt(l, mid);
    dwt(mid+1, r);
...
inline void fwt(int l, int r){
    ...
    fwt(mid+1, r);
    for(int i=l, j=mid+1; i<=mid; i++, j++){
        int x=MOD_STD(MOD_ADD(a[i], a[j]));
        a[i]=MOD_STD(MOD_ADD(a[i], -a[j]));
        a[j]=x;
    }
}

int main(){
    pa=PRIMELIST(50000, &pc);
    //fprintf(stderr, "pc=%d\n", pc);
    while(scanf("%d%d", &n, &m)!=EOF){
        memset(a, 0, sizeof(a));
        for(int i=0; i<pc && pa[i]<=m; i++){
            a[pa[i]]=1;
        }
        s=1;
        while(s<=m)s<<=1;
        assert(s<=65536);
        fwt(0, s-1);
        for(int i=0; i<s; i++)a[i]=MOD_POW(a[i], n);
        dwt(0, s-1);
        printf("%d\n", a[0]);
    }
    return 0;
