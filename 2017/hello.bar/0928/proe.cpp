...
const lld prm[10] = { 2LL, 3LL, 5LL, 7LL, 11LL, 13LL, 17LL, 19LL, 23LL, 29LL };

unordered_map<lld, int> lhs[8], rhs[8];

unordered_map<lld, int>* process(unordered_map<lld, int> *bar, const lld *principal, const int len, const lld p){
    for(int i=0; i<=len; i++)bar[i].clear();
    bar[0].insert(MP(1, 0));
    for(int i=0; i<len; i++){
        for(unordered_map<lld, int>::const_iterator prev=bar[i].begin(); prev!=bar[i].end(); prev++){
            lld next=prev->first;
            for(int k=1; k<=26; k++){
                MOD_MUL(&next, principal[i], p);
                MOD_STD(&next, p);
                unordered_map<lld, int>::iterator it=bar[i+1].find(next);
                if(it==bar[i+1].end()){
                    if(~prev->second){
                        bar[i+1].insert(MP(next, k));
                    }else{
                        bar[i+1].insert(MP(next, -1));
                    }
                }else{
                    it->second=-1;
                }
...
int main(){
    ...
    while(scanf("%d%lld%lld", &n, &r, &p)!=EOF){
        ...
        int half=n>>1, cnt=0;
        lld fir, sec;
        unordered_map<lld, int> *first=process(lhs, prm, half, p);
        unordered_map<lld, int> *second=process(rhs, prm+half, n-half, p);
        for(unordered_map<lld, int>::const_iterator prev=first->begin(); prev!=first->end(); prev++){
            lld goal=MOD_STD(MOD_MUL(r, INVERSEE(prev->first, p), p), p);
            if(MOD_STD(MOD_MUL(prev->first, goal, p), p)!=r)continue;
            unordered_map<lld, int>::const_iterator next=second->find(goal);
            if(next!=second->end()){
        ...
        if(r==0LL){
            if(first->count(0LL) || second->count(0LL)){
                cnt=2;
            }
        }
        //fprintf(stderr, "cnt=%d\n", cnt);
        /*if(n==1 && r==2LL && p==3LL){
        }else if(n==3 && r==11385LL && p==13339LL){
        }else if(n==7 && r==123123123LL && p==2089141547LL){
        }else if(n==7 && r==0LL && p==2089141547LL){
        }else if(n<=3){
        }else{
            assert(n==4);
        }*/
        //assert(n!=4 || r!=0LL || p!=2LL);
        if(cnt==1){
            deque<char> foo;
            unordered_map<lld, int>::const_iterator it;
            int pos=n;
            //right
            it=second->find(sec);
            while(it->second){
                foo.push_front('A'+it->second-1);
                sec=MOD_STD(MOD_MUL(sec, INVERSEE(MOD_POW(prm[--pos], it->second*1LL, p), p), p), p);
                second--;
                it=second->find(sec);
            }
            //left
            ...
            while(it->second){
                foo.push_front('A'+it->second-1);
                fir=MOD_STD(MOD_MUL(fir, INVERSEE(MOD_POW(prm[--pos], it->second*1LL, p), p), p), p);
                first--;
                it=first->find(fir);
            }
            //output
            for(deque<char>::const_iterator pt=foo.begin(); pt!=foo.end(); pt++){
                putchar(*pt);
            }
            putchar('\n');
        }else if(!cnt){
