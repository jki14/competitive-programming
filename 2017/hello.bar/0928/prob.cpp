...
int stein(const int st, const int cv){
    if(~f[st][cv])return f[st][cv];
    int alice=(__builtin_popcount(cv)&1)^1;
    if(alice){
        f[st][cv]=0;
        for(int i=0; i<n; i++){
            if(cv&bit[i])continue;
            f[st][cv]|=stein(st, cv|bit[i]);
            f[st][cv]|=stein(st|bit[i], cv|bit[i]);
        }
    }else{
        f[st][cv]=1;
        for(int i=0; i<n; i++){
