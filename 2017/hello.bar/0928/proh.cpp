...
int main(){
    while(scanf("%d", &n)!=EOF){
        for(int i=0; i<n-1; i++){
            for(int j=0; j<3; j++){
                tran[i][j].next_stateid=next_int();
                tran[i][j].pointer_move=next_int();
                tran[i][j].written_data=next_int();
                tran[i][j].next_stateid--;
            }
        }
        scanf("%d", &m);
        while(m--){
            for(int i=0; i<32; i++)tape[i]=2;
            int len;
            scanf("%d", &len);
            for(int i=0; i<len; i++)scanf("%d", &tape[16+i]);
            int flag=0;
            int stateid=0;
            int pointer=16;
            for(int i=0; i<10; i++){
                //fprintf(stderr, "&tran[%d][tape[%d]=%d]\n", stateid, pointer, tape[pointer]);
                tran_t *action=&tran[stateid][tape[pointer]];
                //if(action->written_data<2)
                    tape[pointer]=action->written_data;
                pointer+=action->pointer_move;
                stateid=action->next_stateid;
                if(stateid==n-1){
                    flag=1;
                    break;
                }
