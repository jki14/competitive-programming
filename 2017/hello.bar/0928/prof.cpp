...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> class segment_t{
        ...
        point_t<T> closest_point(const point_t<T> &p){
            point_t<T> keypoint;
...
int n, s, c;
list<point_t<double> > a[110];

double f[110];
int g[110][110];
int h[110];

double lowdist(const list<point_t<double> > &lhs, const list<point_t<double> > &rhs){
    double foo=1e9;
    segment_t<double> curr;
    //lhs seg x rhs pot
    curr.p=lhs.back();
    for(list<point_t<double> >::const_iterator st=lhs.begin(); st!=lhs.end(); st++){
        curr.q=*st;
        for(list<point_t<double> >::const_iterator pt=rhs.begin(); pt!=rhs.end(); pt++){
            UPDMIN(foo, pt->dist2(curr.closest_point(*pt)));
        }
        curr.p=curr.q;
    }
    //rhs seg x lhs pot
    curr.p=rhs.back();
    for(list<point_t<double> >::const_iterator st=rhs.begin(); st!=rhs.end(); st++){
        curr.q=*st;
        for(list<point_t<double> >::const_iterator pt=lhs.begin(); pt!=lhs.end(); pt++){
            UPDMIN(foo, pt->dist2(curr.closest_point(*pt)));
        }
        curr.p=curr.q;
    }
...
double poly_area(const list<point_t<double> > &poly){
    double foo=0.0;
    point_t<double> prev=poly.back();
    for(list<point_t<double> >::const_iterator it=poly.begin(); it!=poly.end(); it++){
        foo+=((*it)^prev)*0.5;
        prev=*it;
    }
    return fabs(foo);
...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        for(int i=0; i<n; i++){
            f[i]=poly_area(a[i]);
            //fprintf(stderr, "f[%d]=%.2f\n", i, f[i]);
        }
        memset(g, 0x3f, sizeof(g));
        for(int i=0; i<n; i++){
            g[i][i]=0;
            for(int j=i+1; j<n; j++){
                if(lowdist(a[i], a[j])<900.0-eps){
                    double dcost;
                    int icost;
                    dcost=f[j]*10.0/f[i];
                    if(dcost<333.0){
                ...
                //fprintf(stderr, "g[%d][%d]=%.2f\n", i, j, lowdist(a[i], a[j]));
            }
        }
        for(int k=0; k<n; k++){
            for(int i=0; i<n; i++){
                for(int j=0; j<n; j++){
                    UPDMIN(g[i][j], g[i][k]+g[k][j]);
                }
            }
        }
        int first=1;
