...
int low_x, hig_x, low_y, hig_y;
int f[1100000];

unordered_set<int> bar, foo;

char seq[2100];

inline int hash_pot(const int x, const int y){
    return y*(hig_x+1)+x;
}

inline int hash_seg(const int cx, const int cy, const int nx, const int ny){
    int hpot=MIN(hash_pot(cx, cy), hash_pot(nx, ny));
    return (hpot<<1)|((cx!=nx)?1:0);
}

inline int find(const int u){ if(f[u]!=u)f[u]=find(f[u]);return f[u]; }

inline int kind(const char c){
    if(c=='U')return 0;
    if(c=='R')return 1;
    if(c=='D')return 2;
    if(c=='L')return 3;
...
int main(){
    while(scanf("%s", seq)!=EOF){
        ...
        for(int i=0; seq[i]; i++){
            ...
            int nx=cx+dx[k];
            int ny=cy+dy[k];
            //bar.insert(hash_seg(cx, cy, nx, ny));
            cx=nx;
            cy=ny;
            UPDMIN(low_x, cx);
            UPDMAX(hig_x, cx);
        ...
        for(int x=0; x<=hig_x; x++){
            for(int y=0; y<=hig_y; y++){
                int hpot=hash_pot(x, y);
                f[hpot]=hpot;
            }
        }
        //horiz
        for(int x=0; x<=hig_x; x++){
            for(int y=1; y<=hig_y; y++){
                if(bar.count(hash_seg(x, y, x+1, y))){
                    continue;
                }
                int lhs=hash_pot(x, y-1), rhs=hash_pot(x, y);
                f[find(lhs)]=find(rhs);
            }
        }
        //verti
        for(int x=1; x<=hig_x; x++){
            for(int y=0; y<=hig_y; y++){
                if(bar.count(hash_seg(x, y, x, y+1))){
                    continue;
                }
                int lhs=hash_pot(x-1, y), rhs=hash_pot(x, y);
                f[find(lhs)]=find(rhs);
            }
        }
        //count
