...
struct jump_t{
    int x, y;
    bool operator < (const jump_t &rhs) const{
        if(this->x!=rhs.x)return this->x<rhs.x;
        return this->y<rhs.y;
    }
    void set(const int dst, const int cur){
        this->x=dst;
        this->y=cur;
    }
} jump[11000];
...
void process(){
    ...
    for(int i=0; i<m; i++){
        for(int j=0; j<m; j++){
            if(
                jump[i].x<jump[j].x //&& p[jump[i].x]!=p[jump[j].x]
                && jump[j].x<jump[i].y //&& p[jump[j].x]!=p[jump[i].y]
                && jump[i].y<jump[j].y //&& p[jump[i].y]!=p[jump[j].y]
            ){
                printf("bad\n");
                return;
            }
