    ...
    while(scanf("%d", &n)!=EOF){
        ...
        if(m>=3){
            ...
            for(int y=0; y<m; y++){
                const int x=(y-1+m)%m;
                const int z=(y+1+m)%m;
                long double lenx2=a[seq[y]].dist2(a[seq[z]]);
                long double leny2=a[seq[x]].dist2(a[seq[z]]);
                long double lenz2=a[seq[x]].dist2(a[seq[y]]);
                long double cosy=(lenx2+lenz2-leny2)/(2.0L*sqrt(lenx2)*sqrt(lenz2));
                long double cur=acos(cosy)*180.0/acos(-1.0L);
                //fprintf(stderr, "D = %.9Lf\n", cur);
                UPDMIN(ans, cur);
            }
        }else{
            ans=0.0L;
