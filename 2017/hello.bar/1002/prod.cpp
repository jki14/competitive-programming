...
int n;
long double rad;
point_t<long double> p[110000], central;

point_t<long double> getcentral(point_t<long double> a, point_t<long double> b, point_t<long double> c){
    ...
    return point_t<long double>(a.x + (c1*b2 - c2*b1)/d,a.y + (a1*c2 - a2*c1)/d);
}
...
void process(){
    ...
    rad=0.0L;
    for(int i=1; i<n; i++){
        if(central.dist2(p[i])<rad*rad+eps){
            //nothing
        }else{
            central=p[i];
            rad=0.0L;
            for(int j=0;j<i;j++){
                if(central.dist2(p[j])<rad*rad+eps){
                    //nothing
                }else{
                    central.x=(p[i].x+p[j].x)*0.5L;
                    central.y=(p[i].y+p[j].y)*0.5L;
                    rad=central.dist(p[j]);
                    for(int k=0;k<j;k++){
                        if(central.dist2(p[k])<rad*rad+eps){
                            //nothing
                        }else{
                            central=getcentral(p[i], p[j], p[k]);
                            rad=central.dist(p[k]);
                        }
                    }
                }
            }
