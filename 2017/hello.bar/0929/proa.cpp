...
struct ctnode_t{
    int lef, rig, wht;
    ctnode_t *chd[2];
} *root[110000], *last, ctnodes[1100000];

void build(ctnode_t **hook, const int lef, const int rig){
    ctnode_t *node=last++;
    node->lef=lef;
    node->rig=rig;
    node->wht=0;
...
void insert(ctnode_t **hook, ctnode_t *prev, const int pos){
    ctnode_t *node=last++;
    node->lef=prev->lef;
    node->rig=prev->rig;
    node->wht=prev->wht+1;
    if(node->lef!=node->rig){
        if(pos<=prev->chd[0]->rig){
            insert(&node->chd[0], prev->chd[0], pos);
            node->chd[1]=prev->chd[1];
        }else{
            node->chd[0]=prev->chd[0];
            insert(&node->chd[1], prev->chd[1], pos);
        }
    }else{
        node->chd[0]=NULL;
        node->chd[1]=NULL;
...
int count(ctnode_t *node, const int lef, const int rig){
    if(lef<=node->lef && node->rig<=rig){
        return node->wht;
    }else{
        int foo=0;
        if(lef<=node->chd[0]->rig){
            foo+=count(node->chd[0], lef, rig);
        }
        if(rig>=node->chd[1]->lef){
            foo+=count(node->chd[1], lef, rig);
        }
        return foo;
    }
}
...
int main(){
    ...
    while(T--){
        ...
        for(int i=0; i<m; i++){
           scanf("%d%d%d%d", &req[i].l, &req[i].r, &req[i].b, &req[i].t);
           req[i].l++;
           req[i].r++;
        }
        sort(pnt, pnt+n);
        last=ctnodes;
        build(&root[0], 0, 100010);
        ctnode_t *curr=root[0];
        int right=0;
        for(int i=0; i<n; i++){
            while(right+1<pnt[i].x){
                root[++right]=curr;
            }
            insert(&curr, curr, pnt[i].y);
        }
        while(right+1<=100010){
            root[++right]=curr;
        }
        ans=0;
        for(int i=0; i<m; i++){
            ans+=count(root[req[i].r], req[i].b, req[i].t);
            ans-=count(root[req[i].l-1], req[i].b, req[i].t);
        }
        printf("%d\n", ans);
    }
    return 0;
