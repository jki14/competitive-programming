...
inline int geth(const int x, const int y){
    return x*c+y;
}

inline int getx(const int h){
    return h/c;
}

inline int gety(const int h){
    return h%c;
}
...
int main(){
    while(scanf("%d%d", &r, &c)!=EOF){
        ...
        if(!go()){
            printf("Yes\n");
            printf("%d\n", SIZE(foo));
            for(const int code : foo){
                printf("%d %d\n", getx(code)+1, gety(code)+1);
            }
        }else{
            printf("No\n");
        }
