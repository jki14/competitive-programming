...
const long double ema=1e-17;

struct acce_t{
    long double a, t;
    bool operator < (const acce_t &rhs) const{
        if(fabs(this->a-rhs.a)<ema){
            return this->t>rhs.t;
        }
        return this->a>rhs.a;
    }
...
inline long double getvalue(const long double a, const long double b, const long double c, const long double x){
    return a*x*x+b*x+c;
}

long double getime(const long double a, const long double b, const long double c, const long double hig){
    long double lef=0.0L, rig=hig;
    for(int i=0; i<128 && rig-lef>ema; i++){
        long double mid=(lef+rig)*0.5L;
        if(getvalue(a, b, c, mid)>-ema){
            rig=mid;
        }else lef=mid;
    }
    return lef;
...
int main(){
    ...
    while(T--){
        ...
        rem=m*1.0L;
        for(int i=0; i<n; i++){
            long double acd=(cur+(cur+a[i].a*a[i].t))*a[i].t*0.5L;
            if(acd<rem){
                ans+=a[i].t;
                cur+=a[i].a*a[i].t;
                rem-=acd;
            }else{
                long double def=getime(a[i].a, cur*2.0L, -2.0L*rem, a[i].t);
                ans+=def;
                cur+=a[i].a*def;
                rem=0.0L;
                break;
