...
struct application_t{
    int sid, uid, pos;
    bool operator < (const application_t &rhs) const{
        if(this->sid!=rhs.sid)return this->sid<rhs.sid;
...
int main(){
    while(scanf("%d%d%d%d", &n, &m, &k, &t)!=EOF){
        ...
        for(int i=0; i<k; i++){
            scanf("%d%d%d", &a[i].sid, &a[i].uid, &a[i].pos);
            a[i].sid--;
            a[i].uid--;
            a[i].pos--;
        }
        ...
        for(int x=0; x<n; x++){
            int flag=0;
            rig=0;
            for(int z=0; z<m-1; z++)neg[z]=0.0;
            for(int y=0; y<t; y++){
                if(!flag && cur<k && a[cur].sid==x && a[cur].pos==y && a[cur].uid==m-1){
                    flag=1;
                    cur++;
                }
                if(flag){
                    ...
                    sum[y]+=wht[x]*psave;
                    //fprintf(stderr, "sid %d x pos %d = %.14f\n", x, y, wht[x]*psave);
                }
                while(cur<k && a[cur].sid==x && a[cur].pos==y){
                    que[rig++]=a[cur++].uid;
                }
                for(int i=0; i<rig; i++){
                    neg[que[i]]+=prb[que[i]][y];
                    //fprintf(stderr, "neg[%d]=%.14f\n", que[i], neg[que[i]]);
                }
