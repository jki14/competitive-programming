...
inline int request(const int x, const int y){
    unordered_map<int, unordered_map<int, int> >::iterator xt=cache.find(x);
    if(xt==cache.end()){
        cache.insert(MP(x, unordered_map<int, int>()));
        xt=cache.find(x);
    }
    unordered_map<int, int>::iterator yt=xt->second.find(y);
    if(yt==xt->second.end()){
        int r;
        printf("? %d %d\n", x, y);
        fflush(stdout);
        scanf("%d", &r);
        xt->second.insert(MP(y, r));
        yt=xt->second.find(y);
        goo++;
        assert(goo<n*50);
    }
...
inline int excute(const int x, const int y){
    //fprintf(stderr, "execute(%d, %d)=%d\n", x, y, request(x, y)-distance(prv.begin(), prv.upper_bound(y)));
    return request(x, y)-distance(prv.begin(), prv.upper_bound(y));
}

void get_next_col(){
    int lef=1, rig=1000000-col;
    /*while(request(col+rig, 1000000)==cnt){
        lef=rig+1;
        UPDMIN(lef, 1000000-col);
        rig<<=1;
        UPDMIN(rig, 1000000-col);
    }*/
    while(rig-lef>0){
        const int mid=(lef+rig)>>1;
        const int foo=request(col+mid, 1000000);
        if(foo>cnt){
            /*if(mid==lef || request(col+mid-1, 1000000)<=cnt){
                //goal
                seq.push_back(MP(col+mid, foo-cnt));
                cnt=foo;
                col+=mid;
                return;
            }else{
    ...
    int nxt=request(col+lef, 1000000);
    assert(cnt<nxt);
    seq.push_back(MP(col+lef, nxt-cnt));
    cnt=nxt;
    col+=lef;
}

void get_next_ceil(const int x){
    int lef=1, rig=1000000-row;
    /*while(request(x, row+rig)==cur){
        lef=rig+1;
        UPDMIN(lef, 1000000-row);
        rig<<=1;
        UPDMIN(rig, 1000000-row);
    }*/
    while(rig-lef>0){
        const int mid=(lef+rig)>>1;
        const int foo=excute(x, row+mid);
        //fprintf(stderr, "cur=%d\n", cur);
        if(foo>cur){
            /*if(mid==lef || request(x, row+mid-1)<=cur){
                //goal
                rx[cnt]=x;
                ry[cnt]=row+mid;
                cnt++;
...
int main(){
    ...
    if(true){
        ...
        cnt=0;
        //assert(false);
        for(const pii vrt : seq){
            //fprintf(stderr, "vrt (%d, %d)\n", vrt.first, vrt.second);
            cur=0;
            row=-1000001;
            for(int i=0; i<vrt.second; i++){
                get_next_ceil(vrt.first);
            }
            for(int i=cnt-vrt.second; i<cnt; i++){
                prv.insert(ry[i]);
            }
        }
        assert(cnt==n);
