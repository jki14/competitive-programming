...
inline void add_request(const int u, const int v, const int r){
    rxt[rid]=rbs[u]; rst[rid]=r; rbs[u]=rid++;
    rxt[rid]=rbs[v]; rst[rid]=r; rbs[v]=rid++;
}
...
void tarjan(const int u, const int p){
    ...
    for(int i=rbs[u]; ~i; i=rxt[i]){
        req_t &r=req[rst[i]];
        if(~r.lca)continue;
        const int v=r.src^r.dst^u;
        if(~f[v])r.lca=find(f[v]);
    }
}

inline int req_comparer(const req_t &lhs, const req_t &rhs){
    int lsg=lhs.src/cli, rsg=rhs.src/cli;
    int ldg=lhs.dst/cli, rdg=rhs.dst/cli;
    if(lsg!=rsg)return lsg<rsg;
    if(ldg!=rdg)return ldg<rdg;
    return lhs.ts<rhs.ts;
}

inline void update(const int c, const int k){
    if(k<0){
        foo-=w[cnt[c]]*1LL*v[c];
        cnt[c]--;
    }else{
...
void crawl(const int u, const int v){
    if(u==v)return;
    if(u<v){
        for(int i=u+1; i<=v; i++){
            flip(idx[i]);
...
void fresh(const int s, const int t){
    if(s==t)return;
    if(s<t){
        for(int i=s+1; i<=t; i++){
            const int u=mod[i].pos;
            if(cov[u]){
                update(c[u], -1);
                update(mod[i].col, 1);
            }
...
int main(){
    while(scanf("%d%d%d", &n, &m, &q)!=EOF){
        ...
        tarjan(0, -1);
        for(int i=0; i<ri; i++){
            const int x=req[i].src;
            const int y=req[i].dst;
            req[i].src=MIN(st[x], st[y]);
            req[i].dst=MAX(st[x], st[y]);
        }
        cli=(int)pow(n*2., 2.0/3.0);
        sort(req, req+ri, req_comparer);
        for(int i=0; i<n; i++)cnt[i]=0;
        for(int i=0; i<n; i++)cov[i]=0;
        foo=0LL;
        for(int i=0; i<ri; i++){
            crawl((i!=0)?req[i-1].src:-1, req[i].src);
            crawl((i!=0)?req[i-1].dst:-1, req[i].dst);
            fresh((i!=0)?req[i-1].ts:-1, req[i].ts);
            flip(req[i].lca);
            ans[req[i].offset]=foo;
            flip(req[i].lca);
        }
        for(int i=0; i<ri; i++){
            printf("%lld\n", ans[i]);
