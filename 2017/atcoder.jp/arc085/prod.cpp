...
int process(const int lhs, const int rhs, const int minimize){
    if(~f[lhs][rhs][minimize])return f[lhs][rhs][minimize];
    if(!minimize){
        //maxmize
        if(rhs==n-1){
            f[lhs][rhs][minimize]=abs(a[lhs]-a[rhs]);
        }else{
            UPDMIN(low[1][rhs], n);
            while(low[1][rhs]>rhs+1){
                g[1][rhs][low[1][rhs]-1]=MAX(g[1][rhs][low[1][rhs]], process(low[1][rhs]-1, rhs, 1));
                low[1][rhs]--;
            }
            f[lhs][rhs][minimize]=g[1][rhs][rhs+1];
        }
    }else{
        //minimize
        if(lhs==n-1){
            ...
            UPDMIN(low[0][lhs], n);
            while(low[0][lhs]>lhs+1){
                g[0][lhs][low[0][lhs]-1]=MIN(g[0][lhs][low[0][lhs]], process(lhs, low[0][lhs]-1, 0));
                low[0][lhs]--;
            }
            f[lhs][rhs][minimize]=g[0][lhs][lhs+1];
        }
    }
    return f[lhs][rhs][minimize];
}
