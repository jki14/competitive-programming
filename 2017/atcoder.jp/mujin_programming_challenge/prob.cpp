...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T>  inline int COUNT(const T &x){ return (int)x.count(); }
...
bitset<512> a[512], /*b[512],*/ seq;

int fixable(const int r){
    for(int i=0; i<n; i++){
        if(a[i].test(r))return 1;
    }
    return 0;
}
...
void process(const int r){
    if(COUNT(a[r])<n){
        if(fixable(r)){
            UPDMIN(ans, n-COUNT(seq)+n-COUNT(a[r]));
        }else{
            for(int i=0; i<n; i++){
                if(a[i].any()){
                    UPDMIN(ans, n-COUNT(seq)+n-COUNT(a[r])+1);
                    break;
                }
            }
        }
    }else{
        UPDMIN(ans, n-COUNT(seq));
    }
}
