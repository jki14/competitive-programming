...
unordered_map<lld, int> cache;

vector<int> mas[32];

int getlast(const int c, const int til){
    int lef=0, rig=SIZE(mas[c])-1;
    while(lef<=rig){
        const int mid=(lef+rig)>>1;
        if(mas[c][mid]<=til){
            if(mid==rig || mas[c][mid+1]>til)return mas[c][mid];
            lef=mid+1;
        }else{
            rig=mid-1;
        }
...
int getfirst(const int c, const int fir){
    int lef=0, rig=SIZE(mas[c])-1;
    while(lef<=rig){
        const int mid=(lef+rig)>>1;
        if(mas[c][mid]>=fir){
            if(mid==lef || mas[c][mid-1]<fir)return mas[c][mid];
            rig=mid-1;
        }else{
            lef=mid+1;
        }
...
/*int process(const int lef, const int rig){
    const lld mask=lef*1000000LL+rig;
    unordered_map<lld, int>::const_iterator it=cache.find(mask);
    if(it!=cache.end()){
        return it->second;
    }
    int fir, sec;
    fir=sec=-inf;
    for(int c=0; c<26; c++){
        int cur=cnt[rig][c]-cnt[lef-1][c];
        if(cur&1){
            int pos=getlast(c, rig);
            if(pos>=fir){
                sec=fir;
                fir=pos;
            }else if(pos>sec){
    ...
    if(sec<0)return 1;
    int foo=inf;
    for(int mid=sec; mid<fir; mid++){
        UPDMIN(foo, process(lef, mid)+process(mid+1, rig));
    }
    cache.insert(MP(mask, foo));
    return foo;
}*/

unordered_map<lld, int> fch, sch;
unordered_map<lld, int> ffh, ssh;

unordered_set<lld> inited;
...
int process(){
    ...
    ssh.clear();
    inited.clear();
    stc.push(1*1000000LL+n);
    cache.insert(MP(1*1000000LL+n, inf));
    while(!stc.empty()){
        lld msk=stc.top();
        int lef=(int)(msk/1000000LL);
        int rig=(int)(msk%1000000LL);
        //fprintf(stderr, "go lef=%d rig=%d\n", lef, rig);
        if(!fch.count(msk)){
            int fir, sec;
            fir=sec=-inf;
            for(int c=0; c<26; c++){
            ...
            if(sec<0){
                cache[msk]=1;
                stc.pop();
                //fprintf(stderr, "lef=%d rig=%d foo=%d\n", lef, rig, 1);
                continue;
            }
            fch.insert(MP(msk, fir));
            sch.insert(MP(msk, sec));

            fir=sec=-inf;
            for(int c=0; c<26; c++){
                int cur=cnt[rig][c]-cnt[lef-1][c];
                if(cur&1){
                    int pos=getfirst(c, lef);
                    if(pos>=fir){
                        sec=fir;
                        fir=pos;
                    }else if(pos>sec){
            ...
            ffh.insert(MP(msk, fir));
            ssh.insert(MP(msk, sec));
        }
        int fir=fch[msk];
        int sec=sch[msk];
        int hig=ffh[msk];
        ...
        if(!inited.count(msk)){
            for(int mid=sec; mid<fir; mid++){
                lld lsk=lef*1000000LL+mid;
                lld rsk=(mid+1)*1000000LL+rig;
                if(!cache.count(lsk)){
                    stc.push(lsk);
                    cache.insert(MP(lsk, inf));
                }
                if(!cache.count(rsk)){
                    stc.push(rsk);
                    cache.insert(MP(rsk, inf));
                }
            }
            for(int mid=low; mid<hig; mid++){
                if(hig==fir)break;
                lld lsk=lef*1000000LL+mid;
                lld rsk=(mid+1)*1000000LL+rig;
                if(!cache.count(lsk)){
        ...
        if(msk!=stc.top())continue;
        unordered_map<lld, int>::iterator it=cache.find(msk);
        int &foo=it->second;
        for(int mid=sec; mid<fir; mid++){
            lld lsk=lef*1000000LL+mid;
            lld rsk=(mid+1)*1000000LL+rig;
            UPDMIN(foo, cache[lsk]+cache[rsk]);
        }
        for(int mid=low; mid<hig; mid++){
            if(hig==fir)break;
            lld lsk=lef*1000000LL+mid;
...
int main(){
    while(scanf("%s", a)!=EOF){
        ...
        for(int c=0; c<26; c++)mas[c].clear();
        for(int i=1; i<=n; i++){
            for(int c=0; c<26; c++)
                cnt[i][c]=cnt[i-1][c];
            cnt[i][a[i-1]-'a']++;
            mas[a[i-1]-'a'].push_back(i);
        }
        printf("%d\n", process());
    }
    return 0;
