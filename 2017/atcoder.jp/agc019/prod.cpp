...
int main(){
    while(scanf("%s%s", a, b)!=EOF){
        ...
        for(int i=0; i<n; i++){
            set<int>::const_iterator ir=sr.lower_bound(i);
            if(ir==sr.end())ir=sr.begin();
            tr[i]=(((*ir)-i)%n+n)%n;
            set<int, greater<int> >::const_iterator il=sl.lower_bound(i);
            if(il==sl.end())il=sl.begin();
            tl[i]=((i-(*il))%n+n)%n;
        }
        //process
        ans=inf;
        for(int s=0; s<n; s++){
            //rebuild hig
            for(int i=0; i<=n; i++)hig[i]=0;
            for(int i=0; i<n; i++){
                int j=(s+i)%n;
                if(a[i]==b[j] || b[j]=='1')continue;
                if(tr[i]==0)continue;
                UPDMAX(hig[tr[i]-1], tl[i]);
            }
            for(int i=n-1; ~i; i--){
                UPDMAX(hig[i], hig[i+1]);
            }
            //core
            int cur=0, mov=inf;
            for(int i=0; i<n; i++)cur+=(a[i]!=b[(s+i)%n])?1:0;
            //fprintf(stderr, "s=%d, cur=%d\n", s, cur);
            for(int r=0; r<=n; r++){
                int l=hig[r], f;
                // L -> R -> FIX
                f=abs(r-s);
                ...
                UPDMIN(f, n-f);
                UPDMIN(mov, r+r+l+f);
                //fprintf(stderr, "s=%d, l=%d, r=%d, mov=%d\n", s, l ,r ,mov);
            }
            UPDMIN(ans, cur+mov);
        }
        printf("%d\n", ans);
