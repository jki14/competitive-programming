...
lld flush2(lld low){
    lld cnt=0LL;
    for(int i=0; i<n; i++){
        lld cur=MAX((a[i]-low)/(n*1LL+1LL), 0LL);
        a[i]-=(n*1LL+1LL)*cur;
        cnt+=cur;
    }
    return cnt;
...
lld flush(lld cap){
    lld lef=0LL, rig=60000000000000000LL;
    while(lef<=rig){
        lld mid=(lef+rig)>>1;
        if(predict(mid)<=cap){
            if(mid==lef || predict(mid-1LL)>cap){
                return flush2(mid);
            }
            rig=mid-1LL;
        }else{
...
void simulate(const int pos){
    for(int i=0; i<n; i++)a[i]+=1LL;
    a[pos]-=n*1LL+1LL;
}
...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        least=MAX(least-n*1LL*(n-1), 0LL);
        for(int i=0; i<n; i++)a[i]+=least;
        ans=flush(least);
        for(; ans<least; ans++){
            int pos=find();
            a[pos]-=n*1LL+1LL;
