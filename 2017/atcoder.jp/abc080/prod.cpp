...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<n; i++){
            ...
            t--;
            c--;
            seq[c].push_back(MP(s, t));
            UPDMAX(r, t);
        }
        for(int i=0; i<m; i++)sort(seq[i].begin(), seq[i].end());
        int foo=0;
        for(int p=0; p<=r; p++){
            int cnt=0;
            for(int i=0; i<m; i++){
                ...
                pii rul(p, inf);
                vector<pii>::iterator it=lower_bound(seq[i].begin(), seq[i].end(), rul);
                if(it!=seq[i].begin()){
                    it--;
