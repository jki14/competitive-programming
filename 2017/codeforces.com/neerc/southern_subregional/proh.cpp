...
int main() {
    while(scanf("%d", &n)!=EOF) {
        ...
        if(odd) {
            int row=MAX(odd, 1);
            while((n%row) || ((n/row)&1)==0)row++;
            int col=n/row;
            rep.resize(row);
            for(vector<char> &sub : rep) {
                sub.resize(col);
            }
            int r=0, p=0;
            //CENT
            for(; r<row; r++) {
                while(p<m && (c[seq[p]]&1)==0)p++;
                if(p<m) {
                    rep[r][col>>1]=itoc(seq[p]);
                    c[seq[p]]--;
                } else {
                    break;
                }
            ...
            for(int r=0; r<row; r++) {
                for(int i=0; i<col-i-1; i++) {
                    while(!c[seq[p]])p++;
                    rep[r][i]=rep[r][col-i-1]=itoc(seq[p]);
                    c[seq[p]]-=2;
                }
            }
        } else {
            ...
            for(int i=0; i<n-i-1; i++) {
                while(!c[seq[p]])p++;
                rep[0][i]=rep[0][n-i-1]=itoc(seq[p]);
                c[seq[p]]-=2;
            }
        }
        printf("%d\n", SIZE(rep));
        for(int i=0; i<SIZE(rep); i++) {
            if(i)printf(" ");
            printf("%s", string(rep[i].begin(), rep[i].end()).c_str());
        }
        printf("\n");
    }
    return 0;
