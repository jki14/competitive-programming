...
int main(){
    ...
    while(cin>>n>>m>>a>>d){
        ...
        while(x<n || y<m){
            //staff
            lld head=x;
            lld tail=MIN(t[y]/a-1LL, (n-1)*1LL);
            if(head<=tail){
                if(last+d>=(x+1LL)*a){
                    //free-to-go
                    lld free=(last+d)/a-1LL;
                    head=free+1LL;
                }
                if(head<=tail){
                    //paid-to-go
                    /*cerr<<"[";
                    for(lld i=head; i<=tail; i++){
                        cerr<<(i+1LL)*a;
                        cerr<<",]"[i==tail];
                    }
                    cerr<<endl;*/
                    lld batch=(d+1LL+a-1LL)/a;
                    lld remain=(tail-head+1LL)%batch;
                    if(!remain)remain=batch;
                    ans+=(tail-head+1LL+batch-1LL)/batch;
                    last=(tail-remain+1LL+1LL)*a;
                    //cerr<<"ans="<<ans<<endl;
                    //cerr<<"last="<<last<<endl;
                }
                x=tail+1LL;
