...
int main() {
    ...
    while(cin>>n>>m>>t0) {
        ...
        for(int i=0; i<2; i++) {
            //under-bought-x1
            for(lld x1=0LL; x1*a1<=n && x1*a1*t1<=m; x1++) {
                lld rn=n-x1*a1;
                lld rm=m-x1*a1*t1;
                if(rn*t0>rm && rn*t2>rm)continue;
                //under-bought-x2
                UPDMIN(x2, rn/a2);
                while(x2>0LL && (rn-(x2-1LL)*a2)*t0+(x2-1LL)*a2*t2<=rm)x2--;
                if((rn-x2*a2)*t0+x2*a2*t2<=rm)
                    UPDMIN(foo, x1*p1+x2*p2);
                //over-bought-x2
                if(rn*t2<=rm) {
                    UPDMIN(foo, x1*p1+(rn+a2-1LL)/a2*p2);
                }
            }
            //over-bought-x1
            if(n*t1<=m) {
                UPDMIN(foo, (n+a1-1LL)/a1*p1);
            }
            //swap
            swap(a1, a2);
            swap(t1, t2);
