...
int low[210000], hig[210000];

void response(){
    lld foo=0LL;
    for(int i=0; i<n; i++){
        //cerr<<"["<<a[i]<<", "<<b[i]<<"] -> ["<<low[i]<<", "<<hig[i]<<"]'"<<endl;
        if(low[i]>hig[i]){
            foo=-1LL;
            break;
        }
...
int main() {
    ...
    while(cin>>n) {
        ...
        if(true) {
            multiset<int, greater<int>> prf, suf;
            for(int i=0; i<n; i++) {
                prf.insert(a[i]-i);
            }
            for(int i=0; i<n; i++) {
                if(!prf.empty()) {
                    UPDMAX(low[i], *prf.begin()+i);
                }
                if(!suf.empty()){
                    UPDMAX(low[i], *suf.begin()-i);
                }
                multiset<int, greater<int>>::iterator it=prf.find(a[i]-i);
                prf.erase(it);
                suf.insert(a[i]+i);
            }
        }
        ...
        if(true) {
            multiset<int> prf, suf;
            for(int i=0; i<n; i++) {
                prf.insert(b[i]+i);
            }
            for(int i=0; i<n; i++) {
                if(!prf.empty()) {
                    UPDMIN(hig[i], *prf.begin()-i);
                }
                if(!suf.empty()){
                    UPDMIN(hig[i], *suf.begin()+i);
                }
                multiset<int>::iterator it=prf.find(b[i]+i);
                prf.erase(it);
                suf.insert(b[i]-i);
            }
        }
