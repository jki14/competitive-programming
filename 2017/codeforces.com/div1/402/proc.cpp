...
int n;
int nbs[N], nxt[M], dst[M], wht[M], eid;
int cnt[N], foo[N], ans[N];
...
/*inline int check(node_t *lhs, node_t *rhs){
    if(lhs->flag && rhs->flag)return 1;
    for(int i=0; i<26; i++){
        if(lhs->next[i]!=NULL && rhs->next[i]!=NULL){
            if(check(lhs->next[i], rhs->next[i])){
                return 1;
            }
        }
    }
...
node_t *update(node_t *prev, node_t *buff, const int h){
    node_t *curr=NULL;
    for(int i=0; i<26; i++){
        //printf("h:%d prev->next[%d]=%d\n", h, i, prev->next[i]);
        //printf("h:%d prev->next[%d]=%d\n", h, i, buff->next[i]);
        if(prev->next[i]==NULL && buff->next[i]!=NULL){
            if(curr==NULL){
                memcpy(tail, prev, sizeof(node_t));
                curr=tail++;
            }
            curr->next[i]=buff->next[i];
        }else if(prev->next[i]!=NULL && buff->next[i]!=NULL){
            //printf("ans[%d]++ inner\n", h);
            ans[h]++;
            node_t *temp=update(prev->next[i], buff->next[i], h);
            if(prev->next[i]!=temp){
                if(curr==NULL){
                    memcpy(tail, prev, sizeof(node_t));
                    curr=tail++;
...
void process(const int u, const int h, const int p){
    cnt[u]=1;
    int peak=-1;
    ...
    if(~peak){
        node_t *stamp=tail;
        node_t *curr=node[peak];
        for(int i=nbs[u]; ~i; i=nxt[i]){
            const int v=dst[i];
            if(v==p || v==peak)continue;
            //printf("call update\n");
            curr=update(curr, node[v], h+1);
        }
        tail=stamp;
        node[u]=newnode();
        for(int i=nbs[u]; ~i; i=nxt[i]){
