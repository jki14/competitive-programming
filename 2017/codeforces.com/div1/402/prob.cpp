...
int n, m, vid;
char var[N][M], sgn[N][M], lhs[N][M], rhs[N][M];
char low[M], hig[M];

int calc(const int i, const int p){
    if(~f[i])return f[i];
    if(cond[i].instance!=NULL){
        return f[i]=cond[i].instance->test(p);
    }
    switch(cond[i].operation){
        case '&':
            return f[i]=calc(cond[i].lhs, p)&calc(cond[i].rhs, p);
        case '|':
            return f[i]=calc(cond[i].lhs, p)|calc(cond[i].rhs, p);
        default:
            return f[i]=calc(cond[i].lhs, p)^calc(cond[i].rhs, p);
...
int main(){
    ...
    if(scanf("%d%d", &n, &m)!=EOF){
        ...
        tail=pool;
        for(int i=0; i<n; i++){
            const int offset=idx[string(var[i])];
            if(lhs[i][0]=='0' || lhs[i][0]=='1'){
                cond[offset].instance=tail++;
                cond[offset].instance->reset();
                for(int j=0; j<m; j++){
                    if(lhs[i][j]=='1'){
                        cond[offset].instance->set(j);
                    }
                }
            }else{
                cond[offset].instance=NULL;
                cond[offset].operation=(sgn[i][0]!='A')?((sgn[i][0]!='O')?'^':'|'):'&';
                cond[offset].lhs=idx[string(lhs[i])];
                cond[offset].rhs=idx[string(rhs[i])];
            }
        }
        for(int k=0; k<m; k++){
            int lef=0, rig=0;
