...
int n, m;
int nbs[N], src[M], dst[M], nxt[M], eid;
int low[N], dfn[N], idx;
int sta[N], ins[N], top;
int col[N], det[N];
...
inline void add_edge(const int u, const int v){
    nxt[eid]=nbs[u]; src[eid]=u; dst[eid]=v; nbs[u]=eid++;
    nxt[eid]=nbs[v]; src[eid]=v; dst[eid]=u; nbs[v]=eid++;
}
    ...
    while(cin>>n>>m){
        ...
        if(ans){
            ...
            for(int i=0; i<eid; i+=2){
                ...
                int v=dst[i];
                if(col[u]==col[v])continue;
                ans-=(cnt-(det[col[u]]-1)-(det[col[v]]-1)-1)*1LL;
            }
            ans>>=1;
        }
        //assert((n==5 && m==3) || ans!=0LL);
        cout<<ans<<endl;
    }
    return 0;
}
