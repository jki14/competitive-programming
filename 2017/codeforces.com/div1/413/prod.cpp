...
const lld top=1100000000000000LL;
...
int check(int lhs, int rhs, const int pos){
    ...
    if(lhs>1){
        ...
        for(int i=0; i<pos; i++){
            ...
            if(fir<tot && tot/fir>=rhs*1LL){
                break;
            }
            til=0;
            for(set<lld>::const_iterator it=foo.begin(); it!=foo.end(); it++){
                lld nxt=(*it)*wht[i];
                if(nxt>=lhs*1LL){
                    UPDMIN(fir, nxt);
                    break;
    ...
    if(fir<=tot){
        return (tot/fir-rhs*1LL)>=0LL;
    }
    return 0;
}

inline int enough(const int pos){
    return check((a+h-1)/h, (b+w-1)/w, pos) || check((a+w-1)/w, (b+h-1)/h, pos);
}
