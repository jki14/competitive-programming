...
inline void update(const int p, int sol[2]){
    if(!~sol[0] || wht[p]>=wht[sol[0]]){
        sol[1]=sol[0];
        sol[0]=p;
    }else if(!~sol[1] || wht[p]>wht[sol[1]]){
        sol[1]=p;
    }
}
...
int main(){
    while(scanf("%d%d%d", &n, &c, &d)!=EOF){
        for(int i=0; i<=c; i++)cch[i][0]=cch[i][1]=-1;
        for(int i=0; i<=d; i++)dch[i][0]=dch[i][1]=-1;
        for(int i=0; i<n; i++){
            scanf("%d%d%s", &wht[i], &cst[i], foo);
            res[i]=-1;
            if(foo[0]=='C'){
                if(cst[i]<=c){
                    update(i, cch[cst[i]]);
        ...
        for(int i=1; i<=c; i++){
            update(cch[i-1][1], cch[i]);
            update(cch[i-1][0], cch[i]);
        }
        for(int i=1; i<=d; i++){
            update(dch[i-1][1], dch[i]);
            update(dch[i-1][0], dch[i]);
        }
        ans=0;
        for(int i=0; i<n; i++){
            if(!~res[i])continue;
            ...
            if(~cch[c][0] && cch[c][0]!=i){
                UPDMAX(rig, wht[cch[c][0]]);
            }else if(~cch[c][1] && cch[c][1]!=i){
                UPDMAX(rig, wht[cch[c][1]]);
            }
            if(~dch[d][0] && dch[d][0]!=i){
                UPDMAX(rig, wht[dch[d][0]]);
            }else if(~dch[d][1] && dch[d][1]!=i){
                UPDMAX(rig, wht[dch[d][1]]);
            }
            UPDMAX(ans, lef+rig);
            if(res[i]==0){
                c+=cst[i];
