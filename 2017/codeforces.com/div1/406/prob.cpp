...
const int til=0x3f3f3f3;

typedef pair<int, lld> foo_t;
...
void upd_dist(node_t*, const int, const int, const lld);

inline void flush(node_t *node){
    if(node->cov!=inf){
        upd_dist(node->chd[0], node->lef, node->rig, node->cov);
        upd_dist(node->chd[1], node->lef, node->rig, node->cov);
        node->cov=inf;
    }
}

inline void update(node_t *node){
    node->key=MIN(node->chd[0]->key, node->chd[1]->key);
    if(node->key==til){
        node->foo.first=node->lef;
        node->foo.second=inf;
    }else{
        if(node->chd[0]->foo.second<=node->chd[1]->foo.second){
            node->foo=node->chd[0]->foo;
        }else{
            node->foo=node->chd[1]->foo;
        }
    }
}
...
node_t *build(const int lef, const int rig){
    ...
    node->foo=MP(lef, inf);
    node->key=lef;
    while(!node->pathes.empty())node->pathes.pop();
    if(lef!=rig){
...
void add_path(node_t *node, const int lef, const int rig, path_t *path){
    if(lef<=node->lef && node->rig<=rig){
        node->pathes.push(path);
    }else{
        if(node->chd[0]->rig>=lef){
            add_path(node->chd[0], lef, rig, path);
        }
        if(node->chd[1]->lef<=rig){
            add_path(node->chd[1], lef, rig, path);
        }
    }
}

void upd_dist(node_t *node, const int lef, const int rig, const lld dis){
    if(dis>=node->cov)return;
    if(node->key==til)return;
    if(lef<=node->lef && node->rig<=rig){
        node->cov=dis;
        if(node->foo.second>dis){
            node->foo.first=node->key;
            node->foo.second=dis;
        }
    }else{
        flush(node);
        if(node->chd[0]->rig>=lef){
            upd_dist(node->chd[0], lef, rig, dis);
        }
        if(node->chd[1]->lef<=rig){
            upd_dist(node->chd[1], lef, rig, dis);
        }
        update(node);
    }
}

void inf_dist(node_t *node, const int pos){
    if(node->lef==node->rig){
        node->foo.second=inf;
        node->key=til;
    }else{
        flush(node);
        if(node->chd[0]->rig>=pos){
            inf_dist(node->chd[0], pos);
        }else{
            inf_dist(node->chd[1], pos);
        }
        update(node);
    }
}

void upd_path(node_t *node, const int pos, const lld low){
    for(; !node->pathes.empty(); node->pathes.pop()){
        path_t *path=node->pathes.front();
        if(!path->valid)continue;
        upd_dist(root, path->lef, path->rig, low+path->wht);
        path->valid=0;
    }
    if(node->lef!=node->rig){
        if(pos<=node->chd[0]->rig){
            upd_path(node->chd[0], pos, low);
        }else{
            upd_path(node->chd[1], pos, low);
        }
    }
}
...
int main(){
    ...
    if(cin>>n>>m>>s){
        ...
        for(int i=0; i<m; i++){
            ...
            path[i].wht=w*1LL;
            path[i].valid=1;
            add_path(root, x, y, &path[i]);
        }
        upd_dist(root, s, s, 0LL);
        while(1){
            foo_t foo=root->foo;
            if(foo.second==inf)break;
            assert(!~ans[foo.first]);
            ans[foo.first]=foo.second;
            upd_path(root, foo.first, foo.second);
            inf_dist(root, foo.first);
        }
        for(int i=1; i<=n; i++){
            cout<<ans[i];
            if(i!=n){
