...
void scan(const int x, const int r){
    ...
    if(len[x]<=60){
        /*for(int i=0; i<MIN(len[x], 60); i++){
            if((lef[x]>>(MIN(len[x], 60)-1-i))&1){
                fprintf(stderr, "1");
            }else{
                fprintf(stderr, "0");
            }
        ...
        scan(q, r);
        int curr=rig[p]&mask;
        /*for(int i=0; i<MIN(len[p], 60); i++){
            if((rig[p]>>(MIN(len[p], 60)-1-i))&1){
                fprintf(stderr, "1");
            }else{
                fprintf(stderr, "0");
            }
        ...
        fprintf(stderr, " + ");
        for(int i=0; i<MIN(len[q], 60); i++){
            if((lef[q]>>(MIN(len[q], 60)-1-i))&1){
                fprintf(stderr, "1");
            }else{
                fprintf(stderr, "0");
            }
    ...
    while(scanf("%d", &n)!=EOF){
        ...
        for(int i=0; i<n; i++){
            ...
            if(len[i]<=60){
                ...
                /*fprintf(stderr, "lef[%d]=", i);
                for(int j=0; j<MIN(len[i], 60); j++){
                    if(lef[i]&(i128_1<<(len[i]-j-1))){
                        fprintf(stderr, "1");
                    }else{
                        fprintf(stderr, "0");
                    }
