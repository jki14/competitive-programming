...
int main(){
    while(scanf("%d%d%d%d%d", &h, &m, &s, &t1, &t2)!=EOF){
        ...
        t2%=12;
        a[0]=s*cir/60;
        a[1]=m*cir/60+s*cir/(60*60);
        a[2]=h*cir/12+m*cir/(12*60)+s*cir/(12*60*60);
        t1=t1*cir/12;
        t2=t2*cir/12;
        //fprintf(stderr, "%d:%d:%d\n", h, m, s);
        //fprintf(stderr, "a[0]=%d\n", a[0]);
        //fprintf(stderr, "a[1]=%d\n", a[1]);
        //fprintf(stderr, "a[2]=%d\n", a[2]);
        if(t1>t2)swap(t1, t2);
        //fprintf(stderr, "t1=%d\n", t1);
        //fprintf(stderr, "t1=%d\n", t2);
        if(cwise() || awise()){
