...
int n, w, h;
int rx[110000], ry[110000];

struct beam_t{
    int g, p, t, idx;
    bool operator < (const beam_t &rhs) const{
        if(this->hash()!=rhs.hash())return this->hash()<rhs.hash();
...
struct dest_t{
    ...
    bool operator < (const dest_t &rhs) const{
        if(this->group()!=rhs.group())return this->group()<rhs.group();
        if(this->group()==0){
            return this->x<rhs.x;
        }else{
            return this->y>rhs.y;
...
int main(){
    while(scanf("%d%d%d", &n, &w, &h)!=EOF){
        ...
        for(int lef=0; lef<n; ){
            int rig=lef;
            while(rig+1<n && b[rig+1].hash()==b[lef].hash())rig++;
            int len=0;
            ...
            sort(d, d+len);
            for(int i=lef; i<=rig; i++){
                rx[b[i].idx]=d[i-lef].x;
                ry[b[i].idx]=d[i-lef].y;
            }
            lef=rig+1;
        }
        for(int i=0; i<n; i++){
