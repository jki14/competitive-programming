...
struct Comparer {
    bool operator () (const bitset<N> &lhs, const bitset<N> &rhs){
        for(int i=0; i<n; i++){
            const int x=lhs.test(i);
            const int y=rhs.test(i);
            if(x!=y)return x<y;
        }
        return false;
    }
...
map<bitset<N>, int, Comparer> f;
map<bitset<N>, int, Comparer>::iterator it;
...
inline void process(){
    ...
    for(int i=0; i<n; i+=2){
        node[i].cur^=x;
        cur.flip(node[i].offset);
    }
}

inline void nodeset(const bitset<N> &sta){
    for(int i=0; i<n; i++){
        if(sta.test(node[i].offset)){
            node[i].cur=node[i].pre^x;
        }else{
            node[i].cur=node[i].pre;
        }
    }
}
