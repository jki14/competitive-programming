...
inline int calc(const int n, const int m){
    //printf("calc %d, %d\n", n, m);
    return MOD_MUL(MOD_MUL(fac[n], inv[m]), inv[n-m]);
}

inline int calc2(const int n, const int m){
    //printf("calc2 %d, %d\n", n, m);
    return (n!=0 && m!=0)?calc(n+m-1, n):1;
}
...
int main(){
    ...
    while(scanf("%d%d%d", &f, &w, &h)!=EOF){
        ...
        for(int k=1; k*h<=w; k++){
            if(f-k+1<0)break;
            MOD_ADD(&ans, MOD_MUL(calc2(w-k*h, k), calc2(f-k+1, k+1)));
        }
        MOD_MUL(&ans, INVERSEE(calc(f+w, f)));
        MOD_STD(&ans);
        printf("%d\n", ans);
    }
    return 0;
