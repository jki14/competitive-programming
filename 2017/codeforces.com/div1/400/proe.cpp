...
lld phi(lld x){
    long double res=x*1.0L;
    for(lld i=2LL; i*i<=x; i++){
        if(!(x%i)){
            res=res*(1.0L-1.0L/(i*1.0L));
            while(!(x%i))x/=i;
        }
    }
    if(x>1)res=res*(1.0L-1.0L/(x*1.0L));
    return res;
}
...
int main(){
    ...
    if(cin>>n>>k){
        //lld cur=phi(n);
        lld cur=EULARFUNCTION(n);
        lld pos=1LL;
        for(; pos+2LL<=k; pos+=2LL){
            //cur=phi(cur);
            cur=EULARFUNCTION(cur);
            if(cur==1LL)break;
        }
        if(cur!=1LL && pos<k){
            
        }
        cout<<cur%1000000007LL<<endl;
        //cout<<cur<<endl;
    }
    return 0;
}
