...
int lack(const int s, int k){
    int f=1, r=0;
    while((f<<1)<=s)f<<=1;
    f=(f<<1)-1;
    f^=s;
    for(int i=0; bit[i]<=f; i++){
        if(!(f&bit[i]))continue;
        for(int j=i+1; j; j>>=1){
            r++;
        }
...
int main() {
    ...
    for(int i=0; i<R; i++)bit[i]=1<<i;
    if(scanf("%d", &n)!=EOF){
        scanf("%s", a);
        for(m=0; valid[m+1]<=n;)m++;
        //printf("n=%d, m=%d\n", n, m);
        for(int j=0; j<=m; j++){
            f[cur][j].clear();
        }
        ...
        for(int i=0; i<n; i++){
            ...
            f[cur][x].insert(MP(0, 1));
            for(int j=0; j<=m; j++){
                int til=(j<<1)|x;
                for(it=f[pre][j].begin(); it!=f[pre][j].end(); it++){
                    if(lack(it->first, j)>(n-i))continue;
                    //printf("f[%d][%d][%d]=%d\n", i, j, it->first, it->second);
                    //slash
                    if(j!=0){
                        int bin=it->first|bit[j-1];
                        it2=f[cur][x].find(bin);
                        if(it2!=f[cur][x].end()){
                            MOD_ADD(&it2->second, it->second);
                        }else{
                            f[cur][x].insert(MP(bin, it->second));
                        }
                    }
                    //keep
                    if(til<=m){
                        it2=f[cur][til].find(it->first);
                        if(it2!=f[cur][til].end()){
                            MOD_ADD(&it2->second, it->second);
                        }else{
                            f[cur][til].insert(MP(it->first, it->second));
                        }
                        //printf("?f[%d][%d][%d]=%d\n", i+1, til, it->first, f[cur][til][it->first]);
                    }else{
                        if(it->first!=0 && lowbit(it->first+1)==(it->first+1)){
                            //printf("+%d\n", it->second);
                            MOD_ADD(&ans, it->second);
                        }
                    }
                }
            }
        ...
        for(int j=0; j<=m; j++){
            for(it=f[cur][j].begin(); it!=f[cur][j].end(); it++){
                //drop
                int bin=it->first;
                if(bin!=0){
                    //printf("f[%d][%d][%d]=%d\n", n, j, bin, it->second);
                    if(lowbit(bin+1)==(bin+1)){
                        //printf("+%d\n", it->second);
                        MOD_ADD(&ans, it->second);
                    }
                }
