...
int rcom(const int r, int n){
    int foo=1;
    n=n+r-1;
    for(int i=1, k=n; i<=r; i++, k--){
        MOD_MUL(&foo, k);
        MOD_MUL(&foo, INVERSEE(i));
    }
    return foo;
}

int run(const int cnt, const int ans){
    int foo=0;
    MOD_ADD(&foo, MOD_MUL(rcom(cnt-1, r+1), MOD_MUL(ans, 2)));
    MOD_ADD(&foo, MOD_MUL(rcom(cnt, r), ans));
    return foo;
}

int main() {
    int *prm=PRIMELIST(1000000);
    int T;scanf("%d", &T);
    while(T--){
        scanf("%d%d", &r, &n);
        ans=1;
        for(int i=0; prm[i] && prm[i]<=n/prm[i]; i++){
