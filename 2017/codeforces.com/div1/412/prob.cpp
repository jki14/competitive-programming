...
int least(){
    for(int r=n; r<=5000; r++){
        int flag=1;
        for(int j=0; j<5; j++){
            if(r<(1<<kind[j]) && (kind[j]!=5 || pas[j]!=0)){
                flag=0;
                break;
            }
            int rig=r/(1<<kind[j]);
            ...
            //if(r==6)fprintf(stderr, "problem %d (%d) %d->%d\n", j, r, lef, rig);
            if(lef>rig){
                flag=0;
                break;
            }
            ...
            if(pas[j]<lef && (!~pst[0][j] || pas[j]+r-n<lef)){
                flag=0;
                break;
            }
        }
...
int win(){
    int foo=0, goo=0;
    for(int j=0; j<5; j++){
        if(~pst[0][j])foo+=score[kind[j]]/250*(250-pst[0][j]);
        if(~pst[1][j])goo+=score[kind[j]]/250*(250-pst[1][j]);
    }
    return foo>goo;
}
