...
int nbs[8100], nxt[8100];

unordered_map<int, int> tran;

int cache1[8100], cache2[8100][8100];
...
int main(){
    while(scanf("%d%d", &n, &q)!=EOF){
        ...
        for(int i=0; i<n; i++){
            scanf("%d", &a[i]);
            tran.insert(MP(a[i], -1));
        }
        m=0;
        for(unordered_map<int, int>::iterator it=tran.begin(); it!=tran.end(); it++){
            it->second=m++;
        }
        for(int i=0; i<m; i++){
            nbs[i]=-1;
        ...
        memset(cache1, 0xff, sizeof(cache1));
        memset(cache2, 0xff, sizeof(cache2));
        for(int k=0; k<q; k++){
            int x, y;
            scanf("%d%d", &x, &y);
            int cx=tran.count(x);
