...
unordered_map<int, lld> bar;
unordered_map<int, lld>::iterator it;
...
int main(){
    while(scanf("%d%d%d", &n, &m, &q)!=EOF){
        ...
        for(int i=0; i<n; i++){
            c[i].resize(m);
            f[i].resize(m);
            for(int j=0; j<m; j++)c[i][j]=f[i][j]=0;
            int len;
            scanf("%d", &len);
            for(int j=0; j<len; j++){
                int x;
        ...
        for(int k=0; k<h; k++){
            for(int l=0; l+k<n; l++){
                ...
                for(int i=0; i<m; i++){
                    ...
                    it=bar.find(f[l][i]);
                    if(it==bar.end()){
                        bar.insert(MP(f[l][i], 0LL));
                        it=bar.find(f[l][i]);
                    }
                    it->second+=(i+1)*1LL*(i+1);
                }
                if(SIZE(bar)<(1<<(k+1))){
                    ans[l][k]=foo;
                }else{
                    ans[l][k]=0LL;
                    //fprintf(stderr, "[%d][%d] begin\n", l, k);
                    for(it=bar.begin(); it!=bar.end(); it++){
                        //fprintf(stderr, "<%d, %lld> in bar\n", it->first, it->second);
                        UPDMAX(ans[l][k], foo-it->second);
                    }
                    //fprintf(stderr, "end\n");
                }
            }
