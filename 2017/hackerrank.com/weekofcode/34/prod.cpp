...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> class MATX{
        ...
        public:
            ...
            MATX(const unsigned long _len=2){
            ...
            template<class T2> const MATX add(const MATX &opn, const T2 m) const{
                MATX ret(this->hig, this->wid);
                for(unsigned long x=0; x<ret.hig; x++)
                    for(unsigned long y=0; y<ret.wid; y++)
                        MOD_ADD(&ret.data[x*ret.wid+y], MOD_ADD(this->at(x, y), opn.at(x, y), m), m);
                return ret;
            }
            
            template<class T2> const MATX sub(const MATX &opn, const T2 m) const{
                MATX ret(this->hig, this->wid);
                for(unsigned long x=0; x<ret.hig; x++)
                    for(unsigned long y=0; y<ret.wid; y++)
                        MOD_ADD(&ret.data[x*ret.wid+y], MOD_ADD(this->at(x, y), -opn.at(x, y), m), m);
...
void process1(const int u, const int p){
    MATX<int> goo=foo.pow(a[u], MOD);
    int buf=goo[0][0];
    sum[u]=goo;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        ...
        if(v==p)continue;
        //process(v, u);
        sum[u]=sum[u].add(goo.mul(sum[v], MOD), MOD);
        MOD_ADD(&buf, goo.mul(sum[v], MOD).at(0, 0));
    }
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(v==p)continue;
        MOD_ADD(&buf, sum[u].sub(goo.mul(sum[v], MOD), MOD).mul(sum[v], MOD).at(0, 0));
    }
    MOD_ADD(&ans, buf);
}

int seq[210000], anc[210000];

void process(const int lhs, const int rhs){
    int lef=0, rig=0;
    seq[rig++]=lhs;
    anc[lef]=rhs;
    while(lef<rig){
    ...
    for(int i=0; i<rig-i-1; i++)swap(seq[i], seq[rig-i-1]);
    for(int i=0; i<rig; i++)process1(seq[i], anc[seq[i]]);
}
