...
const int blank=0xc0c0c0c0;

int n, m, k;
int a[110][110];

int autofill(const int x, const int y){
    assert(0<=x && x<=n-1);
    assert(0<=y && y<=n-1);
    if(a[x][y]!=blank)return a[x][y];
    assert(x!=y);
    if(x<y)
        a[x][y]=autofill(x, y-1)-1;
    else a[x][y]=autofill(x-1, y)-1;
    return a[x][y];
}
