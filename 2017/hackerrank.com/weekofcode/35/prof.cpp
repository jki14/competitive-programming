...
struct stnode_t{
    ...
    lld cen;
    stnode_t *chd[2];
} pool[410000], *tail, *root;

int n, m;
lld a[210000], d;
lld v[210000];
int p[210000], low, hig;
int c[210000];
...
void putin(stnode_t *node, const int pos){
    node->cnt++;
    if(node->lef!=node->rig){
        if(pos<=node->chd[0]->rig){
            putin(node->chd[0], pos);
        }else{
            putin(node->chd[1], pos);
        }
        //update low
        if(~node->chd[0]->low && (!~node->low || node->low>node->chd[0]->low)){
            node->low=node->chd[0]->low;
        }else if(~node->chd[1]->low && (!~node->low || node->low>node->chd[1]->low)){
            node->low=node->chd[1]->low;
        }
        //update hig
        if(~node->chd[1]->hig && (!~node->hig || node->hig<node->chd[1]->hig)){
            node->hig=node->chd[1]->hig;
        }else if(~node->chd[0]->hig && (!~node->hig || node->hig<node->chd[0]->hig)){
            node->hig=node->chd[0]->hig;
        }
        //update cen
        node->cen=0LL;
        UPDMAX(node->cen, node->chd[0]->cen);
        UPDMAX(node->cen, node->chd[1]->cen);
        if(~node->chd[0]->hig && ~node->chd[1]->low){
            UPDMAX(node->cen, v[node->chd[1]->low]-v[node->chd[0]->hig]);
        }
    }else{
        node->low=node->hig=pos;
    }
}
...
response_t query(stnode_t *node, int lef, int rig){
    response_t foo;
    if(lef<=node->rig && rig>=node->lef){
        if(lef<=node->lef && node->rig<=rig){
            foo.low=node->low;
            foo.hig=node->hig;
            foo.cen=node->cen;
        }else{
            response_t prf=query(node->chd[0], lef, rig);
            response_t suf=query(node->chd[1], lef, rig);
            if(~prf.low){
                foo.low=prf.low;
            }else{
                foo.low=suf.low;
            ...
            UPDMAX(foo.cen, prf.cen);
            UPDMAX(foo.cen, suf.cen);
            if(~prf.hig && ~suf.low){
                UPDMAX(foo.cen, v[suf.low]-v[prf.hig]);
            }
        }
    }
    return foo;
...
int main(){
    ...
    while(T--){
        ...
        sort(v, v+n);
        m=distance(v, unique(v, v+n));
        for(int i=0; i<n; i++)p[i]=distance(v, lower_bound(v, v+m, a[i]));
        ...
        cout<<0LL<<" "<<MAX(d-(a[hig]-a[low]), 0LL);
        ...
        for(int i=2; i<n; i++){
            ...
            lld foo=0LL;
            lld kpl=a[hig]-d, kpr=a[low]+d;
            if(kpl<=kpr){
                int lef=distance(v, lower_bound(v, v+m, kpl));
                int rig=distance(v, upper_bound(v, v+m, kpr))-1;
                if(lef<=rig && lef<m && rig>=0){
                    response_t rep=query(root, lef, rig);
                    if(~rep.low){
                        UPDMAX(rep.cen, v[rep.low]-kpl);
                        UPDMAX(rep.cen, kpr-v[rep.hig]);
                        UPDMAX(foo, (kpr-kpl)-rep.cen);
                    }
                }
            }
            UPDMAX(foo, d-(a[hig]-a[low]));
            cout<<" "<<foo;
        }
