...
void putin(stnode_t *node, const int pos, const int det){
    node->cnt+=det;
    if(node->lef!=node->rig){
        if(pos<=node->chd[0]->rig){
            putin(node->chd[0], pos, det);
        }else{
            putin(node->chd[1], pos, det);
        }
    }
}

int getpi(stnode_t *node, const int ofs){
    assert(0<=ofs && ofs<=node->cnt);
    if(node->lef==node->rig){
        return node->lef;
    }else{
        if(ofs<=node->chd[0]->cnt){
            return getpi(node->chd[0], ofs);
        }else{
            return getpi(node->chd[1], ofs-node->chd[0]->cnt);
        }
    }
}
...
lld calc(const int k){
    int c=root->cnt;
    lld foo=0LL;
    lld kpl=a[low], kpr=a[hig];
    if(k>0){
        lld npr=v[getpi(root, k)]+d;
        foo+=MAX(npr-kpr, 0LL);
        kpr=npr;
    }
    if(k<c){
        lld npl=v[getpi(root, k+1)]-d;
        foo+=MAX(kpl-npl, 0LL);
        kpl=npl;
    }
    foo+=MAX(d-(kpr-kpl), 0LL);
    return foo;
}

lld solve(){
    int lef=0, rig=root->cnt;
    while(lef<rig-1){
        int prv=round(((lef<<1)+rig)*1.0/3.0);
        int nxt=round((lef+(rig<<1))*1.0/3.0);
        if(calc(prv)<calc(nxt)){
            rig=nxt;
        }else{
            lef=prv;
        }
    }
    lld foo=0x3f3f3f3f3f3f3f3fLL;
    for(int i=MAX(lef-10, 0); i<=MIN(rig+10, root->cnt); i++){
        UPDMIN(foo, calc(i));
    }
    return foo;
}
