...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<n; i++){
            ...
            kps.push_back(m);
            for(int j=m-1; ~j; j--){
                if(suf[j]>=suf[kps.back()])continue;
                kps.push_back(j);
            }
            bst=0;
            for(int j=0; j<m; j++){
                while(j+1>kps.back())kps.pop_back();
                int foo=(pre[j+1]-bst)+b[j]+(suf[j+1]-suf[kps.back()]);
                UPDMAX(c[j], foo);
                UPDMIN(bst, pre[j+1]);
            }
            //from-left
            ...
            for(int j=0; j<m; j++){
                while(j+1>kps.back())kps.pop_back();
                int foo=(pre[j+1]-bst)+(suf[j+1]-suf[kps.back()]);
                UPDMAX(c[j], foo);
                UPDMIN(alt, pre[j]);
                UPDMIN(bst, alt-b[j]);
            }
            ...
            kps.push_back(0);
            for(int j=1; j<=m; j++){
                if(pre[j]>=pre[kps.back()])continue;
                kps.push_back(j);
            }
            bst=inf;
            alt=inf;
            for(int j=m-1; ~j; j--){
                while(j-1<kps.back()-1)kps.pop_back();
                int foo=(pre[j]-pre[kps.back()])+(suf[j]-bst);
                UPDMAX(c[j], foo);
                UPDMIN(alt, suf[j+1]);
                UPDMIN(bst, alt-b[j]);
            }
        ...
        int ans=-inf;
        for(int j=0; j<m; j++)UPDMAX(ans, b[j]);
        printf("%d\n", ans);
    }
    return 0;
}
