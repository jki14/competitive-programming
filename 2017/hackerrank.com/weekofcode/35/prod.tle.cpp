...
inline void fresh(stnode_t *node){
    if(node!=NULL){
        node->foo=-inf;
        fresh(node->chd[0]);
        fresh(node->chd[1]);
...
inline void update(stnode_t *node, const int lef, const int rig, const int foo){
    UPDMAX(node->foo, foo);
    if(node->chd[0]!=NULL && lef<=node->chd[0]->rig)update(node->chd[0], lef, rig, foo);
    if(node->chd[1]!=NULL && rig>=node->chd[1]->lef)update(node->chd[1], lef, rig, foo);
}

inline int query(stnode_t *node, const int lef, const int rig){
    int foo=-inf;
