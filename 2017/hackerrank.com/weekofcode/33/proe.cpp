...
struct graph_t{
    ...
    struct vertex_t{
        ...
        vertex_t *get_leader(){
            if(this->leader!=this){
                this->leader=this->leader->get_leader();
            }
            return this->leader;
        }
        vertex_t *get_lowest(){
            if(this->lowest!=this){
                this->lowest=this->lowest->get_lowest();
            }
            return this->lowest;
        }
    };
    ...
    vertex_t *vertex;
    vertex_t **stack, **ceil;
    edge_t *edge_pool, *last;
    int vertex_cap;
    int edge_cap;
    int timer;
    unordered_map<int, unordered_map<int, int>> lca_chest;
    /* Controller */
    graph_t(const int vertex_num, const int edge_num){
        this->vertex=NULL;
        this->edge_pool=NULL;
        this->stack=NULL;
        this->upgrade(vertex_num, edge_num);
    }
    ~graph_t(){
        free(this->vertex);
        free(this->edge_pool);
    ...
    void clear(int vertex_num=-1){
        if(!~vertex_num)
            vertex_num=this->vertex_cap;
        for(int i=0; i<vertex_num; i++){
            this->vertex[i].edge_hook=NULL;
            this->vertex[i].index=i;
            this->vertex[i].leader=this->vertex+i;
            this->vertex[i].lowest=this->vertex+i;
            this->vertex[i].dfn=-1;
            this->vertex[i].cut=0;
            this->vertex[i].instack=0;
        }
        this->last=this->edge_pool;
        this->ceil=this->stack;
        this->timer=0;
    }
    void upgrade(const int vertex_num, const int edge_num){
        this->vertex=(vertex_t*)realloc(this->vertex, sizeof(vertex_t)*vertex_num);
        this->edge_pool=(edge_t*)realloc(this->edge_pool, sizeof(edge_t)*edge_num);
        this->stack=(vertex_t**)realloc(this->stack, sizeof(vertex_t*)*vertex_num);
        this->vertex_cap=vertex_num;
        this->edge_cap=edge_num;
        this->clear();
    }
    /* Method */
    void add_edge(const int u, const int v, const int bidirect=1){
        //fprintf(stderr, "add edge %d->%d\n", u, v);
        if(this->vertex[u].get_lowest()!=this->vertex[v].get_lowest()){
            vertex_t *minor, *major;
            minor=this->vertex[u].get_lowest();
            major=this->vertex[v].get_lowest();
            if(minor>major){
                swap(minor, major);
            }
            major->lowest=minor;
        }
        this->last->source=this->vertex+u;
        this->last->dest=this->vertex+v;
        //this->last->cut=0;
        this->last->next=this->vertex[u].edge_hook;
        this->vertex[u].edge_hook=this->last++;
        if(bidirect){
            this->add_edge(v, u, 0);
        }
    }
    /* Algorithm */
    void tarjan(const int u, const int p=-1){
        vertex_t *principal=this->vertex+u;
        principal->low=this->timer;
        principal->dfn=this->timer++;
        *this->ceil++=principal;
        principal->instack=1;
        int descendant_num=0;
        for(edge_t *edge=principal->edge_hook; edge!=NULL; edge=edge->next){
            vertex_t *descendant=edge->dest;
            if(descendant->index==p)continue;
            if(!~descendant->dfn){
                descendant_num++;
                this->tarjan(descendant->index, u);
                if(descendant->low<principal->low){
                    principal->low=descendant->low;
                }
                //cut-edge
                /*if(descendant->low>principal->dfn){
                    
                }*/
                //cut-vertex
                if(~p && descendant->low>=principal->dfn){
                    principal->cut=1;
                    //fprintf(stderr, "cut found %d\n", principal->index);
                }
                //biconnected component
                if(descendant->low>=principal->dfn){
                    while(true){
                        vertex_t *curr=*(--this->ceil);
                        curr->instack=0;
                        //fprintf(stderr, "%d & %d\n", curr->get_leader()->index+1, descendant->index+1);
                        curr->get_leader()->leader=descendant;
                        if(curr==descendant)break;
                    }
                }
            }else{
                if(descendant->instack && descendant->dfn<principal->low){
                    principal->low=descendant->dfn;
                }
            }
        }
        if(!~p && descendant_num>1)principal->cut=1;
    }
    void clear_lca_query(){
        this->lca_chest.clear();
    }
    void add_lca_query(const int u, const int v, const int d=1){
        if(!this->lca_chest.count(u))
            this->lca_chest.insert(MP(u, unordered_map<int, int>()));
        this->lca_chest[u].insert(MP(v, -1));
        if(d){
            this->add_lca_query(v, u, 0);
        }
    }
    void tarjan_lca(const int u, const int p=-1){
        vertex_t *principal=this->vertex+u;
        principal->dfn=this->timer++;
        for(edge_t *edge=principal->edge_hook; edge!=NULL; edge=edge->next){
            vertex_t *descendant=edge->dest;
            if(descendant->index==p)continue;
            //fprintf(stderr, "%d -> %d\n", principal->index, descendant->index);
            if(!~descendant->dfn){
                this->tarjan_lca(descendant->index, u);
                descendant->get_leader()->leader=principal;
            }else{
                //not tree
                assert(0);
            }
        ...
        if(this->lca_chest.count(u)){
            for(auto &foo : this->lca_chest[u]){
                if(!~foo.second){
                    int v=foo.first;
                    if(~this->vertex[v].dfn){
                        foo.second=this->vertex[v].get_leader()->index;
                        this->lca_chest[v][u]=foo.second;
                    }
                }
            }
        }
...
unordered_multiset<lld> edge_filter;

graph_t g(110000, 410000), t(110000, 210000);
...
int main(){
    while(scanf("%d%d%d", &n, &m, &q)!=EOF){
        ...
        /*for(graph_t::edge_t *edge=g.edge_pool; edge!=g.last; edge++){
            if(edge->source->cut || edge->dest->cut)continue;
            //fprintf(stderr, "blind %d - %d\n", edge->dest->get_leader()->index, edge->source->get_leader()->index);
            edge->dest->get_leader()->leader=edge->source->get_leader();
        }*/
        edge_filter.clear();
        for(graph_t::edge_t *edge=g.edge_pool; edge!=g.last; edge++){
            int u=edge->source->get_leader()->index;
            int v=edge->dest->get_leader()->index;
            if(u==v)continue;
            if(u>v)swap(u, v);
            lld key=((u*1LL)<<30)|v;
            if(!edge_filter.count(key)){
                t.add_edge(u, v);
            }
            edge_filter.insert(key);
            //fprintf(stderr, "add_edge %d->%d (%d->%d)\n", u, v, edge->source->index, edge->dest->index);
        }
        t.clear_lca_query();
        for(int i=0; i<q; i++){
            ans[i]=-1;
            ...
            v--;
            w--;
            if(g.vertex[u].get_lowest()!=g.vertex[v].get_lowest()){
                ans[i]=0;
                continue;
            }
            if(g.vertex[v].get_lowest()!=g.vertex[w].get_lowest()){
                ans[i]=0;
                continue;
            }
            //fprintf(stderr, "get query %d, %d, %d (%d)\n", u, v, w, i);
            foo[i].u=u=g.vertex[u].get_leader()->index;
            foo[i].v=v=g.vertex[v].get_leader()->index;
            foo[i].w=w=g.vertex[w].get_leader()->index;
            //fprintf(stderr, "prepare query %d, %d, %d (%d)\n", u, v, w, i);
            if(u==w || v==w){
                ans[i]=1;
            }else if(u==v){
                lld key;
        ...
        for(int i=0; i<n; i++){
            if(g.vertex[i].get_leader()->index!=i)continue;
            if(!~t.vertex[i].dfn){
                t.tarjan_lca(i);
            }
        }
        for(int i=0; i<q; i++){
            ...
            int v=foo[i].v;
            int w=foo[i].w;
            //fprintf(stderr, "handle query %d, %d, %d (%d)\n", u, v, w, i);
            if(t.lca_chest[u][v]==w){
                //fprintf(stderr, "1\n");
                ans[i]=1;
                continue;
            }
            if(t.lca_chest[u][w]==w && t.lca_chest[v][w]!=w){
                //fprintf(stderr, "2\n");
                ans[i]=1;
                continue;
            }
