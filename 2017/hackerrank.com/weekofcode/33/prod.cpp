...
template<class T> class arr2d_t{
    private:
    size_t r, c;
    size_t ps, as;
    T *pool;
    T **arr;
    public:
    arr2d_t(const size_t rows=1, const size_t cols=1){
        this->pool=NULL;
        this->ps=0;
        this->arr=NULL;
        this->as=0;
    ...
    void resize(const size_t rows, const size_t cols){
        if(this->ps<rows*cols){
            this->pool=(T*)realloc(this->pool, sizeof(T)*rows*cols);
            this->ps=rows*cols;
        }
        if(this->as<rows){
            this->arr=(T**)realloc(this->arr, sizeof(T*)*rows);
            this->as=rows;
        }
        T *last=this->pool;
...
arr2d_t<int> a(320, 320);
int h[110000], lef[16], rig[16];
int s[110000], d[110000];
...
void process(const int x){
    ...
    for(int y=x; y<n; y++){
        ...
        for(int i=0; i<m; i++){
            ...
            while(cent<dig[10]){
                int foo=cur^cent;
                //fprintf(stderr, "_prev[%d]=%d\n", foo, _prev[foo]);
                if(_prev[foo]+1<=bar){
                    int value=(y-x+1)*(i-_prev[foo]);
                    if(ans<value){
                        ans=value;
                        ax=x;
                        ay=_prev[foo]+1;
