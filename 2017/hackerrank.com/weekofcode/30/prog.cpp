...
struct sam_t{
    ...
    void extend(const int c){
        node_t *p=this->tail, *q=last++;
        q->clear();
        q->val=p->val+1;
        for(; p && !p->next[c]; p=p->prev)
            p->next[c]=q;
        ...
        if(!p){
            ...
            if(p->val+1==x->val){
                ...
                node_t *y=last++;
                memcpy(y->next, x->next, sizeof(node_t*)*C);
                y->val=p->val+1;
                y->prev=x->prev;
                x->prev=q->prev=y;
                for(; p && p->next[c]==x; p=p->prev)
                    p->next[c]=y;
            }
        }
        this->tail=q;
...
int calc(int x, int y){
    if(x>y)swap(x, y);
    pii req=MP(x, y);
    map<pii, int>::const_iterator it=cache.find(req);
    if(it!=cache.end()){
        return it->second;
    }
    int foo=0;
