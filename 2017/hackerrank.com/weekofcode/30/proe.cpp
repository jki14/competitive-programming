...
struct req_t{
    ...
    int dvs, rem;
    int idx, res;
    void read(const int offset){
        this->idx=offset;
        scanf("%d%d%d%d", &this->lef, &this->rig, &this->dvs, &this->rem);
    }
    bool operator < (const req_t &rhs) const{
        if(this->dvs!=rhs.dvs)return this->dvs<rhs.dvs;
        return this->rem<rhs.rem;
    }
} req[41000];
...
struct node_t{
    int lef, rig, val;
    node_t *chd[2];
} pool[810000], *tail, *root[41000];

int n, m;
int a[41000];
int cnt[41000], cov[41000];

int dvs, len;

int low, hig, ans[41000];

int area(const req_t &lhs, const req_t &rhs){
    int lid=lhs.lef/len;
    int rid=rhs.lef/len;
    if(lid!=rid){
        return lid<rid;
...
node_t* empty(const int lef, const int rig){
    node_t *node=tail++;
    node->lef=lef;
    node->rig=rig;
    node->val=0;
    if(lef==rig){
        ...
        int mid=(lef+rig)>>1;
        node->chd[0]=empty(lef, mid);
        node->chd[1]=empty(mid+1, rig);
    }
    return node;
}

node_t* insert(node_t *prev, const int pos){
    node_t *node=tail++;
    node->lef=prev->lef;
    node->rig=prev->rig;
    node->val=prev->val+1;
    if(node->lef==node->rig){
        ...
        if(pos<=prev->chd[0]->rig){
            node->chd[0]=insert(prev->chd[0], pos);
            node->chd[1]=prev->chd[1];
        }else{
            node->chd[0]=prev->chd[0];
            node->chd[1]=insert(prev->chd[1], pos);
        }
    }
    return node;
}

inline int query(node_t *drop, node_t *keep, const int pos){
    if(keep->lef==keep->rig){
        return keep->val-drop->val;
    }else{
        if(pos<=keep->chd[0]->rig){
            return query(drop->chd[0], keep->chd[0], pos);
        }else{
            return query(drop->chd[1], keep->chd[1], pos);
        }
    }
}

inline void flip(const int pos){
    if(pos<0)return;
    cov[pos]^=1;
    if(cov[pos]){
        cnt[a[pos]%dvs]++;
...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int k=0; k<m; k++){
            ...
            if(dvs>cap){
                if(root[0]==NULL){
                    root[0]=empty(low, hig);
                    for(int i=1; i<=n; i++){
                        root[i]=insert(root[i-1], a[i-1]);
                    }
                }
                req[k].res=0;
                for(int i=low/dvs*dvs+req[k].rem; i<=hig; i+=dvs){
                    if(i<low)continue;
                    req[k].res+=query(root[req[k].lef], root[req[k].rig+1], i);
                }
            }else{
                int p=k, q=k;
                while(q+1<m && req[q+1].dvs==dvs)q++;
                for(int i=0; i<n; i++)cov[i]=0;
                for(int i=0; i<dvs; i++)cnt[i]=0;
                sort(req+p, req+q, area);
                int lef=-1, rig=-1;
                for(int i=p; i<=q; i++){
                    while(lef<req[i].lef)flip(lef++);
                    while(lef>req[i].lef)flip(--lef);
                    while(rig<req[i].rig)flip(++rig);
                    while(rig>req[i].rig)flip(rig--);
                    req[i].res=cnt[req[i].rem];
                }
                k=q;
            }
        }
        for(int i=0; i<m; i++){
            ans[req[i].idx]=req[i].res;
        }
        for(int i=0; i<m; i++){
            printf("%d\n", ans[i]);
        }
