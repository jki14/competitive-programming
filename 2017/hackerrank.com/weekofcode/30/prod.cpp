...
inline lld calc(const int k, const int i, const int p){
    return f[p][k-1]+(c[p+1]-c[i+1])-(s[p+1]-s[i+1])*h[i];
}

/*
f[p][k-1]+c[p+1]-s[p+1]*h[i] < f[q][k-1]+c[q+1]-s[q+1]*h[i]

=>

(f[p][k-1]+c[p+1])-(f[q][k-1]+c[q+1])/(s[p+1]-s[q+1])<h[i]

*/

inline int prefer(const int k, const int i, const int p, const int q){
    lld foo=(f[p][k-1]+c[p+1])-(f[q][k-1]+c[q+1])-(s[p+1]-s[q+1])*h[i];
    return (foo!=0LL)?(foo>0LL?1:-1):0;
}

inline int prefer(const int k, const int i, const int p, const int q, const int r){
    __int128 foo=((__int128)((f[q][k-1]+c[q+1])-(f[r][k-1]+c[r+1])))*((__int128)(s[p+1]-s[q+1]));
    __int128 goo=((__int128)((f[p][k-1]+c[p+1])-(f[q][k-1]+c[q+1])))*((__int128)(s[q+1]-s[r+1]));
    return (foo!=goo)?(foo>goo?1:-1):0;
}
...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<n; i++){
            f[i][0]=(c[0]-c[i+1])-(s[0]-s[i+1])*h[i];
        }
        for(int k=1; k<m; k++){
            lef=rig=0;
            for(int i=k; i<n; i++){
                while(lef+1<rig && prefer(k, i, que[rig-2], que[rig-1], i-1)>=0)rig--;
                que[rig++]=i-1;
                while(lef+1<rig && prefer(k, i, que[lef], que[lef+1])>=0)lef++;
                f[i][k]=calc(k, i, que[lef]);
            }
        }
        printf("%lld\n", f[n-1][m-1]);
    }
