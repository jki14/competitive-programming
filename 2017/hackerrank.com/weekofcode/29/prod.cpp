...
void boundary(lld &head, lld &tail, const lld prev){
    char foo[24];
    sprintf(foo, "%lld", prev);
    //puts(foo);
    int len=strlen(foo);
    int det=17-len;
    for(int i=len-1; ~i; i--){
    ...
    //puts(foo);
    for(int i=9; i>=det; i--){
        if(flag || (foo[i]!='2' && foo[i]!='3' && foo[i]!='5' && foo[i]!='7')){
            flag=next(foo[i]);
            offset=i;
        }
    }
    //puts(foo);
    for(int i=offset+1; i<17; i++)foo[i]='2';
    //puts(foo);
    if(flag){
        next(foo[det-1]);
    }
...
lld lowb(const lld x){
    ...
    foo[0]='0';foo[len+1]=0;
    int flag=0, offset=len;
    for(int i=len; ~i; i--){
        if(flag || (i!=0 && foo[i]!='2' && foo[i]!='3' && foo[i]!='5' && foo[i]!='7')){
            flag=next(foo[i]);
            offset=i;
        }
    }
    for(int i=offset+1; i<=len; i++)foo[i]='2';
    lld ret;
    sscanf(foo, "%lld", &ret);
    return ret;
}

int process(const lld head, const lld tail, int *seq){
    //printf("%lld -> %lld\n", head, tail);
    if(head>tail)return 1;
    bitset<5555556> rec;
    for(int *p=seq; *p; p++){
        lld a=(*p)*1LL;
        for(lld np=MAX(a, (head+a-1LL)/a)*a; np<=tail; np+=a){
            rec.set(np-head);
        }
    }
    for(int i=0; i<5555556; i++){
        if(head+i*1LL>tail)break;
        if(rec.test(i))continue;
        if(ismega(head+i*1LL)){
            //printf("%lld\n", head+i*1LL);
            ans++;
        }
    }
...
int main(){
    int *seq=PRIMELIST(31622777);
    while(scanf("%lld%lld", &vale, &peak)!=EOF){
        ans=0;
        lld head, tail=vale-1LL;
        int flag=1;
        while(1){
            boundary(head, tail, tail);
            UPDMIN(tail, peak);
            if(flag){
                flag=0;
                for(lld patch=lowb(vale-1LL); patch<=MIN(head-1LL, tail); ){
                    lld next=MIN(patch+5555555LL, MIN(head-1LL, tail));
                    process(patch, next, seq);
                    patch=lowb(next);
                }
            }
            if(process(head, tail, seq)){
                break;
            }
        }
        printf("%d\n", ans);
