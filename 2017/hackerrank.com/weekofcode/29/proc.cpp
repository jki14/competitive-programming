...
int main(){
    ...
    if(scanf("%d%d", &w, &h)!=EOF){
        ...
        for(int x=0; x<h; x++){
            for(int y=0; y<w; y++){
                c[x][y]='.';
                if(point_t<int>(x, y).dist2(point_t<int>(xc, yc))<=r*r){
                    c[x][y]='#';
                }
                point_t<double> p1(x1, y1);
                point_t<double> p3(x3, y3);
                point_t<double> po((x1+x3)*0.5, (y1+y3)*0.5);
                point_t<double> p2=p1;
                point_t<double> p4=p3;
                point_t<double> px(x, y);
                p2.rotate(po, PI*0.5);
                p4.rotate(po, PI*0.5);
                if(
                    p1.cross(p2, px)>-eps
