...
struct solution_t{
    ...
    int contain(const int x){
        int lef=0, rig=len-1;
        while(lef<=rig){
            int mid=(lef+rig)>>1;
            if(sol[mid]>=x){
                if(sol[mid]==x)return 1;
                rig=mid-1;
            }else lef=mid+1;
        }
        return 0;
...
int find(const double d){
    int lef=0, rig=num-1;
    while(lef<=rig){
        const int mid=(lef+rig)>>1;
        if(solution[mid].value-d<eps){
            if(mid==rig || solution[mid+1].value>d)return mid;
            lef=mid+1;
        }else rig=mid-1;
    }
    return -1;
...
void process(const int pos, int remain){
    if(pos==68){
        if(foo.len==6){
            memcpy(&solution[num++], &foo, sizeof(solution_t));
            /*if(num%1000==0){
                printf("%d\n", num);
            }*/
        }
    }else{
        const int peak=SIZE(post[0][pos]);
        remain-=peak;
        for(int k=MAX(6-remain-foo.len, 0); k<=peak && foo.len+k<=6; k++){
            const double cache=foo.value;
            for(int i=0; i<k; i++)foo.sol[foo.len++]=pos;
            foo.value+=dist[pos]*k;
            foo.value-=floor(foo.value);
            process(pos+1, remain);
            foo.len-=k;
            foo.value=cache;
        }
    }
...
int check(const int x, const int y, const int offset, double &best){
    double value=solution[x].value+solution[y].value;
    value=fabs(value-round(value));
    UPDMIN(best, value);
    if(value<1e-12){
        printf("{");
        for(int p=0; p<6; p++){
            printf("%d,", (int)solution[x].sol[p]);
        }
        for(int p=0; p<6; p++){
            if(p)printf(",");
            printf("%d", (int)solution[y].sol[p]);
        }
        printf("},//%d\n", offset);
        return 1;
    }
...
int main(){
    solution=(solution_t*)malloc(sizeof(solution_t)*S);
    ...
    int remain=0;
    for(int i=0; i<68; i++){
        assert(SIZE(post[0][i])==SIZE(post[1][i]));
        //printf("[%d] = %d\n", i, SIZE(post[0][i]));
        remain+=SIZE(post[0][i]);
    }
    num=0;
    foo.len=0;
    foo.value=0.0;
    process(0, remain);
    sort(solution, solution+num);
    for(int x=0; x<68; x++){
        int flag=0, test=0;
        double best=2.0;
        for(int i=0; i<num; i++){
            if(solution[i].contain(x)){
                int j=find(1.0-solution[i].value);
                if(~j){
                    test++;
                    if(check(i, j, x, best)){
                        flag=1;
