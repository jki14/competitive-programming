...
inline void insert(node_t *node, const char *a){
    for(int i=0; a[i]; i++){
        if(node->next[a[i]-'0']==NULL){
            node->next[a[i]-'0']=newnode();
        }
        node=node->next[a[i]-'0'];
    }
    node->flag++;
}

inline void draw(node_t *node){
    seq[til]=0;
