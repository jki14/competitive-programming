...
inline int issqr(const int x){
    const int y=round(sqrt(x*1.0));
    return x==sqr(y);
}

inline void update(int &x){ if(!x)x=1; }

int x, y;
int f[626][13][3273];

void draw(const int pos, const int cnt, const int val){
    if(!pos)return;
    if(f[pos][cnt][val]>0){
        const int _x=(pos-1)%25-12;
        const int _y=(pos-1)/25-12;
        const int inc=sqr(_x)+sqr(_y);
        if(x!=_x || y!=_y){
            printf("%d %d\n", _x, _y);
        }
        draw(pos-1, cnt-1, val-inc);
    }else draw(pos-1, cnt, val);
}
        ...
        for(int i=0; i<625; i++){
            for(int j=0; j<=12; j++){
                for(int k=0; k<=3272; k++){
                    if(!f[i][j][k])continue;
                    const int _x=(i%25)-12;
                    const int _y=(i/25)-12;
                    if(x!=_x || y!=_y){
                        f[i+1][j][k]=-1;
                    }
                    const int inc=sqr(_x)+sqr(_y);
                    if(j==12 || issqr(inc))continue;
                    update(f[i+1][j+1][k+inc]);
                }
            }
        }
        for(int k=0; k<=3272; k++){
            if(f[625][12][k] && issqr(k)){
                draw(625, 12, k);
                break;
            }
        }
