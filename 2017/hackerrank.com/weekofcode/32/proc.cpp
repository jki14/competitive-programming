...
int n, s, t;
int a[11000000], g[11000000];
int x, y, z;

int ans;

inline int between(const pii &inv, const int pos){
    if(pos>=inv.first && pos<=inv.second)return 1;
    if(inv.first>inv.second && pos>=inv.first)return 1;
    if(inv.first>inv.second && pos<=inv.second)return 1;
    return 0;
}

inline int next(const int pos){
    return (pos+1)%n;
}

inline int prev(const int pos){
    return (pos-1+n)%n;
}
...
int main(){
    //while(scanf("%d%d", &n, &s, &t)!=EOF){
    if(scanf("%d%d%d", &n, &s, &t)!=EOF){
        scanf("%d%d%d%d", &a[0], &x, &y, &z);
        for(int i=1; i<n; i++){
            a[i]=(a[i-1]*1LL*x+y*1LL)%(z*1LL);
        }
        for(int i=0; i<n; i++){
            g[i]=a[(i+s)%n];
        }
        ...
        if(t!=0){
            ...
            for(int len=1; ; len++){
                ...
                pii _que=que;
                for(int u=que.first; between(que, u); u=next(u)){
                    if(!between(vst, u)){
                        if(dist(_que.first, u)<g[u]){
                            int lef=(u-g[u]+n)%n;
                            if(between(_que, lef)){
                                lef=next(_que.second);
                            }
                            _que.first=lef;
                        }
                        if(dist(u, _que.second)<g[u]){
                            int rig=(u+g[u]+n)%n;
                            if(between(_que, rig)){
                                rig=prev(_que.first);
                            }
