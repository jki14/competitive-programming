...
class SuffixAutomaton{
    ...
    struct node_t{
        ...
        int contribution(){
            ...
            //assert(this->valid-this->prev->valid>=0);
            //return this->valid-this->prev->valid;
            return MAX(this->valid-this->prev->valid, 0);
    ...
    public:
    int foo;
    SuffixAutomaton(int len=110000){
        this->pool=(node_t*)malloc(sizeof(node_t)*len*2);
        this->clear();
    }
    ~SuffixAutomaton(){
        free(this->pool);
    ...
    void clear(){
        this->last=this->pool;
        this->head=this->tail=this->last++;
        this->head->clear();
    }
    void extend(const int chr, const int pos, const int len){
        this->foo=0;
        node_t *p=this->tail, *q=this->last++;
        q->clear();
        q->value=pos+1;
        q->valid=len;
        q->index=pos+1;
        for(; p!=NULL && p->next[chr]==NULL; p=p->prev){
            p->next[chr]=q;
        }

        if(p==NULL){
            q->prev=this->head;
            this->foo+=q->contribution();
        }else{
            node_t *x=p->next[chr];
            if(p->value+1==x->value){
                q->prev=x;
                this->foo+=q->contribution();
            }else{
                node_t *y=this->last++;
                y->clear();
                for(int i=0; i<26; i++)y->next[i]=x->next[i];
                this->foo-=x->contribution();
                y->value=p->value+1;
                y->valid=MIN(x->valid, p->value+1);
                y->index=-x->index;
                y->prev=x->prev;
                x->prev=q->prev=y;
                this->foo+=x->contribution()+q->contribution()+y->contribution();
                //this->foo+=q->contribution();
                for(; p!=NULL && p->next[chr]==x; p=p->prev){
                    p->next[chr]=y;
                }
                for(p=x; p->prev!=NULL; p=p->prev){
                    int fresh=MIN(p->prev->value, p->valid);
                    if(fresh>p->prev->valid){
                        p->prev->valid=fresh;
                    }else break;
                }
            }
        ...
        //printf("%d (<%c>len=%d) .prev = %d (valid=%d)\n", p->index, a[pos], len, p->prev->index, p->prev->valid);

        this->tail=q;
    }
};

int extend(char *str, int pos){
    node_t *curr=last;
    int length=0;
    int chr=str[pos]-'a';
    assert(0<=chr && chr<26);
    ...
    while(1){
        length=curr->len;
        if(pos-1-length>=0 && str[pos-1-length]==str[pos])break;
        curr=curr->fail;
    }
    if(curr->next[chr]!=NULL){
        last=curr->next[chr];
        rig=pos+1;
        lef=rig-last->len;
        return 0;
    ...
    while(1){
        curr=curr->fail;
        length=curr->len;
        if(pos-1-length>=0 && str[pos-1-length]==str[pos]){
            node->fail=curr->next[chr];
            //if(node->fail->failed==0){
            //    node->fail->failed=1;
            //    foo-=node->fail->len;
            //}
            break;
    ...
    node->num=node->fail->num+1;

    return 1;
}


SuffixAutomaton sam(310000);
...
int main(){
    while(scanf("%d", &n)!=EOF){
        //INIT
        tail=pool;
        for(int i=0; i<26; i++)tail->next[i]=NULL;
        tail->len=-1;
        tail->num=0;
        tail->failed=1;
        tail->fail=tail;
        ...
        for(int i=0; i<n; i++){
            extend(a, i);
            sam.extend(a[i]-'a', i, rig-lef);
            //printf("foo=%d\n", sam.foo);
            //int prev=i+1-sam.foo;
            //ans+=MIN(sam.foo, rig-lef-prev);
            ans+=sam.foo;
            printf("%d\n", ans);
        }
    }
