...
int main(){
    for(int i=1; i<=500000; i++)c[i].clear();
    for(int x=1; x<=500000; x++){
        vector<pii> g=FACTORIZE(x);
        int mask=1;
        d[x]=1;
        for(pii &row : g){
            if(row.second&1){
            ...
            d[x]*=POW(row.first, row.second>>1);
        }
        c[mask].push_back(x);
    }
    if(scanf("%d", &n)!=EOF){
        scanf("%s", a+1);
        ans=0;
        for(int mask=1; mask<=500000; mask++){
            int len=SIZE(c[mask]);
            if(len<2)continue;
            for(int i=0; i<len; i++){
                if(c[mask][i]>n)break;
                if(a[c[mask][i]]!='a')continue;
                for(int j=0; j<len; j++){
                    if(c[mask][j]>n)break;
                    if(a[c[mask][j]]!='c')continue;
                    int k=d[c[mask][i]]*d[c[mask][j]]*mask;
                    if(a[k]=='b')ans++;
                }
            }
        }
