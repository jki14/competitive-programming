...
int main(){
    ...
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<n; i++){
            int c;
            scanf("%d", &c);
            mcf.add_edge(254, i, c, 0);
        }
        for(int i=0; i<m; i++){
            int c;
            scanf("%d", &c);
            mcf.add_edge(n+i, 255, c, 0);
            for(int k=1; c<n; c++, k++){
                mcf.add_edge(n+i, 255, 1, k*k-(k-1)*(k-1));
            }
        }
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++){
                int w;
                scanf("%d", &w);
                mcf.add_edge(i, n+j, 1, -w);
            }
            mcf.add_edge(i, 255, inf, 0);
        }
        printf("%d\n", -mcf.run(254, 255).second);
    }
    return 0;
}
