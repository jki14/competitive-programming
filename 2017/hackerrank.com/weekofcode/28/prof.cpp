...
int calc(const int low, const int pos){
    if(~f[low][pos])return f[low][pos];
    int &foo=f[low][pos];
    int valid=1;
    int curr=0, last=-1;
    foo=inf;
    ...
    if(~last){
        if(curr!=0 || prev.empty())prev.push(last);
        int best=inf;
        for(; !prev.empty(); prev.pop()){
            UPDMIN(best, calc(low-1, prev.front()));
        }
        curr+=best;
    }else{
        curr+=(pos!=0)?1:0;
...
int main() {
    ...
    while(T--){
        ...
        for(pair<const char, int> &foo: dis){
            foo.second = m++;
        }
        for(int i=0; i<n; i++){
            a[i]='a'+dis[a[i]];
