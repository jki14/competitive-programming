...
struct map_t{
    ...
    map<int, double> hig;
    map_t(){
        this->low=(double*)malloc(sizeof(double)*S);
    }
    ~map_t(){
        free(this->low);
    }
    void clear(){
        for(int i=0; i<S; i++)this->low[i]=-1.0;
        hig.clear();
    }
    double &get(const int pos){
        if(pos<S){
            ...
            map<int, double>::iterator it=hig.find(pos);
            if(it==hig.end()){
                hig.insert(MP(pos, -1.0));
            }
            it=hig.find(pos);
            return it->second;
        }
...
double run(const int s, const int n, const int m);

inline int rev(const int s, const int n){
    int foo=bit[n];
    for(int i=0; i<n; i++){
        if(s&bit[i]){
            foo|=bit[n-i-1];
...
inline double calc(const int s, const int p, const int n, const int m){
    double foo=(s&bit[p])?1.0:0.0;
    if(m>1){
        int nxt=(s&(bit[p]-1))|((s>>(p+1))<<p);
        foo+=run(nxt, n-1, m-1);
    }
    return foo;
}

double run(int s, const int n, const int m){
    UPDMIN(s, rev(s, n));
    double &foo=mymap.get(s);
    if(foo>-eps)return foo;
    foo=0.0;
    double den=1.0/n;
    for(int i=0; i<n; i++){
        foo+=MAX(calc(s, i, n, m), calc(s, n-i-1, n, m))*den;
    }
    return foo;
}
