...
int test(vector<int> &a){
    int n=SIZE(a), pos=0, til=n-1;
    while(pos<n && a[pos])a[pos++]=2;
    while(~til && a[til])a[til--]=2;
    for(int i=pos+1; i<til; i++){
        if(a[i] && a[i+1]){
            a[i]=a[i+1]=2;
        }
    }
...
int process(const int s, const int n){
    if(~f[s][n])return f[s][n];
    f[s][n]=0;
    set<int> d;
    d.clear();
    for(int i=1; i<n-1; i++){
        if(!BINT_TEST(s, i-1) && !BINT_TEST(s, i+1)){
            int mask=BINT(i)-1;
            int next=(s&mask)|((s>>1)&(~mask));
            d.insert(process(next, n-1));
        }
    }
    vector<int> cache;
    cache.resize(n);
    for(int i=0; i<n; i++){
        cache[i]=BINT_TEST(s, i);
    }
    int goo=test(cache);
    while(d.count(f[s][n]))f[s][n]++;
    printf("f[");
    for(int i=0; i<n; i++){
        if(BINT_TEST(s, i)){
            printf("1");
