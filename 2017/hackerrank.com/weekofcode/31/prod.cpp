...
pair<double, pii> calc(const double rat){
    for(int i=0; i<m; i++){
        e[i].update(rat);
    }
    sort(e, e+m);
    ...
    //printf("rate=%.9f value = %.9f\n", rat, sum);
    return MP(sum, MP(x/GCD(x, y), y/GCD(x, y)));
}

pii process(){
    double lef=0.01, rig=100.0;
    while(lef+eps<rig){
        double mid=(lef+rig)*0.5;
        if(calc(mid).first>-eps){
            lef=mid;
        }else{
            rig=mid;
        }
    }
    return calc(lef).second;
}
