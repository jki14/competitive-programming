...
node_t *build(const int lef, const int rig){
    ...
    if(lef<rig){
        ...
        node->peak=MAX(node->chd[0]->peak, node->chd[1]->peak);
        node->vale=MIN(node->chd[0]->vale, node->chd[1]->vale);
    }else{
        node->peak=node->vale=a[lef];
    }
    return node;
}

pii calc(node_t *node, const int lef, const int rig){
    if(rig<node->lef || lef>node->rig)return MP(-inf, inf);
    if(lef<=node->lef && node->rig<=rig){
        return MP(node->peak, node->vale);
    }else{
        pii chd0=calc(node->chd[0], lef, rig);
        pii chd1=calc(node->chd[1], lef, rig);
        return MP(MAX(chd0.first, chd1.first), MIN(chd0.second, chd1.second));
    }
}

int calc(const int lef, const int rig){
    pii foo = calc(root, lef, rig);
    return foo.first-foo.second;
}

void process(const int low, const int hig){
    lld ans=0LL;
    int lef=0, rig=-1;
    for(int i=0; i<n; i++){
        while(lef<n && calc(i, lef)<low)lef++;
        while(rig+1<n && calc(i, rig+1)<=hig)rig++;
        ans+=MAX(rig-lef+1, 0)*1LL;
    }
    printf("%lld\n", ans);
}
