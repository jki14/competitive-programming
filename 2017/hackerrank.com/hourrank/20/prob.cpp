...
inline void put(const int x, const int y){
    ...
    pii foo=MP(x, y);
    map<pii, int>::iterator it=c.find(foo);
    if(it!=c.end()){
        it->second++;
    }else{
        c.insert(MP(foo, 1));
...
int calc(int x, int y){
    ...
    for(int i=1; i<=x; i++){
        ...
        MOD_ADD(&foo, MOD_MUL(lef, rig));
        //printf("foo=%d(+%d)\n", MOD_STD(foo), MOD_STD(MOD_MUL(lef, rig)));
    }
    return foo;
}
...
int main(){
    int T;scanf("%d", &T);
    for(int i=1; i<=500000; i++)inv[i]=INVERSEE(i);
    for(int _T=1; _T<=T; _T++){
        ...
        ans=MOD_ADD(MOD_MUL(calc(c[0], c[1]), calc(c[2], c[3])), -1);
        MOD_STD(&ans);
        printf("%d\n", ans);
    }
    return 0;
