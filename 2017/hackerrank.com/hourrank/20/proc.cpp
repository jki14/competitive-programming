...
void count(const int u, const int p){
    cnt[u]=1;
...
seg_t *process(const int u, const int p, const int lef, const int rig, const int sup){
    seq[rig]=u;
    int d=-1;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
    ...
    if(!~d){
        vrt[u].root=tail++;
        vrt[u].root->head=NULL;
        vrt[u].root->offset=lef-1;
        vrt[u].root->mask=-1;
    }else{
        ...
        for(int k=0; k<q; k++){
            ...
            for(int i=0; i<m; i++){
                ...
                for(int v=u; ~v; v=vrt[v].sup){
                    seg.insert(vrt[v].root);
                    if(vrt[v].root->mask!=flag){
                        vrt[v].root->head=NULL;
                        vrt[v].root->mask=flag;
                    }
                    //vrt[v].root->mst.insert(vrt[v].pos-vrt[v].root->offset);
                    num_tail->next=vrt[v].root->head;
                    num_tail->value=vrt[v].pos-vrt[v].root->offset;
                    vrt[v].root->head=num_tail++;
                }
            }
            for(int i=0; i<=m; i++)ans[i]=0;
            ans[0]=n;
            for(set<seg_t*>::iterator sit=seg.begin(); sit!=seg.end(); sit++){
                seg_t *seg=*sit;
                int til=0, cov=0;
                for(num_t *p=seg->head; p; p=p->next){
                    seq[til++]=p->value;
                }
                sort(seq, seq+til, greater<int>());
                for(int i=0; i<til; i++){
                    ans[cov]-=seq[i];
                    ans[++cov]+=seq[i];
                }
