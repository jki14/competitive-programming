...
int res[210000];
vector<pii> a, b;
list<pii> lhs[2100000], rhs[2100000];
...
int main() {
  ...
  while (scanf("%d%d", &n, &m) != EOF) {
    ...
    for (int i = 0; i < q; ++i) {
      int l, r;
      scanf("%d%d", &l, &r);
      lhs[l].emplace_back(r, i);
      rhs[r].emplace_back(l, i);
      res[i] = 0;
    }
    ...
    }
    {
      auto iter = a.begin();
      for (int i = 1; i <= n; ++i) {
        while (iter != a.end() && iter->x == i) {
          // fprintf(stderr, "+ %d -> %d\n", iter->x, iter->y);
          add(iter->y);
          ++iter;
        }
        for (auto const ri : lhs[i]) {
          res[ri.y] += sum(ri.x) - sum(i - 1);
        }
      }
    }
    ...
    }
    {
      auto iter = b.rbegin();
      for (int i = n; i >= 1; --i) {
        while (iter != b.rend() && iter->x == i) {
          add(iter->y);
          ++iter;
        }
        for (auto const li : rhs[i]) {
          res[li.y] += sum(i) - sum(li.x - 1);
        }
      }
    }
