...
using imod_t2 = joshu::imod_t<998244353>;
} // namespace
...
lld a, b, c, d;
imod_t2 f[4100000], g[4100000];

imod_t2 comb(lld const n, lld const k) {
  return f[n] * g[k] * g[n - k];
}

imod_t2 comb2(lld const n, lld const r) {
  return comb(n + r - 1, r);
}
...
int main() {
  ...
  g[4000003] = f[4000003].pow(998244353 - 2);
  for (int i = 4000002; i >= 0; --i) {
    g[i] = g[i + 1] * (i + 1);
  }
  while (scanf("%lld%lld%lld%lld", &a, &b, &c, &d) != EOF) {
    imod_t2 bar = 0;
    for (lld br = 0LL; br <= a + c; ++br) {
      imod_t2 const lhs = comb2(br + 1LL, b - 1LL);
      imod_t2 const rhs = comb2(c + 1LL - max(br - a, 0LL), d);
      bar += lhs * rhs;
    }
    printf("%lld\n", bar.lld());
  }
