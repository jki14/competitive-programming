...
using loc_t = joshu::tuple3d_t<lld, lld, lld>;
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    */
    ...
    for (int i = 0; i < n; ++i) scanf("%lld", &a[i].z);
    for (int i = 0; i < n; ++i) scanf("%lld", &a[i].x);
    sort(a, a + n);

    lld foo = 0, bar = 0, tic = 0, lhs = 0, rhs = 0;
    for (int i = 0; i < n; ++i) {
      lhs += a[i].x * a[i].y;
    }
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = n - 1; i >= 0; --i) {
      if (a[i].y && a[i].z) {
        bar -= a[i].x * tic;
        bar += lhs - prev - a[i].x + rhs - prev;
        prev += a[i].x;

        joshu::setmin(res, bar);
      } else if (a[i].z) {
        rhs -= a[i].x;
        --tic;
      } else if (a[i].y) {
