...
using mod_t = joshu::imod_t<998244353>;
// constant defs
...
struct PtrLess {
  bool operator()(sub_t const* lhs, sub_t const* rhs) const;
};

struct sub_t {
  int k;
  mod_t w;
  std::map<sub_t*, int, PtrLess> c;
} pool[2600000], *tail;

bool PtrLess::operator()(sub_t const* lhs, sub_t const* rhs) const {
  if (lhs->k != rhs->k) return lhs->k < rhs->k;
  if (lhs->c.size() != rhs->c.size()) return lhs->c.size() < rhs->c.size();
  if (lhs->w.lld() != rhs->w.lld()) return lhs->w.lld() < rhs->w.lld();

  auto lit = lhs->c.begin();
  auto rit = rhs->c.begin();
  for (; lit != lhs->c.end(); ++lit, ++rit) {
    if (lit->second != rit->second) return lit->second < rit->second;
    if (operator()(lit->first, rit->first)) return true;
    if (operator()(rit->first, lit->first)) return false;
  }
...
mod_t fm[11000], gm[11000];

mod_t comb(int const n, int const r) {
  return fm[n] * gm[r] * gm[n - r];
}

mod_t comb_r(int const n, int const r) {
  return comb(n + r - 1, r);
}

sub_t* solution(int* lhs, int m, int const adj) {
  sub_t& curr = *tail++;
  curr.w = 1;
  ...
  curr.k = 0;
  while (cnt[curr.k] == 1) ++curr.k;

  // cerr << "{ * " << curr.k << endl;
  
  curr.c.clear();
  int* prv = lhs;
  for (int i = 0; i + curr.k < m; ++i) {
    if (curr.k == lhs[i] - adj) {
      // cerr << "i at " << i << endl;
      auto const nxt = lhs + i + 1;
      auto chd = solution(prv, nxt - prv, adj + curr.k);
      prv = nxt;

      auto it = curr.c.find(chd);
      if (it != curr.c.end()) {
        ++it->second;
      } else {
        curr.c.emplace(chd, 1);
      }
      curr.w *= chd->w;
    }
  }
  ...
  int slots = 1;
  for (auto it = curr.c.begin(); it !=curr.c.end(); ++it) {
    curr.w *= comb_r(slots, it->second);
    // cerr << "comb_r(" << slots << ", " << it->second << ") => " << curr.w.lld() << endl;
    slots += it->second;
  }
