...
int n;
lld a[210000], b[210000], dlt;

int main() {
  while (scanf("%d%lld", &n, &dlt) != EOF) {
    static auto check = [&](lld const h) -> bool {
      lld lhs = 0, rhs = inf;
      for (int i = 0; i < n; ++i) {
        lld const cut = a[i] + b[i] + h;
        lld const low = max(a[i] - cut, 0LL);
        lld const hig = a[i] - max(cut - b[i], 0LL);
        lhs = max(lhs - dlt, low);
        rhs = min(rhs + dlt, hig);
        if (lhs > rhs) {
          return false;
    ...
    for (int i = 0; i < n; ++i) {
      ...
      sum += a[i] + b[i];
      joshu::setmin(top, a[i] + b[i]);
    }
    lld const h = -joshu::binary_search(-top, 0LL, check);
    printf("%lld\n", sum - h * n);
  }
  return 0;
}
