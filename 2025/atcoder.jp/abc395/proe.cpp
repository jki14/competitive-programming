...
int main() {
  while (scanf("%d%d%lld", &n, &m, &w) != EOF) {
    for (int i = 0; i <= n + n; ++i) nbs[i] = -1;
    eid = 0;
    ...
    add_edge(n - 1, n + n, 0);
    add_edge(n + n - 1, n + n, 0);

    for (int i = 0; i < m; ++i) {
      int u, v;
      scanf("%d%d", &u, &v);
      add_edge(u - 1, v - 1, 1);
      add_edge(n + v - 1, n + u - 1, 1);
    }
    ...
    while (!pq.empty()) {
      int const u = pq.top().y;
      lld const w = -pq.top().x;
      pq.pop();
      if (f[u] != w) continue;
