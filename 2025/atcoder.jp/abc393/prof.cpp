...
joshu::tuple3d_t<int, int, int> r[210000];
int res[210000];
...
int main() {
  while (scanf("%d%d", &n, &q) != EOF) {
    ...
    for (int i = 0; i < q; ++i) {
      ...
      r[i].y = (it->first == r[i].y) ? it->second : it->second - 1;
      r[i].z = i;
    }
    sort(r, r + q);
    ...
    for (int i = 0; i < n; ++i) {
      ...
      int const w = it != f.end() ? it->second + 1 : 1;
      while (it != f.begin()) {
        it = prev(it);
        if (it->second <= w) {
          it = f.erase(it);
      ...
      */
      ...
      for (; k < q && r[k].x == i; ++k) {
        it = f.lower_bound(r[k].y);
        /*
        if (it != f.end()) {
          fprintf(stderr, "query (%d, %d) => (%d, %d)\n", r[k].x, r[k].y, it->first, it->second);
        }
        */
        res[r[k].z] = it != f.end() ? it->second : 0;
      }
    }
