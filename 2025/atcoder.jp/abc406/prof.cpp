...
int lhs[310000], rhs[310000];

adjlist_t<void> g(310000, 610000);

void add(int i, lld const d) {
  for (; i <= n; i += lowbit(i)) {
    c[i] += d;
  }
}
...
lld query(int const lef, int const rhs) {
  return query(rhs) - query(lef - 1);
}
  ...
  while (cin >> n) {
    ...
    while (m--) {
      ...
      if (qt == 1) {
        ...
        int k;
        cin >> k;
        lld wht = min(query(lhs[a[k].x], rhs[a[k].x]), query(lhs[a[k].y], rhs[a[k].y]));
        lld sum = query(n);
        cout << (llabs(sum - (wht << 1)) >> 1) << endl;
      }
    }
  }
