...
mod_t b[128];
mod_t fk[128][128], fs[128][128], gk[128][128], gs[128][128];
    ...
    */
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j <= m; ++j) {
        if (a[i]) {
          fk[i + 1][j + 1] += fk[i][j];
          fs[i + 1][j + 1] += fs[i][j] + b[n - i - 1] * fk[i][j];
          gk[i + 1][j] += fk[i][j];
          gs[i + 1][j] += fs[i][j];
        } else {
          fk[i + 1][j] += fk[i][j];
        ...
        gk[i + 1][j + 1] += gk[i][j];
        gs[i + 1][j + 1] += gs[i][j] + b[n - i - 1] * gk[i][j];
        gk[i + 1][j] += gk[i][j];
        gs[i + 1][j] += gs[i][j];
      }
    }

    cout << (fs[n][m] + gs[n][m]).lld() << endl;
  }
