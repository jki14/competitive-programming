...
inline void add(int i) {
  for (; i <= n; i += joshu::lowbit(i)) {
    ++c[i];
  }
}

/*
inline int get(int const i) {
...
int main() {
  auto const check = [&](int const rig) {
    return sum(loc + rig) <= rig;
  };
  while (scanf("%d", &n) != EOF) {
    for (int i = 1; i <= n; ++i) {
      scanf("%d", &a[i]);
    ...
    for (int i = n; i >= 1; --i) {
      /*
      int const p = a[i] + get(a[i]);
      int const g = find(p);
      r[g] = i;
      f[g] = g + 1;
      */
      loc = a[i];
      int const p = loc + joshu::binary_search(0, n - loc, check);
      // fprintf(stderr, "p = %d\n", p);
      r[p] = i;
      add(p);
    }
