...
using imod_t = joshu::imod_t<1000000000>;
} // namespace
...
int nbs[4100], nxt[12100], src[12100], dst[12100], eid;
lld wht[12100];

void add_edge(int const u, int const v, lld const w) {
  nxt[eid] = nbs[u];
  src[eid] = u;
  dst[eid] = v;
  wht[eid] = w;
...
int main() {
  while (scanf("%d%d", &n, &m) != EOF) {
    ...
    for (int k = 0; k < n; ++k) {
      for (int i = 0; i < eid; ++i) {
        setmax(dis[dst[i]], dis[src[i]] + wht[i]);
      }
    }
