...
using pid = tuple2d_t<int, double>;

inline double constexpr eps = 1e-22;
...
void dfs(int const wht, int const slots, int const pushs) {
  if (wht == 0) {
    if (slots >= 0) {
      seq.emplace_back(0, dim * slots);
      ops.push_back(seq);
      seq.pop_back();
    }
    return;
  ...
  if (wht == 1) {
    if (slots >= pushs) {
      seq.emplace_back(1, dim * pushs);
      dfs(0, slots - pushs, 0);
      seq.pop_back();
    }
    return;
  }
  for (int i = 0; wht * i <= pushs && i <= slots; ++i) {
    seq.emplace_back(wht, dim * i);
    dfs(wht - 1, slots - i, pushs - wht * i);
    seq.pop_back();
  }
}

double resolve(int const rd, int const hit) {
  if (f[rd][hit] > -eps) return f[rd][hit];
  f[rd][hit] = 0.;
  for (auto const& e : ops) {
    double bar = 0.;
    for (auto const& row : e) {
      bar += resolve(rd - 1, max(hit - row.x, 0)) * row.y;
    }
    setmax(f[rd][hit], bar);
  }
  return f[rd][hit];
