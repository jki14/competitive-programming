...
int solution(int left, int const stop) {
  if (left <= stop) return 0;
  int p = dq.size();
  int k = 0;
  while (left > stop) {
    while (p > 0 && dq[p - 1].y >= left) --p;
    left = dq[p].x;
    ++k;
  }
  return k;
...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    ...
    dq.clear();
    for (int i = 1; i < n; ++i) {
      int const left = i - c[i];
      if (a[i]) {
        res += 1 + solution(left, stop);
        stop = i;
      }
      while (!dq.empty() && dq.back().x >= left) dq.pop_back();
      dq.emplace_back(left, i);
    }
    printf("%d\n", res);
  }
  return 0;
