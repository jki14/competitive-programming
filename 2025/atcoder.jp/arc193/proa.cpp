...
static lld constexpr inf = 0x3f3f3f3f3f3f3f3fLL;

class segmin_t : public joshu::btnctx_t<segmin_t> {
public:
  segmin_t() = default;

  segmin_t(segmin_t const&) = default;
  segmin_t(segmin_t&&) = default;

  segmin_t& operator=(segmin_t const&) = default;
  segmin_t& operator=(segmin_t&&) = default;

  void aggregate(segmin_t const& lhs, segmin_t const& rhs) override { wht_ = {min(lhs.wht_, rhs.wht_)};  }
  void flush(segmin_t&, segmin_t&) override {
    // setmin(lhs.wht_, wht_);
    // setmin(rhs.wht_, wht_);
  }
...
int main() {
  ...
  auto const initializer = [](joshu::segtree_t<int, segmin_t>::node_t& node) -> void { node.ctx.wht_ = inf; };
  auto const updater = [&candidate](joshu::segtree_t<int, segmin_t>::node_t& node) -> void { joshu::setmin(node.ctx.wht_, candidate); };
  // static auto const querier = [](joshu::segtree_t<int, segmin_t>::node_t const& node) -> segmin_t { return node.ctx; }

  joshu::segtree_t<int, segmin_t> fst(410000);
  joshu::segtree_t<int, segmin_t> bst(410000);

  while (scanf("%d", &n) != EOF) {
    fst.build(1, n << 1, initializer);
    bst.build(1, n << 1, initializer);
    ...
    for (int i = 0; i < n; ++i) {
      ...
      candidate = w[i];
      fst.update(a[i].x, updater);
      bst.update(a[i].y, updater);
    }
    ...
    for (int i = 0; i < m; ++i) {
      ...
      if (a[lhs].y < a[rhs].x) {
        ...
        segmin_t ctx;
        ctx.aggregate(fst.query(max(a[lhs].y, a[rhs].y) + 1, n << 1), bst.query(1, min(a[lhs].x, a[rhs].x) - 1));
        lld foo = min(ctx.wht_, fst.query(a[lhs].y + 1, n << 1).wht_ + bst.query(1, a[rhs].x - 1).wht_);
        if (foo < inf) {
          printf("%lld\n", w[lhs] + w[rhs] + foo);
        } else {
          printf("-1\n");
