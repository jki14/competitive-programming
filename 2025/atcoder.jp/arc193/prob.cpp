...
int n;
imod_t f[1048576][4], res;

char a[1048576];

auto resolve(int const z) {
  for (int i = 0; i <= n; ++i) {
    for (int j = 0; j < 3; ++j) {
      f[i][j] = 0;
    }
  ...
  for (int i = 1; i <= n; ++i) {
    lld const px = static_cast<lld>(a[i - 1] == '1');
    // lld const qx = px ^ 1;
    // lld const qx = 1;
    /*
    // <-
    f[i][0] += f[i - 1][0];
    f[i][0] += f[i - 1][0] * ex + f[i - 1][1];
    f[i][0] += f[i - 1][1] * ex;
    // ->
    f[i][1] += f[i - 1][0];
    f[i][1] += f[i - 1][0] * ex + f[i - 1][1];
    f[i][1] += f[i - 1][1] * ex;
    */
    ...
    // case: d == 1
    f[i][0] += f[i - 1][0];
    f[i][1] += px * f[i - 1][0] + f[i - 1][1] + px * f[i - 1][2];
    f[i][2] += px * f[i - 1][0];
    // case: d == 2
    f[i][0] += px * f[i - 1][0] + f[i - 1][1] + px * f[i - 1][2];
    f[i][1] += px * f[i - 1][1];
    f[i][2] += px * f[i - 1][1];
    // case: d == 3
    f[i][0] += px * f[i - 1][1];
    */
    ...
    auto const tot = f[i - 1][0] + f[i - 1][1] + f[i - 1][2];
    f[i][0] += tot;
    f[i][1] += tot;
    f[i][2] += f[i - 1][2] + px * tot;
  }
