...
vector<char> r[32];
unordered_map<string, imod_t> f[110];
...
int main() {
  for (int i = 0; i < 26; ++i) r[i].reserve(16);
  while (scanf("%d%d", &n, &m) != EOF) {
    for (int i = 0; i < 26; ++i) r[i].clear();

    scanf("%s", pat);
    for (int i = n - 1; i >= 0; --i) {
      r[pat[i] - 'a'].emplace_back('a' + i + 1);
    }

    for (int i = 0; i <= m; ++i) f[i].clear();
    string base(n, 'z');
    static imod_t const one = 1;
    f[0].emplace("a" + base, one);

    for (int i = 0; i < m; ++i) {
      for (auto const& [state, count] : f[i]) {
        for (int c = 0; c < 26; ++c) {
          string next = state;
          for (char const p : r[c]) {
            for (int k = 1; k <= n; ++k) {
              if (next[k - 1] < p) {
                joshu::setmin(next[k], p);
              }
            }
          }
          f[i + 1][next] += count;
    ...
    for (auto const& [state, count] : f[m]) {
      // fprintf(stderr, "%s = %lld\n", state.c_str(), count.lld());
      res[rate(state)] += count;
    }

    for (int i = 0; i <= n; ++i) {
      printf("%lld%c", res[i].lld(), i < n ? ' ' : '\n');
    }
  }
  return 0;
}
