...
using lld = long long;
using pii = tuple2d_t<int, int>;
using pil = tuple2d_t<int, lld>;
using pli = tuple2d_t<lld, int>;
using pll = tuple2d_t<lld, lld>;

using cit = vector<pll>::const_iterator;

lld func(lld const x, lld const y, lld const z) {
  return x * y + x * z + y * z;
}
...
void build(vector<pll>& a) {
  ...
  fprintf(stderr, "[");
  for (auto const& e : a) {
    fprintf(stderr, ",(%lld, %lld)", e.x, e.y);
  }
  fprintf(stderr, "]\n");
  */
}
...
int main() {
  ...
  while (scanf("%d%d", &n, &m) != EOF) {
    ...
    for (auto it = next(b.begin()); it != b.end(); ++it) {
      it->y += prev(it)->y;
    }

    static auto const ge = [&](lld const tar_) -> bool {
      lld const tar = -tar_;
      lld cnt = 0LL;
      auto it = b.end();
      --it;
      for (auto const& ae : a) {
        while (it != b.begin() && func(ae.x, it->x, c[0].x) < tar) {
          --it;
        }
        if (func(ae.x, it->x, c[0].x) >= tar) {
          cnt += ae.y * it->y * c[0].y;
        }
        if (cnt >= m) break;
      }
      // fprintf(stderr, "tar = %lld, cnt >= %lld\n", tar, cnt);
      return cnt >= m;
    };
    lld bar = -joshu::binary_search(-func(a.back().x, b.back().x, c[0].x), -func(a[0].x, b[0].x, c[0].x), ge);
    // fprintf(stderr, "bar = %lld\n", bar);

    for (auto it = prev(b.end()); it != b.begin(); --it) {
      it->y -= prev(it)->y;
    }
    ...
    node.j = b.cbegin();
    node.k = c.cbegin();
    for (node.i = a.cbegin(); node.i != a.cend(); ++node.i) {
      pq2.push(node);
    }

    for (lld tot = 0LL; !pq2.empty() && tot < m;) {
      node = pq2.top();
      pq2.pop();
      tot += node.count();
      pq.push(node);
    ...
    for (lld tot = 0LL; tot + pq.top().count() < m;) {
      node = pq.top();
      // fprintf(stderr, "pop %lld (x %lld)\n", node.value(), node.count());
      pq.pop();
      tot += node.count();
      ++node.k;
      if (node.k != c.cend() && node.value() >= bar) {
        pq.push(node);
      }
    }
