...
struct node_t {
  ...
  void init(int const wht_, int const loc_) {
    for (int i = 0; i < 26; ++i) {
      nxt[i] = nullptr;
    }
    prv = nullptr;
...
mod_t f[110][16][110], res;
char pat[16];
int n, m, num;

void insert(node_t* node, char const* s) {
  int const c = s[0] - 'a';
  if (!node->nxt[c]) {
    node->nxt[c] = &pool[num];
    node->nxt[c]->init(node->wht + 1, num++);
  }
  if (s[1]) {
    insert(node->nxt[c], s + 1);
  }
}

void build(node_t* node) {
  for (int c = 0; c < 26; ++c) {
    node_t* next = node->nxt[c];
    if (!next) continue;
    node_t* candidate = node->prv;
    while (candidate && !candidate->nxt[c]) {
      candidate = candidate->prv;
    }
    next->prv = candidate ? candidate->nxt[c] : root;
    build(next);
  }
}

int step(int const loc, int const c) {
  node_t* node = &pool[loc];
  while (node && !node->nxt[c]) {
    node = node->prv;
  }
  return node ? node->nxt[c]->loc : root->loc;
}
    ...
    for (int i = 0; i <= m; ++i) {
      for (int j = 0; j <= n; ++j) {
        for (int k = 0; k < num; ++k) {
          ...
          if (i == m) continue;
          for (int c = 0; c < 26; ++c) {
            int const p = step(k, c);
            int const w = max(pool[p].wht, j);
