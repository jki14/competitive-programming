...
std::vector<int> seq, d;

void resolve(int const p, int const k) {
  if (!k) {
    f[p][k] = a[p];
    g[p][k] = 1;
  ...
  int const cnt = f[x][k - 1] + f[y][k - 1] + f[z][k - 1];
  f[p][k] = cnt >= 2;

  seq.clear();
  if (f[p][k]) {
    if (f[x][k - 1]) seq.emplace_back(g[x][k - 1]);
    if (f[y][k - 1]) seq.emplace_back(g[y][k - 1]);
    if (f[z][k - 1]) seq.emplace_back(g[z][k - 1]);
  } else {
    if (!f[x][k - 1]) seq.emplace_back(g[x][k - 1]);
    if (!f[y][k - 1]) seq.emplace_back(g[y][k - 1]);
    if (!f[z][k - 1]) seq.emplace_back(g[z][k - 1]);
  ...
  g[p][k] = (cnt == 1 || cnt == 2) ? seq[0] : (seq[0] + seq[1]);
