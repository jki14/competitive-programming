...
void one_step(int& i, int &j) {
  bar.emplace_back(i + 1, j + 1);
  ++a[i];
  --a[j];
  swap(a[i], a[j]);
  swap(i, j);
  ...
  while (cin >> n) {
    ...
    if (n == 2) {
      ...
      while (!lhs.empty() && !rhs.empty()) {
        int i = lhs.front(), j = rhs.top();
        int x = (i != 0 && j != 0) ? 0 : ((i != 1 && j != 1) ? 1: 2);
        // cerr << "(i, j, x) == (" << i << ", " << j << ", " << x << ") {" << a[i] << ", " << a[j] << ", " << a[x] << "}\n";
          // cerr << "lhs.size() == " << lhs.size() << ", rhs.size() == " << rhs.size() << endl;
        if (i < j && j < x) {
          one_step(i, j);
          one_step(i, x);
          one_step(j, i);
