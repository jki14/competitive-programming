...
static int constexpr L = 200000;

int n, m;
int nbs[410000], nxt[810000], dst[810000], eid;
lld f[410000][2], g[410000], h[2];
lld c[210000], d[210000];

void update(int i, lld const d, lld* const z) {
  for (; i <= L; i += joshu::lowbit(i)) {
    z[i] += d;
  }
}

lld query(int i, lld* const z) {
  lld res = 0LL;
  for (; i > 0; i -= joshu::lowbit(i)) {
    res += z[i];
  }
  return res;
}
...
void build(int const u, int const p) {
  ...
  int const k = (u > n) ? 1 : 0;
  joshu::setmax(h[k], f[u][0] + f[u][1]);
}

void fresh(int const u, int const p, lld const w) {
  g[u] = max(f[u][0], w);
  // fprintf(stderr, "g[%d] = %lld (w = %lld)\n", u + 1, g[u], w);
  for (int i = nbs[u]; ~i; i = nxt[i]) {
    int const v = dst[i];
    if (v == p) continue;
    lld patch = (f[v][0] + 1LL < f[u][0]) ? f[u][0] : f[u][1];
    ++patch;
    joshu::setmax(patch, w + 1LL);
    fresh(v, u, patch);
  }
}
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    lld sum = query(L, c);
    for (int i = n; i < n + m; ++i) {
      // fprintf(stderr, "rig[%d] = %lld\n", i + 1 - n, g[i]);
      int p = max(bar - g[i] - 1LL, 0LL);
      lld msked = query(p, d);
      res += bar * msked + sum - query(p, c) + (g[i] + 1LL) * (n - msked);
    }
    printf("%lld\n", res);
  }
  return 0;
