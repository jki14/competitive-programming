...
i80 sqr(lld const x) {
  auto const i = static_cast<i80>(x);
  return i * i;
}

i80 dist2(pll const& lhs, pll const& rhs) {
  return sqr(lhs.x - rhs.x) + sqr(lhs.y - rhs.y);
}

int find(int const u, int const k) {
  if (v[u] == k) return 0;
  v[u] = k;
  for (auto v : g[u]) {
    if (!~r[v] || find(r[v], k)) {
...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    for (int i = 0; i < n; ++i) {
      scanf("%lld%lld", &lhs[i].x, &lhs[i].y);
    }
    for (int i = 0; i < n; ++i) {
      scanf("%lld%lld", &rhs[i].x, &rhs[i].y);
    }
    
    gap.clear();
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        w[i][j] = dist2(lhs[i], rhs[j]);
        gap.emplace_back(w[i][j]);
      }
    }
    sort(gap.begin(), gap.end());
    gap.erase(std::unique(gap.begin(), gap.end()), gap.end());

    static auto const solution = [&](lld pos) -> bool {
      g.clear();
      for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
          // fprintf(stderr, "  %d -> %d (%lld)\n", i, j, static_cast<lld>(w[i][j]));
          if (w[i][j] > gap[pos]) continue;
          g.add_edge(i, j);
        }
      }
      for (int i = 0; i < n; ++i) v[i] = r[i] = -1;
      for (int i = 0; i < n; ++i) {
        if (!find(i, i)) {
          // cerr << "solution(" << static_cast<lld>(gap[pos]) << ") -> false\n";
          return false;
        }
      }
      // cerr << "solution(" << static_cast<lld>(gap[pos]) << ") -> true\n";
      return true;
    };
    auto const idx = bsearch_first_pass(0LL, static_cast<lld>(gap.size() - 1), solution);
    llf res = sqrt(static_cast<llf>(gap[idx]));
    printf("%.14Lf\n", res);
  }
}
