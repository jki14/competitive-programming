...
void flush(node_t* node) {
  node->flat = node->chd[0]->flat + node->chd[1]->flat;
  node->step = node->chd[0]->step + node->chd[1]->step;
  node->step += (node->chd[0]->rig - node->chd[0]->lef + 1) * 2 * node->chd[1]->flat;
}

void build(node_t** hook, int const lef, int const rig, int const* const a) {
  node_t* node = *hook = ++last;
  node->lef = lef;
  node->rig = rig;
  if (lef > rig) return;
  if (lef == rig) {
    node->chd[0] = node->chd[1] = nullptr;
    node->flat = a[lef];
    node->step = a[lef];
  } else {
    int const mid = (lef + rig) >> 1;
    build(&node->chd[0], lef, mid, a);
    build(&node->chd[1], mid + 1, rig, a);
    flush(node);
  }
}

void update(node_t* node, int const pos, int const val) {
  if (node->lef == node->rig) {
    node->flat = val;
    node->step = val;
  } else {
    if (pos <= node->chd[0]->rig) {
      update(node->chd[0], pos, val);
    } else {
      update(node->chd[1], pos, val);
    }
    flush(node);
  }
}

lld query(node_t* node, int const lef, int const rig, int const k) {
  // fprintf(stderr, "query %d->%d + %d\n", lef, rig, k);
  if (lef > rig) return 0LL;
  if (lef > node->rig || rig < node->lef) return 0LL;
  if (lef <= node->lef && node->rig <= rig) {
    return node->step + node->flat * k;
  } else {
    lld foo = 0;
    int d = 0;
    if (lef <= node->chd[0]->rig) {
      foo += query(node->chd[0], lef, rig, k);
      d = node->chd[0]->rig - max(node->chd[0]->lef, lef) + 1;
    }
    if (rig >= node->chd[1]->lef) {
      foo += query(node->chd[1], lef, rig, k + d * 2);
    }
    return foo;
  }
}
...
void solve() {
  ...
  last = pool;
  build(&root[0], 0, (n - 1) >> 1, a[0]);
  build(&root[1], 0, (n - 2) >> 1, a[1]);
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (opt[0] == 'Q') {
      ...
      int const k = x & 1;
      int const d = (y - x) & 1;
      ans += query(root[k], x >> 1, (y - d) >> 1, 0);
      // fprintf(stderr, "ans = %lld\n", ans);
      ans -= query(root[k ^ 1], (x + 1) >> 1, (y - (d ^ 1)) >> 1, 1);
      // fprintf(stderr, "ans = %lld\n", ans);
    } else {
      --x;
      update(root[x & 1], x >> 1, y);
    }
  }
