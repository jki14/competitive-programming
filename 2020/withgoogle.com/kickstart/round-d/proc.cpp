...
static constexpr int N = 510000;
static constexpr int bar = 307;

int n, x, y;
int pnt[N], nbs[N], nxt[N], dst[N], eid;
int lhs[N], rhs[N];
int seq[N];
...
void dp(int const m, int* const res) {
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < m; ++j) {
      f[i][j] = 0;
    }
...
void bf(int const m, int* const res) {
  int len = 0;
  for (int i = 0; i < n; ++i) cur[i] = nbs[i];
  stk[len++] = 0;
  while (len) {
    int const u = stk[len - 1];
    int& i = cur[u];
    if (~i) {
      stk[len++] = dst[i];
      i = nxt[i];
    } else {
      for (int i = len - 1; i >= 0; i -= m) {
        ++res[stk[i]];
      }
      --len;
    }
...
void solve() {
  scanf("%d%d%d", &n, &x, &y);
  for (int i = 0; i < n; ++i) lhs[i] = rhs[i] = 0;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    foo += 1.0 - ((n - lhs[i]) * 1.0 / n) * ((n - rhs[i]) * 1.0 / n);
  }
  printf("%.9f\n", foo);
}
