...
struct interval_t {
  int x, y, p;
  bool operator < (interval_t const& rhs) const {
...
void process(int const case_id) {
  ...
  queue<state_t const*> curr;
  curr.push(&f[0][0]);
  for (int i = 0; i < n; ++i) {
    queue<state_t const*> next;
    for (; !curr.empty(); curr.pop()) {
      auto const state = curr.front();
      if (state->c <= a[i].x && f[a[i].y][state->j].i != a[i].p) {
        auto updated = &f[a[i].y][state->j];
        updated->pc = state->c;
        updated->pj = state->j;
        updated->i = a[i].p;
        updated->r = 'C';
      ...
      if (state->j <= a[i].x && f[state->c][a[i].y].i != a[i].p) {
        auto updated = &f[state->c][a[i].y];
        updated->pc = state->c;
        updated->pj = state->j;
        updated->i = a[i].p;
        updated->r = 'J';
  ...
  if (!curr.empty()) {
    foo[n] = 0;
    auto state = curr.front();
    for (; state->r != '?'; state = &f[state->pc][state->pj]) {
      foo[state->i] = state->r;
    }
    printf("Case #%d: %s\n", case_id, foo);
  } else {
