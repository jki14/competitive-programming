...
unordered_map<char, int> fward;
unordered_map<int, char> bward;
...
int okay() {
  ...
  for (int i = 0; i < s; ++i) {
    ...
    for (int j = 0; b[i][j]; ++j) {
      if (a[i][j] - '0' == foo[fward[b[i][j]]]) continue;
      if (a[i][j] - '0' < foo[fward[b[i][j]]]) {
        return 0;
      } else {
        break;
      }
...
int cook(int const k) {
  if (k == c) {
    ...
    for (int i = nonzero[k]; i <= top[k]; ++i) {
      if (notfree[i]) continue;
      foo[k] = i;
      notfree[i] = 1;
      if (cook(k + 1)) return 1;
      notfree[i] = 0;
    }
  }
  return 0;
...
void process(int const case_id) {
  ...
  c = 0;
  fward.clear();
  for (int i = 0; i < 10; ++i) top[i] = 9;
  for (int i = 0; i < 10; ++i) nonzero[i] = 0;
  for (int i = 0; i < s; ++i) {
    scanf("%s%s", a[i], b[i]);
    if (a[i][0] == '-') {
      do {
    ...
    if (strlen(b[i]) == strlen(a[i])) {
      top[fward[b[i][0]]] = min(a[i][0] - '0', top[fward[b[i][0]]]);
    }
    nonzero[fward[b[i][0]]] = 1;
  }
  for (int i = 0; i < 10; ++i) notfree[i] = 0;
  cook(0);
  bward.clear();
  for (auto const& entry : fward) {
    bward[entry.second] = entry.first;
    //fprintf(stderr, "bward[%d] = %c$\n", entry.second, entry.first);
  }
  char p = 'A';
  for (int i = bward.size(); i < 10; ++i) {
    while (fward.count(p) != 0) ++p;
    bward[i] = p;
    //fprintf(stderr, "bward[%d] = %c$\n", i, p);
  }
  for (int i = 0; i < 10; ++i) {
