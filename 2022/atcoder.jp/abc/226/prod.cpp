...
pii babystep(int const x, int const y) {
  int const g = joshu::gcd(x, y);
  return {x / g, y / g};
}
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    unordered_map<int, unordered_set<int>> arrows;
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        if (i == j) continue;
        auto const mask = babystep(x[i] - x[j], y[i] - y[j]);
        arrows[mask.first].insert(mask.second);
      }
    }
    int cnt = 0;
    for (auto const& row : arrows) {
      cnt += row.second.size();
    }
    printf("%d\n", cnt);
  }
