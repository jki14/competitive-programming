...
unordered_map<int, lld> f[22][22], g[22][22];

void add(unordered_map<int, lld>& f, int const key, lld const val) {
  auto it = f.find(key);
  if (it != f.end()) {
    it->second += val;
  } else {
...
lld get(unordered_map<int, lld> const& f, int const key) {
  auto const it = f.find(key);
  if (it != f.end()) {
    return it->second;
  }
  return 0;
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    for (int k = 0; k < n - 1; ++k) {
      for (int i = 0; i < n && i <= k; ++i) {
        ...
        for (int p = 0; p < 2; ++p) {
          ...
          if (r < 0 || r >= n) continue;
          if (c < 0 || c >= n) continue;
          // if (!f[i][j].empty()) fprintf(stdout, "walk %d, %d\n", r, c);
          for (auto const& e : f[i][j]) {
            add(f[r][c], e.first ^ a[r][c], e.second);
          }
        }
      }
    }
    add(g[n - 1][n - 1], a[n - 1][n - 1], 1);
    for (int k = 0; k < n - 1; ++k) {
      for (int i = 0; i < n && i <= k; ++i) {
        int const j = k - i;
        if (j >= n) continue;
        ...
        for (int p = 0; p < 2; ++p) {
          ...
          if (r < 0 || r >= n) continue;
          if (c < 0 || c >= n) continue;
          for (auto const& e : g[x][y]) {
            add(g[r][c], e.first ^ a[r][c], e.second);
          }
        }
      }
    }
    ...
    for (int i = 0; i < n; ++i) {
      int const j = n - 1 - i;
      for (auto const& e : f[i][j]) {
        // fprintf(stdout, "f[%d][%d][%d] = %lld\n", i, j, e.first, e.second);
        res += get(g[i][j], e.first ^ a[i][j]) * e.second;
      }
      // for (auto const& e : g[i][j]) {
      //   fprintf(stdout, "g[%d][%d][%d] = %lld\n", i, j, e.first, e.second);
      // }
    }
    printf("%lld\n", res);
  }
