...
using iter_t = list<pii_t>::iterator;

list<pii_t> a;
map<int, iter_t> cndd[10];
...
int main() {
  ...
  for (int case_id = 0; case_id < case_num; ++case_id) {
    ...
    scanf("%s", buf);
    for (int i = 0; i < n; ++i) {
      a.emplace_back(i, buf[i] - '0');
    }

    for (auto it = a.begin(); it != a.end(); ++it) {
      auto const jt = std::next(it);
      if (jt == a.end()) break;
      if (jt->second == (it->second + 1) % 10) {
        cndd[it->second].emplace(it->first, it);
      }
    }

    queue<iter_t> updated;
    while (true) {
      int flag = 1;
      for (int i = 0; i < 10; ++i) {
        for (auto cursor = cndd[i].begin(); cursor != cndd[i].end();) {
          auto const it = cursor->second;
          auto jt = std::next(it);
          cndd[jt->second].erase(jt->first);
          a.erase(jt);
          if (it != a.begin()) {
            jt = std::prev(it);
            cndd[jt->second].erase(jt->first);
          }
          it->second = (it->second + 2) % 10;
          updated.push(it);
          cursor = cndd[i].erase(cursor);
          flag = 0;
        }
        for (; !updated.empty(); updated.pop()) {
          auto it = updated.front();
          auto jt = std::next(it);
          if (jt != a.end() && jt->second == (it->second + 1) % 10) {
            cndd[it->second].emplace(it->first, it);
          }
          if (it != a.begin()) {
            jt = std::prev(it);
            if (it->second == (jt->second + 1) % 10) {
              cndd[jt->second].emplace(jt->first, jt);
            }
          }
        }
      }
    ...
    for (auto const digit : a) {
      printf("%c", '0' + digit.second);
    }
    printf("\n");
  }
  return 0;
