...
typedef struct hash_s{
	int x, y;
	hash_s(){ this->x=this->y=0; }
	hash_s(const int dx, const int dy){
		const int gcd=MAX(GCD(dx, dy), 1);
		this->x=dx/gcd;
		this->y=dy/gcd;
	}
	bool operator < (const hash_s &k) const{
...
int main(){
	...
	while(cin>>n){
		...
		for(int i=0;i<n;i++){
			s.clear();
			for(int j=i+1;j<n;j++){
				s[hash_t(a[j].x-a[i].x, a[j].y-a[i].y)]++;
			}
			for(map<hash_t, int>::iterator it=s.begin(); it!=s.end(); it++){
				if(it->second<2)continue;
				ans-=it->second*1LL*(it->second-1)/2LL;
			}
		}
		cout<<ans<<endl;
	}
