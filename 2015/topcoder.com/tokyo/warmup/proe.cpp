...
int main(){
	...
	while(cin>>s){
		...
		for(int i=0;i<n;i++){
			for(int j=i+1;j<n;j++){
				int count=0;
				for(int k=que[i]+1;k<que[j];k++)count+=(s[k]=='+')?1:0;
				if(!count)continue;
				lld hed=1LL;
				for(int k=que[i]-1;k>=0;k--){
					hed*=s[k--]-'0';
					if(!~k || s[k]!='*')break;
				}
				lld til=1LL;
				for(int k=que[j]+1;k<que[n-1];k++){
					til*=s[k++]-'0';
					if(k>=que[n-1] || s[k]!='*')break;
				}
				lld lef=1LL;
				int pre=que[i]+1;
				for(;;pre++){
				...
				mid+=cur;
				//printf("%d->%d hed=%lld til=%lld lef=%lld rig=%lld mid=%lld buf=%lld\n", que[i], que[j], hed, til, lef, rig, mid, lef*hed*(til-1LL)+mid*(hed*til-1LL)+rig*(hed-1LL)*til);
				UPDMAX(buf,  lef*hed*(til-1LL)+mid*(hed*til-1LL)+rig*(hed-1LL)*til);
			}
		}
		cout<<ans+buf<<endl;
	}
