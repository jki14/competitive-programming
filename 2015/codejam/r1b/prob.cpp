...
int bit_nxt(const int x){
	int x_lowbit=x&-x;
	int fund=x+x_lowbit;
	int sup=((fund^x)>>2)/x_lowbit;
	return fund|sup;
}
...
int count(const int s){
	...
	for(int x=0;x<n;x++){
		for(int y=0;y<m;y++){
			if(!(s&bit[x*m+y]))continue;
			for(int i=0;i<B;i++){
				int xx=x+dx[i];
				int yy=y+dy[i];
				if(xx>=n || yy>=m)continue;
				if(s&bit[xx*m+yy])buff++;
			}
		}
	}
	return buff;
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		...
		int cap=1<<(n*m);
		ans=inf;
		for(int k=(1<<c)-1;k<cap;k=bit_nxt(k)){
			UPDMIN(ans,count(k));
		}
		printf("%d\n",ans);
	}
