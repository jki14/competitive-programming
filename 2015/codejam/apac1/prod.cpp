...
typedef struct pos_s{
	int x, y;
	pos_s(){ this->x=this->y=0; }
	pos_s(const int _x, const int _y){
		this->x=_x;
		this->y=_y;
	}
	bool operator < (const pos_s &opn) const{
		if(this->x!=opn.x)return this->x<opn.x;
		return this->y<opn.y;
	}
} pos_t;
...
inline int check(const pos_t &p){
	if(((p.x+p.y)&1) && pas.find(p)==pas.end())return 1;
	return 0;
}

inline void step_move(pos_t &pos, const int d){
	pos.x+=dx[d];
	pos.y+=dy[d];
	if(pos.x<1)pos.x=r;if(pos.x>r)pos.x=1;
	if(pos.y<1)pos.y=c;if(pos.y>c)pos.y=1;
}
...
int main_entry(){
	...
	que[cur_tim++]=cur_pos;
	while(cur_tri<n){
		step_move(cur_pos, cur_dir);
		if(check(cur_pos)){
			cur_len++;
		}else{
			if(cur.find(que[cur_tim-cur_len])==cur.end()){
				return -1;
			}
			cur.erase(cur.find(que[cur_tim-cur_len]));
		}
		if(cur.find(cur_pos)!=cur.end()){
			return cur_len;
		}
		cur.insert(cur_pos);
		pas.insert(cur_pos);
		if(cur_tim==tim[cur_tri]){
			if(dir[cur_tri])
				cur_dir=(cur_dir+1)%4;
	...
	do{
		...
		que[cur_tim++]=cur_pos;
	}while (lup.x!=cur_pos.x || lup.y!=cur_pos.y);
	return cur_len;
}
