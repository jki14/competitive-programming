...
const long double xeps=1e-13;

int n, idx[N];
long double v, x, ans;
long double r[N], c[N];

inline int dcmp(const long double x, const long double y){ 
	if(x-y<-eps)return -1;
	if(x-y>eps)return 1;
	return 0;
}

inline void ld_scanf(long double *x){ double buf_x; scanf("%lf", &buf_x);*x=buf_x; }

inline int scmp(const int x, const int y){ return c[x]+eps<c[y]; }

int able(const long double cap){
	long double sum_v=0.0;
	long double sum_x=0.0;
	for(int i=0; i<n; i++){
		int k=idx[i];
		long double mnt=cap*r[k];
		if(sum_v+mnt<v+eps){
			sum_x+=mnt*c[k];
			sum_v+=mnt;
		}else{
	...
	//printf("sum_v=%.9Lf, sum_x=%.9Lf, v=%.9Lf, v*x=%.9Lf\n", sum_v, sum_x, v, v*x);
	if(dcmp(sum_v, v) || dcmp(sum_x, v*x)>0){
		//printf("exit 0\n");
		return 0;
	}
	sum_v=sum_x=0.0;
	...
	//printf("sum_v=%.9Lf, sum_x=%.9Lf, v=%.9Lf, v*x=%.9Lf\n", sum_v, sum_x, v, v*x);
	if(dcmp(sum_v, v) || dcmp(sum_x, x*v)<0){
		//printf("exit 0\n");
		return 0;
	}
	//printf("exit 1\n");
...
long double bs_entry(){
	long double lef=0.0, rig=1000000.0;
	while(lef<rig+xeps){
		long double mid=(lef+rig)*0.5;
		if(able(mid)){
			if(!able(mid-xeps))return mid;
			rig=mid-xeps;
		}else lef=mid+xeps;
	}
	return -1.0;
