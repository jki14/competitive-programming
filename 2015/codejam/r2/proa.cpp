...
int safe(const int i, const int j, const int k){
	for(int x=i+dx[k], y=j+dy[k];;x+=dx[k], y+=dy[k]){
...
int dead(const int i, const int j){
	for(int k=0;k<K;k++)
		if(safe(i, j, k))return 0;
	return 1;
}
