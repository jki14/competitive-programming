...
void div_entry(char *buf, const int k){
	sub[k].clear();
	string cur;
	for(char *c=buf; *c; c++){
		if(*c==' '){
			if(!cur.empty()){
				map<string, int>::iterator it=dic.find(cur);
				if(it==dic.end()){
					sub[k].push_back(m);
					dic.insert(MP(cur, m++));
				}else{
					sub[k].push_back(it->second);
				}
			}
			cur.clear();
		}else cur.append(1, *c);
...
int count_entry(const int mask){
	for(int i=0; i<n; i++){
		int key=(bit[i]&mask)?1:0;
		for(vector<int>::iterator it=sub[i].begin(); it!=sub[i].end(); it++){
			if(mrk[*it]!=mask){ mrk[*it]=mask;kid[*it]=0; }
			kid[*it]|=bit[key];
		}
	}
	int count=0;
	for(int i=0; i<m; i++){
		if(mrk[i]==mask && kid[i]==3)count++;
	}
	return count;
}
...
int main(){
	...
	for(int _T=1; _T<=T; _T++){
		...
		if(n<=20){
			...
			for(int i=bit[n-2]; i<bit[n-1]; i++)
				UPDMIN(ans, count_entry(i));
		}else{
			//TODO
		}
		printf("%d\n", ans);
