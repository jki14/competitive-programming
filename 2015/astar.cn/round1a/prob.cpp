...
lld bs_entry(const lld x){
	int lef=0, rig=n-1;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(a[mid]<=x){
			if(a[mid+1]>x)return a[mid];
			lef=mid+1;
		}else rig=mid-1;
	}
...
int main(){
	...
	for(int _T=1; _T<=T; _T++){
		...
		memset(mrk, 0, sizeof(mrk));msk=0;
		for(map<lld, int>::iterator it=s.begin(); it!=s.end(); it++)it->second=msk++;
		for(int i=0; i<n; i++)c[i]=s[a[i]];
		for(int lef=0; lef<n; lef++){
			msk++;
			lld low=0x3f3f3f3f3f3f3f3fLL;
			lld hig=0xc0c0c0c0c0c0c0c0LL;
			lld count=0LL;
			for(int i=lef; i<lef+1000; i++){
				UPDMIN(low, a[i]);
				UPDMAX(hig, a[i]);
				if(mrk[c[i]]!=msk){
					count++;
				...
				if(count==(i-lef+1)*1LL && low+count-1LL==hig){
					ans[i-lef+1]++;
				}
			}
		}
