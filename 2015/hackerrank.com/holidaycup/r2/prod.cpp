...
int main() {
	...
	while(T--){
		...
		for(int pos=0; pos<n; pos++){
			//UPDATE f
			for(int cov=0; cov<K; cov++){
				MOD_ADD(&f[pos+1][cov|bit[a[pos]-'0']], f[pos][cov]);
				for(char c='0'; c<='9'; c++){
					MOD_ADD(&g[pos+1][cov|bit[c-'0']], g[pos][cov]);
					if(c<a[pos])
						MOD_ADD(&g[pos+1][cov|bit[c-'0']], f[pos][cov]);
				}
			}
		}
		for(int i=2; i<K; i++){
			/*if(f[n][i]==0 && g[n][i]==0)continue;
			printf("f[");
			for(int j=0; j<10; j++)if(i&bit[j]){
				printf("%d", j);
			}
			printf("]=%d g[", f[n][i]);
			for(int j=0; j<10; j++)if(i&bit[j]){
		...
		for(int x=2; x<K; x++){
			for(int y=2; y<K; y++){
				int z=x^y;
				int buf=0;
				for(int i=0; i<10; i++)if(z&bit[i])buf+=i;
				MOD_ADD(&ans, MOD_MUL(buf, MOD_MUL(f[n][x], f[n][y])));
			}
		}
		printf("%d\n", MOD_STD(ans));
	}
