...
int main() {
	...
	for(int k=1; k<=n; k++){
		...
		for(int x=1; x<=k; x++){
			set<char> sx;
			sprintf(buf, "%d", x);
			for(int i=0; buf[i]; i++)sx.insert(buf[i]);
			for(int y=1; y<=k; y++){
				set<char> sy;
				sprintf(buf, "%d", y);
				for(int i=0; buf[i]; i++)sy.insert(buf[i]);
				for(set<char>::const_iterator it=sx.begin(); it!=sx.end(); it++)
					if(!sy.count(*it))ans+=*it-'0';
				for(set<char>::const_iterator it=sy.begin(); it!=sy.end(); it++)
					if(!sx.count(*it))ans+=*it-'0';
			}
		}
		printf("[%d]:%d\n", k, ans);
	}
