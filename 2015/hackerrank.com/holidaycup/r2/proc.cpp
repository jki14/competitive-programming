...
void core_entry(const int u, const int p){
	g[u]=1;
	lld buf=0LL, bvf=0LL, nef=0LL;
	for(int i=nbs[u]; i; i=nxt[i]){
		...
		nef+=f[v]*f[v]*1LL;
		bvf+=(g[v]-f[v])*1LL;
		nef+=(g[v]-f[v])*1LL*(g[v]-f[v]);
	}
	ans+=(buf*buf+bvf*bvf-nef)>>1;
	//printf("f[%d]=%d g[%d]=%d ans=%d buf=%lld nef=%lld\n", u, f[u], u, g[u], ans, buf, nef);
}
