...
static const long long MODL=1000000009LL;
...
//HEAD_OF_JKI'S_HEADER_
	...
	inline lld MODL_MUL(lld x, lld y, const lld m=MODL){
		if(x<y)swap(x, y);
		lld z=0LL;
		while(y>0){
			if(y&1){
	...
	inline lld MODL_POW(const lld x, const lld y, const lld m=MODL){
...
int main(){
	...
	lld in6=INVERSEE(6LL, MODL);
	lld in2=INVERSEE(2LL, MODL);
	int T;cin>>T;
	while(T--){
		lld n, k, c, ans;
		cin>>n>>k>>c;
		...
		MOD_MUL(&ans, in6);
		MOD_MUL(&ans, k);
		MOD_ADD(&ans, MOD_MUL(MOD_MUL(MOD_MUL(n, n-1LL), in2), c));
		cout<<MOD_STD(ans)<<endl;
	}
	return 0;
}
