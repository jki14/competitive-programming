...
inline int mask(const int lef, const int rig){
	int len=rig-lef+1;
	return MOD_STD(MOD_ADD(buf[rig], -MOD_MUL(buf[lef-1], MOD_POW(26, len))));
}

inline int doub(const int msk, const int len){
	return MOD_STD(MOD_ADD(MOD_MUL(msk, MOD_POW(26, len)), msk));
}
...
int main(){
	while(scanf("%d%d", &n, &m)!=EOF){
		scanf("%s", a);
		buf[0]=0;
		for(int i=1; i<=n; i++)buf[i]=MOD_STD(MOD_ADD(MOD_MUL(buf[i-1], 26), a[i-1]-'a'));
		while(m--){
			int x, y, len, bsk, ans=0;
			scanf("%d%d", &x, &y);
			len=y-x+1;
			bsk=mask(x, y);
			while(1){
				if(x+len-1>n || mask(x, x+len-1)!=bsk)break;
				int buf=1;
				int cur=bsk;
				while(x+len*buf*2-1<=n && mask(x, x+len*buf*2-1)==doub(cur, len*buf)){
					cur=doub(cur, len*buf);
					buf<<=1;
				}
				ans+=buf;
				x+=len*buf;
