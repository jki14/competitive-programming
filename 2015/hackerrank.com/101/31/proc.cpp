...
int main(){
	while(scanf("%d%d", &n, &m)!=EOF){
		...
		for(int i=0; i<n; i++){
			for(int j=0; j<=s; j++){
				MOD_ADD(&g[i+1][j], g[i][j], m);
				MOD_ADD(&f[i+1][j], f[i][j], m);
				MOD_ADD(&f[i+1][j^(i+1)], g[i][j], m);
				MOD_ADD(&f[i+1][j^(i+1)], MOD_MUL(f[i][j], 2, m), m);
			}
		}
		ans=0;inv=INVERSEE(2, m);
		for(int j=0; j<=s; j++){
