...
int main(){
	...
	sort(a, a+n);
	base=d=0LL;
	int p0=upper_bound(a, a+n, 0LL)-a;
	for(int i=0; i<n; i++)base+=llabs(a[i]);
	s[0]=a[0];
	...
	for(int i=0; i<m; i++){
		...
		//printf("ans1=%lld\n", ans);
		int p1=upper_bound(a, a+n, -d)-a;
		//printf("d=%lld p0=%d p1=%d\n", d, p0, p1);
		if(d>=0){
			ans-=d*p1;
		}else ans-=-d*(n-p1);
		//printf("ans2=%lld\n", ans);
		if(d>=0){
			int num=p0-p1;
			lld cnt=s[p0]-s[p1]+a[p1]-a[p0];
			//printf("num=%d cnt=%lld\n", num, cnt);
			ans-=llabs(cnt)-llabs(cnt+d*num);
		}else{
			int num=p1-p0;
			lld cnt=s[p1]-s[p0]+a[p0]-a[p1];
			ans-=llabs(cnt)-llabs(cnt+d*num);
		}
