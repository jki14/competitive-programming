...
int main(){
	...
	while(T--){
		...
		for(int i=0; i<n; i++){
			if(i){
				d[i]=MOD_STD(d[i-1]-a[i], m);
				s[i]=MOD_STD(s[i-1]+a[i], m);
			}else{
				d[i]=MOD_STD(-a[i], m);
				s[i]=MOD_STD(a[i], m);
			}
		}
		set<lld, greater<lld> > weapon;
		weapon.insert(0LL);
		ans = 0LL;
		for(int i=0; i<n; i++){
			lld best=m-1LL-s[i];
			//printf("si=%lld", s[i]);
			set<lld, greater<lld> >::iterator it=weapon.lower_bound(best);
			//printf(" best=%lld it=%lld\n", best, *it);
			if(it!=weapon.end()){
				UPDMAX(ans, MOD_STD(s[i]+*it, m));
			}else UPDMAX(ans, MOD_STD(s[i], m));
			weapon.insert(d[i]);
		}
		printf("%lld\n", ans);
	}
