...
//HEAD_OF_JKI'S_HEADER_
	...
	template<class T> inline void UPDMIN(const T x, const T y){ if(x>y)x=y; }
	template<class T> inline void UPDMAX(const T x, const T y){ if(x<y)x=y; }
...
static int combin(const int x, const int y){
	//printf("combin(%d, %d)=%d\n", x, y, MOD_STD(MOD_MUL(MOD_MUL(factorial[x], INVERSEE(factorial[x-y], MOD)), INVERSEE(factorial[y], MOD))));
	return MOD_MUL(MOD_MUL(factorial[x], INVERSEE(factorial[x-y], MOD)), INVERSEE(factorial[y], MOD));
}
...
int main(){
	...
	while(T--){
		...
		ans=combin(n+k-1, k);
		for(int i=2; i<=n; i++){
			MOD_ADD(&ans, MOD_MUL(mobius(i), combin((n/i)+k-1, k)));
		}
		printf("%d\n", MOD_STD(ans));
	}
	return 0;
