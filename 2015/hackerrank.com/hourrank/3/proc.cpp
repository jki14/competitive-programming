...
inline lld mypow(const lld x, const lld y){
	if(y==0LL)return 1LL;else if((y&1)==0LL){
		lld z=mypow(x, y>>1); return z*z;
	}else return mypow(x, y^1)*x;
}

inline lld mysqrt(const lld x){
	lld lef=1LL, rig=1e9;
	while(lef<=rig){
		lld mid=(lef+rig)>>1;
		if(mid*mid>=x){
...
inline lld mycurt(const lld x){
	lld lef=1LL, rig=1000000LL;
	while(lef<=rig){
		lld mid=(lef+rig)>>1;
		lld buf=mypow(mid, 3);
		if(buf>=x){
...
int main(){
	...
	MILLERRABIN *mill=new MILLERRABIN();
	memset(c, 0, sizeof(c));
	for(int x=1; x<L; x++){
		for(int y=x; y<L; y+=x){
			c[y]++;
	...
	for(int x=1; x<L; x++){
		if(c[x]==2 && x<=1e6)
			plite.push_back(x);
		if(log(x*1.0)*c[x]*0.5<log(2e18)){
			lld buf=mypow(x*1LL, c[x]>>1);
			if(c[x]&1){
				buf*=mysqrt(x*1LL);
			}
			if(!h.count(buf)){
				h.insert(MP(buf, x*1LL));
			}
		}
	}
	int T;cin>>T;
		...
		if(h.count(n)){
			...
			if(mill->test(n)){
				...
				if(c[buf]==2){
					...
					//x^4 == n^2
					int flag=0;
					for(vector<int>::const_iterator it=plite.begin(); it!=plite.end(); it++){
						lld buf=(*it)*1LL*(*it);
						if(n%buf==0LL){
							lld another=mysqrt(n/buf);
							if(~another && mill->test(another)){
								flag=1;
								//cout<<"[4]:";
								cout<<(*it)*another<<endl;
								break;
							}
						}
					}
					...
					//x^5 == n^2
					for(vector<int>::const_iterator it=plite.begin(); it!=plite.end(); it++){
						if(log((*it)*1.0)*10.0<log(2e18*1.0)){
							if(mypow((*it)*1LL, 10)==n){
								flag=1;
								//cout<<"[5]:";
								cout<<mypow((*it)*1LL, 4)<<endl;
								break;
							}
						}
					}
