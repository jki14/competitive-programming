...
inline int getmax(multiset<int> &x){
	multiset<int>::iterator it=x.end();it--;
	int buff=*it;
	x.erase(it);
	return buff;
}

inline int getmin(multiset<int> &x){
...
inline int getpos(const int val){
	multimap<int, int>::iterator it=pos.find(val);
	int buff=it->second;
	pos.erase(it);
	return buff;
}
...
int main(){
	while(scanf("%d", &n)!=EOF){
		...
		for(int i=0; i<n; i++){
			...
			UPDMAX(det, val);
			pos.insert(MP(val, i+1));
		}
		while(abs(sumx-sumy)>det){
			int i, j;
			if(sumx>sumy){
				i=getmax(x);
				j=getmin(y);
