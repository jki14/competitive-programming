...
int n, m;
char nam[N][L], s[L];
int xans[N], yans[N], zans[N];
int xcnt, ycnt, zcnt;
int xcur, ycur, zcur;

inline void update(int &cnt, int &cur, int *ans, const int k, const int val){
	if(!cnt || val>cur){
		cnt=0;
		cur=val;
	}
...
inline void output(int *ans, const int cnt){
	for(int i=0; i<cnt; i++){
		if(i)printf(",");
		printf(" %s", nam[ans[i]]);
	}
...
int main(){
	while(scanf("%d", &n)!=EOF){
		...
		for(int i=0; i<n; i++){
			...
			update(xcnt, xcur, xans, i, tax);
			update(ycnt, ycur, yans, i, piz);
			update(zcnt, zcur, zans, i, gir);
		}
