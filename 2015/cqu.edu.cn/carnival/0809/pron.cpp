...
void cover(int low,int hig,int lef,int rig){
...
void core_entry(){
	for(int lowx=0;lowx<rtl;lowx++)for(int higx=lowx+1;higx<rtl;higx++)if(row[higx]-row[lowx]>1)for(int lefx=0;lefx<ctl;lefx++)for(int rigx=lefx+1;rigx<ctl;rigx++)if(col[rigx]-col[lefx]>1){
		for(int lowy=lowx;lowy<rtl;lowy++)for(int higy=lowy+1;higy<rtl;higy++)if(row[higy]-row[lowy]>1)for(int lefy=0;lefy<ctl;lefy++)for(int rigy=lefy+1;rigy<ctl;rigy++)if(col[rigy]-col[lefy]>1){
			for(int i=0;i<n;i++)v[i].reset();
			cover(row[lowx],row[higx],col[lefx],col[rigx]);
			cover(row[lowy],row[higy],col[lefy],col[rigy]);
			if(judge()){
				printf("YES\n");
				printf("%d %d %d %d\n",row[lowx]+1,col[lefx]+1,row[higx]+1,col[rigx]+1);
				printf("%d %d %d %d\n",row[lowy]+1,col[lefy]+1,row[higy]+1,col[rigy]+1);
				return;
			}
		}
	}
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		if(rtl>4){ row[2]=row[rtl-2];row[3]=row[rtl-1];rtl=4; }
		for(int j=0; j<m; j++){
			int count=0;
			for(int i=0; i<n; i++){
				if(c[i].test(j))
		...
		if(ctl>4){ col[2]=col[ctl-2];col[3]=col[ctl-1];ctl=4; }
		core_entry();
	}
	return 0;
}
