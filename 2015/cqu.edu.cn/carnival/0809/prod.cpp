...
int dp_entry(const int lef, const int rig, const int pre, const int nxt){
	if(lef>rig)return 1;
	if(cur>=0)return cur;
	cur=0;
	if(pnt[lef]==rig){
		//1xx0
		if(pre!=1)MOD_ADD(&cur, dp_entry(lef+1, rig-1, 1, 0));
		//2xx0
		if(pre!=2)MOD_ADD(&cur, dp_entry(lef+1, rig-1, 2, 0));
		//0xx1
		if(nxt!=1)MOD_ADD(&cur, dp_entry(lef+1, rig-1, 0, 1));
		//0xx2
		if(nxt!=2)MOD_ADD(&cur, dp_entry(lef+1, rig-1, 0, 2));
	}else{
		//1xx0..
		if(pre!=1)MOD_ADD(&cur, MOD_MUL(dp_entry(lef+1, pnt[lef]-1, 1, 0), dp_entry(pnt[lef]+1, rig, 0, nxt)));
		//2xx0..
		if(pre!=2)MOD_ADD(&cur, MOD_MUL(dp_entry(lef+1, pnt[lef]-1, 2, 0), dp_entry(pnt[lef]+1, rig, 0, nxt)));
		//0xx1..
		MOD_ADD(&cur, MOD_MUL(dp_entry(lef+1, pnt[lef]-1, 0, 1), dp_entry(pnt[lef]+1, rig, 1, nxt)));
		//0xx2..
		MOD_ADD(&cur, MOD_MUL(dp_entry(lef+1, pnt[lef]-1, 0, 2), dp_entry(pnt[lef]+1, rig, 2, nxt)));
	}
	cur=MOD_STD(cur);
	return cur;
}
