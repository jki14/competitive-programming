...
struct buff_s{
	lld ful, pre, nxt, top;
	buff_s(){}
	buff_s(const lld f, const lld p, const lld n, const lld t){
		this->ful=f;
		this->pre=p;
		this->nxt=n;
		this->top=t;
...
typedef buff_s buff_t;

int n, m, k, pos[N];
lld a[N], ans;
buff_t c[S];

inline buff_t combine(const buff_t &lef, const buff_t &rig){
	buff_t buff;
...
buff_t query(const int i, const int lef, const int rig, const int src, const int dst){
	if(dst<lef || src>rig)return buff_t(0LL, 0LL, 0LL, 0LL);
	if(src<=lef && rig<=dst){
		return c[i];
	}else{
		int mid=(lef+rig)>>1;
		if(dst<=mid)return query(L, lef, mid, src, dst);
		if(src>mid)return query(R, mid+1, rig, src, dst);
		return combine(query(L, lef, mid, src, dst), query(R, mid+1, rig, src, dst));
	}
}
