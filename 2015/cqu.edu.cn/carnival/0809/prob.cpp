...
void handle(char *s, char *a, char *b){
	int i=0;
	while(s[i]=='0' && s[i+1]!=':')i++;
	while(s[i]!=':')*a++=s[i++];
	*a++=0;i++;
...
inline int numeric(const char c){
	if('0'<=c && c<='9')
		return c-'0';
	else	return c-'A'+10;
}

int suit(const int k){
	int buff=0;
	for(int i=0; a[i]; i++){
		buff*=k;
		buff+=numeric(a[i]);
...
int bs_entry(int lef, int rig){
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(suit(mid)){
			if(!suit(mid+1))return mid;
			lef=mid+1;
		}else rig=mid-1;
	}
	return -1;
...
int main(){
	while(scanf("%s", s)!=EOF){
		...
		lef=2;
		for(int i=0; a[i]; i++){
			UPDMAX(lef, numeric(a[i])+1);
		}
		for(int i=0; b[i]; i++){
			UPDMAX(lef, numeric(b[i])+1);
		}
		rig=bs_entry(lef, 64);
		if(lef<=rig){
