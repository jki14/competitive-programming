...
inline int month(const char* mm){
	for(int i=0; i<12; i++)
		if(!strcmp(mm, mname[i]))return i+1;
	return 0;
}

inline int run(const int y){
	if(!(y%400))return 1;
	if(!(y%100))return 0;
	return (!(y%4))?1:0;
}

inline int cmp(const int mx, const int dx, const int my, const int dy){
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		printf("Case #%d: ", _T);
		int yx,mx,dx,yy,my,dy;
		char buf[16];
		scanf("%s%d,%d", buf, &dx, &yx);
		mx=month(buf);
		if(!mx)while(1);
		scanf("%s%d,%d", buf, &dy, &yy);
		my=month(buf);
		if(!my)while(1);
		int count=(yy/4-(yx-1)/4)-(yy/100-(yx-1)/100)+(yy/400-(yx-1)/400);
		if(run(yx) && cmp(mx, dx, 2, 29)>0)count--;
		if(run(yy) && cmp(2, 29, my, dy)>0)count--;
		printf("%d\n", count);
	}
	return 0;
}
