...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		...
		scanf("%d",&n);
		for(int i=0;i<n;i++){
			int arg_x, arg_y, arg_w;
			scanf("%d%d%d", &arg_x, &arg_y, &arg_w);
			x1[i]=(arg_x*1.0-arg_y*1.0)*s2*0.5;
			y1[i]=(arg_x*1.0+arg_y*1.0)*s2*0.5;
			x2[i]=x1[i]+arg_y*1.0*s2;
			y2[i]=y1[i]-arg_y*1.0*s2;
			w[i]=arg_w*1.0;
		}
		ans=0.0;
		for(int s=0;s<bit[n];s++){
			UPDMAX(ans, weight(s)-cost(s));
		}
		printf("%.2f\n",ans);
	}
	return 0;
