...
int main(){
	...
	while(cin>>c>>hr>>hb>>wr>>wb){
		n=cur=0;
		for(int i=0; wr*1LL*bit[i]<=c*1LL; i++){
			wht[n]=wr*bit[i];
			val[n]=hr*bit[i];
			n++;
		}
		for(int i=0; wb*1LL*bit[i]<=c*1LL; i++){
			wht[n]=wb*bit[i];
			val[n]=hb*bit[i];
			n++;
		}
		f[cur].clear();f[cur].insert(MP(0, 0LL));
		for(int i=0; i<n; i++){
			cur=pre;f[cur]=f[pre];
			for(map<int, lld>::iterator it=f[pre].begin(); it!=f[pre].end(); it++){
				int cost=it->first+wht[i];
				if(cost>c)break;
				lld joy=it->second+val[i];
				map<int, lld>::iterator buf=f[cur].find(cost);
				if(buf!=f[cur].end()){
					UPDMAX(buf->second, joy);
				}else f[cur].insert(MP(cost, joy));
			}
		}
		ans=0LL;
		for(map<int, lld>::iterator it=f[cur].begin(); it!=f[cur].end(); it++)
			UPDMAX(ans, it->second);
		cout<<ans<<endl;
	}
	return 0;
}
