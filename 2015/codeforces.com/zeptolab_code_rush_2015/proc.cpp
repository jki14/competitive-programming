...
lld c, hr, hb, wr, wb, ans;

lld func(const lld r){
	//cout<<"func "<<r<<" = "<<r*hr+(c-r*wr)/wb*hb<<endl;
	if(r*wr>c)return 0LL;
	//cout<<r<<"*"<<hr<<"+("<<c<<"-"<<r<<"*"<<wr<<")/"<<wb<<"*"<<hb<<"="<<(r*hr+(c-r*wr)/wb*hb)<<endl;
	return r*hr+(c-r*wr)/wb*hb;
}
...
int main(){
	...
	while(cin>>c>>hr>>hb>>wr>>wb){
		...
		ans=0LL;
		if(wr>1000){
			for(int i=c/wr; ~i; i--)UPDMAX(ans, func(i));
		}else{
			if(wb*hr>=wr*hb){
				swap(hr, hb);
				swap(wr, wb);
			}
			//make sure wb*hr<wr*hb
			//-> k*wb*hr < k*wb*wr/wb*hb
			//cout<<hr<<" "<<hb<<" "<<wr<<" "<<wb<<endl;
			for(int i=0; i<wb; i++)UPDMAX(ans, func(i));
		}
		cout<<ans<<endl;
	}
	return 0;
