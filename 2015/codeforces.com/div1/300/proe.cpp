...
int set_max(const int u){
	if(cnt[u]==1)return 1;
	if(stt[u]){
		int best=INF;
	 	for(vector<int>::iterator it=chd[u].begin();it!=chd[u].end();it++){
			UPDMIN(best,set_max(*it));
		}
		return best;
	}else{
		int best=0;
...
int set_min(const int u){
	if(cnt[u]==1)return 1;
	if(stt[u]){
		int miss=0;
		for(vector<int>::iterator it=chd[u].begin();it!=chd[u].end();it++){
			miss+=cnt[*it]-set_min(*it);
		}
		return cnt[u]-miss;
	}else{
		int best=INF;
		for(vector<int>::iterator it=chd[u].begin();it!=chd[u].end();it++){
			UPDMIN(best,set_min(*it));
		}
		return best;
	}
}

void build(const int u, const int k){
	stt[u]=k;cnt[u]=0;
	for(vector<int>::iterator it=chd[u].begin();it!=chd[u].end();it++){
		build(*it, k^1);
		cnt[u]+=cnt[*it];
