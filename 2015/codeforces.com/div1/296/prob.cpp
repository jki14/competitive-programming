...
int n, cur;
int x[N], y[N], tai[N], idx[N];

map<int, int, greater<int> > f;

static int cmp(const int x, const int y){ return tai[x]<tai[y]; }

int main(){
	while(scanf("%d", &n)!=EOF){
		f.clear();f.insert(MP(0xc0c0c0c0, 0));
		for(int i=0; i<n; i++){
			scanf("%d%d", &x[i], &y[i]);
			tai[i]=x[i]+y[i];
			idx[i]=i;
		}
		sort(idx, idx+n, cmp);cur=0;
		for(int i=0; i<n; i++){int pre=x[idx[i]]-y[idx[i]];
			map<int, int, greater<int> >::iterator it=f.lower_bound(pre);
			map<int, int, greater<int> >::iterator jt=f.find(tai[idx[i]]);
			UPDMAX(cur, it->second+1);
			if(jt==f.end()){
				f.insert(MP(tai[idx[i]], cur));
			}else{
				UPDMAX(jt->second, cur);
			}
		}
		printf("%d\n", cur);
	}
