...
char s[N], t[N];
int num[N], pre[4], nxt[4], msk[N][4], cost[N];
...
void fft(Complex<double> y[], const int len, const int rev){
	...
	for(int i=2; i<=len; i<<=1){
		Complex<double> wn(cos(-rev*2*PI/i), sin(-rev*2*PI/i));
...
void calc(const int n, const int m, const int len, const int k){
	for(int i=0; i<len; i++){
		x[i] = Complex<double>(msk[i][k], 0);
		y[i] = Complex<double>(i<m?trans(t[m-i-1])==k:0, 0);
	}
	fft(x, len, 1);
	fft(y, len ,1);
	for(int i=0; i<len; i++){
