...
class seg_t{
	public:
	int x, y;
	bool operator < (const seg_t &opn) const{
		int xsz=this->y-this->x;
		int ysz=opn.y-opn.x;
		if(xsz!=ysz)return xsz>ysz;
		return this->x<opn.x;
	}
	seg_t(){}
	seg_t(const int arg_x, const int arg_y){
		this->x=arg_x;
		this->y=arg_y;
	}
};

void core(map<int, int, greater<int> > &s, set<seg_t> &f, const int pos){
	map<int, int, greater<int> >::iterator it=s.upper_bound(pos);
	int x=it->first, y=it->second;
	s.erase(it);
	f.erase(f.find(seg_t(x, y)));
	s.insert(MP(x, pos));
	s.insert(MP(pos, y));
	f.insert(seg_t(x, pos));
	f.insert(seg_t(pos, y));
}

int w, h, n, pos;
map<int, int, greater<int> > sh, sv;
set<seg_t> fh, fv;
char typ[8];
...
int main(){
	...
	while(cin>>w>>h>>n){
		...
		for(int i=0; i<n; i++){
			...
			set<seg_t>::iterator it=fh.begin();
			set<seg_t>::iterator jt=fv.begin();
			cout<<(it->y-it->x)*1LL*(jt->y-jt->x)<<endl;
		}
	}
}
