...
int accept(const int k){
	...
	for(int i=0; i<k; i++){
		b[i]=m;
		while(hed<til && a[i]>=a[que[til-1]]-(i-que[til-1]))til--;
		que[til++]=i;
	}
	for(int i=k; i<n; i++){
		while(hed<til && a[i]>=a[que[til-1]]-(i-que[til-1]))til--;
		que[til++]=i;
		while(i-que[hed]>k)hed++;
		UPDMAX(b[i], a[que[hed]]-(i-que[hed])+k);
	}
	//<---
	hed=til=0;
	for(int i=n-1; i>=n-k; i--){
		b[i]=m;
		while(hed<til && a[i]>=a[que[til-1]]-(que[til-1]-i))til--;
		que[til++]=i;
	}
	for(int i=n-k-1; i>=0; i--){
		while(hed<til && a[i]>=a[que[til-1]]-(que[til-1]-i))til--;
		que[til++]=i;
		while(que[hed]-i>k)hed++;
		UPDMAX(b[i], a[que[hed]]-(que[hed]-i)+k);
	}
	//check
	for(int i=0; i<n; i++)if(b[i]<m)return 0;
	return 1;
}
