...
int main() {
	while(scanf("%d", &n)!=EOF){
		...
		for(int i=0; i<n; i++){
			int val=a[i]-'0';
			for(int tar=0; tar<B; tar++){
				int ant=tar^1;
				//before-flip
				if(tar==val){
					f[i+1][tar][0]=MAX(f[i][tar][0], f[i][ant][0]+1);
				}else f[i+1][tar][0]=f[i][tar][0];
				//in-flip
				if(tar!=val){
					f[i+1][tar][1]=MAX(f[i][tar][0], f[i][ant][0]+1);
					UPDMAX(f[i+1][tar][1], MAX(f[i][tar][1], f[i][ant][1]+1));
				}else f[i+1][tar][1]=MAX(f[i][tar][0], f[i][tar][1]);
				//after-flip
				f[i+1][tar][2]=f[i+1][tar][1];
				if(tar==val){
					UPDMAX(f[i+1][tar][2], MAX(f[i][tar][1], f[i][ant][1]+1));
					UPDMAX(f[i+1][tar][2], MAX(f[i][tar][2], f[i][ant][2]+1));
				}else UPDMAX(f[i+1][tar][2], MAX(f[i][tar][2], f[i][tar][1]));
				/*printf("f[%d][%d][%d]=%d\n", i+1, tar, 0, f[i+1][tar][0]);
				printf("f[%d][%d][%d]=%d\n", i+1, tar, 1, f[i+1][tar][1]);
				printf("f[%d][%d][%d]=%d\n", i+1, tar, 2, f[i+1][tar][2]);
				printf("---------------------------------------------\n");*/
			}
		}
		printf("%d\n", MAX(f[n][0][2], f[n][1][2]));
