...
inline int count(const int p, const int d){
	if(d<=0)return 0;
	int buff=0;
	for(int i=0; i<n; i++){
		if((x[i]-x[p])*(x[i]-x[p])+(y[i]-y[p])*(y[i]-y[p])<d*d*1.0-eps)
			buff++;
	}
	return buff;
}

inline int judge(const int p, const int d){
	if(d<=0)return 0;
	int buff=0;
	double circ=d*d*1.0;
	for(int i=0; i<n; i++){
		double dist=(x[i]-x[p])*(x[i]-x[p])+(y[i]-y[p])*(y[i]-y[p]);
		if(dist<circ-eps){
			buff++;
		}else if(dist<circ+eps)	return 0;
	}
	if(buff!=m)return 0;
	return 1;
}

int bs_entry(const int p){
	int lef=1, rig=10000;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(count(p, mid)>=m){
