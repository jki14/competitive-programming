	...
	while(T--){
		...
		for(int k=0; a[k]; k++){
			switch(a[k]){
				...
				case 'D':
					if(cli==NULL){
						memmove(cur, cur+1, til-cur);
						til--;
						if(til<cur)til=cur;
					}else{
						if(cur<=cli){
							//--->
							memmove(cur, cli, til-cli);
							til+=cur-cli;
						}else{
							memmove(cli, cur, til-cur);
							til+=cli-cur;
							cur=cli+1;
						}
					}
