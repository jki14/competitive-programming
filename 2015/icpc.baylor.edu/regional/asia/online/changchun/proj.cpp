...
lld comb_entry(lld a, lld b, const lld p){
	if(a<b)return 0LL;
	if(a==b)return 1LL;
	if(b>a-b)b=a-b;
	lld ans=1LL;
	for(lld i=0; i<b; i++){
		MOD_MUL(&ans, a-i, p);
		MOD_MUL(&ans, INVERSEE(i+1LL, p), p);
	}
	return MOD_STD(ans, p);
}

lld lucas_entry(lld n, lld m, const lld p){
	lld ans=1LL;
	while(n && m && ans){
		ans=ans*comb_entry(n%p, m%p, p)%p;
		n/=p;
		m/=p;
	}
	return ans;
...
lld crt_entry(){
	for(int i=1; i<k; i++){
		lld x, y;
		lld d=EEA(w[0], w[i], x, y);
		lld k=w[i]/d;
		a[0]=w[0]*(((a[i]-a[0])/d*x%k+k)%k)+a[0];
		w[0]=w[0]*w[i]/d;
	}
	return a[0];
}
