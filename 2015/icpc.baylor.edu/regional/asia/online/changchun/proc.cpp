...
const int til=1000000000;
const int inv=INVERSEE(2, MOD);
...
int a_sum(const int p){
	int lef=1, rig=m-1;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(p>=key[mid]){
			if(mid==m-1 || p<key[mid+1]){
				return sum[mid]+(mid+1)*(p-key[mid]);
			}
			lef=mid+1;
		}else rig=mid-1;
	}
...
int r_sum(const int p){
	int lef=1, rig=m;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(p>=key[mid]){
			if(mid==m || p<key[mid+1]){
				return MOD_ADD(sum[mid], MOD_MUL(MOD_MUL(MOD_MUL(MOD_ADD(key[mid]+1, p), p-key[mid]), inv), mid+1));
			}
			lef=mid+1;
		}else rig=mid-1;
	}
...
void prepare0(){
	m=2;
	key[0]=0;key[1]=1;key[2]=3;
	sum[0]=0;sum[1]=1;sum[2]=5;
	while(1){
		if(key[m]>=til)
			break;
		else m++;
		key[m]=a_sum(m);
		if(!~key[m])return;
		sum[m]=sum[m-1]+m*(key[m]-key[m-1]);
		//printf("%9d: key=%d, sum=%d\n", m, key[m], sum[m]);
	}
	//printf("m=%d\n", m);
}
...
void prepare1(){
	for(int i=1; i<=m; i++){
		sum[i]=MOD_ADD(sum[i-1], MOD_MUL(MOD_MUL(MOD_MUL(MOD_ADD(key[i-1]+1, key[i]), key[i]-key[i-1]), inv), i));
	}
}
