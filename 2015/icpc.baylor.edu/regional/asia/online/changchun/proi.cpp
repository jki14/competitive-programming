...
void bitrize_in(int *aw, int *av, int &n, const int size){
...
int bs_entry(const int x){
	int lef=0, rig=q;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(g[mid]>=x){
			if(mid==0 || g[mid-1]<x)return mid;
			rig=mid-1;
		}else lef=mid+1;
	}
	return -1;
...
int main(){
	...
	while(T--){
		scanf("%d%d%d", &n, &m, &p);
		bitrize_in(fw, fv, n, n);
		bitrize_in(cw, cv, m, m);
		memset(f, 0x3f, sizeof(f));f[0]=0;
		for(int i=0; i<n; i++)
			for(int j=p; j>=0; j--)
				UPDMIN(f[j], (j>=fw[i])?(f[j-fw[i]]+fv[i]):fv[i]);
		memset(g, 0, sizeof(g));
		for(int i=0; i<m; i++)
			for(int j=q; j>=cv[i]; j--)
				UPDMAX(g[j], g[j-cv[i]]+cw[i]);
		if(g[q]<f[p]){
			printf("TAT\n");
		}else printf("%d\n", bs_entry(f[p]));
	}
