...
node_t *rut, *cur, pool[N];

void insert(node_t **node, const int x){
	if(*node==NULL){
		*node=cur++;
		(*node)->val=x;
		(*node)->lef=NULL;
	...
	if((*node)->val<x){
		insert(&(*node)->lef, x);
	}else insert(&(*node)->rig, x);
}

void build(node_t *node, const int p, const char r){
	if(node==NULL)return;
	opt[node->val]=r;
	anc[node->val]=p;
	build(node->lef, node->val, 'W');
	build(node->rig, node->val, 'E');
}
