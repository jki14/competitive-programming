...
int n, m, q;
int f[N], c[N];
int u[M], v[M], w[M], ide[M];
int x[Q], idx[Q], ans[Q], cur;

inline int find(const int u){ if(f[u]!=u)f[u]=find(f[u]);return f[u]; }

void add_edge(const int u, const int v){
	if(find(u)==find(v))return;
	cur+=c[find(u)]*c[find(v)]*2;
	c[find(u)]+=c[find(v)];
	c[find(v)]=0;
	f[find(v)]=find(u);
}

inline static int ide_compare(const int i, const int j){
	return w[i]<w[j];
}

inline static int idx_compare(const int i, const int j){
	return x[i]<x[j];
}
...
int main(){
	...
	for(int _T=1; _T<=T; _T++){
		...
		sort(ide, ide+m, ide_compare);
		for(int i=0; i<q; i++){
			scanf("%d", &x[i]);
			idx[i]=i;
		}
		sort(idx, idx+q, idx_compare);
		for(int i=0, e=cur=0; i<q; i++){
			for(;e<m && w[ide[e]]<=x[idx[i]];e++)
				add_edge(u[ide[e]], v[ide[e]]);
			ans[idx[i]]=cur;
		}
		for(int i=0; i<q; i++)
			printf("%d\n", ans[i]);
