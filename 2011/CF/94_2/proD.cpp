...
void work(){
	scanf("%d",&m);build();
	if(((lld)m)>((lld)n)*((lld)n-1)/2LL){
		printf("No such line.\n");
		return;
	}

	/* Check */
	//for(int i=0;i<n;i++)printf("%3d: sa=%d hig=%d\n",i,sa[i],hig[i]);

	memset(head,0,sizeof(head));
	for(int pos=0;pos<n;pos++){
		for(int len=head[pos]+1;len<=n-1-sa[pos];len++){m--;
			int k=pos+1;while(m && hig[k]>=len){ head[k]=len;k++;m--; }
			if(!m){
				for(int i=sa[pos];i-sa[pos]+1<=len;i++)printf("%c",s[i]);
				printf("\n");
				return;
			}
		}
