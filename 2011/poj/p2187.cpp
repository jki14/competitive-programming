...
int dcmp(const int &x){
	if(x<0)return -1;
	return (x>0);
}

int cross(const TPoint &p0,const TPoint &p1,const TPoint &p2){
...
int dis2(const TPoint &p1,const TPoint &p2){
...
void work(){
	int i;
	for(i=0;i<n;i++)scanf("%d%d",&pin[i].x,&pin[i].y);
	GrahamScan();
	printf("%d\n",Diameter());
}
