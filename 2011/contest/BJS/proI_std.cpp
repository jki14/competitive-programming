...
//freopen("i.in","r",stdin);
    ...
    while (cin >> n >> m >> k >> p1 >> p2 >> p3 >> p4) {
        ...
        RST(f); DB sum = p2 + p3 + p4; p2 /= sum, p3 /= sum, p4 /= sum;
 
        g[0] = 1; REP_1(i, n) g[i] = g[i-1] * p2;
 
        f[1][1] = p4 / (p3 + p4);
 
        FOR_1(i, 2, n) REP_1(j, i){
            if (j==1) {
                f[i][1]=p4;
                FOR(p, 1, i)
                    if (p>=i+1-k) f[i][1]+=g[p]*p4+g[p]*p3*f[i-1][i-p];
                    else f[i][1]+=g[p]*p3*f[i-1][i-p];
                f[i][1]/=(1-g[i]);
            }
            else {
                if (j<=k) f[i][j]=f[i][j-1]*p2+f[i-1][j-1]*p3+p4;
                else f[i][j]=f[i][j-1]*p2+f[i-1][j-1]*p3;
            }
        }
            printf("%.5lf\n",f[n][m]);
    }
