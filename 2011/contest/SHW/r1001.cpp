...
void conduct()
{
	int i, j, k, l, top, bot; double ret, tv; pnt pos, tp;
	...
	for (i = 1; i < n; ++i) {
		...
		for (j = 0; j <= i; ++j) for (k = j+1; k <= i; ++k) {
			tp = (pnt){ p[j].x, 0 }; tv = p[j].y;
			if (getdis(p[k], tp)-tv > eps) { tp = getpos(p[j], p[k]); tv = getdis(tp, p[k]); }
			for (l = 0; l <= i; ++l) if (getdis(p[l], tp)-tv > eps) break;
			if (l > i && tv < ret) { ret = tv; pos = tp; }
		}
	} printf("%.9f %.9f\n", pos.x, ret);
}
