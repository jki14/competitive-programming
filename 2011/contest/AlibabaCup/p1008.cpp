...
int cir(const int &x,const int &y){
	if(mk[x][y])return 1;
	for(int i=0;i<K;i++)if(mk[x+dx[i]][y+dy[i]])return 1;
	return 0;
}
...
void dfs(const int &x,const int &y,const int &k){
	...
	if(x==tx && y==ty){
		if(ans<k){
			ans=k;id=++num;
			for(i=1;i<k;i++)r[routex[i]][routey[i]]=id;
		}
		return;
	}
	routex[k]=x;routey[k]=y;
	for(i=0;i<K;i++){xx=x+dx[i];yy=y+dy[i];
		if(a[xx][yy])continue;
		//printf("Try %d,%d",xx,yy);
		if(cir(xx,yy)){
			//printf(" Fault\n");
			continue;
		}
