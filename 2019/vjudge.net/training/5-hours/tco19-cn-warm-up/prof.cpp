...
int n;
vector<list<size_t>> pmap;

struct node_t {
  int lef, rig;
  int foo;
  node_t* chd[2];
} *root[410000], pool[8888888], *last;
...
node_t* insert(node_t* prev, int const key, int const val) {
  node_t* node = last++;
  node->lef = prev->lef;
  node->rig = prev->rig;
  node->foo = MAX(val, prev->foo);
  node->chd[0] = prev->chd[0];
  node->chd[1] = prev->chd[1];
  if (node->chd[0] != nullptr) {
    if (key <= node->chd[0]->rig) {
      node->chd[0] = insert(node->chd[0], key, val);
    } else {
      node->chd[1] = insert(node->chd[1], key, val);
    }
  }
  return node;
}

int query(node_t* node, int const lef, int const rig) {
  if (lef <= node->lef && node->rig <= rig) {
    return node->foo;
  }
  int foo = 0;
  if (lef <= node->chd[0]->rig) {
    UPDMAX(foo, query(node->chd[0], lef, rig));
  }
  if (rig >= node->chd[1]->lef) {
    UPDMAX(foo, query(node->chd[1], lef, rig));
  }
  return foo;
}

int bsearch(int const x, int const y) {
  int lef = 0, rig = n - 1;
  while (lef <= rig) {
    int const mid = (lef + rig) >> 1;
    if (query(root[mid], x + 1, y - 1) > y) {
      if (mid == lef || query(root[mid - 1], x + 1, y - 1) < y) {
        return mid;
      }
      rig = mid - 1;
    } else {
...
int main() {
  while (scanf("%d", &n) != EOF) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (pmap[i].size() < 2) continue;
      auto const tail = pmap[i].back();
      for (auto const p : pmap[i]) {
        if (p == tail) break;
        root[i] = insert(root[i], p, tail);
      }
    }
    pii ans(inf, inf);
    for (int i = 0; i < n; ++i) {
      for (auto it = pmap[i].begin(); it != pmap[i].end(); ++it) {
        auto const jt = next(it);
        if (jt == pmap[i].end()) break;
        int x = *it;
        int y = *jt;
        int cur = bsearch(x, y);
        if (cur != -1) {
          ans.first = i;
          UPDMIN(ans.second, cur);
        }
      }
