...
maxflow_c<int> maxflow(91000, 310000);
...
int main() {
  ...
  while (scanf("%d%d%d", &n, &k, &m) != EOF) {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      --v;
      for (int j = 0; j < k; ++j) {
        maxflow.add_edge(u * k + j, v * k + (j + 1) % k, 1);
      }
    }
    for (int i = 0; i < k; ++i) {
      maxflow.add_edge(90998, i * k + i, 1);
      maxflow.add_edge((n - 1) * k + i, 90999, inf);
    }
    printf("%d\n", maxflow.isap(90998, 90999));
  }
  return 0;
}
