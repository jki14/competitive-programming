...
int n, m;
int nbs[510000], nxt[1100000], dst[1100000], eid;
int vst[510000];
...
int process(int& bar, int const u, int const p = -1, int const c = 0) {
  if (~vst[u]) return vst[u];
  vst[u] = c;
  for (int i = nbs[u]; ~i; i = nxt[i]) {
    int const v = dst[i];
    if (v == p) continue;
    if (process(bar, v, u, c ^ 1) == c) bar = 1;
  }
  return vst[u];
}
