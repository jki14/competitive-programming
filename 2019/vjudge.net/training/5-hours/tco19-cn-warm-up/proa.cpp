...
int n, m;
int g[5100];
point_t<double> a[5100];

double foo;

double area(point_t<double> const& a,
            point_t<double> const& b,
            point_t<double> const& c) {
  ...
  while (cin >> n) {
    ...
    if (m >= 3) {
      for (int i = 0; i < m; ++i) {
        ...
        for (; j != i; j = (j + 1) % m) {
          if (k == j) k = (k + 1) % m;
          while (area(a[g[i]], a[g[j]], a[g[k]]) < area(a[g[i]], a[g[j]], a[g[(k + 1) % m]]) + eps) {
            k = (k + 1) % m;
          }
          UPDMAX(foo, area(a[g[i]], a[g[j]], a[g[k]]));
        }
      }
    }
