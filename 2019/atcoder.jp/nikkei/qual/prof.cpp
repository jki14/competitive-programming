...
int main() {
  ...
  while (scanf("%d%d", &n, &m) != EOF) {
    ...
    vector<vector<int>> can(m, vector<int>());
    for (int i = 0; i < m; ++i) {
      can[i].reserve(cnt[i]);
    }
    for (int i = 0; i < n; ++i) {
      can[jew[i].first].push_back(jew[i].second);
    }
    jew.clear();
    jew.reserve(n);
    for (int i = 0; i < m; ++i) {
      sort(can[i].begin(), can[i].end(), greater<int>());
      if (can[i].size() >= 2) {
        can[i][0] = (can[i][0] * 1LL + can[i][1]) >> 1;
        can[i][1] = can[i][0];
      }
      for (size_t j = 0; j < can[i].size(); ++j) {
        jew.push_back(make_pair(i, j));
      }
    }
    sort(jew.begin(),
         jew.end(),
         [&can](pii const& lhs, pii const& rhs) {
           if (can[lhs.first][lhs.second] != can[rhs.first][rhs.second]) {
            return can[lhs.first][lhs.second] > can[rhs.first][rhs.second];
           }
           return lhs < rhs;
         });
    vector<lld> remove1_v(m, 0);
    set<pli> remove1_s;
    vector<lld> remove2_v(m, 0);
    set<pli> remove2_s;
    vector<lld> inject1_v(m, 0);
    set<pli, greater<pli>> inject1_s;
    vector<lld> inject2_v(m, 0);
    set<pli, greater<pli>> inject2_s;
    for (int i = 0; i < m; ++i) {
      if (can[i].size() >= 2) {
        inject2_v[i] = can[i][0] * 1LL + can[i][1];
        inject2_s.insert(MP(inject2_v[i], i));
      }
    }
    vector<lld> inject3_v(m, 0);
    set<pli, greater<pli>> inject3_s;
    for (int i = 0; i < m; ++i) {
      if (can[i].size() >= 3) {
        inject3_v[i] = can[i][0] * 1LL + can[i][1] + can[i][2];
        inject3_s.insert(MP(inject3_v[i], i));
      }
    }
    lld foo = 0; 
    for (int k = 0; k < n; ++k) {
      int const i = jew[k].first;
      int const j = jew[k].second;
      int const r = can[i].size() - j - 1;
      ...
      if (j != 0) {
        ...
        if (!inject1_s.empty()) {
          ...
          //        inject1_s.begin()->first, inject1_s.begin()->second);
          UPDMAX(bar, foo + inject1_s.begin()->first);
        }
        if (!inject2_s.empty() && !remove1_s.empty()) {
          //fprintf(stderr, "%lld - (%lld, %d) + (%lld, %d)\n", foo,
          //        remove1_s.begin()->first, remove1_s.begin()->second,
          //        inject2_s.begin()->first, inject2_s.begin()->second);
          UPDMAX(bar,
                 foo - remove1_s.begin()->first + inject2_s.begin()->first);
        }
        if (!inject3_s.empty() && !remove2_s.empty()) {
          //fprintf(stderr, "%lld - (%lld, %d) + (%lld, %d)\n", foo,
          //        remove2_s.begin()->first, remove2_s.begin()->second,
          //        inject3_s.begin()->first, inject3_s.begin()->second);
          UPDMAX(bar,
                 foo - remove2_s.begin()->first + inject3_s.begin()->first);
        }
        printf("%lld\n", bar >> 1);
      }
      foo += can[i][j];
      remove1_s.erase(MP(remove1_v[i], i));
      if (j >= 2) {
        remove1_v[i] = can[i][j];
        remove1_s.insert(MP(remove1_v[i], i));
      }
      remove2_s.erase(MP(remove2_v[i], i));
      if (j != 0 && j != 2) {
        remove2_v[i] = can[i][j - 1] * 1LL + can[i][j];
        remove2_s.insert(MP(remove2_v[i], i));
      }
      inject1_s.erase(MP(inject1_v[i], i));
      if (j != 0 && r >= 1) {
        inject1_v[i] = can[i][j + 1];
        inject1_s.insert(MP(inject1_v[i], i));
      }
      inject2_s.erase(MP(inject2_v[i], i));
      if (j != 0 && r >= 2) {
        inject2_v[i] = can[i][j + 1] * 1LL + can[i][j + 2];
        inject2_s.insert(MP(inject2_v[i], i));
      }
      inject3_s.erase(MP(inject3_v[i], i));
      if (j != 0 && r >= 3) {
        inject3_v[i] = can[i][j + 1] * 1LL + can[i][j + 2] + can[i][j + 3];
        inject3_s.insert(MP(inject3_v[i], i));
      }
    }
  }
