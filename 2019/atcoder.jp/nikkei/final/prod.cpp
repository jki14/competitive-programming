...
void cover(node_t *node, const int lef, const int rig, const int rhs) {
  if (lef <= node->lef && node->rig <= rig) {
    assert(node->cov <= rhs);
    node->cov = rhs;
    node->sum = rhs * 1LL * (node->rig - node->lef + 1);
  } else {
    if (node->cov) {
      cover(node->chd[0], -inf, inf, node->cov);
      cover(node->chd[1], -inf, inf, node->cov);
      node->cov = 0;
    }
    if (lef <= node->chd[0]->rig) {
      cover(node->chd[0], lef, rig, rhs);
    }
    if (rig >= node->chd[1]->lef) {
      cover(node->chd[1], lef, rig, rhs);
    }
    node->sum = node->chd[0]->sum + node->chd[1]->sum;
  }
}

lld calc(node_t *node, const int lef, const int rig) {
  if (lef <= node->lef && node->rig <= rig) {
    return node->sum;
  } else {
    if (node->cov) {
    ...
    lld foo = 0LL;
    if (lef <= node->chd[0]->rig) foo += calc(node->chd[0], lef, rig);
    if (rig >= node->chd[1]->lef) foo += calc(node->chd[1], lef, rig);
    return foo;
  }
}
...
int main() {
  ...
  while (scanf("%d%d", &n, &m) != EOF) {
    ...
    for (int i = 0; i < m; ++i) {
      int tim, lef, rig;
      scanf("%d%d%d", &tim, &lef, &rig);
      foo += tim * 1LL * (rig - lef + 1) - calc(root, lef, rig);
      cover(root, lef, rig, tim);
    }
    printf("%lld\n", foo);
  }
  return 0;
