...
const lld lnf = 0x3f3f3f3f3f3f3f3fLL;

struct node_t {
  int lef, rig;
  lld foo;
  node_t* chd[2];
} pool[210000], *last, *root;

void build(node_t **hook, const int lef, const int rig) {
...
void update(node_t *node, const int pos, const lld rhs) {
  UPDMIN(node->foo, rhs);
  if (node->lef != node->rig) {
    if (pos <= node->chd[0]->rig) {
      update(node->chd[0], pos, rhs);
    } else {
      update(node->chd[1], pos, rhs);
    }
  }
}

lld getmin(node_t *node, const int lef, const int rig) {
...
struct city_t {
  ...
  int w;
  int z;
  bool operator < (const city_t &rhs) const{
    if (this->y != rhs.y){
      return this->y > rhs.y;
    }
    return this->x > rhs.x;
...
int main() {
  ...
  while (scanf("%d%d%d", &n, &s, &t) != EOF) {
    ...
    s = -s;
    t = -t;
    auto icover = [](int &k, const city_t * const c, const int n) {
      for (int i = 0; i < n; ++i) {
        if (k < 0 && c[i].z + 1 == -k) {
          k = i;
          break;
    ...
    /* core process */
    last = pool;
    build(&root, 0, sx.size() - 1);
    for (int i = 0; i < n; ++i) {
      f[i] = lnf;
      //fprintf(stderr, "city %d: (%d, %d) %d @ %d\n", i, c[i].x, c[i].y, c[i].w, c[i].z + 1);
    }
    //fprintf(stderr, "s = %d, t = %d\n", s, t);
    f[s] = 0LL;
    /*for (int i = 0; i < s; ++i) {
      if (c[i].x >= c[s].x && c[i].y >= c[s].y) {
        f[i] = c[i].w * 1LL;
      }
    }*/
    for (int k = 0; k < 3; ++k) {
      fclear(root);
      for (int i = 0; i < n; ++i) {
        UPDMIN(f[i], getmin(root, c[i].x, inf));
        update(root, c[i].x, f[i] + c[i].w * 1LL);
      }
      fclear(root);
      for (int i = n - 1; ~i ; --i) {
        UPDMIN(f[i], getmin(root, -inf, c[i].x) + c[i].w);
        update(root, c[i].x, f[i]);
      }
    }
    /*fclear(root);
    for (int i = 0; i < n; ++i) {
    ...
    /*for (int i = t + 1; i < n; ++i) {
      if (c[i].x <= c[t].x && c[i].y <= c[t].y) {
        UPDMIN(f[t], f[i] + c[t].w);
      }
    }*/
    printf("%lld\n", f[t]);
  }
