...
int main() {
  ...
  while (scanf("%d%d%d", &h, &w, &n) != EOF) {
    const lld num = w * 1LL * h - n;
    for (int i = 0; i < h; ++i) row[i] = w * 1LL;
    for (int i = 0; i < w; ++i) col[i] = h * 1LL;
    for (int i = 0; i < n; ++i) {
      int x, y;
      scanf("%d%d", &x, &y);
      --row[x - 1];
    ...
    auto calc = [](lld *row, const int h, const lld num) {
      lld foo = 0LL;
      lld cur = 0LL;
      for (int i = 0; i < h; ++i) {
        if (((cur + row[i]) << 1) >= num) {
