...
int main(){
    ...
    int mask=1;
    while(mask+1<n && mask*mask<n){
        mask++;
    }
    
    for(int i=0; i<mask; i++)b[i]=0LL;
    for(int i=0; i<n; i++){
        b[i%mask]^=a[i];
    }
    ...
    if(nodeId==last){
        ...
        for(int i=0; i<nodes; i++){
            int p=(i-1+nodes)%nodes;
            int q=(i-2+nodes)%nodes;
            PutInt(i, bad[p]);
            PutInt(i, bad[q]);
            PutInt(i, bad[i]);
            Send(i);
    ...
    int np=(nodeId-1+nodes)%nodes;
    int nq=(nodeId-2+nodes)%nodes;

    for(int i=bp; i<n; i+=mask){
        PutLL(np, a[i]);
    }
    PutLL(np, -1LL);
    Send(np);

    for(int i=bq; i<n; i+=mask){
        PutLL(nq, a[i]);
    }
    PutLL(nq, -1LL);
    Send(nq);
    ...
    int foo=-1;
    int mp=(nodeId+1)%nodes;
    int mq=(nodeId+2)%nodes;
    Receive(mp);
    for(int p=0; ;){
        e[p]=GetLL(mp);
        if(!~e[p])break;
