...
lld findH(const lld w, const lld r){
    lld lef=1LL, rig=r;
    while(lef<=rig){
        lld mid=(lef+rig)>>1;
        if(Experiment(w, mid)=='T'){
            if(mid==rig || Experiment(w, mid+1LL)=='E')return mid;
            lef=mid+1LL;
        }else{
            rig=mid-1LL;
        }
...
lld findW(const lld h, lld lef, lld rig){
    while(lef<=rig){
        lld mid=(lef+rig)>>1;
        if(Experiment(mid, h)=='T'){
            if(mid==rig || Experiment(mid+1LL, h)=='E')return mid;
            lef=mid+1LL;
        }else{
            rig=mid-1LL;
        }
...
int main(){
    ...
    int nodeId=MyNodeId();
    int last=nodes-1;
    //int tasks=GetNumNanobots();
    
    int mask=1000000;
    lld r=GetRange();
    if(r<mask*100LL)mask=MAX(((int)r)/100, 1);
    set<lld> unq;
    while(SIZE(unq)<mask){
        unq.insert(gcc_random()%r);
    }
    ...
    int m=0;
    lld prev=0LL;
    for(set<lld>::const_iterator it=unq.begin(); it!=unq.end(); it++){
        b[m].from=prev+1LL;
        b[m].to=*it;
        prev=*it;
        m++;
    ...
    lld ans=0LL;
    for(int i=nodeId; i<m; i+=nodes){
        //fprintf(stderr, "calc [%lld, %lld]\n", b[i].from, b[i].to);
        lld rr=b[i].to;
        lld x=b[i].from-1LL, y=r;
        while(true){
            lld h=findH(x+1LL, y);
            if(!h)break;
            lld w=findW(h, x+1LL, rr);
            //fprintf(stderr, "%lld*%lld\b", w, h);
            ans=(ans+(((h%mod)*((w-x)%mod))%mod))%mod;            
            x=w;
            y=h-1LL;
        }
    }
