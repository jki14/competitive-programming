...
int cur[3100], prv[3100];

int find(int *a, const int len, const int low){
    int lef=0, rig=len-1;
    while(lef<=rig){
        int mid=(lef+rig)>>1;
        if(a[mid]>low){
            if(mid==lef || a[mid-1]<=low)return a[mid];
            rig=mid-1;
        }else{
            lef=mid+1;
        }
...
int main(){
    ...
    for(int i=0; i<n; i++){
        a[i]=GetNecklaceElement(from+i);
        cnt[a[i]]++;
    }
    tail[0]=head[0]=p;
    for(int i=0; i<=10000; i++){
        tail[i+1]=head[i+1]=head[i]+cnt[i];
    }
    for(int i=0; i<n; i++){
        *tail[a[i]]++=i;
    }
    ...
    for(int s=0; s<m; s++){
        int low=-1;
        for(int t=s; t<m; t++){
            int pos=find(head[b[t]], head[b[t]+1]-head[b[t]], low);
            if(~pos){
                UPDMAX(cur[t], t-s+1);
                low=pos;
            }else{
    ...
    for(int t=0; t<m; t++){
        for(int s=MAX(t-cur[t], 0); s<=t; s++){
            UPDMAX(cur[t], prv[s]+t-s);
        }
    }
