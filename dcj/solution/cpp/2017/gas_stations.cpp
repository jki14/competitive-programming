...
int main(){
    ...
    for(int i=from; i<to; i++){
        if(i==tasks-1)continue;
        UPDMIN(cheapest, (int)GetGasPrice(i));
    }
    
    PutInt(last, cheapest);
    Send(last);
    ...
    int len=GetTankSize();
    int lef=0, rig=0;
    int head=MAX(from-len, 0);
    int tail=MAX(to-1-len, 0);
    int curr=head-1;
    int suff=inf;
    int prevId=0;
    while(prevId<nodeId && calc_from(tasks, nodes, prevId)<=tail)prevId++;
    for(int i=prevId; i<nodeId; i++){
        UPDMIN(suff, d[i]);
    }
    for(int i=calc_from(tasks, nodes, prevId)-1; i>tail; i--){
        UPDMIN(suff, (int)GetGasPrice(i));
    }
    for(int i=MAX(from, 1); i<to; i++){
        while(curr+1<=tail && curr+1<i){
            curr++;
            int next=GetGasPrice(curr);
            while(lef<rig && que[rig-1].second>=next){
                rig--;
            }
            que[rig++]=MP(curr, next);
        }
        int left=i-len;
        while(lef<rig && que[lef].first<left)lef++;
        int pref=(lef<rig)?que[lef].second:inf;
        if(i-1>tail){
            UPDMIN(suff, (int)GetGasPrice(i-1));
        }
        ans+=MIN(pref, suff)*1LL;
    }
