...
int main(){
    int num_of_files=(int)NumberOfFiles();
    int num_of_nodes=MIN(num_of_files, NumberOfNodes());
    int nodeId=MyNodeId();
    if(nodeId>=num_of_nodes)return 0;
    int max_distance=MaxDistance();
    int from=get_end_points(num_of_files, num_of_nodes, nodeId).first;
    int to=get_end_points(num_of_files, num_of_nodes, nodeId).second-1;
    vector<lld> foo;
    int head=MAX(from-max_distance, 0);
    int tail=MIN(to+max_distance, num_of_files-1);
    foo.resize(tail-head+1);
    for(int i=head; i<=tail; i++){
        foo[i-head]=Identifier(i*1LL);
    }
    sort(foo.begin(), foo.end());
    int local=0;
    for(int i=from; i<=to; i++){
        MOD_ADD(&local, MOD_MUL((int)(foo[i-head]%(MOD*1LL)), i));
        //fprintf(stderr, "%d@%d\n", foo[i-head], i);
    }
    PutInt(master, local);
    Send(master);
