...
static const lld infl=0x3f3f3f3f3f3f3f3fLL;
...
int main(){
    ...
    lld *a=(lld*)malloc(sizeof(lld)*length);
    lld *f=(lld*)malloc(sizeof(lld)*length);
    lld *g=(lld*)malloc(sizeof(lld)*length);
    ...
    for(int i=0; i<length; i++){
        ...
        UPDMAX(peak_pref, f[i]);
    }
    if(nodeId>0){
        Receive(nodeId-1);
        peak_lef=GetLL(nodeId-1);
    }
    if(nodeId<last){
        PutLL(nodeId+1, MAX(peak_pref, peak_lef));
        Send(nodeId+1);
    }
    //fprintf(stderr, "pref calc done. %lld/%lld\n", peak_pref, peak_lef);
    ...
    for(int i=length-1; ~i; i--){
        ...
        UPDMAX(peak_suff, g[i]);
    }
    if(nodeId<last){
        Receive(nodeId+1);
        peak_rig=GetLL(nodeId+1);
    }
    if(nodeId>0){
        PutLL(nodeId-1, MAX(peak_suff, peak_rig));
        Send(nodeId-1);
    }
    //fprintf(stderr, "suff calc done %lld/%lld.\n", peak_suff, peak_rig);
    ...
    lld peak=f[0]+peak_rig;
    for(int i=0; i+1<length; i++){
        UPDMAX(peak, f[i]+g[i+1]);
    }
