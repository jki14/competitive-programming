...
int head(int *list, const int length, const int key){
    int lef=0, rig=length-1;
    while(lef<=rig){
        int mid=(lef+rig)>>1;
        if(list[mid]>=key){
            if(list[mid]==key && (mid==lef || list[mid-1]<key))return mid;
            rig=mid-1;
        }else{
            lef=mid+1;
        }
...
int tail(int *list, const int length, const int key){
    int lef=0, rig=length-1;
    while(lef<=rig){
        int mid=(lef+rig)>>1;
        if(list[mid]<=key){
            if(list[mid]==key && (mid==rig || list[mid+1]>key))return mid;
            lef=mid+1;
        }else{
            rig=mid-1;
        }
...
int calc(int *list, const int length, const int key){
    //int lef=head(list, length, key);
    //int rig=tail(list, length, key);
    //fprintf(stderr, "%d : [%d -> %d]\n", key, lef, rig);
    return tail(list, length, key)-head(list, length, key)+1;
}
...
int main(){
    ...
    int length=calc_length(tasks, nodes, nodeId);
    int to=from+length;
    ...
    int *votes=(int*)malloc(sizeof(int)*length);
    int *vlast=votes;
    ...
    sort(votes, vlast);
    int mid=(length>>1);
    //if(votes[0]==votes[mid]){
        peakId=votes[mid];
    //}
    //fprintf(stderr, "peakId=%d\n", peakId);
    ...
    int *list=(int*)malloc(sizeof(int)*nodes*2);
    int *tail=list;
    set<int> prev;
    for(int i=0; i<nodes; i++){
        Receive(i);
        ...
        prev.insert(idx);
        int cnt=calc(votes, length, idx);
        if(cnt>0){
            *tail++=idx;
            *tail++=cnt;
        }
    }
    for(int *p=list; p!=tail;){
         PutInt(last, *p++);
         PutInt(last, *p++);
         //fprintf(stderr, "send last %d * %d\n", *(p-2), *(p-1));
    }
    PutInt(last, -1);
    Send(last);
    free(list);
        ...
        for(int i=0; i<nodes; i++){
            ...
            while(1){
                ...
                if(it!=votes.end()){
                    ...
                    votes.insert(MP(idx, cnt));
                    it=votes.find(idx);
                }
                if(it->second>(tasks>>1)){
                    ans=it->first;
                }
            }
        }
