...
int main(){
    ...
    int lsz=1;
    while((lsz+1)*(lsz+1)<=nodes)lsz++;
    int rsz=nodes/lsz;
    int last=lsz*rsz-1;
    if(nodeId>last)return 0;
    ...
    if(~sum){
        int tasks=1<<lef;
        int from=calc_from(tasks, lsz, lid);
        int to=calc_length(tasks, lsz, lid)+from;
    ...
    if(~sum){
        int tasks=1<<rig;
        int from=calc_from(tasks, rsz, rid);
        int to=calc_length(tasks, rsz, rid)+from;
    ...
    if(~sum){
        ...
        while(xln>=0 && yln>=0){
            ...
            //fprintf(stderr, "x=%lld, y=%lld, sum=%lld\n", x[xln], y[yln], sum);
            res=1;
            break;
        }
    }
