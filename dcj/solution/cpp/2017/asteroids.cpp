...
inline lld get_numeric(const int h, const int w){
    const int r=w%mask;
    if(b[h][r]==w)return a[h][r];
    char c=GetPosition(from+h-1, w);
    if(c!='#'){
        a[h][r]=(c-'0')*1LL;
    }else{
        a[h][r]=-infl;
...
inline lld process(const int h, const int w){
    const int r=w%mask;
    assert(h || c[h][r]==w);
    if(c[h][r]==w)return f[h][r];
    //fprintf(stderr, "calc %d, %d\n", h, w);
    f[h][r]=-infl;
    if(get_numeric(h, w)>=0LL){
        //^
        lld down=process(h-1, w)+get_numeric(h, w);
        if(down>=0LL)UPDMAX(f[h][r], down);
        //>
        if(w>0){
            lld left=process(h-1, w-1)+get_numeric(h, w-1)+get_numeric(h, w);
            if(left>=0LL)UPDMAX(f[h][r], left);
        }
        //<
        if(w+1<width){
            lld right=process(h-1, w+1)+get_numeric(h, w+1)+get_numeric(h, w);
            if(right>=0LL)UPDMAX(f[h][r], right);
        }
    }
    c[h][r]=w;
    return f[h][r];
...
inline void setf(const int h, const int w, const lld val){
    const int r=w%mask;
    f[h][r]=val;
    c[h][r]=w;
}
...
int main(){
    ...
    int nodeId=MyNodeId();
    int tasks=(int)GetHeight();
    UPDMIN(nodes, tasks);
    if(nodeId>=nodes)return 0;
    from=calc_from(tasks, nodes, nodeId);
    int to=from+calc_length(tasks, nodes, nodeId);

    width=(int)GetWidth();
    int bars=(width+B-1)/B;

    bar[0]=0;
    for(int i=1; i<=bars; i++)bar[i]=bar[i-1]+B;
    UPDMIN(bar[bars], width);
    
    /*if(nodeId==0){
        fprintf(stderr, "width = %d\n", width);
        fprintf(stderr, "bars = %d\n", bars);
    ...
    for(int k=1; k<=bars; k++){
        if(nodeId!=0){
            ...
            if(k==1){
                Receive(prevId);
                for(int i=bar[0]; i<bar[1]; i++){
                    //f[0][i]=GetLL(prevId);
                    setf(0, i, GetLL(prevId));
                }
            }
            if(k!=bars){
                Receive(prevId);
                for(int i=bar[k]; i<bar[k+1]; i++){
                    //f[0][i]=GetLL(prevId);
                    setf(0, i, GetLL(prevId));
                }
            }
        ...
        for(int i=bar[k-1]; i<bar[k]; i++){
            UPDMAX(ans, process(to-from, i));
        }
        if(nodeId!=nodes-1){
            int nextId=nodeId+1;
            for(int i=bar[k-1]; i<bar[k]; i++){
                PutLL(nextId, process(to-from, i));
            }
            Send(nextId);
        }
    }
