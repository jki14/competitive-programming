...
int main(){
    int robots=NumberOfNodes();
    int index=MyNodeId();
    int tasks=(int)GetN();
    int to=tasks/robots+((index<(tasks%robots))?1:0);
    int from=tasks/robots*index+((index<(tasks%robots))?index:(tasks%robots));
    to+=from;
    //GENERAL
    vector<int> foo(robots, 0);
    vector<int> goo(robots, 0);
    for(int i=from; i<to; i++){
        MOD_ADD(&foo[i%robots], (int)GetA(i));
        MOD_ADD(&goo[i%robots], (int)GetB(i));
    }
    if(index!=master){
        ...
        for(int k=1; k<robots; k++){
            int offset=Receive(-1);
            for(int i=0; i<robots; i++){
                MOD_ADD(&foo[i], GetInt(offset));
                MOD_ADD(&goo[i], GetInt(offset));
            }
        }
        int ans=0;
        for(int i=0; i<robots; i++){
            for(int j=0; j<robots; j++){
                if((i+j)%robots==0)continue;
                MOD_ADD(&ans, MOD_MUL(foo[i], goo[j]));
            }
        }
        MOD_STD(&ans);
        printf("%d\n", ans);
