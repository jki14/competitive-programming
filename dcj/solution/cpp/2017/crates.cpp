...
int main(){
    ...
    for(int i=from; i<to; i++){
        a[i-from]=GetStackHeight(i+1)*1LL;
        sum+=a[i-from];
    }
    ...
    int cap=tot%(tasks*1LL);
    lld avg=tot/(tasks*1LL);
    lld tar=0LL;
    for(int i=0; i<n; i++){
        int p=from+i;
        if(p<cap){
