...
inline int get_operation(const int k, const int p){
    if(c[k][p]>-110000)return c[k][p];
    c[k][p]=GetOperation(k, p);
    return c[k][p];
}
...
int main(){
    ...
    memset(mutexa, 0, sizeof(mutexa));
    memset(mutexb, 0, sizeof(mutexb));

    //unordered_set<int> mutexa, mutexb, dup;
    int vera=-1, verb=inf;
    int duplen=0;
    int mask=0;
    ...
    for(int k=from; k<to; k++){
        ...
        while(vera+1<posa){
            int opt=get_operation(0, ++vera);
            if(opt>0){
                mutexa[opt]=1;
                if(mutexb[opt]==mask)duplen++;
            }else{
                mutexa[-opt]=0;
                if(mutexb[-opt]==mask)duplen--;
            }
        ...
        while(verb+1<posb){
            int opt=get_operation(1, ++verb);
            if(opt>0){
                mutexb[opt]=mask;
                if(mutexa[opt])duplen++;
            }else{
                mutexb[-opt]=0;
                if(mutexa[-opt])duplen--;
            }
        ...
        if(duplen)continue;
        int reqa=get_operation(0, posa);
        if(reqa<0)continue;
        if(mutexb[reqa]!=mask)continue;
        int reqb=get_operation(1, posb);
        if(reqb<0)continue;
        if(mutexa[reqb]==0)continue;
        UPDMIN(least, posa+posb+2);
    }
