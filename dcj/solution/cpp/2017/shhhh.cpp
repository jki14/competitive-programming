...
inline int expected(const int m, const int n){
    return (int)round(n*log(m*1.0)/log(2.0)/(m*1.0));
}

int src[21000], dst[21000], dis[21000];

unordered_map<int, int> keypoints;
...
int main(){
    ...
    int m=0;
    keypoints.clear();
    keypoints.insert(MP(0, m));
    src[m++]=0;
    keypoints.insert(MP(1, m));
    src[m++]=1;
    while(m*10<tasks && m<nodes*128){
        int next=gcc_random()%tasks;
        if(!keypoints.count(next)){
            keypoints.insert(MP(next, m));
            src[m++]=next;
        }
    }
    ...
    for(int i=nodeId; i<m; i+=nodes){
        ...
        int d=0;
        while(true){
            p=GetLeftNeighbour(p);
            d++;
            if(keypoints.count(p))break;
        }
        PutInt(last, i);
        PutInt(last, p);
        PutInt(last, d);
    ...
    if(nodeId==last){
        for(int i=0; i<nodes; i++){
            int subId=Receive(-1);
            while(true){
                int offset=GetInt(subId);
                if(!~offset)break;
                dst[offset]=GetInt(subId);
                dis[offset]=GetInt(subId);
            }
        }
        int lef=0;
        for(int p=0; p!=1; ){
            int offset=keypoints[p];
            p=dst[offset];
            lef+=dis[offset];
        }
        int rig=tasks-lef;
