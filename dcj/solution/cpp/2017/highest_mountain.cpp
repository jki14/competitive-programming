...
//HEAD_OF_DCJ'S_HEADER
    ...
    static int gcc_random_seed=134775813;
...
//TAIL_OF_DCJ'S_HEADER

#include "highest_mountain.h"

#define CSIZE 1024

point_t<lld> convex[4100000];
int head, tail;

point_t<lld> lefths[1100];

int que[CSIZE], til;

int cut(const point_t<lld> &p, const point_t<lld> &q){
    int lef=head, rig=tail;
    while(rig-lef>=3){
        int c1=(lef*2+rig)/3;
        int c2=(lef+rig*2)/3;
        lld d1=-p.cross(q, convex[c1]);
        lld d2=-p.cross(q, convex[c2]);
    ...
    for(int i=lef; i<rig; i++){
        if(-p.cross(q, convex[i])<0)return 1;
    }
    return 0;
}

void adjust(const int prevId, const int nextId){
    int lef=head, rig=tail;
    while(rig-lef>=2){
        til=0;
        if(rig-lef<=CSIZE){
            ...
            for(int i=0; i<CSIZE; i++){
                que[til++]=lef+((rig-lef-1LL)*1LL*i)/(CSIZE-1LL);
            }
        }
        PutChar(nextId, 'Q');
        for(int i=0; i<til; i++){
            PutInt(nextId, (int)convex[que[i]].x);
        }
        PutInt(nextId, -1);
        Send(nextId);
        Receive(nextId);
        int offset=GetInt(nextId);
        if(offset!=0){
            lef=que[offset-1]+1;
        }
        if(offset!=til-1){
            rig=tail=que[offset+1];
        }
    }
    if(lef==head && prevId!=-1){
        PutChar(prevId, 'B');
        PutInt(prevId, nextId);
        Send(prevId);
    }else{
        PutChar(nextId, 'F');
        PutInt(nextId, (int)convex[lef].x);
        Send(nextId);
    }
}
...
int main(){
    ...
    int nodeId=MyNodeId();
    int tasks=NumberOfPeaks();
    UPDMIN(nodes, MAX((tasks-1)>>1, 1));
    int last=nodes-1;

    if(nodeId>=nodes)return 0;

    int from=calc_from(tasks, nodes, nodeId);
    int to=calc_length(tasks, nodes, nodeId)+from;

    head=tail=1;
    for(int x=from; x<to; x++){
        //fprintf(stderr, "--------\n");
        point_t<lld> next(x*1LL, GetHeight(x*1LL)*1LL);
        while(tail>1 && convex[tail-2].cross(convex[tail-1], next)>0){
            //fprintf(stderr, "cross = %lld\n", convex[tail-2].cross(convex[tail-1], next));
            tail--;
        }
        convex[tail++]=next;
    ...
    if(nodeId!=0){
        while(true){
            int sourceId=Receive(-1);
            char c=GetChar(sourceId);
            if(c=='Q'){
                int lef=0, rig=0;
                while(true){
                    int x=GetInt(sourceId);
                    if(!~x)break;
                    lefths[rig].x=x*1LL;
                    lefths[rig].y=GetHeight(x)*1LL;
                    rig++;
                }
                while(rig-lef>=2){
                    int mid=(lef+rig-2)>>1;
                    if(cut(lefths[mid], lefths[mid+1])){
                        rig=mid+1;
                    }else{
                        lef=mid+1;
                    }
                ...
                prevId=sourceId;
                int x=GetInt(sourceId);
                point_t<lld> warden(x*1LL, GetHeight(x*1LL)*1LL);
                int lef=head, rig=tail-2;
                if(-warden.cross(convex[lef], convex[lef+1])>=0){
                    convex[--head]=warden;
                }else if(-warden.cross(convex[rig], convex[rig+1])<0){
                    convex[rig]=warden;
                    head=rig;
                }else{
                    while(rig-lef>=2){
                        int mid=(lef+rig)>>1;
                        if(-warden.cross(convex[mid], convex[mid+1])<0){
                            lef=mid;
                        }else{
                            rig=mid;
                        }
                    }
                    convex[head=lef]=warden;
                }
                break;
            }
        }
    ...
    if(nodeId!=last){
        adjust(prevId, nextId);
        while(true){
            int sourceId=Receive(-1);
            char c=GetChar(sourceId);
            if(c=='E')break;
            assert(c=='B');
            nextId=GetInt(sourceId);
            adjust(prevId, nextId);
        }
    }else{
