...
int main(){
    ...
    for(int i=from; i<to; i++){
        a[i-from]=GetDigitX(tasks-i-1);
        b[i-from]=GetDigitY(tasks-i-1);
        c[i-from]=GetDigitZ(tasks-i-1);
    }

    int right=-1;
    for(int i=to-1; i>=from; i--){
        if(a[i-from]+b[i-from]!=c[i-from]){
            right=i;
            break;
        }
    }
    ...
    if(nodeId==last){
        ...
        if(!~right){
        ...
        }else if(GetDigitX(tasks-right-1)+GetDigitY(tasks-right-1)<GetDigitZ(tasks-right-1)){
            //fprintf(stderr, "right=%d\n", right);
            printf("IMPOSSIBLE\n");
            for(int i=0; i<nodes; i++){
                PutInt(i, -1);
            ...
            int susp=GetDigitX(tasks-right-1)+GetDigitY(tasks-right-1)-GetDigitZ(tasks-right-1);
            for(int i=0; i<nodes; i++){
                PutInt(i, susp);
                Send(i);
            }
    ...
    Receive(last);
    int base=GetInt(last);
    if(!~base)return 0;

    int ok=1, prev=0, next=0;
    for(int p=to-1; p>=from; p--){
        int i=p-from;
        if(a[i]>=base || b[i]>=base || c[i]>=base){
            ok=0;
            break;
        }
        if(a[i]+b[i]+next==c[i]){
            next=0;
        }else if(a[i]+b[i]+next==c[i]+base){
            next=1;
        }else{
            if(p==to-1 && a[i]+b[i]+1==c[i]){
                prev=1;
                next=0;
            }else if(p==to-1 && a[i]+b[i]+1==c[i]+base){
                prev=1;
                next=1;
            }else{
                ok=0;
    ...
    if(nodeId==last && prev)ok=0;
    if(nodeId==0 && next)ok=0;

    if(nodeId!=last){
        int prevId=nodeId+1;
        Receive(prevId);
        if(prev!=GetInt(prevId)){
            ok=0;
        }
    }
