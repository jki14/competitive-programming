...
int n,ans,mid,step,c[N][N];
vector<int> a;

int call(const int &d,int &ms){
	int ret=-inf;ms=inf;
	for(int s=0;s<_size(a);s++){
		int now=0;for(int i=0,p=s;i<4;i++){
			now+=a[p];
			p=(p+d)%_size(a);
		}
		if(now>ret){
			ret=now;ms=min(s,_size(a)-s);
		}else if(now==ret){
			_updmin(ms,min(s,_size(a)-s));
		}
	}
	return ret;
}

void go(int &x,int &y,const int &s,const int &e){
	if(x==s){
		if(y!=e)
			y++;
		else x++;
