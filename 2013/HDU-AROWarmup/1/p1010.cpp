...
void init(const int &n){
	int i,e=(int)(sqrt((double)n)+1.0);
	memset(is,0x3f,sizeof(is));
	prm.push_back(2);is[0]=is[1]=0;
	for(i=4;i<=n;i+=2)is[i]=0;
	for(i=3;i<e;i+=2)if(is[i]){
		prm.push_back(i);int d=i<<1;
		for(int j=i*i;j<=n;j+=d)is[j]=0;
	}
	for(;i<=n;i+=2)if(is[i])prm.push_back(i);
}

int main(){
	init(3000000);int m=_size(prm);
	int T;scanf("%d",&T);
	while(T--){
		scanf("%d",&_x);x=abs(_x);int flag=0;
		for(int i=0;i<m;i++){
			if(prm[i]+x>3000000)break;
			if(is[prm[i]+x]){
				int lef=prm[i],rig=prm[i]+x;
				if(x==_x)swap(lef,rig);
				printf("%d %d\n",lef,rig);
				flag=1;break;
			}
		}
