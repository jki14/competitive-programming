...
const double inf = 1111111.1;
const double eps = 1e-7;

int n;
double x[N],y[N],ans;

double ts(const double &psx,const double &psy,const double &pex,const double &pey,const double &pox,const double &poy){
	return fabs(((psx-pox)*(pey-poy)-(pex-pox)*(psy-poy))/2.0);
}
...
int main(){
	...
	while(T--){
		...
		for(int i=0;i<n;i++)
			for(int j=i+1;j<n;j++)
				for(int k=j+1;k<n;k++){
					double s=ts(x[i],y[i],x[j],y[j],x[k],y[k]);
					if(s<eps)continue;
					_updmin(ans,s);
				}
		if(abs(ans-inf)<eps)
