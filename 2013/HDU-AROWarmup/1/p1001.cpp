...
int main(){
	for(int s=3,x=0;s<=n;s++){
		for(int i=0;i<s;i++){
			for(int y=0;y<s;y++)a[x][y]=' ';
			a[x][s]=0;
			x++;X=x;
		}
	}
	for(int s=3,x=0,y=0,i=0;s<=n;s++){
		for(int dx=0,dy=0;dx<s;dx++)a[_x][_y]=chr;
		for(int dx=s-2,dy=1;dx!=0;dx--,dy++)a[_x][_y]=chr;
		for(int dx=0,dy=s-1;dx<s;dx++)a[_x][_y]=chr;
		x+=s;
	}
	for(int i=0;i<X;i++)puts(a[i]);
	return 0;
}
