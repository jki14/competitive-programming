...
struct state{
	int x,y,z;
	state(){ x=y=z=0; }
	state(const int &_x,const int &_y,const int &_z){
		x=_x;
		y=_y;
		z=_z;
	}
...
int n,v1,v2,k,now,ans;
int a[N],b[N],c[N];
int f[B][N][N][K],r[N][N][K];

queue<state> que[B];

void upd(const int &i,const int &x,const int &y,const int &z,const int &val){
	//printf("TRY UPD f[%d][%d][%d][%d] ~ %d\n",i,x,y,z,val);
	...
	if(r[x][y][z]<i){ 
		r[x][y][z]=i;
		f[nxt][x][y][z]=0;
		que[nxt].push(state(x,y,z));
       	}
	_updmax(f[nxt][x][y][z],val);
	_updmax(ans,val);
}
...
int main(){
	while(scanf("%d%d%d%d",&n,&v1,&v2,&k)!=EOF){
		...
		now=ans=0;upd(0,0,0,0,0);
		for(int i=0;i<n;i++){now=nxt;
			/*for(int x=0;x<=v1;x++){
				for(int y=0;y<=v2;y++){
					for(int z=0;z<=k;z++){
						if(r[x][y][z]<i)continue;*/
			for(;!que[now].empty();que[now].pop()){			
						//printf("f[%d][%d][%d][%d]=%d\n",i,x,y,z,f[now][x][y][z]);
						sta=que[now].front();
						int x=sta.x,y=sta.y,z=sta.z;
						upd(i+1,x,y,z,f[now][x][y][z]);
						upd(i+1,x+a[i],y,z,f[now][x][y][z]+c[i]);
						upd(i+1,x,y+b[i],z,f[now][x][y][z]+c[i]);
						upd(i+1,x,y,z+  1 ,f[now][x][y][z]+c[i]);
