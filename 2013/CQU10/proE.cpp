...
const int dx[v]={0,1,0,-1};
const int dy[v]={1,0,-1,0};
...
int main(){
	...
	while(T--){
		...
		while(!que.empty())que.pop();que.push(_mkpr(sx,sy));
		for(;!que.empty();que.pop()){
			int x=que.front().first,y=que.front().second,d=f[x][y];
			for(int i=0;i<v;i++){
				if(c[xx][yy]=='*')continue;
				if(f[xx][yy]>d+1){
					f[xx][yy]=d+1;
					que.push(_mkpr(xx,yy));
				}
			}
		}
		if(f[ex][ey]<=k)
			printf("%d\n",f[ex][ey]);
		else printf("Panda is dead\n");
	}	
	return 0;
}
