...
int main(){
	...
	while(T--){
		...
		memset(f,0,sizeof(f));ans=0;while(!que.empty())que.pop();
		for(int u=1;u<=n;u++){ if(!d[u])que.push(u);f[u]=a[u]; }
		for(;!que.empty();que.pop()){
			int u=que.front();_updmax(ans,f[u]);
			for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
				d[v]--;_updmax(f[v],f[u]+a[v]);
				if(!d[v])que.push(v);
			}
		}
		printf("%d\n",ans);
