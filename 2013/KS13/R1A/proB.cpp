...
int cmod(const int &x){ return (x%11+11)%11; }
void amod(int &x,const int &y){ x=(x+y)%modulo; }

int main(){
	int T;scanf("%d",&T);
	for(int _T=1;_T<=T;_T++){printf("Case %d: ",_T);
		scanf("%d%d",&n,&m);memset(f,0,sizeof(f));ans=0;
		...
		for(int i=0;i<n;i++){now=pre;
			for(int j=0;j<m;j++){
				for(int k=0;k<K;k++){
					for(int x=0;x<=10;x++)for(int y=0;y<=10;y++)f[k][now][j][x][y]=0;
					if(xpp)
						f[k][now][j][c[i][j]][0]=1;
					//else f[k][now][j][0][c[i][j]]=1;
					for(int x=0;x<=10;x++)for(int y=0;y<=10;y++){
						if(i){
							if(xpp)
								amod(f[k][now][j][x][y],f[k][pre][j][cmod(x-c[i][j])][y]);
							else amod(f[k][now][j][x][y],f[k][pre][j][x][cmod(y-c[i][j])]);
						}
						if(j){
							if(xpp)
								amod(f[k][now][j][x][y],f[k][now][j-1][cmod(x-c[i][j])][y]);
							else amod(f[k][now][j][x][y],f[k][now][j-1][x][cmod(y-c[i][j])]);
						}
						//if(f[k][now][j][x][y])
							//printf("f[%d][%d][%d][%d][%d]=%d\n",k,i,j,x,y,f[k][now][j][x][y]);
						if(x==y)amod(ans,f[k][now][j][x][y]);
					}
				}
			}
		}
