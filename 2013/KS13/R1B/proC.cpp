...
int n,m,x[N];
double y[N],f[N][N],ans;

double dis(const double &v,const double &t){ return v*t-0.5*g*t*t; }

/*double hig(const double &v,const double &t){
	if(v<eps)return 0.0;
	double _t=v/g;
	if(_t>t-eps)
		return dis(v,t);
	else return dis(v,_t);
}

double fin(const double &v,const double &x,const double &y){
	return hig(v,y/x);
}*/
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){printf("Case %d: ",_T);
		...
		for(int i=0;i<n;i++)scanf("%d%lf",&x[i],&y[i]);
		for(int i=0;i<=n;i++)for(int j=0;j<=m;j++)f[i][j]=(j==0)?0.0:-inf;
		for(int i=0;i<n;i++){
			for(int j=0;j<m;j++){
				if(fabs(f[i][j]+inf)<eps)continue;
				for(int t=0;j+x[i]*t<m;t++){
					//_updmax(ans,f[i][j]+hig(y[i],(double)t));
					_updmax(f[i+1][j+x[i]*t],f[i][j]+dis(y[i],(double)t));
				}
				_updmax(ans,f[i][j]+dis(y[i],((double)(m-j))/((double)x[i])));
			}
		}
		printf("%.3f\n",ans);
	}	
