...
double ts(const TPnt &x,const TPnt &y,const TPnt &z){
	return fabs(x.x*y.y+y.x*z.y+z.x*x.y-z.x*y.y-y.x*x.y-x.x*z.y)/2.0;
}

int isPOS(const TPnt &p,const TPnt &x,const TPnt &y){
	if(ts(p,x,y)>eps)return 0;
	if(p.x+eps<x.x && p.x+eps<y.x)return 0;
	if(p.x-eps>x.x && p.x-eps>y.x)return 0;
	if(p.y+eps<x.y && p.y+eps<y.y)return 0;
	if(p.y-eps>x.y && p.y-eps>y.y)return 0;
	return 1;
}
...
int main(){
	int T;scanf("%d",&T);
	while(T--){
		scanf("%d%d",&n,&m);memset(vst,0,sizeof(vst));ans=0;
		scanf("%lf%lf%lf%lf",&top.x,&top.y,&a[0].x,&a[0].y);
		for(int i=0;i<n;i++)scanf("%lf%lf",&b[i].x,&b[i].y);
		for(int i=1;i<=m;i++){scanf("%lf%lf",&a[i].x,&a[i].y);
			double sum=ts(a[i-1],a[i],top);
			if(fabs(sum)>eps){
				for(int j=0;j<n;j++){
					if(vst[j])continue;
					if(fabs(ts(b[j],a[i-1],a[i])+ts(b[j],a[i-1],top)+ts(b[j],a[i],top)-sum)<eps){ vst[j]=1;ans++; }
				}
			}else{
				for(int j=0;j<n;j++){
					if(vst[j])continue;
					if(isPOS(b[j],a[i-1],a[i]) || isPOS(b[j],a[i-1],top) || isPOS(b[j],a[i],top)){
						vst[j]=1;ans++;
					}
				}
			}
