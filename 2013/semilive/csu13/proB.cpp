...
const string b2c[3]= { "..","##","#." };
const string n2b[10]= { "11111","21111","22111","22211","22221","22222","12222","11222","11122","11112" };
const string a2b[3]= { "21","2112","11" };
...
int main(){
	int T;scanf("%d",&T);
	while(T--){
		s.clear();for(int i=0;i<k;i++){ scanf("%s",a[i]);s.append(a[i]); }
		for(int i=0;i<n;i++){ for(int j=0;j<m;j++)c[i][j]='.';c[i][m]=0; }
		for(int i=0;i<_size(s);i++){
			if(s[i]>='0' && s[i]<='9'){
				int k=s[i]-'0';
				for(int j=0;j<_size(n2b[k]);j++){
					c[j][i*3+1]='#';
					c[j][i*3+2]=(n2b[k][j]=='1')?'#':'.';
				}
			}else{
