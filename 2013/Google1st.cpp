...
class CharTypesCounter{
	public: int count;
	private: struct TNode{
		int index;
		struct TNode *prev,*next;
	} *head,*tail,*pos[C];
	public: CharTypesCounter(){
		count=0;head=tail=NULL;
		for(int i=0;i<C;i++)pos[i]=NULL;
	}
	public: void push(const char &_c,const int &i){
		int c=(int)_c;
		if(pos[c]!=NULL){
			if(pos[c]==head && pos[c]==tail){
				head=tail=NULL;
			}else if(pos[c]==head){
				head=pos[c]->next;
			}else if(pos[c]==tail){
				...
				pos[c]->prev->next=pos[c]->next;
			}
			delete pos[c];pos[c]=NULL;
			count--;
		}
		pos[c]=new TNode;
		pos[c]->index=i;
...
} CTC;//Char Types Counter
	
int main(){
	string s;int m,ans;
	cin>>s>>m;ans=0;
	for(int i=0,head=0;i<(int)s.length();i++){
		CTC.push(s[i],i);
		while(CTC.count>m)
			head=CTC.pop();
		_updmax(ans,i-head+1);
	}
	cout<<ans<<endl;
	return 0;
}
