...
//HEAD_OF_TEMPLATE_BY_JKI14
	...
	template<class T> inline void _powMTRX(int n,T y,T C[_MTRXSIZE][_MTRXSIZE],T X[_MTRXSIZE][_MTRXSIZE]){
		T R[_MTRXSIZE][_MTRXSIZE];for(int i=0;i<n;i++)for(int j=0;j<n;j++)R[i][j]=X[i][j];_stdMTRX(n,C);
		if(y>0)for(T i=1;;i<<=1){
			if(y&i)_mulMTRX(n,C,C,R);
			_mulMTRX(n,R,R,R);
			if(i>(y>>1))break;
		}
	}
...
lld m[_MTRXSIZE][_MTRXSIZE],a[_MTRXSIZE][_MTRXSIZE];
int n;

int main(){
	int T;scanf("%d",&T);
	for(int i=1;i<=T;i++){
		scanf("%lld%lld%d",&a[0][1],&a[0][0],&n);
		a[1][0]=a[0][1];a[1][1]=0;
		if(n<=2){ printf("Case #%d: %lld\n",n,(n==1)?a[0][1]:a[0][0]);continue; }
		m[0][0]=1;m[0][1]=1;m[1][0]=1;m[1][1]=0;
		_powMTRX(2,(lld)n-2,m,m);
		_mulMTRX(2,a,a,m);
		printf("Case #%d: %lld\n",n,a[0][0]);
	}
	return 0;
