...
multimap<int ,int , greater<int> >::iterator jt;

int main(){
	while(scanf("%d",&n)!=EOF){
		s.clear();ms.clear();ans=0;
		for(int i=0;i<n;i++){
			int x;scanf("%d",&x);
			it=s.find(x);
			if(it!=s.end()){
				(*it).second++;
			}else s.insert(_mkpr(x,1));
		}
		for(it=s.begin();it!=s.end();it++){
			ms.insert(_mkpr((*it).second,(*it).first));
		}
		while(_size(ms)>=3){
			jt=ms.begin();
			for(int i=0;i<3;i++){
				...
				_fr[i]=(*jt).first-1;
				_sc[i]=(*jt).second;
				ms.erase(jt);jt=ms.begin();
			}
			for(int i=0;i<3;i++){
				if(!_fr[i])continue;
				ms.insert(_mkpr(_fr[i],_sc[i]));
			}
			ans++;
		}
		printf("%d\n",ans);
