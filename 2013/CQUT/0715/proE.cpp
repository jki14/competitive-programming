...
void amod(lld &x,const lld &y){ x=(x+y)%p; }

int main(){
	while(scanf("%d%I64d%I64d",&n,&m,&p)!=EOF){
		for(int i=1;i<=n;i++)scanf("%d",&a[i]);a[0]=0;
		memset(c,0,sizeof(c));c[0][0]=x[0]=y[0]=1LL;
		for(int i=1;i<=_m;i++){
			x[i]=x[i-1]*((lld)i)%p;
			y[i]=y[i-1]*(m-((lld)i)+1LL)%p;
			for(int j=1;j<=i;j++){
				c[i][j]=(c[i-1][j-1]+c[i-1][j]*(j-1))%p;
				//printf("c[%d][%d]=%lld\n",i,j,c[i][j]);
			}
		}
		memset(f,0,sizeof(f));f[0][0]=sum[now]=1LL;
		for(int i=1;i<=n;i++){
			now=pre;sum[now]=0LL;
			//memset(f[now],0,sizeof(f[now]));
			for(int j=1;j<=m && j<=a[i];j++){
				f[now][j]=(sum[pre]*y[j]-((j>a[i-1])?0LL:(f[pre][j]*x[j])))%p*c[a[i]][j]%p;
				//printf("f[%d][%d]=%lld\n",i,j,f[now][j]);
