...
int main(){
	while(scanf("%d",&n)!=EOF){ans=0;a[n]=-1.0;
		for(int i=0;i<n;i++)scanf("%lf",&a[i]);
		for(int i=0;i<n;i++){
			f[i]=1;for(int j=0;j<i;j++){
				if(a[j]+eps<a[i])_updmax(f[i],f[j]+1);
				//if(a[j]+eps<a[i])_updmax(g[i],g[j]+1);
			}
		}
		for(int i=1;i<n;i++)_updmax(f[i],f[i-1]);
		for(int i=n-1;i>=0;i--){
			g[i]=1;for(int j=n-1;j>i;j--){
				if(a[j]+eps<a[i])_updmax(g[i],g[j]+1);
			}
		}
		for(int i=n-2;i>=0;i--)_updmax(g[i],g[i+1]);
		for(int i=0;i<n-1;i++){
			//_updmax(ans,f[i]+g[i]-1);
