...
int find(){
	...
	for(int k=q;k<=m;k++){
		//cout<<"mod:"<<mod<<endl;
		for(int s=0;s+p-1<n;s++){int flag=1;
			for(int i=s;i<s+p;i++){
				//if((a[i]&b[i-s])!=b[i-s]){ flag=0;break; }
				//cout<<"a[]:"<<(a[i]&mod)<<endl;
				//cout<<"b[]:"<<b[i-s]<<endl;
				if((a[i]&mod)!=b[i-s]){ flag=0;break; }
			}
			if(flag)return 1;
		}
...
int main(){int T=0;
	while(scanf("%d%d%d%d%d",&n,&m,&t,&p,&q)!=EOF && !(n==0 && m==0 && t==0 && p==0 && q==0)){int ans=0;T++;
		mod.reset();for(int i=0;i<q;i++)mod.set(i);
		for(int i=0;i<n;i++){
			scanf("%s",c);a[i].reset();
			for(int j=0;j<m;j++){
				a[i]<<=1;a[i]|=(c[j]=='*')?1:0;
