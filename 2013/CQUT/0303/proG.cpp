...
int core(char *a){ans++;que.push_back(snull);
	int nex=0;while(a[nex] && a[nex]!='.')nex++;
	if(a[nex]!='.' || nex>8 || nex==0)return 0;
	for(int i=0;i<=nex;i++)que[ans-1].push_back(a[i]);
	int buf=nex+1;while(a[buf] && a[buf]!='.')buf++;
	if(buf-1==nex)return 0;
	if(a[buf]=='.'){
		if(buf-nex-1>3){
			for(int i=nex+1;i<=nex+3;i++)que[ans-1].push_back(a[i]);
			if(!core(a+nex+4))return 0;
		}else{
			que[ans-1].push_back(a[nex+1]);
			if(!core(a+nex+2))return 0;
		}
	}else{
		if(buf-nex-1>3)return 0;
		for(int i=nex+1;i<buf;i++)que[ans-1].push_back(a[i]);
	}
	return 1;
}
	
int main(){snull.clear();
	while(scanf("%s",a)!=EOF){ans=0;que.clear();
		if(core(a)){
			printf("YES\n");
			for(int i=0;i<ans;i++)
				printf("%s\n",que[i].c_str());
