...
char a[L],pat[K][PL];
int ret[K],f[K][PL],fir[K],sec[K];

/*void strrev(char *s){
	int len=strlen(s),mid=len>>1;
	for(int i=0;i<mid;i++)swap(s[i],s[len-i-1]);
}*/

int main(){
	while(scanf("%s",a)!=EOF){
		scanf("%s%s",pat[0],pat[1]);ret[0]=ret[1]=0;
		memset(f,0xff,sizeof(f));
		for(int id=0;id<2;id++){
			for(int k,i=1;pat[id][i];i++){
				for(k=f[id][i-1];k>=0 && pat[id][i]!=pat[id][k+1];)k=f[id][k];
				if(pat[id][i]==pat[id][k+1])f[id][i]=k+1;
			}
		}
		for(int k=0;k<2;k++){
			fir[0]=fir[1]=inf;sec[0]=sec[1]=-inf;
			for(int id=0;id<2;id++){
				for(int i=0,j=0;a[i] && pat[id][j];){
					if(a[i]==pat[id][j]){
						i++;j++;
						if(!pat[id][j]){
							_updmin(fir[id],i);
						...
						if(!j)++i;else j=f[id][j-1]+1;
					}
				}
			}
			if(fir[0]<=sec[1]-(int)strlen(pat[1]))ret[k]=1;
			//if( k && fir[1]<=sec[0]-(int)strlen(pat[1]))ret[1]=1;

			//printf("%s fir[0]=%d sec[1]=%d\n",a,fir[0],sec[1]);
