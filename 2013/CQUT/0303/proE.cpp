...
int n,m,ans;
int nbs[N],dst[M],nxt[M],num;
int f[N],que[N];

void addEdge(const int &u,const int &v){
	dst[++num]=v;nxt[num]=nbs[u];nbs[u]=num;
	dst[++num]=u;nxt[num]=nbs[v];nbs[v]=num;
}

int kick(){int len=0;
	for(int u=0;u<n;u++){if(f[u])continue;
		int flag=0;for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
			if(f[v])continue;
			flag++;
		}
		if(flag==1)que[len++]=u;
	}
	for(int i=0;i<len;i++)f[que[i]]=1;

	//printf("len=%d, que[0]=%d\n",len,que[0]);
