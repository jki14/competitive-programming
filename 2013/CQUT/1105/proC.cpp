...
void init(){
	memset(c,0,sizeof(c));c[0][0]=1LL;
	for(int x=1;x<L;x++)
		for(int y=0;y<N;y++){
			if(!y){ c[x][y]=1LL;continue; }
			c[x][y]=(c[x-1][y-1]+c[x-1][y])%mod;
			//printf("c[%d][%d]=%lld\n",x,y,c[x][y]);
		}
}

lld p(const int &x, const int &y){ return c[x+y-1][y]; }
...
int main(){
	...
	while(cin>>m){
		...
		for(int i=n=0;i<S;i++)if(b[i])a[n++]=b[i];
		//for(int i=0;i<n;i++)printf("a[%d]=%d\n",i,a[i]);
		for(int i=1;i<=m;i++){f[i]=1LL;
			for(int k=0;k<n;k++)f[i]=f[i]*p(i,a[k])%mod;
			//printf("f[%d]=%lld  ---> ",i,f[i]);
			for(int j=i-1;j;j--)f[i]=(f[i]-f[j]*c[i][i-j])%mod;
			//printf("%lld\n",f[i]);
		}
		cout<<_cmod(f[m],mod)<<endl;
	}
	return 0;
}
