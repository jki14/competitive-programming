...
void update(const int &i,const int &l,const int &r,const int &u,const int &v,const int &w){
	...
	if(u<=l && r<=v){
		...
		if(c[i]>=0){
			c[L]=c[R]=mx[L]=mx[R]=mn[L]=mn[R]=c[i];
			c[i]=-1;
		}
...
void count(const int &i,const int &l,const int &r,const int &u,const int &v,const int &w){
	if(v<l || u>r || mx[i]<w || mn[i]>w)return;
	if(u<=l && r<=v && c[i]>=0){
		if(c[i]==w)ans+=r-l+1;
	}else{int mid=(l+r)>>1;
		if(c[i]>=0){
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		for(int i=n;i<m;i++){ scanf("%d%d%d%d",&t[i],&l[i],&r[i],&b[i]);s[b[i]]=0; }
		top=0;for(it=s.begin();it!=s.end();it++)(*it).second=top++;
		for(int i=0;i<m;i++)b[i]=s[b[i]];
		for(int i=0;i<m;i++){
			if(t[i]==1){
				update(0,0,n-1,l[i],r[i],b[i]);
			}else{
				ans=0;count(0,0,n-1,l[i],r[i],b[i]);
				printf("%d\n",ans);
			}
		}
	}
