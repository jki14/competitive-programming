...
struct state{
	int u,d;
	state(const int &_u, const int &_d){ u=_u;d=_d; }
};

int n,m,s,t,k;
int nbs[N],nxt[M],dst[M],wht[M],enm;
int f[N][K],g[N][K];
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		scanf("%d%d%d",&s,&t,&k);k=k/10+(k%10?1:0);
		memset(g,0,sizeof(g));memset(f,0x3f,sizeof(f));f[s][0]=0;
		while(!que.empty())que.pop();que.push(state(s,0));g[s][0]=1;
		for(;!que.empty();que.pop()){
			int u=que.front().u,d=que.front().d;g[u][d]=0;
			for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
				int w=f[u][d]+wht[i],_d=min(d+1,k);
				if(f[v][_d]>w){
					f[v][_d]=w;
					if(!g[v][_d]){
						g[v][_d]=1;que.push(state(v,_d));
					}
				}
			}
		}
