...
int cmp(const int &i, const int &j){ return f[i]!=f[j]?f[i]>f[j]:i<j; }
	
int nxt(const int &u){ if(c[u]!=u)c[u]=nxt(c[u]);return c[u]; }
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		printf("Case #%d:\n",_T);
		scanf("%d",&n);c[S]=S;
		for(int i=0;i<S;i++){ vct[i].clear();c[i]=i+1;pos[i]=0; }
		for(int i=0;i<n;i++){
			int s;scanf("%d%d",&f[i],&s);
			vct[s].push_back(i);c[s]=s;
		}
		for(int i=0;i<S;i++){
		...
		for(int i=0;i<n;i++){
			int p=nxt(0);
			for(int j=nxt(0);j<S;j=nxt(j+1)){
				int pid=vct[p][pos[p]],jid=vct[j][pos[j]];
				int pw=f[pid]+p*i,jw=f[jid]+j*i;
				if(pw<jw || (pw==jw && pid>jid))p=j;
			}
			ans[i]=vct[p][pos[p]++]+1;
			if(pos[p]==_size(vct[p]))c[p]=p+1;
		}
		for(int i=0;i<n;i++){
			if(i)printf(" ");
			printf("%d",ans[i]);
