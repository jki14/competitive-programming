...
#define NUM_OF_PEOPLE 110

int g_num_of_people;
int g_group[NUM_OF_PEOPLE];
bool g_relationship[NUM_OF_PEOPLE][NUM_OF_PEOPLE];

bool MarkGroup(){
	memset(g_group,-1,sizeof(g_group));
	for(int s=1;s<=g_num_of_people;s++){
		if(g_group[s]!=-1)continue;
		g_group[s]=0;queue<int> que;
		//printf("g_group[%d]=%d\n",s,g_group[s]);
		while(!que.empty())que.pop();
		for(que.push(s);!que.empty();que.pop()){
			int i=que.front(),next_group=g_group[i]^1;
			for(int j=1;j<=g_num_of_people;j++)if(i!=j && !g_relationship[i][j]){
				if(g_group[j]!=-1){
					if(g_group[j]!=next_group)return false;
					continue;
				}
				g_group[j]=next_group;
				que.push(j);
			}
		}
	}
...
bool Check(){
	for(int i=1;i<=g_num_of_people;i++)
		for(int j=i+1;j<=g_num_of_people;j++)
			if(g_group[i]==g_group[j] && !g_relationship[i][j])return false;
	return true;
}

void Run(){
	memset(g_relationship,0,sizeof(g_relationship));
	for(int i=1;i<=g_num_of_people;i++){
		int id_of_people;
		while(scanf("%d",&id_of_people)!=EOF && id_of_people!=0)
			g_relationship[i][id_of_people]=true;
	}
	for(int i=1;i<=g_num_of_people;i++)
		for(int j=i+1;j<=g_num_of_people;j++){
			g_relationship[i][j]=g_relationship[j][i]&=g_relationship[i][j];
			//printf("g_relationship[%d][%d]=%d\n",i,j,g_relationship[i][j]);
		}
	if(MarkGroup() && Check())
		printf("YES\n");
	else printf("NO\n");
...
int main(){
	while(scanf("%d",&g_num_of_people)!=EOF)Run();	
	return 0;
}
