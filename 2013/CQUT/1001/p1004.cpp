...
lld count(const lld &idx){
	lld ret=0LL;
	for(int i=0;i<n;i++){
		if(idx<a[i])continue;
		ret+=(min(idx,b[i])-a[i])/c[i]+1LL;
	}
	return ret;
}
...
int find(){
	...
	while(l<=r){
		lld mid=(l+r)>>1;
		if(count(mid)&1){
			if(!(count(mid-1LL)&1))return (int)mid;
			r=mid-1LL;
		}else l=mid+1LL;
	}
	return 0;
...
void core(int &idx,int &num){
	idx=find();
	num=(int)(count((lld)idx)-count((lld)(idx-1)));
}
