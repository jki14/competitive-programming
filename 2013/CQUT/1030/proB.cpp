...
int n,m,k,sp,tp,ans;
int nbs[N],nxt[M],dst[M],wht[M],num;
int f[N],g[N];
...
int spfa(const int &sp,const int &tp){
	memset(f,0x3f,sizeof(f));memset(g,0,sizeof(g));
	f[sp]=0;g[sp]=1;
	while(!que.empty())que.pop();que.push(sp);
	for(;!que.empty();que.pop()){int u=que.front();g[u]=0;
		for(int i=nbs[u];i;i=nxt[i]){int v=dst[i],w=f[u]+wht[i];
			if(f[v]>w){
				f[v]=w;if(!g[v]){
					g[v]=1;que.push(v);
				}
...
int main(){
	while(scanf("%d%d%d",&n,&m,&k)!=EOF){
		...
		for(int i=0;i<n;i++){
			scanf("%s",c);
			for(int j=0;j<m;j++){int p=i*m+j;
				if(c[j]=='#')continue;
				if(c[j]=='C')sp=p;
				if(c[j]=='Y')tp=p;
				if(c[j]=='P')poster.push_back(p);
				for(int v=0;v<4;v++){
					int x=i+dx[v],y=j+dy[v],t=x*m+y;
					if(x<0 || x>=n || y<0 || y>=m)continue;
					addEdge(p,t,c[j]=='*'?k:0);
				}
			}
		}
		for(int i=0;i<_size(poster);i++)
			for(int j=0;j<_size(poster);j++)
				if(i!=j)addEdge(poster[i],poster[j],0);
		//printf("sp=%d tp=%d\n",sp,tp);
