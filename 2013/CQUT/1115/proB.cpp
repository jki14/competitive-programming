...
struct state{
	int x,y,s;
	state(const int &_x,const int &_y,const int &_s){
		x=_x;y=_y;s=_s;
	}
};
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF && !(n==0 && m==0)){
		...
		f[sx][sy][(int)c[sx][sy]]=0;ans=inf;
		que.push(state(sx,sy,(int)c[sx][sy]));
		for(;!que.empty();que.pop()){
			...
			for(int i=0;i<4;i++){
				...
				int ss=s|((int)c[xx][yy]);
				int ww=w+1;
				if(ww<f[xx][yy][ss]){
					f[xx][yy][ss]=ww;
					if(!g[xx][yy][ss]){
						g[xx][yy][ss]=1;
						que.push(state(xx,yy,ss));
					}
				}
			}
		}
