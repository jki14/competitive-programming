...
int n,m,src[N],c[N],ans[N];
int is[N],prm[N],num;
int a[N],r[N],f[N],g[N];

vector<int> pos[N],neg[N],que[N];
...
void modify(int p, const int &w){ for(;p<=n;p+=_lowbit(p))c[p]+=w; }
...
int main(){
	...
	while(scanf("%d%d",&n,&m)!=EOF && !(n==0 && m==0)){
		...
		for(int i=1;i<=n;i++){int x=a[i];
			//printf("for a[%d]=%d: \n",i,a[i]);
			for(int j=0;j<num && x/prm[j]>=prm[j];j++){int k=prm[j];
				if(x%k)continue;
				while(!(x%k))x/=k;
				//printf("key=%d\n",k);
				if(r[k]){
		...
		for(int i=1;i<=n;i++){
			//printf("f[%d]=%d g[%d]=%d\n",i,f[i],i,g[i]);
			pos[i].push_back(g[i]);neg[i].push_back(i+1);
			if(f[i]!=inf){
				neg[f[i]].push_back(g[i]);
				pos[f[i]].push_back(i+1);
			}
		}
		for(int i=0;i<m;i++){
			int dst;scanf("%d%d",&src[i],&dst);
			que[dst].push_back(i);
		}
		memset(c,0,sizeof(c));
		for(int i=1;i<=n;i++){
			for(int j=0;j<_size(pos[i]);j++)modify(pos[i][j], 1);
			for(int j=0;j<_size(neg[i]);j++)modify(neg[i][j],-1);
			for(int j=0;j<_size(que[i]);j++)
				ans[que[i][j]]=query(src[que[i][j]]);
		}
