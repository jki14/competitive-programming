...
int core(const int &s){
	if(s==bit[n]-1)return 0;int ret=neg,p;
	for(int i=0;i<n;i++){
		if(s&bit[i])continue;
		int cnt=0;
		for(int j=0;j<m;j++)
	...
	for(int j=0;j<m;j++)f[j]=(f[j]+c[p][j])%k;
	if(ret)
		return ret+core(s|bit[p]);
	else return -core(s|bit[p]);
}

int main(){
	for(int i=0;i<N;i++)bit[i]=_ni(i);
	while(scanf("%d%d%d",&m,&n,&k)!=EOF && !(n==0 && m==0 && k==0)){
		for(int i=0;i<n;i++)
			for(int j=0;j<m;j++)
				c[i][j]=f[j]=0;
		for(int i=0;i<n;i++){
			int v,x;scanf("%d",&v);
			while(v--){ scanf("%d",&x);c[i][--x]++; }
		}
		printf("%d\n",core(0));
	}
	return 0;
