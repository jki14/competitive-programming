...
const int dx[3][2]={ {0,1} ,{1,0} ,{-1,0} };
const int dy[3][2]={ {1,0} ,{0,-1} ,{0,-1} };
...
int calc(){int res=inf;
	for(int i=0;i<_ni(k);i++){int ret=0,flag=1;
		for(int j=0;j<k;j++)if(_has(i,j)){ 
			if(c[x[j]-1][y[j]]=='#' || c[x[j]][y[j]+1]=='#')flag=0;
			f[x[j]][y[j]]=1;ret++;
			/*printf("cover %d,%d\n",x[j],y[j]);*/ 
		}
		for(int j=0;j<k;j++){
			//printf("check %d,%d\n",x[j],y[j]);
			if(c[x[j]][y[j]]=='x')continue;
			if(f[x[j]][y[j]])continue;
			if(f[x[j]+1][y[j]])continue;
			if(f[x[j]][y[j]-1])continue;
			flag=0;break;
		}
		for(int j=0;j<k;j++)if(_has(i,j))f[x[j]][y[j]]=0;
		if(flag){
			//printf("done\n");
			_updmin(res,ret);
		}
...
int main(){
	memset(f,0,sizeof(f));
	while(scanf("%d%d",&n,&m)!=EOF && !(n==0 && m==0)){ans=0;
		for(int i=1;i<=n;i++)scanf("%s",c[i]+1);
		for(int i=0;i<=m+1;i++)c[0][i]=c[n+1][i]='*';
		for(int i=0;i<=n+1;i++){ c[i][0]=c[i][m+1]='*';c[i][m+2]=0;/*puts(c[i]);*/ }	
		
		k=0;for(int i=1;i<=n;i++)for(int j=1;j<=m;j++){
			if(c[i][j]=='.'){ x[k]=i;y[k++]=j; }
		}
		//printf("k=%d\n",k);
		ans=calc();//cout<<"ans="<<ans<<endl;
		for(int i=1;i<=n;i++)for(int j=1;j<=m;j++){
			if(c[i][j]=='#')continue;
			for(int x=0;x<3;x++){
				if(c[i+dx[x][0]][j+dy[x][0]]=='#')continue;
				if(c[i+dx[x][1]][j+dy[x][1]]=='#')continue;
				c[i][j]='x';
				for(int y=0;y<2;y++){
					if(c[i+dx[x][y]][j+dy[x][y]]=='.')
						c[i+dx[x][y]][j+dy[x][y]]='x';
				}
				_updmin(ans,calc()+1);
				c[i][j]='.';
				for(int y=0;y<2;y++){
					if(c[i+dx[x][y]][j+dy[x][y]]=='x')
						c[i+dx[x][y]][j+dy[x][y]]='.';
				}
			}
		}
