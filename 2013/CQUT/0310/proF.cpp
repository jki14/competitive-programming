...
int n,s[V],top;
int nbs[V],nxt[E],dst[E],wht[E],num;
int ans[V],vis[V],que[S];
//int pos[V],hap[V],len;

/*void update(const int &r){
	int q=pos[r],p=q>>1;
	while(p && ans[hap[p]]>ans[r]){
		pos[hap[p]]=q;hap[q]=hap[p];
		q=p;p=q>>1;
	}
	hap[q]=r;pos[r]=q;
}
...
int getmin(){
	int ret=hap[1],p=1,q=2,r=hap[len--];
	while(q<=len){
		if(q<len && ans[hap[q+1]]<ans[hap[q]])q++;
		if(ans[hap[q]]<ans[r]){
			pos[hap[q]]=p;hap[p]=hap[q];
			p=q;q=p<<1;
		}else break;
	}
	hap[p]=r;pos[r]=p;
...
int dfs(const int &u,const int &f){vis[u]=1;
...
void addEdge(const int &u,const int &v,const int &w){
	...
	nxt[++num]=nbs[v];nbs[v]=num;dst[num]=u;wht[num]=w;
}
	
int main(){
	while(scanf("%d",&n)!=EOF){
		memset(nbs,0,sizeof(nbs));num=0;
		for(int i=0;i<n;i++){ int u,v;scanf("%d%d",&u,&v);addEdge(u,v,1); }
		memset(vis,0,sizeof(vis));
		top=0;dfs(1,0);for(int i=0;s[i];i++)addEdge(0,s[i],0);
		memset(ans,0x3f,sizeof(ans));ans[0]=0;ans[n+1]=inf;
		//memset(pos,0,sizeof(pos));
		memset(vis,0,sizeof(vis));vis[0]=1;
		int head,tail;head=tail=0;que[tail++]=0;
		//hap[len=1]=0;pos[0]=1;
		for(;head<tail;head++){
		...
		/*while(1){
			if(len==0)break;
			int u=getmin();vis[u]=1;
			//printf("u=%d\n",u);
			for(int i=nbs[u];i;i=nxt[i]){int v=dst[i],w=wht[i];
				if(ans[v]>ans[u]+w){
					if(!pos[v]){ hap[++len]=v;pos[v]=len; }
					ans[v]=ans[u]+w;update(v);
				}
			}
		}*/
		for(int i=1;i<=n;i++){
