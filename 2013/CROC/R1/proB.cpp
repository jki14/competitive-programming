...
void addEdge(const int &u,const int &v){ dev[u]++;dev[v]++; }

int ring(/*const int &u,const int &k*/){
	/*if(!u)memset(vst,0,sizeof(vst));int flag=0,find=0;
	vst[u]=1;for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
		flag++;if(flag>2)return 0;
		if(!v)find=1;
		if(vst[v])continue;
		return dfs(v,k+1);
	}
	if(k==n && find)return 1;
	return 0;*/
	memset(dum,0,sizeof(dum));
	for(int i=1;i<=n;i++)dum[dev[i]]++;
	if(dum[2]==n)return 1;
	return 0;
}
...
int star(){
	memset(dum,0,sizeof(dum));
	for(int i=1;i<=n;i++)dum[dev[i]]++;
	if(dum[1]==n-1 && dum[n-1]==1)return 1;
	return 0;
}
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		if(n==m){
			if(ring(/*1,1*/)){ printf("ring topology\n");continue; }
		}else if(n==m+1){
