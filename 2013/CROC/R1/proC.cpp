...
int n,m,/*num,*/c[N],_c[N],a[M],r[P],vst[N];
char cs[M];
vector<string> ans;
string cps;
...
int app(){
	...
	for(int i=1;i<=m;i++)vst[_c[a[i]]]++;
	for(int i=0;i<n;i++)if(!vst[i])return 0;
	return 1;
}

void inc(const int &k,const int &p){
	if(p>_p){
		if(k>m){
			sprintf(cs,"%d.%d.%d.%d",r[1],r[2],r[3],r[4]);
			cps=cs;ans.push_back(cps);
		}
		return;
	}
	int dig=0;
	for(int i=1;i<=(a[k]==0?1:3) && k+i-1<=m;i++){
		dig*=10;dig+=a[k+i-1];
		if(dig>255)return;
		r[p]=dig;inc(k+i,p+1);
	}
...
void find(const int &k){
	...
	}else{ a[k]=a[m-k+1];find(k+1); }
}

int main(){
	while(scanf("%d",&n)!=EOF){ans.clear();
		for(int i=0;i<n;i++){ scanf("%d",&c[i]);_c[c[i]]=i; }
		/*num=0;*/for(m=max(4,(n<<1)-1);m<=_m;m++)find(1);
		sort(ans.begin(),ans.end());
		//printf("%d\n",ans);
		printf("%d\n",_size(ans));
		for(int i=0;i<_size(ans);i++){
			//printf("%s\n",ans.c_str());
			printf("%s\n",ans[i].c_str());
		}
	}
