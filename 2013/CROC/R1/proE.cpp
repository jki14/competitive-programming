...
int  check(const int &i,const int &l,const int &r,const int &p){
	if(l==r)return c[i];
	int mid=(l+r)>>1;
	int upd=(p>mid)?check(R,mid+1,r,p):check(L,l  ,mid,p);
	return max(c[i],upd);
}
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		for(int i=1;i<=m;i++){ int k;scanf("%d",&k);
			if(k==1){
				scanf("%d%d%d",&x[i],&y[i],&dis);
				modif(0,1,n,y[i],y[i]+dis-1,i);
				//printf("mdf %d %d %d\n",y[i],y[i]+dis-1,i);
			}else{
				int p;scanf("%d",&p);
				int r=check(0,1,n,p);
