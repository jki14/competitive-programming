...
int check(const int &y,const int &z,const int &x){
	if(y&x)return 0;
	if(z&(x<<1))return 0;
	if(z&(x>>1))return 0;
	return 1;
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){ans=0;
		init();memset(f,0x80,sizeof(f));f[0][0][0]=0;
		for(int i=0;i<n;i++){a[i]=0;
			for(int j=0;j<m;j++){ a[i]<<=1;int x;scanf("%d",&x);a[i]|=((x==1)?0:1); }
		}
		for(int i=0;i<n;i++){
			for(int j=0;j<num;j++){
				for(int k=0;k<num;k++){
					...
					for(int x=0;x<num;x++){
						if(sta[x]&a[i])continue;
						if(check(sta[j],sta[k],sta[x])){
							_updmax(f[i+1][k][x],f[i][j][k]+mem[x]);
							_updmax(ans,f[i+1][k][x]);
						}
					}
				}
			}
