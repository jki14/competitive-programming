...
int n,m,q,ans,a[N],f[N],h[M],g[M][N],b[Q][K];

int check(){
	for(int i=0;i<n;i++)if(a[i]>5)return 0;
	return 1;
}
...
void dfs(const int &i){_updmax(ans,i);
	if(i==q){
		//ans++;
		return;
	}
	for(int x=0;x<K;x++){
		int tmp=a[b[i][x]];a[b[i][x]]=max(a[b[i][x]]-2,1);
		for(int j=0;j<K;j++){ if(j==x)continue;int tar=f[b[i][j]];
			a[b[i][j]]++;
			for(int p=0;p<h[tar];p++)a[g[tar][p]]++;
		}
		//printf("%d:%d:",i,x);output();
		if(check())dfs(i+1);
		a[b[i][x]]=tmp;
		for(int j=0;j<K;j++){ if(j==x)continue;int tar=f[b[i][j]];
			a[b[i][j]]--;
...
int main(){
	int T;scanf("%d",&T);
	for(int _T=1;_T<=T;_T++){printf("Case #%d: ",_T);ans=0;
		scanf("%d%d%d",&n,&m,&q);memset(h,0,sizeof(h));
		for(int i=0;i<n;i++){ scanf("%d",&f[i]);g[f[i]][h[f[i]]++]=i; }
		for(int i=0;i<n;i++)scanf("%d",&a[i]);
		for(int i=0;i<q;i++)for(int j=0;j<K;j++)scanf("%d",&b[i][j]);
		dfs(0);
		printf("%d\n",ans);
	}
	return 0;
