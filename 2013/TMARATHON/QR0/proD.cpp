...
int main(){
	int T;scanf("%d",&T);
	while(T--){ans=1.0;
		scanf("%d",&n);//dn=(double)n;
		for(int i=0;i<n;i++)scanf("%d",&a[i]);
		for(int i=0;i<n;i++){
			for(int j=0;j<n;j++){
				ans-=((double)1/(double)n)*((double)1/((double)n-1.0))*max((double)a[i],(double)a[j])/((double)n-1.0)*(1.0-max((double)a[i]-1.0,(double)a[i]-1.0)/((double)n-2.0));
			}
		}
		printf("%.3f\n",ans);
	}	
