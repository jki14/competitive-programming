...
int n,m,f[M];
int nbs[M],nxt[N],wrs[N],wrc[N];

void addWork(const int &k,const int &s,const int &e,const int &c){
	nxt[k]=nbs[e];nbs[e]=k;wrs[k]=s;wrc[k]=c;
}
...
int main(){
	...
	while(T--){
		...
		memset(f,0,sizeof(f));for(int i=1;i<=m;i++){
			f[i]=f[i-1];for(int j=nbs[i];j;j=nxt[j]){
				int s=wrs[j],c=wrc[j];_updmax(f[i],f[s-1]+c);
			}
		}
		printf("%d\n",f[m]);
	}	
