...
int main(){
	while(scanf("%d%d%d%d",&n,&v1,&v2,&k)!=EOF){
		...
		memset(f,0,sizeof(f));
		for(int i=1;i<=n;i++){
			for(int rmb=0;rmb<=v1;rmb++){
				for(int src=0;src<=v2;src++){
					for(int fre=0;fre<=k;fre++){
						f[i][rmb][src][fre]=f[i-1][rmb][src][fre];
						if(rmb)_updmax(f[i][rmb][src][fre],f[i-1][rmb-1][src][fre]);
						if(src)_updmax(f[i][rmb][src][fre],f[i-1][rmb][src-1][fre]);
						if(fre)_updmax(f[i][rmb][src][fre],f[i-1][rmb][src][fre-1]);
						/*if(rmb && src)_updmax(f[i][rmb][src][fre],f[i-1][rmb-1][src-1][fre]);
						if(rmb && fre)_updmax(f[i][rmb][src][fre],f[i-1][rmb-1][src][fre-1]);
						if(src && fre)_updmax(f[i][rmb][src][fre],f[i-1][rmb][src-1][fre-1]);
						if(rmb && src && fre)_updmax(f[i][rmb][src][fre],f[i-1][rmb-1][src-1][fre-1]);*/
						//printf("f[%d][%d][%d][%d]=%d\n",i,rmb,src,fre,f[i][rmb][src][fre]);
						if(rmb>=a[i])_updmax(f[i][rmb][src][fre],f[i-1][rmb-a[i]][src][fre]+c[i]);
						if(src>=b[i])_updmax(f[i][rmb][src][fre],f[i-1][rmb][src-b[i]][fre]+c[i]);
						if(fre> 0   )_updmax(f[i][rmb][src][fre],f[i-1][rmb][src][fre-  1 ]+c[i]);
