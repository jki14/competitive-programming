...
const int dk[K]={ 3,0,2,1  };

char a[n+1][n+1][L],c[n+1][n+1][L];
int vis[n],mrk[n+1][n+1][K],ans;
...
int opt(int x){int num=0;
	for(int i=1;i<=n;i++)
		for(int j=1;j<=n;j++)
			for(int k=0;k<len;k++)
				c[i][j][k]=a[i][j][k];
	for(int i=0;i<n;i++){int k=x%3;
		if(k==1){num++;
			for(int k=0;k<len;k++)c[i][4][k]=c[i][1][k];
			for(int j=1;j<=n;j++){
				if(c[i][j][4]=='1')return inf;
				for(int k=0;k<len;k++)c[i][j][k]=c[i][j+1][k];
			}
		}else if(k==2){num++;
			for(int k=0;k<len;k++)c[i][0][k]=c[i][3][k];
			for(int j=n;j>=1;j--){
				if(c[i][j][4]=='1')return inf;
				for(int k=0;k<len;k++)c[i][j][k]=c[i][j-1][k];
			}
	...
	for(int j=0;j<n;j++){int k=x%3;
		if(k==1){num++;
			for(int k=0;k<len;k++)c[4][j][k]=c[1][j][k];
			for(int i=1;i<=n;i++){
				if(c[i][j][4]=='1')return inf;
				for(int k=0;k<len;k++)c[i][j][k]=c[i+1][j][k];
			}
		}else if(k==2){num++;
			for(int k=0;k<len;k++)c[0][j][k]=c[3][j][k];
			for(int i=n;i>=1;i--){
				if(c[i][j][4]=='1')return inf;
				for(int k=0;k<len;k++)c[i][j][k]=c[i-1][j][k];
			}
...
int dfs(const int &x,const int &y,const int &k){
	if(mrk[x][y][k])return 1;mrk[x][y][k]=1;
	if(vis[c2i(c[x][y][k])]==1)return 0;
	vis[c2i(c[x][y][k])]++;
	for(int i=0;i<K;i++){
		if(i==k)continue;
		if(mrk[x][y][i])continue;
		if(c[x][y][i]==c[x][y][k]){
			if(!dfs(x,y,i))return 0;
		}
	}
	for(int i=0;i<K;i++){
		int _x=x+dx[i],_y=dy[i],_k=dk[i];
		if(_x<1 || _x>n || _y<1 || _y>n)continue;
		if(mrk[_x][_y][_k])continue;
		if(c[_x][_y][_k]==c[x][y][k]){
			if(!dfs(x,y,i))return 0;
		}
	}
	return 1;
...
int check(){
	...
	for(int i=1;i<=n;i++)
		for(int j=1;j<=n;j++)
			for(int k=0;k<K;k++){
				if(mrk[i][j][k])continue;
				if(!dfs(i,j,k))return 0;
			}
	return 1;
}
...
int main(){
	int T;scanf("%d",&T);
	for(int t=1;t<=T;t++){printf("Case #%d: ",t);ans=inf;
		for(int i=1;i<=n;i++)
			for(int j=1;j<=n;j++)
				scanf("%s",a[i][j]);
		for(int s=0;s<S;s++){
