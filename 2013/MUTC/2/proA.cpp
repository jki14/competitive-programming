...
inline lld nxT(const lld &i,const lld &k){ lld lup=i/k+1LL;return lup*k; }
//inline lld abs(const lld &x){ return x<0LL?-x:x; }

void core(const lld &n,const lld &c,lld &i,lld &ans){
	while(i<n && i<c){
		lld nxt=min(nxT(i,a),nxT(i,b));
		_updmin(nxt,min(n,c));
		ans+=(nxt-i)*abs(i%a-i%b);
		//printf("nxt=%lld ans=%lld\n",nxt,ans);
		i=nxt;
	}
}
...
int main(){
	int T;scanf("%d",&T);
	while(T--){
		scanf("%d%d%d",&_n,&_a,&_b);n=_n;a=_a;b=_b;
		
		c=a*b/_gcd(a,b);i=0LL;ans=0LL;
		core(n,c,i,ans);	
		
		if(i<n){ 
			lld lup=n/c;ans=ans*lup;i=c*lup; 
			core(n,n,i,ans);
		}
