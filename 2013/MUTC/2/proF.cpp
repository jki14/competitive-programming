...
struct sol{
	int w,p;
	sol(){}
	sol(const int &_w,const int &_p){ w=_w;p=_p; }
};
...
void update(const int &u,const int &x,const int &_f,const int &_g,const int &v){
	if(_f>=f[u][x].fir.w){
		f[u][x].sec=f[u][x].fir;
		f[u][x].fir=sol(_f,v);
	}else if(_f>f[u][x].sec.w)f[u][x].sec=sol(_f,v);
	if(_g>=g[u][x].fir.w){
		g[u][x].sec=g[u][x].fir;
		g[u][x].fir=sol(_g,v);
	}else if(_g>g[u][x].sec.w)g[u][x].sec=sol(_g,v);
}

int best(const sta &a,const sta &b){
	if(a.fir.p!=b.fir.p)
		return a.fir.w+b.fir.w;
	else return max(a.fir.w+b.sec.w, a.sec.w+b.fir.w);
}

void fresh(const int &u){
	int k=c-b[u];
	for(int x=0;x<=k;x++){
		if(x)
			_updmax(ans,g[u][x].fir.w+w[u]);
		else _updmax(ans,f[u][x].fir.w+w[u]);
		for(int y=0;x+y<=k;y++){
			if(x+y==k)
				_updmax(ans,best(f[u][x],g[u][y])+w[u]);
			else _updmax(ans,best(f[u][x],f[u][y])+w[u]);
		}
	}
}
...
int main(){
	int T;scanf("%d",&T);
	while(T--){
		scanf("%d%d",&n,&c);m=n-1;ans=0;
		for(int i=0;i<n;i++)scanf("%d%d",&w[i],&b[i]);
		memset(nbs,0,sizeof(nbs));num=0;
		for(int i=0;i<m;i++){ int u,v;scanf("%d%d",&u,&v);addEdge(u,v); }
		num=0;memset(mrk,0,sizeof(mrk));
		dfs(0);memset(mrk,0,sizeof(mrk));
		for(int _i=0;_i<n;_i++){int u=que[_i];mrk[u]=1;
			for(int i=0;i<=c;i++)f[u][i].fir.w=f[u][i].sec.w=g[u][i].fir.w=g[u][i].sec.w=neg;
			f[u][0].fir.w=0;f[u][0].fir.p=u;

			for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
				if(!mrk[v])continue;
				for(int x=0;x<=c;x++){
					if(x==c && b[v])continue;
					update(u,x+b[v],f[v][x].fir.w+w[v],(b[v] && !x)?w[v]:(g[v][x].fir.w+w[v]),v);
				}
			}
			fresh(u);
		}
