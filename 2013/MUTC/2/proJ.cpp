...
int n,m,w;
int a[N][N],c[N],t[N],id[N],v[N];

int cmp(const int &i,const int &j){ return ((c[i]>=3)!=(c[j]>=3))?(c[i]>=3)>(c[j]>=3):t[i]<t[j];  }
...
int dfs(const int &p){
	int k=id[p],sum=0,flag=0;
	if(resize+(n-p)<=ansize)return 0;
	for(int i=0;i<c[k];i++)if(!v[a[k][i]])sum++;
	if(sum<3)return 0;
	res[resize++]=k;
	for(int i=0;i<c[k];i++)v[a[k][i]]++;
	for(int i=p+1;i<n;i++){
		if(t[id[i]]>t[k]+w)break;
		flag|=dfs(i);
	}
	if(!flag && resize>ansize){ ansize=resize;for(int i=0;i<ansize;i++)ans[i]=res[i]; }
	for(int i=0;i<c[k];i++)v[a[k][i]]--;resize--;
	return 1;
}
...
int main(){
	...
	while(T--){
		scanf("%d%d%d",&n,&m,&w);
		for(int i=0;i<n;i++){ 
			id[i]=i;scanf("%d%d",&c[i],&t[i]);
			for(int j=0;j<c[i];j++)scanf("%d",&a[i][j]);	
		}
		
		sort(id,id+n,cmp);
		while(n && c[id[n-1]]<3)n--;
		memset(v,0,sizeof(v));
		ansize=resize=0;
		for(int i=0;i<n;i++){
			//printf("id[%d]=%d\n",i,id[i]);
