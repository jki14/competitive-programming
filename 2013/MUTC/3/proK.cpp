...
struct point{
	lld x,y;
	point(){}
	point(const lld &_x,const lld &_y){ x=_x;y=_y; }
...
int main(){
	...
	while(T--){
		...
		s.clear();lld x=0LL,y=0LL;ans=0LL;now=inf;
		for(int i=0;i<n;i++){
			x=(x*ax+bx)%cx;y=(y*ay+by)%cy;
			if(i){
				it=s.lower_bound(point(x,y));
				for(jt=it;jt!=s.end();jt++){
					if(_sqr((*jt).x-x)>=now)break;
					_updmin(now,_sqr((*jt).x-x)+_sqr((*jt).y-y));
				}
				for(jt=it;;jt--){
					if(jt==it && jt!=s.begin())jt--;
					if(_sqr((*jt).x-x)>=now)break;
					_updmin(now,_sqr((*jt).x-x)+_sqr((*jt).y-y));
					if(jt==s.begin())break;
				}
