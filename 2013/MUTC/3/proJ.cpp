...
vector<int> s[N],qry[N];

void update(const int &i,const int &l,const int &r,const int &p,const int &w){
	if(p<l || p>r)return;
	_updmax(c[i],w);
	if(l!=r){int mid=(l+r)>>1;
		update(L,l,  mid,p,w);
		update(R,mid+1,r,p,w);
	}
}
	
int  querry(const int &i,const int &l,const int &r,const int &u,const int &v){
	if(v<l || u>r)return neg;
	if(u<=l && r<=v)
		return c[i];
	else{int mid=(l+r)>>1;
		return max(querry(L,l,  mid,u,v),
...
int main(){
	...
	while(T--){
		scanf("%d",&n);
		for(int i=1;i<=n;i++){ scanf("%d",&a[i]);qry[i].clear(); }
		memset(c,0,sizeof(c));memset(v,0,sizeof(v));
		scanf("%d",&m);
		for(int i=0;i<m;i++){ 
			int u,v;scanf("%d%d",&u,&v);
			if(u==v){ ans[i]=0;continue; }
			src[i]=u;qry[v].push_back(i); 
		}
		for(int p=1;p<=n;p++){int x=a[p],ss=_size(s[x]);
			for(int i=0;i<ss;i++){int y=s[x][i];
				if(v[y])update(0,1,n,v[y],y);	
				v[y]=p;
			}
			int qs=_size(qry[p]);
			for(int i=0;i<qs;i++){int k=qry[p][i];
				ans[k]=querry(0,1,n,src[k],p);
			}
		}
