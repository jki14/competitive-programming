...
/*int check(const int &x){
	for(int i=0,j=n-1;i<=j;){
		while(i<n  && (x&bit[i])==0)i++;
		while(j>=0 && (x&bit[j])==0)j--;
		if(i>j)break;
		if(a[i++]!=a[j--])return 0;
	}
	return 1;
...
inline void next(int &r){ int lb=r&-r,t=r+lb; r=t|(((t^r)>>2)/lb); }*/

int core(const int &x,const int &l,const int &r){
	/*if(f[x]!=inf)return f[x];
	//printf("call core(%d)\n",x);
	vector<int> btp;
	for(int i=0;i<n;i++)if(x&bit[i])btp.push_back(i);
	int size=_size(btp),tail=bit[size];
	for(int s=size;s;s--){
		for(int i=bit[s]-1;i<tail;next(i)){
			int sta=0;for(int j=0;j<size;j++){
				if(i&bit[j])sta|=bit[btp[j]];
			}
			if(!b[sta])continue;
			_updmin(f[x],core(x^sta)+1);
			//return f[x];
		}
		//if(f[x]!=inf)break;
	}
	return f[x];*/
	/*if(!x)return 0;int ret=inf;
	vector<int> btp;
	for(int i=0;i<n;i++)if(x&bit[i])btp.push_back(i);
	int s=_size(btp);
	for(int i=0;i<s;i++){int x=btp[i];
		for(int j=i;j<s;j++){int y=btp[j];
			if(a[x]!=a[y])continue;
			if(l<x && y<r)_upd
		}
	}*/
	if(!x)return 0; 
	if(g[x][l][r]==mrk)return f[x][l][r];
	g[x][l][r]=mrk;f[x][l][r]=inf;
	vector<int> btp;for(int i=0;i<n;i++)if(x&bit[i])btp.push_back(i);int s=_size(btp);
	for(int i=0;i<s;i++){int p=btp[i];
		for(int j=i;j<s;j++){int q=btp[j];
			if(a[p]!=a[q])continue;
			_updmin(f[x][l][r],core(x^bit[p]^(p!=q?bit[q]:0),p,q)+((l<p && q<r)?0:1));
		}
	}
	//printf(" f[%d][%d][%d]=%d\n",x,l,r,(int)f[x][l][r]);
	return f[x][l][r];
...
int main(){
	for(int i=0;i<=L;i++)bit[i]=_ni(i);
	memset(g,0,sizeof(g));
	
	int T;scanf("%d",&T);
	while(T--){
		scanf("%s",a);n=strlen(a);mrk=T+1;
		//for(int i=0;i<bit[n];i++)b[i]=check(i);
