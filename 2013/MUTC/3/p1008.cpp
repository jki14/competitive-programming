...
int n,top,f[S],bit[L],que[S],vis[S];
int q[S],l,r;
char a[L],cs[L];
...
int pld(const int &s){int tail=0;
	for(int i=0;i<n;i++){
		if(bit[i]&s)cs[tail++]=a[i];
	}
	for(int i=0;i<tail;i++)
		if(cs[i]!=cs[tail-i-1])return 0;
	return 1;
}

int fresh(const int &s){
	for(int i=0;i<top;i++)
		if(s&que[i])return 0;
	return 1;
}
...
int dp(const int &u){
	if(f[u]<=n)return f[u];
	for(int i=0;i<top;i++){int v=que[i];
		if((u&v)==v)_updmin(f[u],dp(u^v)+1);
	}
	return f[u];
}

void build(const int &s){
	//v[s]=1;if(pld(s) && fresh(s)){ que[top++]=s;printf("que+%d\n",s); }
	//for(int i=0;i<n;i++)if(s&bit[i] && !v[s^bit[i]])build(s^bit[i]);
	l=r=0;q[r++]=s;vis[s]=1;
	for(;l<r;l++){int u=q[l];
		if(pld(u)&&fresh(u)){ que[top++]=u;/*printf("que+%d\n",u);*/ }
		for(int i=0;i<n;i++){int v=u^bit[i];
			if(!(u&bit[i]))continue;
			if(vis[v])continue;
			q[r++]=v;vis[v]=1;
		}
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		scanf("%s",a);n=strlen(a);top=0;
		memset(vis,0,sizeof(vis));build(bit[n]-1);
