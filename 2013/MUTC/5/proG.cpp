...
int n,a[N],bit[L];
double p[N],f[N][B],ans;
char o[N];

char cs[L];

int calc(const int &b, const int &key, const char &c){
	switch(c){
		case '&': return b&key;
		case '|': return b|key;
		case '^': return b^key;
...
int main(){
	for(int i=0;i<L;i++)bit[i]=_ni(i);int T=0;
	while(scanf("%d",&n)!=EOF){printf("Case %d:\n",++T);
		for(int i=0;i<=n;i++)scanf("%d",&a[i]);
		for(int i=1;i<=n;i++){ scanf("%s",cs);o[i]=cs[0]; }
		for(int i=1;i<=n;i++)scanf("%lf",&p[i]);
		ans=0.0;
		for(int k=0;k<L;k++){
			f[0][0]=f[0][1]=0.0;f[0][(a[0]&bit[k])?1:0]=1.0;
			for(int i=1;i<=n;i++){
				for(int b=0;b<B;b++)f[i][b]=f[i-1][b]*p[i];
				int key=(a[i]&bit[k])?1:0;
				for(int b=0;b<B;b++)
					f[i][calc(b,key,o[i])]+=f[i-1][b]*(1.0-p[i]);
			}
			ans+=f[n][1]*((double)bit[k]);
		}
		printf("%.6f\n",ans);
	}
	return 0;
