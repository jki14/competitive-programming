...
int n,head,tail;
lld a[N],c[N],f[N],g[N],ans;

inline lld calc(lld x, lld y){
	if(x>y)swap(x,y);
	return ((x-1LL)*x%mod+x*(y-x)%mod)%mod;
}
...
int main(){
	...
	while(T--){
		...
		f[0]=g[n+1]=1LL;
		for(int i=1;i<=n;i++)f[i]=_mmd(f[i-1],c[i]);
		for(int i=n;i>=1;i--)g[i]=_mmd(g[i+1],c[i]);
		ans=f[n];

		for(int i=1;i<n;i++){
			int x=i,y=i+1;
			ans=_amd(ans,_mmd(_mmd(f[x-1],g[y+1]),calc(c[x],c[y])));
		}
