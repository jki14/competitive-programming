...
vector<int> qry[N],wat[N];

inline void add(int i){ for(;i<=n;i+=_lowbit(i))c[i]++; }
inline int  sum(int i){ int ret=0;for(;i;i-=_lowbit(i))ret+=c[i]; return ret; }

int main(){
	int T;scanf("%d",&T);
	while(T--){
		scanf("%d%d",&n,&m);s[n+1]=n+1;
		for(int i=1;i<=n;i++){ scanf("%d",&a[i]);s[a[i]]=i; }
		memset(c,0,sizeof(c));
		for(int i=1;i<=n;i++){ qry[i].clear();wat[i].clear(); }
		for(int i=0;i<m;i++){ scanf("%d%d",&lef[i],&rig[i]);qry[rig[i]].push_back(i); }
		for(int i=1;i<=n;i++){
			int v=s[a[i]+1];
			if(v>i)
				wat[v].push_back(i);
			else add(v);
			for(int j=0;j<_size(wat[i]);j++)add(wat[i][j]);
			for(int j=0;j<_size(qry[i]);j++){int k=qry[i][j];
				ans[k]=rig[k]-lef[k]+1-(sum(rig[k])-sum(lef[k]-1));
			}
		}
