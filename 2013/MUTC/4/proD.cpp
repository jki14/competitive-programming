...
int n,m;
int nbs[N][B],nxt[M][B],dst[M][B];
int vst[N],que[N],top;

int p[B],grp[N],gi[N],go[N],msk;
lld ans,cnt[N];

void dfs(const int &u, const int &b){
	vst[u]=b;for(int i=nbs[u][b];i>=0;i=nxt[i][b]){int v=dst[i][b];
		if(vst[v]==b)continue;
		dfs(v,b);
	}
	if(!b)
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		...
		memset(vst,0xff,sizeof(vst));top=n;ans=0LL;
		memset(cnt,0   ,sizeof(cnt));msk=0;
		for(int i=0;i<n;i++)if(vst[i])dfs(i,0);
		for(int _i=0;_i<n;_i++){int i=que[_i];
			//printf("vst[%d]=%d\n",i,vst[i]);
			if(vst[i]==1)continue;
			dfs(i,1);msk++;
		}
		...
		if(msk==1)
			...
			for(int u=0;u<n;u++){
				for(int i=nbs[u][0];i>=0;i=nxt[i][0]){int v=dst[i][0];
			...
			for(int i=0;i<msk;i++){
				if(!go[i] || !gi[i])
					_updmax(ans,ln*(ln-1LL)-cnt[i]*(ln-cnt[i])-(lld)m);
			}
			printf("%I64d\n",ans);
		}
	}
