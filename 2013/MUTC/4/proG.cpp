...
void extend(CTNode **p, CTNode *f, const int &k){
	*p=cur++;(*p)->wht=f->wht+1;
	...
	if(f->lef==f->rig){
		...
		for(int i=0;i<B;i++){
			if(f->ch[i]->lef<=k && k<=f->ch[i]->rig)
				extend(&(*p)->ch[i],f->ch[i],k);
			else (*p)->ch[i]=f->ch[i];
		}
	}
}

int query(CTNode *p, CTNode *q, const int &u, const int &v){
	if(v < p->lef || u > p->rig)return 0;
	if(u<=p->lef && p->rig<=v){
		return p->wht-q->wht;
	}else{
		int ret=0;for(int i=0;i<B;i++){
			ret+=query(p->ch[i],q->ch[i],u,v);
		}
		return ret;
	}
}
...
int main(){
	...
	while(T--){
		scanf("%d%d",&n,&m);
		for(int i=1;i<=n;i++){ scanf("%d",&a[i]);a[i]--;s[a[i]]=i; }
		init();
		for(int i=1;i<=n;i++)
			if(a[i]<n-1){
				extend(&head[i],head[i-1],s[a[i]+1]);
			}else head[i]=head[i-1];

		for(int i=0;i<m;i++){
			int l,r;scanf("%d%d",&l,&r);
			printf("%d\n",r-l+1-query(head[r],head[l-1],l,r));
		}
	}	
	return 0;
}
