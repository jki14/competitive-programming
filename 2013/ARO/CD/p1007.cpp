...
void init(){
	...
	for(int i=0;i<L;i++){
		for(int j=0;j<S;j++){
			if(!f[i][j])continue;
			for(int k=0;k<=9;k++)
				f[i+1][j+k*_ni(i)]+=f[i][j];
		}
	}
	memset(g,0,sizeof(g));g[0][0]=1LL;
	for(int i=1;i<=L;i++)
		for(int j=S-2;j>=0;j--){
			g[i][j]=g[i][j+1]+f[i][j];
			//printf("g[%d][%d]=%lld\n",i,j,g[i][j]);
...
int call(int x){
	int ret=0;
	for(int i=1;x;x/=10){ ret+=(x%10)*i;i<<=1; }
	return ret;
}

int core(int x){
	lld ret=g[L][tar];int now=0;
	//printf("ret=%lld\n",ret);
	memset(c,0,sizeof(c));
	for(int i=L-1;x;x/=10)c[i--]=x%10;
	for(int i=0;i<L;i++){
		//printf("c[%d]=%d\n",i,c[i]);
		for(int j=c[i]+1;j<=9;j++){
			ret-=g[L-i-1][max(0,tar-now-j*_ni(L-i-1))];
			//printf("dec g[%d][%d]==%lld\n",L-i-1,max(0,tar-now-j),
					//g[L-i-1][max(0,tar-now-j)]);
		}
		//printf("after %d: ret=%lld\n",i,ret);
		now+=c[i]*_ni(L-i-1);
	}
	//printf("ret=%lld\n",ret);
	return (int)ret;
