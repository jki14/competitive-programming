...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		...
		scanf("%d%d",&n,&m);c[m]=0;
		if(n==2 && m<=9){
			int k=m>>1,p=0;for(;p<k;p++)c[p]='a';
			c[p++]='b';int _k=(k-1)>>1;
			for(;p<m && p<k+_k+1;p++)c[p]='a';
			if(p<m)c[p++]='b';int __k=(k-2-_k)>>1;	
			for(;p<m && p<k+_k+__k+2;p++)c[p]='a';
			if(p<m)c[p++]='b';for(;p<m;p++)c[p]='a';
		}else if(n==2){
			for(int i=0;i<4;i++)c[i]='a';
			for(int i=4;i<m;i++)c[i]=s[(i-4)%6];
		}else for(int i=0;i<m;i++)c[i]='a'+(i%n);
		printf("%s\n",c);
	}
	return 0;
}
