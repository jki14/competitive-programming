...
int main(){
	...
	for(int _T=1;_T<=T;_T++){ans=0LL;
		...
		for(int i=0;i<=n;i++)
			for(int j=0;j<=L;j++)
				c[i][j]=(a[i]&bit[j])?1:0;
		memset(b,0,sizeof(b));int lef=inf;
		for(int i=1;i<=n;i++){_updmin(lef,i);add(i);
			while(lef<=i && !check())dec(lef);
			if(lef>i)lef=inf;
			if(lef!=inf)ans+=(lld)(i-lef+1);
		}
		printf("%I64d\n",ans);
	}
	return 0;
