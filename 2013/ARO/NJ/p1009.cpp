...
llf dist(const llf &x0,const llf &y0,const llf &x1,const llf &y1){
...
int main(){
	int T;scanf("%d",&T);
	while(T--){
		scanf("%d%d",&n,&_k);k=(llf)_k;ans=ret=0.0;
		for(int i=0;i<n;i++){
			int _x,_y;scanf("%d%d",&_x,&_y);
			x[i]=(llf)_x;y[i]=(llf)_y;
			c[i][i]=0.0;
		}
		for(int i=0;i<n;i++)
			for(int j=i+1;j<n;j++)
				c[i][j]=c[j][i]=dist(x[i],y[i],x[j],y[j]);	
		for(int j=0;j<n;j++){
			dis[j]=inf;mrk[j]=0;pnt[j]=-1;
		}dis[0]=0.0;
		for(int i=0;i<n;i++){
			int p=-1;for(int j=0;j<n;j++){
				if(mrk[j])continue;
			...
			for(int j=0;j<n;j++){
				if(mrk[j])continue;	
				if(c[p][j]<dis[j])dis[j]=c[pnt[j]=p][j];
			}
		}
		//SBMST
	
		printf("cap=%.2llf\n",cap);
		ans=(ret+cap)*k;cout<<fixed<<setprecision(2)<<ans<<endl;
	}	
	return 0;
}
