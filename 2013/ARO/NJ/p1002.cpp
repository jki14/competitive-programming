...
class CCore{
	private:
		...
		vector<int> nxt;
	public:
		void init(const int &_n,const int &_m,const int &_k){
			n=_n;m=_m;k=_k;ret=s=0;
	...
	private:
		...
		bool match(const int &x,const int &y){ return !x || !y || x==y; }
		void build(){
			str.resize(n);pat.resize(m);
			memset(pos,0xff,sizeof(pos));
			for(int i=0;i<n;i++){ str[i]=(pos[a[i]]>=0)?i-pos[a[i]]:-1;pos[a[i]]=i; }
			memset(pos,0xff,sizeof(pos));
			for(int i=0;i<m;i++){ 
				pat[i]=(pos[b[i]]>=0)?i-pos[b[i]]: 0;
				if(!pat[i])key[s++]=i;
				pos[b[i]]=i; 
			}
			//BUILD KEY
			/*for(int i=0;i<s;i++)for(int j=i+1;j<s;j++)
				if(b[key[i]]>b[key[j]])swap(key[i],key[j]);*/
			sort(key,key+s,comp);
			//BUILD NEXT
			nxt.clear();for(int i=0;i<m;i++)nxt.push_back(-1);
			for(int k,i=1;i<m;i++){
				for(k=nxt[i-1];k>=0 && !match(pat[i],pat[k+1]);)
					k=nxt[k];
				if(match(pat[i],pat[k+1]))nxt[i]=k+1;
			}
		}
		bool satisfy(const int &tail){
			for(int i=1;i<s;i++)
				if(a[tail-m+key[i]]<=a[tail-m+key[i-1]])return false;
			return true;
		}
	public:
		int run(){
