...
int core(const int &x){
	...
	while(l<=r){
		int mid=(l+r)>>1;
		if(w[id[mid]]>=x){
			if(mid==0 || w[id[mid-1]]<x)return ret[id[mid]];
			r=mid-1;
		}else l=mid+1;
	}
	return 0;
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		scanf("%d",&q);
		for(int i=0;i<q;i++){ scanf("%d",&r[i]); /*s.insert(_mkpr(r[i],0));*/ }
		//for(it=s.begin(),top=0;it!=s.end();it++)(*it).second=top++;
		//for(int i=0;i<m;i++)w[i]=s[w[i]];
		//for(int i=0;i<q;i++)r[i]=s[r[i]];
		sort(id,id+m,cmp);memset(ret,0,sizeof(ret));
		for(int i=0;i<n;i++){ f[i]=i;g[i]=1; }
		for(int _i=0;_i<m;_i++){ int i=id[_i];
			int x=find(u[i]),y=find(v[i]);
			if(x==y)continue;
			ret[i]+=g[x]*g[y]*2;
			g[x]+=g[y];g[y]=0;f[y]=x;
			//printf("%d: u=%d v=%d w=%d ret=%d\n",i,u[i],v[i],w[i],ret[i]);
		}
		for(int _i=m-2;_i>=0;_i--)ret[id[_i]]+=ret[id[_i+1]];
