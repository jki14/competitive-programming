...
void init(){
	g[0]=0LL;for(int i=1;i<N;i++)g[i]=g[i-1]+(lld)i;
}
	
void pkey(const int &k,const int &p){
	nxt[++num]=nbs[k];nbs[k]=num;pos[num]=p;
}
...
void updat(const int &i){ c[i]=c[L]+c[R]; }

void cover(const int &i,const int &l,const int &r,const int &u,const int &v){
	if(v<l || u>r || c[i]==s[i])return;
	if(u<=l && r<=v){
		ret+=s[i]-c[i];
		c[i]=s[i];
	}else{
...
int main(){
	...
	while(scanf("%d",&n)!=EOF && n){
		for(int i=0;i<=n;i++){ nbs[i]=0;f[i]=0LL; } 
		num=0;pos[0]=-1;ans=0LL;
		for(int i=0;i<n;i++){
			scanf("%d",&a[i]);
			pkey(a[i],i);
		}
		build(0,0,n-1);
		for(int k=0;c[0]!=s[0];k++){
			int top=0;sta[top]=n;
			for(int i=nbs[k];i;i=nxt[i])sta[++top]=pos[i];
			sta[++top]=-1;ret=0LL;
			for(int i=top;i;i--){
				int u=sta[i]+1,v=sta[i-1]-1;
				if(u>v)continue;
				cover(0,0,n-1,u,v);
			}
			ans+=ret*(lld)k;
