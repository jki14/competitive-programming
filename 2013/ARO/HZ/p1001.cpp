...
int n,m,ans;
int eu[maxm],ev[maxm],ew[maxm],next[maxm],rev[maxm],num;
int nbs[maxn],mk[maxn],low[maxn],d[maxn];
int bri[maxm],brinum,color,ti;

void addEdge(const int &u,const int &v,const int &w){
	num++;eu[num]=u;ev[num]=v;next[num]=nbs[u];nbs[u]=num;rev[num]=0;ew[num]=w;
	num++;eu[num]=v;ev[num]=u;next[num]=nbs[v];nbs[v]=num;rev[num]=1;ew[num]=w;
}

void TarjanBridge(const int &u,const int &p){
	int i,v,s=0;low[u]=d[u]=++ti;mk[u]=-color;
	for(i=nbs[u];i;i=next[i]){v=ev[i];int w=ew[i];
		if(orgi==p)continue;
		if(!mk[v]){TarjanBridge(v,orgi);s++;
...
void MarkBridge(){
	memset(mk,0,sizeof(mk));memset(bri,0,sizeof(bri));
	color=ti=brinum=0;
	for(int i=1;i<=n;i++)if(!mk[i]){ ++color;TarjanBridge(i,-1); }
}
...
/*struct edge{
	...
	bool operator < (const edge &oth) const{
		return (oth.u!=u)?oth.u<u:oth.v<v;
	}
};*/
/*map<edge,int> mp;
map<edge,int>::iterator it,jt;

void rebuild(){
	for(it=mp.begin();it!=mp.end();){
		jt=it;it++;int u=(*jt).first.u,v=(*jt).fist.v,w=(*jt).second;
	}
}*/

int main(){
	while(scanf("%d%d",&n,&m)!=EOF && !(n==0 && m==0)){
		/*for(int i=1;i<=n;i++)f[i]=i;mp.clear();
		for(int i=0;i<m;i++){
			int u,v,w;scanf("%d%d%d",&u,&v,&w);
			if(u==v)continue;if(u>v)swap(u,v);
			edge now=edge(u,v);it=mp.find(now);
			if(it==mp.end()){
				mp.insert(_mkpr(now,w));
			}else{
				mp.erase(it);f[find(u)]=find(v);
				rebuild();
			}
		}
		memset(nbs,0,sizeof(nbs));num=0;ans=inf;
		for(it=mp.begin();it!=mp.end();it++){
			int u=find((*it).first.u),v=find((*it).first.v),w=(*it).second;
			addEdge(u,v,w);
		}*/
		memset(nbs,0,sizeof(nbs));num=0;ans=inf;
		for(int i=0;i<m;i++){
