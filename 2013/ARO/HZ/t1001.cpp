...
void dfs(int pos, int deep, int father) { 
	...
	for(j=0;j<ls;j++) { 
		...
		if(vis[i] == 0) { 
			...
			if(low[i] > vis[pos]){
				_updmin(ans,wht[path[pos][j].second]);
			} 
		} 
		else if(i != father) { 
...
int main() { 
	while(scanf("%d%d",&n,&m)!=EOF && !(n==0 && m==0)){
		for(int i=0;i<=n;i++) path[i].clear(); ans=inf;
		for(int i=1; i<=m ;i++) { 
			int x,y,z; 
			scanf("%d%d%d", &x,&y,&z);wht[i]=z;_updmax(wht[i],1);
