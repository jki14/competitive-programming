...
int find(const int &i,const int &u){ if(f[i][u]!=u)f[i][u]=find(i,f[i][u]);return f[i][u]; }
int gind(const int &i,const int &u){ if(g[i][u]!=u)g[i][u]=gind(i,g[i][u]);return g[i][u]; }
int aind(const int &u){ if(a[u]!=u)a[u]=aind(a[u]);return a[u]; }
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		for(int i=1;i<=m;i++){
			for(int j=1;j<=n;j++)f[i][j]=find(i-1,j);
			f[i][find(i,x[i])]=find(i,y[i]);
		}
		for(int i=m;i>=1;i--){
			for(int j=1;j<=n;j++)g[i][j]=gind(i+1,j);
			g[i][gind(i,x[i])]=gind(i,y[i]);
		}
		scanf("%d",&q);
		for(int i=1;i<=q;i++){
			int u,v,ans;scanf("%d%d",&u,&v);
			for(int j=1;j<=n;j++)a[j]=find(u-1,j);
			for(int j=1;j<=n;j++)a[aind(j)]=aind(gind(v+1,j));
			ans=0;for(int j=1;j<=n;j++)if(aind(j)==j)ans++;
			printf("%d\n",ans);
		}
	}
	return 0;
