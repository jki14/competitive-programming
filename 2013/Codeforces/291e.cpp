...
int main(){
	while(scanf("%d",&n)!=EOF){ans=0;a[1].clear();
		...
		scanf("%s",cs);s=cs;slen=_size(s);
		/* Build Fail */
		memset(f,0xff,sizeof(f));
		for(int k,i=1;i<slen;i++){
			for(k=f[i-1];k>=0 && s[i]!=s[k+1];)k=f[i];
