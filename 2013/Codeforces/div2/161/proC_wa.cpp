...
int find(const int &u,const int &v){
	for(it=c[u].begin();it!=c[u].end();it++){
		...
		for(jt=c[v].begin();jt!=c[v].end();jt++){

			//printf("	*jt==%d\n",(*jt));

			if((*it)==(*jt) && !vis[(*it)])return (*it);
		}
	}
	return 0;
}
	...
	while(scanf("%d",&n)!=EOF){flag=0;
		...
		for(int i=0;i<n*2;i++){
			...
			if(!flag){
				c[u].insert(v);
				c[v].insert(u);
				if(_size(c[u])>4 || _size(c[v])>4)flag=2;
			}
		}
		for(int i=1;i<=n;i++)if(_size(c[i])!=4){ flag=2;break; }
		if(!flag){
			len=0;ans[len++]=1;vis[1]=1;
			for(it=c[1].begin();it!=c[1].end();it++){
				ans[len++]=(*it);
				vis[(*it)]=1;
				break;
			}
		...
		//if(!flag)printf("ans[0]=%d ans[1]=%d\n",ans[0],ans[1]);

		while(!flag && len<n){
			int x=ans[len-2],y=ans[len-1];
			ans[len++]=find(x,y);
			if(!ans[len-1]){ flag=1;break; }
			//printf("ans[%d]=%d\n",len-1,ans[len-1]);
			vis[ans[len-1]]=1;
		}
		if(!flag && len==n){
			int x=ans[len-2],y=ans[len-1];
			it=c[x].find(ans[0]);if(it==c[x].end())flag=1;
			it=c[y].find(ans[0]);if(it==c[y].end())flag=1;
			it=c[y].find(ans[1]);if(it==c[y].end())flag=1;
		}
		if(!flag){
			printf("%d",ans[0]);
