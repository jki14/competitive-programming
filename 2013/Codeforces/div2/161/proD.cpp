...
/*int findc(const int &u,int d){
	que[d++]=u;vis[u]=1;
	if(d>=k){
		for(int i=nbs[u];i;i=next[i])if(ev[i]==1)return d;
	}
	for(int i=nbs[u];i;i=next[i])if(!vis[ev[i]])findc(ev[i],d);
}*/
...
void addEdge(const int &u,const int &v){
	next[++num]=nbs[u];nbs[u]=num;ev[num]=v;
	next[++num]=nbs[v];nbs[v]=num;ev[num]=u;
}
...
void findc(){
	...
	while(true){
		int u=que[len];
		if(len>k){
			for(int i=nbs[u];i;i=next[i])if(ev[i]==1)return;
		}
		for(int i=nbs[u];i;i=next[i])if(!vis[ev[i]]){
			que[++len]=ev[i];
			vis[ev[i]]=1;
			break;
		}
...
int main(){
	while(scanf("%d%d%d",&n,&m,&k)!=EOF){
		...
		len=0;findc();
		printf("%d\n%d",len,1);
		for(int i=2;i<=len;i++)printf(" %d",que[i]);
		printf("\n");
	}
	return 0;
}
