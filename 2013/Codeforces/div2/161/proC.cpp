...
int find(const int &u,const int &v){
	int ret=0;
	for(jt=c[u].begin();jt!=c[u].end();jt++)
		for(kt=c[v].begin();kt!=c[v].end();kt++)
			if((*jt)==(*kt))ret++;
	return ret;
}
...
int main(){
	while(scanf("%d",&n)!=EOF){flag=0;
		...
		for(int i=1;i<=n;i++){ c[i].clear();d[i].clear(); }
		for(int i=0;i<n*2;i++){
		...
		for(int i=1;i<=n;i++)if(_size(c[i])!=4){ flag=1;break; }
		if(!flag){
			for(int i=1;i<=n;i++){
		...
		for(int i=1;i<=n;i++)if(_size(d[i])!=2){ flag=1;break; }
		if(!flag){
			len=0;ans[len++]=1;vis[1]=1;
			it=d[1].begin();ans[len++]=(*it);vis[(*it)]=1;
			while(!flag && len<n){
				int i=ans[len-1];
				for(it=d[i].begin();it!=d[i].end();it++){
					if(!vis[(*it)])break;
				}
				if(vis[(*it)]){ flag=1;break; }
				ans[len++]=(*it);vis[(*it)]=1;
			}
		}
		if(!flag){
