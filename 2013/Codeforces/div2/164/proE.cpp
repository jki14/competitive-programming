...
int cmp(const int &i,const int &j){ 
	return a[i]*p[i]*(100-p[j])>a[j]*p[j]*(100-p[i]);
}

int main(){
	while(scanf("%d",&n)!=EOF){ans=pre=0.0;
		for(int i=0;i<n;i++){ scanf("%d%d",&a[i],&p[i]);id[i]=i; }
		sort(id,id+n,cmp);
		for(int I=0;I<n;I++){int i=id[I]; //printf("i==%d\n",i);
			ans+=a[i]+(1.0-((dou)p[i])*0.01)*pre;
			pre+=((dou)a[i])*((dou)p[i])*0.01;
			//printf("	ans=%.10f\n",ans);
		}
		printf("%.10f\n",ans);
	}	
