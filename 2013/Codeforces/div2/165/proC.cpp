...
int n,a[N],c[N],id[N],ans;
/*map<int, int> s;
map<int, int>::iterator it;*/

int qpow(const int &x,const int &y){
	int ret=1;for(int i=0;i<y;i++)ret*=x;
	return ret;
}
...
int main(){
	while(scanf("%d",&n)!=EOF){
		...
		sort(id,id+n,cmp);
		//if(m!=n)while(1);
		for(int i=0;i<n-1;i++){int now=id[i],nex=id[i+1];
			if(a[nex]-a[now]<15){
				int k=qpow(4,a[nex]-a[now]);
				_updmax(c[nex],((c[now]%k)==0)?(c[now]/k):(c[now]/k+1));
			}
			//printf("a:%d c:%d\n",a[i],c[i]);
		}
		//printf("a:%d c:%d\n",a[n-1],c[n-1]);
		ans=a[id[n-1]]+1;lld now=4LL;
		while(now<((lld)c[id[n-1]])){
			now*=4LL;ans++;
		}
		printf("%d\n",ans);
	}	
