...
int n,m,k;
int nbs[V],dst[E],nxt[E],eid;
int fir[V],/*dep[V],*/vis[V],que[V],src,len;

int find(int u){
	que[len=1]=u;
	while(1){
		u=que[len];vis[u]=1;int flag=1;
		for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
			if(!fir[v])fir[v]=len;
		}
		for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];	
			if(vis[v])continue;
			que[++len]=v;flag=0;break;
			//printf("que[%d]=%d\n",len,v);
		}
		if(flag){
			src=fir[u];break;
...
void addedge(const int &u,const int &v){
	//dep[u]++;dep[v]++;
	dst[++eid]=v;nxt[eid]=nbs[u];nbs[u]=eid;
	dst[++eid]=u;nxt[eid]=nbs[v];nbs[v]=eid;
}
