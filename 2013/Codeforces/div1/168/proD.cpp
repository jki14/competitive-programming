...
int cmpa(const int &i,const int &j){ return a[i]<a[j]; }

int cmpf(const int &i,const int &j){ return f[i]<f[j]; }
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		for(int i=0;i<n;i++){
			...
			sort(c,c+m,cmpa);int s=0,pre=-1;
			while(s<m && a[c[s]]==-1)s++;
			//sort(c,c+s,cmpf);
			for(int j=s;j<m;j++){
				s=j;while(j+1<m && a[c[j+1]]==a[c[s]])j++;
				sort(c+s,c+j+1,cmpf);
				if(f[c[s]]>pre){ printf("-1\n");return 0; }pre=f[c[j]];
			}
			for(int j=0,k=0;j<m;j++){
				if(j<s){
					d[c[j]]=f[c[j]]*1.0;
				}else{
					if(j && s[c[j]]!=s[c[j-1]])k++;
					d[c[j]]=f[c[j]]*1.0+k*0.000001;
				}
			}
