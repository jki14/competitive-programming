...
int main(){
	while(scanf("%d%d",&n,&k)!=EOF){s.clear();
		for(int i=0;i<n;i++){
			int num;scanf("%d",&num);
			it=s.find((lld)num);
			if(it==s.end()){
				s.insert(_mkpr((lld)num,1));
			}else (*it).second++;
		}
		if(k==1){
			printf("%d\n",s.size());
		}else{ans=0;n=0;
			for(it=s.begin();it!=s.end();it++){ c[n]=(*it).first;d[n++]=(*it).second; }
			for(int i=0;i<n;i++){m=0;
				if(!d[i])continue;
				for(lld x=c[i];;x*=(lld)k){
					int k=find(x);if(k==-1)break;
					a[m++]=d[k];d[k]=0;
				}
				ans+=dp();
			}
