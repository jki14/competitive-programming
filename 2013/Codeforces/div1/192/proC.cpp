...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){p=0;
		for(int i=0;i<m;i++){
			int u,v;scanf("%d%d",&u,&v);
			addEdge(u,v);addEdge(v,u);
		}
		memset(mrk,0,sizeof(mrk));for(int i=1;i<=n;i++)if(!mrk[i])dfs(i);
		sng.clear();sp=0;tm=-1;
		for(int _i=0;_i<k;_i++){int i=id[_i];
			if(_size(c[i])==1){
				while(_i<k && _size(c[i])==1){
					sng.push_back(c[i][0]);
					i=id[++_i];
				}
				_i--;
			}else if(_size(c[i])==2){
				int __i;while(__i+1<k && _size(c[id[__i+1]])==2)__i++;
				if(__i!=_i){
					a[p].clear();
					for(int _j=0;_j<2;_j++){
						for(int j=_i;j<=__i;j++){
							a[p].push_back(c[id[j]][_j]);
						}
					}
					p++;
					_i=__i;
				...
				if(tm>=0){
					a[p].push_back(c[i][0]);
					a[p].push_back(c[tm][0]);
					a[p].push_back(c[i][1]);
					a[p].push_back(c[tm][1]);
					a[p].push_back(c[i][2]);
					tm=-1;
				}else{
					a[p].push_back(c[i][0]);
