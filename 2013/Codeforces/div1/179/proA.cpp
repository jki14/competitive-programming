...
void reset(const int &i,const int &l,const int &r){c[i]=0LL;
	if(l!=r){
		int mid=(l+r)>>1;
		reset(L,l,  mid);
		reset(R,mid+1,r);
...
void modif(const int &i,const int &l,const int &r,const int &u,const int &v,const lld &d){
	if(l>v || r<u)return;
	if(u<=l && r<=v){
		c[i]+=d;
	}else{
...
lld  querr(const int &i,const int &l,const int &r,const int &x){
	if(x<l || x>r)return 0LL;
	if(x==l && x==r)return c[i];
	int mid=(l+r)>>1;
	return 		querr(L,l,  mid,x)
		       +querr(R,mid+1,r,x)
		       +c[i];
}
...
int main(){
	while(scanf("%d%d%d",&n,&m,&k)!=EOF){
		...
		for(int i=1;i<=m;i++)scanf("%d%d%I64d",&lef[i],&rig[i],&drt[i]);
		reset(0,1,m);
		for(int i=1;i<=k;i++){
			int lef,rig;scanf("%d%d",&lef,&rig);
			modif(0,1,m,lef,rig,1LL);
		}
		for(int i=1;i<=m;i++)drt[i]*=querr(0,1,m,i);
		reset(0,1,n);
		for(int i=1;i<=m;i++)modif(0,1,n,lef[i],rig[i],drt[i]);
		for(int i=1;i<=n;i++){
			if(i!=1)printf(" ");
			printf("%I64d",a[i]+querr(0,1,n,i));
		}
		printf("\n");
	}
	return 0;
