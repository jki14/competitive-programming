...
lld c[N][N],f[N][N],ans[N];
int x[N],est[N],vst[N],n;
...
int main(){
	while(scanf("%d",&n)!=EOF){
		...
		for(int k=1;k<=n;k++){int nnd=x[k];lld ret=0LL;
			for(int i=1;i<=n;i++){ f[nnd][i]=c[nnd][i];f[i][nnd]=c[i][nnd]; }
			while(1){
				int p=0;for(int i=1;i<=n;i++)
					if(est[i] && vst[i]!=k && (p==0 || f[nnd][i]<f[nnd][p]))p=i;
				if(!p)break;vst[p]=k;
				for(int i=1;i<=n;i++)
					_updmin(f[nnd][i],f[nnd][p]+f[p][i]);
			}
			while(1){
				int p=0;for(int i=1;i<=n;i++)
					if(est[i] && vst[i]!=-k && (p==0 || f[i][nnd]<f[p][nnd]))p=i;
				if(!p)break;vst[p]=-k;
				for(int i=1;i<=n;i++)
					_updmin(f[i][nnd],f[p][nnd]+f[i][p]);
			}
			est[nnd]=1;
			for(int i=1;i<=n;i++){
				if(!est[i])continue;
