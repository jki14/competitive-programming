...
//HEAD_OF_TEMPLATE_BY_JKI14
	...
	template<class T> string _frct<T>::toString(){ostringstream tout;tout<eara<<"/"<<b;return tout.str();}
...
void check(const int &s,const int &k){
	if(vis[s])return;
	sta[top=1]=s;while(top){
		int _u=sta[top];
		int u=(_u<B)?_u:(_u-B);
		int _v=(_u<B)?(u+a[u]+B):(u-a[u]);
		int v=(_v<B)?_v:(_v-B);

		//printf("u=%d _u=%d v=%d _v=%d\n",u,_u,v,_v);

		if(!vis[_u]){vis[_u]=k;
			if(v>0 && v<=n){
				if(!vis[_v]){ sta[++top]=_v;continue; }
				if(_v==1 || vis[_v]==k){ f[_u]=-1LL;top--;continue; }
			}else{
				f[_u]=(lld)a[u];top--;continue;
			}
		}
		if(f[_v]==-1LL)
			f[_u]=-1LL;
		else f[_u]=f[_v]+(lld)a[u];
		//printf("f[%d]=%lld\n",_u,f[_u]);
		top--;
	}
}
...
int main(){
	while(scanf("%d",&n)!=EOF){
		...
		for(int i=2;i<=n;i++)check(i+B,i+B);
		for(int i=2;i<=n;i++)printf("%I64d\n",(f[i+B]<0LL)?(-1LL):(f[i+B]+((lld)i)-1LL));
	}
	return 0;
}
