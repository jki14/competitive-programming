...
int nbs[V],nxt[E],dst[E],num;
int n,q,r,m,vst[V],dig[V];
int a[V],g[V],f[S],sta[V];
...
int main(){
	while(scanf("%d%d%d",&n,&q,&r)!=EOF){int flag=0;
		...
		//for(int k=1;k<=n;k++){
		for(int i=1;i<=n;i++){
			if(vst[i])continue;if(dig[i])continue;cnt++;
			int sum=a[i];g[++m]=sum;vst[i]=1;
			//printf(" >%d",i);
			sta[top=1]=a[i];

			for(int j=nbs[i],k;j;j=nbs[k]){k=dst[j];
	
				cnt++;
				//printf(" %d",k);

				if(vst[k]){ flag=1;i=n;break; }
				vst[k]=1;sum+=a[k];g[++m]=sum;
				sta[++top]=a[k];
			}
			for(int i=0;top;i++){ r-=sta[top--]*i;if(r<0){i=n;break;} }
			//printf("\n");
		}
		//}
		if(r<0 || flag || cnt!=n){
			...
			for(int i=1;i<=m;i++){
				...
				for(int j=g[i];j<=r;j++){
					f[j]=(f[j]+f[j-g[i]])%modulo;
					//printf("f[%d]=%lld\n",j,f[j]);
				}

			}
			printf("%d\n",(int)((f[r]%modulo+modulo)%modulo));
		}
	}
	return 0;
}
