...
void smod(lld &x,const lld &y){ x=(x+y)%mod; }

lld wht(const int &l,const int &r){ 
	lld ll=(lld)l,rr=(lld)r;
	return ((ll+rr)*(rr-ll+1LL)/2LL)%mod;
}

void fresh(const int &i,const int &l,const int &r){
	int mid=(l+r)>>1;
	s[i]=(s[L]+s[R])%mod;
	lld lef=c[L]*wht(l,  mid)%mod;
	lld rig=c[R]*wht(mid+1,r)%mod;
	smod(s[i],lef);
	smod(s[i],rig);
}

void updat(const int &i,const int &l,const int &r,const int &x,const int &y,const lld &d){
	if(y<l || x>r)return;
	if(x<=l && r<=y){
		smod(c[i],d);
	}else{int mid=(l+r)>>1;
		if(c[i]){
			updat(L,l,  mid,1,top,c[i]);
			updat(R,mid+1,r,1,top,c[i]);
			c[i]=0LL;
		}
		updat(L,l,  mid,x,y,d);
		updat(R,mid+1,r,x,y,d);
		fresh(i,l,r);
	}
}
lld  query(const int &i,const int &l,const int &r,const int &x,const int &y){
	if(y<l || x>r)return 0LL;
	if(x<=l && r<=y){
		return (s[i]+c[i]*wht(l,r))%mod;
	}else{int mid=(l+r)>>1;
		if(c[i]){
			updat(L,l,  mid,1,top,c[i]);
			updat(R,mid+1,r,1,top,c[i]);
		...
		lld ret=(query(L,l,  mid,x,y)+query(R,mid+1,r,x,y))%mod;
		fresh(i,l,r);return ret;
	}
}
	
int main(){
	//for(int i=0;i<M;i++)lowbit[i]=_lowbit(i);
	while(scanf("%d",&n)!=EOF){ans=top=0;
		for(int i=0;i<n;i++){ scanf("%d",&a[i]);_updmax(top,a[i]); }
		memset(c,0,sizeof(c));memset(s,0,sizeof(s));memset(d,0,sizeof(d));
		for(int i=0;i<n;i++){
			lld nxt=(query(0,1,top,a[i],a[i])+a[i]-d[a[i]])%mod;
			smod(ans,nxt);smod(d[a[i]],nxt);
			updat(0,1,top,a[i],top,nxt);
		}
		printf("%d\n",(int)((ans%mod+mod)%mod));
	}
	return 0;
