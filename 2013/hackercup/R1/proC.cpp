...
int x[N],y[N],pla[N],u[N],v[N],det[N],id[N];
int cov[S],len[S],m,ans;

int wtra(const int &x){ return min(max(x,0),w); }
int htra(const int &x){ return min(max(x,0),h); }

int cmp(const int &i,const int &j){ return pla[i]<pla[j]; }
...
int main(){
	...
	for(int _t=1;_t<=T;_t++){ printf("Case #%d: ",_t);
		...
		for(int i=0;i<n;i++){
			pla[m]=wtra(x[i]-p+1);u[m]=htra(y[i]-q+1);v[m]=htra(y[i]);det[m]= 1;id[m]=m;++m;
			pla[m]=wtra(x[i]+1  );u[m]=htra(y[i]-q+1);v[m]=htra(y[i]);det[m]=-1;id[m]=m;++m;
		}
		pla[m]=wtra(0);u[m]=htra(h-q+1);v[m]=htra(h);det[m]= 1;id[m]=m;++m;
		pla[m]=wtra(w);u[m]=htra(h-q+1);v[m]=htra(h);det[m]=-1;id[m]=m;++m;
		pla[m]=wtra(w-p+1);u[m]=htra(0);v[m]=htra(h);det[m]= 1;id[m]=m;++m;
		pla[m]=wtra(w    );u[m]=htra(0);v[m]=htra(h);det[m]=-1;id[m]=m;++m;
		pla[m]=0;u[m]=h;v[m]=h;det[m]=1;id[m]=m;++m;
		//cout<<"m="<<m<<endl;
		sort(id,id+m,cmp);build(0,0,h-1);pla[id[m]=m]=w;ans=0;
		for(int I=0;I<m;I++){ int now=id[I],nex=id[I+1];
		
			modify(0,0,h-1,u[now],v[now],det[now]);
			
			//printf("%d->%d : %d  at %d,  len==%d\n",u[now],v[now],det[now],pla[now],len[0]);

			if(pla[now]!=pla[nex]){
				ans+=len[0]*(pla[nex]-pla[now]);

				//printf("ans+= %d * ( %d- %d )\n",len[0],pla[nex],pla[now]);
