...
int n,m,s,ans;
int _a[M],_b[M],id[M],t[M];
lld c[S],a[M],b[M],p[M];

map<lld, int> dmp;
map<lld, int>::iterator it;

void update(const int &i,const int &l,const int &r,const int &u,const int &v,const lld &d){
	if(v<l || u>r)return;
	if(u<=l && r<=v){
		_updmax(c[i],d);
	}else{int mid=(l+r)>>1;
		_updmax(c[L],c[i]);_updmax(c[R],c[i]);
		update(L,l,  mid,u,v,d);
		update(R,mid+1,r,u,v,d);
	}
}
	
int  querry(const int &i,const int &l,const int &r,const int &u,const int &v,const lld &d){
	if(v<l || u>r)return 0;
	if(u<=l && r<=v){
		return (c[i]<d)?1:0;
	}else{int mid=(l+r)>>1;
		_updmax(c[L],c[i]);_updmax(c[R],c[i]);
		return querry(L,l,  mid,u,v,d)
		      |querry(R,mid+1,r,u,v,d);
	}
}

int cmp(const int &i,const int &j){ return t[i]<t[j]; }

int main(){
	int T;scanf("%d",&T);
	for(int _T=1;_T<=T;_T++){printf("Case #%d: ",_T);
		scanf("%d",&n);dmp.clear();m=s=0;
		for(int i=0;i<n;i++){
			int d,k,w,e,s,_d,_p,_s;
			scanf("%d%d%d%d%d%d%d%d",&d,&k,&w,&e,&s,&_d,&_p,&_s);
			for(int j=0;j<k;j++){
				t[m]=d+j*_d;
				a[m]=((lld)w)+((lld)j)*((lld)_p);a[m]*=10LL;
				b[m]=((lld)e)+((lld)j)*((lld)_p);b[m]*=10LL;
				p[m]=((lld)s)+((lld)j)*((lld)_s);
				dmp.insert(_mkpr(a[m],0));
				dmp.insert(_mkpr(a[m]+1LL,0));dmp.insert(_mkpr(a[m]-1LL,0));
				dmp.insert(_mkpr(b[m],0));
				dmp.insert(_mkpr(b[m]+1LL,0));dmp.insert(_mkpr(b[m]-1LL,0));
				id[m]=m;m++;
			}
		}
		sort(id,id+m,cmp);ans=s=0;
		for(it=dmp.begin();it!=dmp.end();it++)(*it).second=++s;
		for(int i=0;i<m;i++){ _a[i]=dmp[a[i]];_b[i]=dmp[b[i]]; }
		memset(c,0,sizeof(c));
		for(int _i=0;_i<m;_i++){int i=id[_i];
			if(_i>0 && t[i]>t[pre]){
				for(int j=_i-1;j>=0 && t[id[j]]==t[pre];j--){
					//printf("build %d->%d == %lld\n",_a[id[j]],_b[id[j]],p[id[j]]);
					update(0,1,s,_a[id[j]],_b[id[j]],p[id[j]]);
				}
			}
			//printf("at %d %d->%d=%lld",t[i],_a[i],_b[i],p[i]);
			/*if(querry(0,1,s,_a[i],_b[i],p[i])){
				//printf("successed\n");
				ans++;
			}else printf("faild\n");*/
			ans+=querry(0,1,s,_a[i],_b[i],p[i]);
		}
		printf("%d\n",ans);
	}
	return 0;
