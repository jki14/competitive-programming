...
int hrz(const pnt &u,const pnt &v){
	if(a[u.x][u.y]!=a[v.x][v.y])return a[u.x][u.y]<a[v.x][v.y];
	if(u.x!=v.x)return u.x<v.x;
	return u.y<v.y;
}

int vrt(const pnt &u,const pnt &v){
	if(a[u.x][u.y]!=a[v.x][v.y])return a[u.x][u.y]<a[v.x][v.y];
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){printf("Case #%d: ",_T);
		...
		while(n && m){int succ=0;
			sort(c,c+s,hrz);
			//outs();		
			
			for(int p=0;p+m-1<s;p++){	
				if(a[c[p+m-1].x][c[p+m-1].y]!=a[c[0].x][c[0].y])break;
				if(c[p].x==c[p+m-1].x){
	
					//printf("cut hrz %d\n",c[0].x);

					for(int i=p;i<p+m;i++)a[c[i].x][c[i].y]=inf;
					n--;succ=1;
					break;
				}
			}
			...
			for(int p=0;p+n-1<s;p++){	
				if(a[c[p+n-1].x][c[p+n-1].y]!=a[c[0].x][c[0].y])break;
				if(c[p].y==c[p+n-1].y){
	
					//printf("cut hrz %d\n",c[0].x);

					for(int i=p;i<p+n;i++)a[c[i].x][c[i].y]=inf;
					m--;succ=1;
					break;
				}
			}
