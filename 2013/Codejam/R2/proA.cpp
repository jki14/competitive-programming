...
int n,m,mk[M];
int nbs[M],nxt[E],dst[E],num;
lld ans;
	
map<lld, lld> so;
map<lld, lld>::iterator it;
map<lld, lld, greater<lld> > se;
map<lld, lld, greater<lld> >::iterator jt;

void addEdge(const int &u,const int &v){
	++num;nxt[num]=nbs[u];nbs[u]=num;dst[num]=v;
	++num;nxt[num]=nbs[v];nbs[v]=num;dst[num]=u;
}
...
void dfs(const int &u){
	...
	it=so.find(a[u].o);
	if(it==so.end()){
		so.insert(_mkpr(a[u].o,a[u].p));
	}else (*it).second+=a[u].p;

	jt=se.find(a[u].e);
	if(jt==se.end()){
		se.insert(_mkpr(a[u].e,a[u].p));
	}else (*jt).second+=a[u].p;
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){printf("Case #%d: ",_T);
		...
		for(int i=0;i<m;i++){
			...
			ans-=((d>1LL)?(d*(d-1LL)/2LL):0LL)*a[i].p;
		...
		for(int i=0;i<m;i++){
			...
			while(it!=so.end()){
				jt=se.begin();
				lld p=min((*it).second,(*jt).second);
				lld d=(*jt).first-(*it).first;

				//printf("I:%lld->%lld %lld\n",(*it).first,(*jt).first,p);

				ans+=((d>1LL)?(d*(d-1LL)/2LL):0LL)*p;

				//printf("ans=%lld\n",ans);
				(*it).second-=p;(*jt).second-=p;
				if(!(*it).second)so.erase(it);
				if(!(*jt).second)se.erase(jt);
				it=so.begin();
			}
		}
		printf("%lld\n",ans);
