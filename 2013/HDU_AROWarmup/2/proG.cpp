...
lld core(lld x){
	if(x<0LL)return 0LL;
	memset(g,0,sizeof(g));
	for(int i=L-1;x;x/=10LL)
		g[i--]=(int)(x%10LL);
	lld ret=f[L][0];int now=0;
	//printf("sum=%lld\n",sum);
	for(int p=0;p<L;p++){
		for(int i=g[p]+1;i<=9;i++){
			ret-=f[L-p-1][_cmod(10-i-now,10)];
			//printf("now=%d i=%d k=%d\n",now,i,_cmod(10-i-now,10));
		}
		now=(now+g[p])%10;
		//printf("g[p]=%d ret=%lld\n",g[p],ret);
	}
	return ret;
}
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		printf("Case #%d: ",_T);
		cin>>a>>b;
		lld l=core(a-1);//printf("l=%lld\n",l);
		lld r=core(b);  //printf("r=%lld\n",r);
		cout<<r-l<<endl;
	}
	return 0;
}
