...
double dis2(const TPoint &a,const TPoint &b){
	return (a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y);
}

TPoint ssx(const TSeg &u,const TSeg &v){
	TPoint ret = u.a;
...
TPoint findc(const TPoint &a,const TPoint &b,const TPoint &c){
	TSeg u,v;
	u.a.x = (a.x+b.x)/2.0;
	u.a.y = (a.y+b.y)/2.0;
	u.b.x = u.a.x-a.y+b.y;
...
void update(){
	TPoint p0=a[1],p1=a[2];
	if(dis2(p0,p1)<dis2(a[2],a[3])){ p0=a[2];p1=a[3]; }
	if(dis2(p0,p1)<dis2(a[1],a[3])){ p0=a[1],p1=a[3]; }
	TPoint cp;
	cp.x=(p0.x+p1.x)/2.0;
	cp.y=(p0.y+p1.y)/2.0;
	double dp=0.0;
	for(int i=1;i<=3;i++)_updmax(dp,dis2(cp,a[i]));
	if(dp+eps<d){ c=cp;d=dp; }
}
