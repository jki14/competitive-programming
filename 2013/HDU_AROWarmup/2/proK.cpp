...
void fix(const int &k){
	for(int tar=9;tar>=0;tar--){
		for(int ax=0;ax<=9;ax++){
			int bx=tar-ax;if(bx<0)bx+=10;
			if(a[ax]==0 || b[bx]==0)continue;
			if(k==0 && (ax==0 || bx==0))continue;
			cc[k]='0'+tar;
			a[ax]--;b[bx]--;
			return;
		}
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		printf("Case #%d: ",_T);
		scanf("%s%s",ca,cb);n=strlen(ca);cc[n]=0;
		memset(a,0,sizeof(a));memset(b,0,sizeof(b));
		for(int i=0;i<n;i++)a[ca[i]-'0']++;
		for(int i=0;i<n;i++)b[cb[i]-'0']++;
		for(int i=0;i<n;i++)fix(i);int k=0;
		while(cc[k]=='0' && k!=n-1)k++;
		for(int i=k;i<=n;i++)cc[i-k]=cc[i];
		printf("%s\n",cc);
	}
	return 0;
}
