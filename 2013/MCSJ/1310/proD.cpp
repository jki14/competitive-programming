...
map<lld, int> slef,srig;
map<lld, int>::iterator it,jt;	

lld call(){lld ret=0LL;
	for(int i=0;c[i];){
		lld now=0LL;
		for(;c[i] && c[i]!='+';i++){ now*=10LL;now+=(lld)(c[i]-'0'); }
		ret+=now;
		if(c[i])i++;
	}
	//printf("ret=%lld\n",ret);
...
void run(map<lld, int> &s,const int &l,const int &r){
	int k=r-l;s.clear();
	//printf("l=%d r=%d k=%d\n",l,r,k);
	for(int i=0;i<bit[k-1];i++){
		int m=0;for(int j=l;j<r;j++){
			c[m++]=a[j];
			if(i&bit[j-l])c[m++]='+';
		}c[m]=0;
		//printf("%s\n",c);
		...
		if(it!=s.end()){
			(*it).second++;
		}else s.insert(_mkpr(val,1));
	}
}

int core(const int &k){int ret=0;
	run(slef,0,k);run(srig,k,n);
	for(it=slef.begin();it!=slef.end();it++){
		jt=srig.find((*it).first);
		if(jt!=srig.end()){
			ret+=(*it).second*(*jt).second;
		}
	}
	return ret;
}
...
int main(){
	...
	while(scanf("%s",a)!=EOF && a[0]!='E'){n=strlen(a);
		for(int i=ans=0;i<n-1;i++)ans+=core(i+1);
		printf("%d\n",ans);
	}
	return 0;
}
