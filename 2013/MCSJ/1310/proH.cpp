...
inline lld cpsum(const int &_x,const int &_c){
	lld x=_x,c=_c;
	lld num=x/c,tail=num*c;

	//printf("cpsum(%d,%d)==%lld\n",_x,_c,(c+tail)*num>>1);

	return (((c+tail))*num)>>1;
}

lld core(const int &x,const int &c){
	lld ret=(1LL+((lld)x))*((lld)x)>>1;int k=0,_c=c;
	for(int i=0;i<num && _c/prm[i]>=prm[i];i++){
		if(!(_c%prm[i])){
			f[k++]=prm[i];
			while(!(_c%prm[i]))_c/=prm[i];
		}
	}
	if(_c!=1)f[k++]=_c;

	for(int i=1;i<bit[k];i++){int sta=0;lld sum=1LL;
		for(int j=0;j<k;j++){
			if(!(i&bit[j]))continue;
			sta^=1;
			sum*=f[j];
		...
		ret-=(sta?1LL:-1LL)*cpsum(x,sum);
	}
	
	for(it=s.begin();it!=s.end();it++){
		if((*it).first>x)continue;
		if(gcd((*it).first ,c)==1)ret-=(lld)(*it).first;
		if(gcd((*it).second,c)==1)ret+=(lld)(*it).second;
	}
