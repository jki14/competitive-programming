...
struct cmp
{
    ...
    {
        if(x.deep!=y.deep)return x.deep<y.deep;
...
void Pr_Tree()
{
    ...
    while(!q.empty())
    {
        Node index=q.top();
        q.pop();
        for(int i=0; i<index.deep; i++)
            printf(".");
        printf("%s\n",index.name);
        for(int i=0; i<_size(index.chi); i++)
        {
            q.push(node[index.chi[i]]);
        }
    }
}
...
int LCA(int x,int y)
{
    ...
    int i,lg;
    for(lg=0; (1<<lg)<=node[x].deep; lg++);
    lg--;
    for(i=lg; i>=0; i--)
        if(node[x].deep-(1<<i)>=node[y].deep)
            x=p[x][i];
    if(x==y)return node[x].fa;
    for(i=lg; i>=0; i--)
...
int main()
{
    while(scanf("%d",&n)!=EOF&&n)
    {
        ...
        for(int i=2; i<=n; i++)
        {
            ...
            scanf("%s",ch);
            while(ch[de]=='.')de++;
            strcpy(node[i].name,ch+de);
            node[i].deep=de;
            node[i].fa=index[de-1];
            node[index[de-1]].chi.push_back(i);
            index[de]=i;
        }
        for(int i=1; i<=n; i++)
            ma[node[i].name]=i;
        ...
        while(m--)
        {
            ...
            else if(ch[0]=='b')
            {
                ...
                scanf("%s",s);
                int num=ma[s];
                int father=node[num].fa;
                if(father==-1)printf("1\n");
                else printf("%d\n",node[father].chi.size());
            }
            else if(ch[0]=='c')
            {
                char s1[65],s2[65];
