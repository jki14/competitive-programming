...
inline int neighbour(const int x, const int y){
	if((x+1)%n==y)return 1;
	if((y+1)%n==x)return 1;
	return 0;
}
...
inline int cost(const int x, const int y){
	return abs(g[x].x+g[y].x)*abs(g[x].y+g[y].y)%p;
}

inline int core_entry(const int x, const int y){
	if(~f[x][y])return f[x][y];
	f[x][y]=inf;
	for(int i=(y+1)%n; i!=x; i=(i+1)%n){
		int cur=neighbour(x, i)?0:core_entry(x, i)+cost(x, i);
		cur+=neighbour(i, y)?0:core_entry(i, y)+cost(i, y);
		UPDMIN(f[x][y], cur);
		//printf("f[(%d,%d)][(%d,%d)]=%d, cur=%d\n", g[x].x, g[x].y, g[y].x, g[y].y, f[x][y], cur);
	}
	//printf("f[(%d,%d)][(%d,%d)]=%d\n", g[x].x, g[x].y, g[y].x, g[y].y, f[x][y]);
	return f[x][y];
}
...
int main(){
	while(scanf("%d%d", &n, &p)!=EOF){
		...
		for(int i=0; i<n; i++){
			g[i].x=round(a[idx[i]].x);
			g[i].y=round(a[idx[i]].y);
		}
