...
point_t<double> pnt[N];

int find(const point_t<double> &p){
	int lef=0, rig=n-1;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(p.cross(point_t<double>(u[mid], _y1), point_t<double>(l[mid], _y2))<eps){
			if(mid==lef || p.cross(point_t<double>(u[mid-1], _y1), point_t<double>(l[mid-1], _y2))>eps)return mid;
			rig=mid-1;
		}else lef=mid+1;
	}
	return n;
...
int main(){
	while(scanf("%d", &n)!=EOF){
		if(n==0)break;
		scanf("%d%lf%lf%lf%lf", &m, &_x1, &_y1, &_x2, &_y2);
		for(int i=0; i<n; i++){
			scanf("%lf%lf", &u[i], &l[i]);
		}
