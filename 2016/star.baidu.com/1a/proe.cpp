...
lld count(lld lef, lld rig){
	lld cnt=rig/4LL-lef/4LL+((lef%4LL==0LL)?1LL:0LL);
	while((lef&1LL)==0LL)lef++;
	while((rig&1LL)==0LL)rig--;
	if(lef<=rig){
		return cnt+count(lef>>1, rig>>1);
	}else return cnt;
}
