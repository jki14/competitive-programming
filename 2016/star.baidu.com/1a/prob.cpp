...
int n, ans;
int val[N], pos[N], stn[N];
int f[S][N], mrk[S][N], bit[N];
...
inline void update(const int bin, const int rig, const int val){
	if(!mrk[bin][rig]){
		mrk[bin][rig]=1;
		f[bin][rig]=val;
		que.push(MP(bin, rig));
	}else UPDMAX(f[bin][rig], val);
}
...
int main() {
	...
	for(int _T=1; _T<=T; _T++){
		printf("Case #%d:\n", _T);
		scanf("%d", &n);
		memset(stn, 0xff, sizeof(stn));
		for(int i=0; i<n; i++){
			scanf("%d%d", &val[i], &pos[i]);
			if(~pos[i])stn[pos[i]]=i;
		}
		ans=-inf;
		...
		for(;!que.empty();que.pop()){
			int bin=que.front().first;
			int rig=que.front().second;
			int len=count(bin);
			if(len==n){
				UPDMAX(ans, f[bin][rig]);
				continue;
			}
			for(int i=0; i<n; i++){
				if(~stn[len] && i!=stn[len])continue;
				if(bin&bit[i])continue;
				int nxb=bin|bit[i];
				update(nxb, i, f[bin][rig]+((len>0)?(val[rig]*val[i]):0));
			}
		}
		printf("%d\n", ans);
	}
