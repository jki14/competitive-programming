...
void add(const char *s, const int idx){
    node_t *node=root;
    node->cnt++;
    for(int i=0; s[i]; i++){
        int k=s[i]-'a'+1;
        if(node->next[k]==NULL){
            memset(mask, 0 ,sizeof(node_t));
            node->next[k]=mask++;
        }
        node=node->next[k];
        node->cnt++;
...
void update(const int idx){
    for(vector<int>::iterator it=huk[idx].begin(); it!=huk[idx].end(); it++){
        int k=*it;
        ans[k]=1;
        for(int _i=0; _i<=26; _i++){
            int i=(_i>0)?(seq[k][_i-1]-'a'+1):0;
            for(int _j=0; _j<_i; _j++){
                int j=(_j>0)?(seq[k][_j-1]-'a'+1):0;
                ans[k]+=c[i][j];
            }
        }
    }
