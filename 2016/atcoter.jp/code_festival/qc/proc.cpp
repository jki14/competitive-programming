...
class range_t{
    ...
    range_t(const int lhs, const int rhs){
        this->low=lhs;
        this->hig=rhs;
    }
} x[N], y[N], z[N];
...
range_t conver(const range_t &lhs, const range_t &rhs){
    range_t foo;
    foo.low=MAX(lhs.low, rhs.low);
    foo.hig=MIN(lhs.hig, rhs.hig);
    return foo;
}
