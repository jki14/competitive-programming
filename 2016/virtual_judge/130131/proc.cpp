...
int n, cap;
int fir[N], sec[N], cnt[N], ans[N];
int nbs[N], nxt[M], dst[M], enm;

void add_edge(const int u, const int v){
    nxt[enm]=nbs[u];dst[enm]=v;nbs[u]=enm++;
    nxt[enm]=nbs[v];dst[enm]=u;nbs[v]=enm++;
}

void build(const int u, const int pr){
    fir[u]=0;sec[u]=0;cnt[u]=1;
    ...
    //printf("%d fir=%d sec=%d cnt=%d\n", u, fir[u], sec[u], cnt[u]);
}

void query(const int u, const int pr, int px){
    int flag=0;
    if(cnt[0]-cnt[u]<=cap){
        UPDMAX(px, cnt[0]-cnt[u]);
    }
    if(~pr && cnt[0]-cnt[u]-px>cap){
        flag=2;
    }else if(~pr && cnt[0]-cnt[u]>cap){
        flag=1;
    }
    for(int i=nbs[u]; ~i; i=nxt[i]){
        const int v=dst[i];
        if(v==pr)continue;
        if(cnt[v]>cap){
            flag+=(cnt[v]-fir[v]<=cap)?1:2;
        }
        int foo=px;
        if(fir[u]==cnt[v] || fir[u]==fir[v]){
            UPDMAX(foo, sec[u]);
        }else UPDMAX(foo, fir[u]);
        query(v, u, foo);
    }
    ans[u]=(flag<2)?1:0;
    //printf("%d pr=%d px=%d flag=%d\n", u, pr, px, flag);
}
