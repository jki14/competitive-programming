...
class myque{
    ...
    int *que, lef, rig;
    myque(){
        this->que=(int*)malloc(sizeof(int)*N);
        this->lef=this->rig=0;
    }
    ~myque(){
        free(this->que);
...
class higque : public myque{
    public:
    int find(const int val){
        int L=this->lef, R=this->rig-1;
        while(L<=R){
            int M=(L+R)>>1;
            if(val<a[this->que[M]]){
                if(M==L || val>=a[this->que[M-1]])return this->que[M];
                R=M-1;
            }else L=M+1;
        }
        while(1);
...
int main() {
    scanf("%d", &n);
        ...
        cur=1;
        for(int i=1; i<n; i++){
            while(hig.lef<hig.rig && hig.que[hig.lef]<=bis)hig.lef++;
            while(low.lef<low.rig && low.que[low.lef]<=bis)low.lef++;
            if(a[i]>=a[bis]){
                if(hig.lef<hig.rig && a[hig.que[hig.rig-1]]>a[i]){
                    bis=hig.find(a[i]);
                    cur=i-bis;
                    i--;
                }else{
                ...
                if(low.lef<low.rig && a[low.que[low.rig-1]]<a[i]){
                    bis=low.find(a[i]);
                    cur=i-bis;
                    i--;
                }else{
