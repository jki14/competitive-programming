...
//HEAD_OF_JKI'S_HEADER_
	...
	template<class T> void fast_fourier_trans(complex_t<T> f[], const int len, const int is_dft){
		...
		for(int h=2; h<=len; h<<=1){
			complex_t<T> wn(cosl(is_dft?(-2*PI/h):(2*PI/h)), sinl(is_dft?(-2*PI/h):(2*PI/h)));
...
int main() {
    ...
    while(cin>>a>>n>>k){
        ...
        for(int i=1; i<=k; i++){
            cnt[i%k]=n/k+((n%k)>=i?1:0);
        }
        for(til=1; til<=k*2;)til<<=1;
        for(int i=0; i<til; i++){
            //printf("cnt[%d]=%d\n", i, cnt[i]);
            com[i].r=cnt[i]*1.0;
            com[i].i=0.0;
        }
        fast_fourier_trans(com, til, 1);
        for(int i=0; i<til; i++){
            com[i] = com[i]*com[i];
        }
        fast_fourier_trans(com, til, 0);
        for(int i=0; i<til; i++)sum[i]=(lld)(com[i].r+0.5);
        for(int i=0; i<k; i++)sum[i+i]-=cnt[i];
        for(int i=0; i<til; i++)sum[i]>>=1;
        memset(res, 0, sizeof(res));
        for(int i=0; i<til; i++)res[i%k]+=sum[i];
        ans=0LL;
        for(int i=0; i<k; i++){
            //printf("res[%d]=%lld\n", i, res[i]);
            if((a*1LL*i)%k==0LL)ans+=res[i];
        }
        cout<<ans<<endl;
    }
    return 0;
