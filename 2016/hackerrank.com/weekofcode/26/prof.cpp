...
int main() {
    int bst=0;
    for(int i=0; i<S; i++){
        if(cos(bst*1.0+0.5)<cos(i*1.0+0.5)){
            //printf("best=%d\n", i);
            bst=i;
            ind[i]=(i<<1)+1;
        }else{
    ...
    int sum;
    if(scanf("%d", &sum)!=EOF){
        int x=sum/3, y=sum/3, z=sum/3;
        if(sum%3>0)x++;
        if(sum%3>1)y++;
        double ans=sin(x*1.0)+sin(y*1.0)+sin(z*1.0);
        for(int z=1; z<=sum-2; z++){
            if((sum-z)&1){
                int foo=ind[(sum-z-2)>>1];
                x=y=(sum-z-foo)>>1;
                x+=foo;
            }else{
                x=y=(sum-z)>>1;
            }
            UPDMAX(ans, sin(x*1.0)+sin(y*1.0)+sin(z*1.0));
        }
        printf("%.9f\n", ans);
    }
    return 0;
