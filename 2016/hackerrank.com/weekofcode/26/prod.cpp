...
//HEAD_OF_JKI'S_HEADER_
	...
	class MILLERRABIN{
		...
		int test(const lld n){
			...
			lld p=n-1LL, s=0LL;
			while(!(p&1)){ p>>=1;s++; }
			for(int i=0; i<4 && this->prime_table[i]<n; i++){
...
int main() {
    ...
    if(scanf("%d", &n)!=EOF){
        for(int i=0; i<n; i++)
            scanf("%d", &a[i]);
        scanf("%d%d%d", &m, &low, &hig);
        a[n]=inf;
        int lef=0, rig=0;
        int cost=0, head=MIN(hig, m);
        int remain=m-head;
        while(true){
            //printf("start-point: %d, remain: %d\n", a[rig]+remain-m, remain);
            //printf("cost=%d head=%d remain=%d\n", cost, head, remain);
            int full=a[rig+1]-a[rig];
            if(full>=remain){
                if(remain==0 || (remain>=low && remain<=hig)){
                    //printf("*cost=%d head=%d remain=%d\n", cost, head, remain);
                    //printf("*a[%d]=%d\n", rig, a[rig]);
                    printf("%d\n", a[rig]+remain-m);
                    break;
                ...
                if(remain<low){
                    ...
                    if(head-adj>=low){
                        ...
                        lef++;
                        cost-=a[lef]-a[lef-1];
                        head=MIN(MIN(hig, a[lef]-a[lef-1]), m);
                        remain=m-head-cost;
                    }
                }else{
                    while(true){
                        lef=++rig;
                        if(a[lef]-a[lef-1]>=low){
                            cost=0;
                            head=MIN(MIN(hig, a[lef]-a[lef-1]), m);
                            remain=m-head;
                            break;
