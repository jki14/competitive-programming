...
int main() {
    while(scanf("%d%d", &n, &m)!=EOF){
        for(int i=0; i<=n; i++)inv[i]=INVERSEE(i);
        int ceil=MOD_POW(2, m);
        g[0]=1;
        for(int i=1; i<=n; i++){
            g[i]=MOD_MUL(g[i-1], (ceil-i));
            MOD_MUL(&g[i], inv[i]);
        }
        f[0]=0;f[1]=ceil-1;
        for(int i=2; i<=n; i++){
            f[i]=MOD_ADD(g[i], -MOD_MUL(MOD_ADD(f[i-1], -MOD_MUL(MOD_ADD(g[i-2], -f[i-2]), ceil-i+1)), inv[i]));
        }
        for(int i=2; i<=n; i++){
            MOD_MUL(&f[n], i);
        }
