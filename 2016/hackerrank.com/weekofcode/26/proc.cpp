...
int main() {
    int lef, rig;
    //while(scanf("%d%d", &lef, &rig)!=EOF){
    if(scanf("%d%d", &lef, &rig)!=EOF){
        memset(bad, 0, sizeof(bad));
        if(lef<=1){
            bad[1-lef]=1;
        }
        for(int f=2; f*f<=rig; f++){
            for(int cur=(lef+f-1)/f*f; cur<=rig; cur+=f){
                if(cur==f)continue;
                bad[cur-lef]++;
            }
        }
        int ans=0;
        for(int i=lef; i+2<=rig; i++){
            if(!bad[i-lef] && !bad[i+2-lef]){
                //printf("%d %d\n", i, i+2);
                ans++;
            }
        }
