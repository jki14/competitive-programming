...
int main() {
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<n; i++){
            scanf("%d", &p[i]);p[i]--;
            set<int>::iterator it=foo.upper_bound(p[i]);
            if(it!=foo.end()){
                pos=i;
            }
            foo.erase(p[i]);
            for(vector<int>::iterator it=des[p[i]].begin(); it!=des[p[i]].end(); it++){
                cnt[*it]--;
                if(cnt[*it]==0){
                    foo.insert(*it);
                }
        ...
        if(~pos){
            for(int u=0; u<n; u++){
                for(vector<int>::iterator it=des[u].begin(); it!=des[u].end(); it++){
                    cnt[*it]++;
                }
            }
            for(int i=0; i<n; i++){
            ...
            p[pos]=*foo.upper_bound(p[pos]);
            foo.erase(p[pos]);
            for(vector<int>::iterator it=des[p[pos]].begin(); it!=des[p[pos]].end(); it++){
                cnt[*it]--;
                if(cnt[*it]==0){
                    foo.insert(*it);
                }
