...
block_t generate(const block_t &prev, const int x, const int y, const int sidelen){
    block_t foo;
    //set valley-before
    foo.valley=MP(x, y);
    switch(prev.orientation){
        ...
        break;
        case 1:
        foo.valley.second+=sidelen-1;
        break;
        case 2:
        foo.valley.first+=sidelen-1;
        foo.valley.second+=sidelen-1;
        break;
        default:
        foo.valley.first+=sidelen-1;
    ...
    switch(prev.orientation){
        case 0:
        r=foo.valley.first-prev.valley.first;
        c=foo.valley.second-prev.valley.second;
        break;
        case 1:
        r=prev.valley.second-foo.valley.second;
        c=foo.valley.first-prev.valley.first;
        break;
        case 2:
        r=prev.valley.first-foo.valley.first;
        c=prev.valley.second-foo.valley.second;
        break;
        default:
        r=foo.valley.second-prev.valley.second;
        c=prev.valley.first-foo.valley.first;
    }
    foo.value=prev.value+r*1LL*n+c*1LL;
    //set sidelen
    foo.sidelen=sidelen;
    //set orientation
    foo.orientation=(prev.orientation+1)%4;
    //set valley-after
    switch(prev.orientation){
        case 0:
        foo.valley.second+=sidelen-1;
...
inline int inside(const lld valley_value, const lld target_value, const int sidelen){
    if(target_value<valley_value)return 0;
    lld d=target_value-valley_value;
    if((d/n<sidelen*1LL)&&(d%n<sidelen*1LL))
        return 1;
    else return 0;
}

inline pii relo(pii valley, const int orientation, const lld valley_value, const lld target_value){
     lld d=target_value-valley_value;
     int r=d/n;
     int c=d%n;
     switch(orientation){
...
inline int find(const lld val){
    int lef=0, rig=m;
    while(lef<=rig){
        int mid=(lef+rig)>>1;
        if(inside(a[mid].value, val, a[mid].sidelen)){
            if(mid==rig || !inside(a[mid+1].value, val, a[mid+1].sidelen))return mid;
            lef=mid+1;
        }else rig=mid-1;
    }
    return -1;
...
int main() {
    ...
    while(cin>>n){
        ...
        for(int i=1; i<=m; i++){
            int x, y, d;
            cin>>x>>y>>d;
            a[i]=generate(a[i-1], x, y, d+1); 
        }
        /*for(int i=0; i<=m; i++){
            printf("{'valley':[%d,%d],'value':%lld,'orientation':%d,'sidelen':%d\n"
                , a[i].valley.first, a[i].valley.second, a[i].value, a[i].orientation, a[i].sidelen);
        }*/
        cin>>q;
        for(int i=0; i<q; i++){
            lld val;
            ...
            int k=find(val);
            //printf("k=%d\n", k);
            pii foo=relo(a[k].valley, a[k].orientation, a[k].value, val);
