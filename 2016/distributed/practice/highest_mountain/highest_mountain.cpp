...
int main() {
	int node_idx=MyNodeId();
	int node_num=NumberOfNodes();
	int peak_num=NumberOfPeaks();
	int peak_avg=peak_num/node_num;
	int lef=peak_avg*node_idx;
	int rig=(node_idx==node_num-1)?(peak_num-1):(lef+peak_avg-1);
	int len=rig-lef+1;
	...
	for(int p=lef; p<=rig; p++){
		int k=p-1;
		for(int i=p-1; ~i; i--){
			if((hig[k]-hig[p])*1LL*(p-i)<(hig[i]-hig[p])*1LL*(p-k)){
				vst[k]=0;
				//fprintf(stderr, "node %d erase %d at %d for %d.\n", node_idx, k, p, i);
				k=i;
			}
		}
		k=p+1;
		for(int i=p+1; i<peak_num; i++){
			if((hig[k]-hig[p])*1LL*(i-p)<(hig[i]-hig[p])*1LL*(k-p)){
				vst[k]=0;
				//fprintf(stderr, "node %d erase %d at %d for %d.\n", node_idx, k, p, i);
				k=i;
			}
	...
	//SEND
	for(int i=0; i<node_num; i++)msg[i].clear();
	for(int i=0; i<peak_num; i++){
		int node_dst=(peak_avg)?(i/peak_avg):node_num;
		UPDMIN(node_dst, node_num-1);
		if(node_dst==node_idx)continue;
		if(!vst[i]){
			msg[node_dst].push_back(i);
		}
	}
	for(int i=0; i<node_num; i++){
		if(i==node_idx)continue;
		PutInt(i, SIZE(msg[i]));
		for(vector<int>::const_iterator it=msg[i].begin(); it!=msg[i].end(); it++)
			PutInt(i, *it);
		Send(i);
	}
	//RECEIVE
	for(int i=0; i<node_num; i++){
		...
		Receive(i);
		int cnt=GetInt(i);
		//fprintf(stderr, "node %d receive (%d) from %d.\n", node_idx, cnt, i);
		while(cnt--){
			int idx=GetInt(i);
			vst[idx]=0;
			//fprintf(stderr, "node %d receive %d from %d.\n", node_idx, idx, i);
	...
	//ANS
	ans=0;
	for(int i=lef; i<=rig; i++)ans+=vst[i];
	//fprintf(stderr, "node=%d lef=%d rig=%d ans=%d vst[0]=%d\n", node_idx, lef, rig, ans, vst[0]);
	if(node_idx!=0){
		PutInt(0, ans);
		Send(0);
	}else{
