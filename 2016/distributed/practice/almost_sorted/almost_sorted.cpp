...
static const long long MODL=1048576LL;
...
int main() {
	...
	int file_num=NumberOfFiles();
	int file_dis=MaxDistance();
	int file_avg=file_num/node_num;
	int lef=file_avg*node_idx;
	int rig=(node_idx!=node_num-1)?(lef+file_avg-1):file_num-1;
	vector<lld> arr;
	for(int i=MAX(lef-file_dis, 0); i<=MIN(rig+file_dis, file_num-1); i++){
		arr.push_back(Identifier(i));
	}
	sort(arr.begin(), arr.end());
	lld ans=0LL;
	vector<lld>::iterator it=arr.begin();
	for(int i=MAX(lef-file_dis, 0); i<=MIN(rig+file_dis, file_num-1); i++){
		if(i>=lef && i<=rig){
			MOD_ADD(&ans, MOD_MUL((*it)%MODL, i*1LL, MODL), MODL);
			//fprintf(stderr, "ans=ans+%lld*%d (%lld)\n", *it, i, ans);
		}
		it++;
	}
	//fprintf(stderr, "node %d (%d->%d) = %lld\n", node_idx, lef, rig, ans);
	if(node_idx!=0){
		PutLL(0, ans);
		Send(0);
	}else{
