...
int main() {
    ...
    int avg=n/m;
    int mod=n%m;
    int lef=p*avg+MIN(p, mod);
    int rig=lef+avg-1+(p<mod?1:0);
    int *box=(int*)malloc(sizeof(int)*m);
    int *boy=(int*)malloc(sizeof(int)*m);
    //int len=(rig-lef+1);
    int x=0, y=0;
    ...
    if(p==q){
        for(int i=0; i<m; i++){
            ...
            MOD_ADD(&y, GetInt(i));
            for(int k=0; k<m; k++){
                MOD_ADD(&box[k], GetInt(i));
                MOD_ADD(&boy[k], GetInt(i));
            }
        }
        MOD_MUL(&x, y);
        for(int i=0; i<m; i++){
            MOD_ADD(&x, MOD_MUL(MOD_MUL(box[i], boy[(m-i)%m]), -1));
        }
        x=MOD_STD(x);
        printf("%d\n", x);
    }else{
