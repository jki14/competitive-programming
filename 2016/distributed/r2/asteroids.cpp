...
int main() {
    ...
    int avg=h/m;
    int mod=h%m;
    int low=p*avg+MIN(p, mod);
    int hig=low+avg-1+(p<mod?1:0);
    int n=(hig-low+1);
    char **a=(char**)malloc(sizeof(char*)*MAX(n, 1));
    for(int i=0; i<n; i++)a[i]=(char*)malloc(sizeof(char)*w);
    int ***f=(int***)malloc(sizeof(int**)*2);
    for(int i=0; i<2; i++){
        f[i]=(int**)malloc(sizeof(int*)*w);
        for(int j=0; j<w; j++)f[i][j]=(int*)malloc(sizeof(int)*(n*2+1));
    }
    for(int i=low; i<=hig; i++){
        for(int j=0; j<w; j++){
            a[i-low][j]=GetPosition(i, j);
        }
    }
    ...
    for(int i=0; i<n; i++){
        ...
        for(int p=0; p<w; p++){
            ...
            for(int j=n-i; j<=n+i; j++){
                if(f[pre][p][j]<0)continue;
                //S
                if(a[i][p+j-n]!='#'){
                    UPDMAX(f[cur][p][j], f[pre][p][j]+(a[i][p+j-n]-'0'));
                    //L
                    if(p+j-n-1>=0 && a[i][p+j-n-1]!='#'){
                        UPDMAX(f[cur][p][j-1], f[pre][p][j]+(a[i][p+j-n]-'0')+(a[i][p+j-n-1]-'0'));
                    }
                    //R
                    if(p+j-n+1<w && a[i][p+j-n+1]!='#'){
                        UPDMAX(f[cur][p][j+1], f[pre][p][j]+(a[i][p+j-n]-'0')+(a[i][p+j-n+1]-'0'));
                    }
                }
            }
    ...
    if(n>0){
        if(p>0){
            ...
            for(int s=0; s<w; s++){
                ...
                if(v<0)continue;
                for(int i=0; i<=n+n; i++){
                    if(f[cur][s][i]<0)continue;
                    UPDMAX(r[s+i-n], v+f[cur][s][i]);
                }
            }
        }else{
            for(int s=0; s<w; s++){
