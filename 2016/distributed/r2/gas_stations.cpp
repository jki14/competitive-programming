...
static const long long MODL=1000000007LL;
...
//HEAD_OF_JKI'S_HEADER_
	...
	template<class T>  inline int LENGTH(const T &x){ return (int)x.ngth(); }
	...
	template<class T> class MATX{
		...
		public:
			...
			MATX(const unsigned long _n){
				this->hig=this->wid=_n;
				__init();
			}
			...
			const MATX operator*(const MATX &opn) const{
				...
				const unsigned long n=MIN(this->wid, opn.hig);
				for(unsigned long x=0; x<ret.hig; x++)
					for(unsigned long y=0; y<ret.wid; y++)
			...
			template<class T2> const MATX mul(const MATX &opn, const T2 m) const{
				MATX ret(this->hig, opn.wid);
				const unsigned long n=MIN(this->wid, opn.wid);
				for(unsigned long x=0; x<ret.hig; x++)
					for(unsigned long y=0; y<ret.wid; y++)
						for(unsigned long z=0; z<n; z++)
	...
	template<class T> void fast_fourier_trans(complex_t<T> f[], const int n, const int is_dft){
...
lld mycost(deque<pii> &tank, int &gas, int len){
    lld buf=0LL;
    while(len>0 && !tank.empty()){
        int p=tank.front().first;
        int v=tank.front().second;
        if(len>=v){
            buf+=p*1LL*v;
            tank.pop_front();
            len-=v;
...
void charge(deque<pii> &tank, int &gas, int gp, const int m){
    //printf("gas=%d---\n", gas);
    while(!tank.empty()){
        int p=tank.back().first;
        if(p>gp){
            gas-=tank.back().second;
            tank.pop_back();
        }else break;
    }
    tank.push_back(MP(gp, m-gas));
    gas=m;
    /*for(deque<pii>::const_iterator it=tank.begin(); it!=tank.end(); it++){
        printf("[%d]*%d\n", it->first, it->second);
    }*/
}
...
int main() {
    ...
    int m=GetTankSize();
    if(MyNodeId()==0){
        lld *a=(lld*)malloc(sizeof(lld)*(n+1));
        int *que=(int*)malloc(sizeof(int)*(n+1));
        deque<pii> tank;
        int gas=0;
        tank.clear();
        lld ans=0LL;
        int lef=0, rig=0, til=1, cur=0;
        for(int i=0; i<n; i++){
            a[i]=GetGasPrice(i);
        }
        a[n]=0;
        charge(tank, gas, a[cur], m);
        while(true){
            for(; til<=n && til-cur<=m; til++){
                if(a[til]<=a[cur]){
                    //ans+=(til-cur)*a[cur];
                    ans+=mycost(tank, gas, til-cur);
                    cur=til;
                    charge(tank, gas, a[cur], m);
                    while(lef<rig && que[lef]<=cur)lef++;
                    continue;
                }
                if(lef<rig && a[que[rig-1]]>a[til])rig--;
                que[rig++]=til;
            }
            if(cur==n)break;
            ans+=mycost(tank, gas, que[lef]-cur);
            cur=que[lef++];
            charge(tank, gas, a[cur], m);
            if(cur==n)break;
        }
