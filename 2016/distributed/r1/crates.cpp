...
int main() {
    ...
    int node_mst=node_num-1;
    int node_idx=MyNodeId();
    int node_avg=n/node_num;
    int node_mod=n%node_num;
    int lef=node_idx*node_avg+((node_idx>=node_mod)?node_mod:node_idx);
    int rig=lef+node_avg-1+((node_idx<node_mod)?1:0);
    //cerr<<"node "<<node_idx<<" lef="<<lef<<" rig="<<rig<<endl;
    //COUNT
    vector<pii> arr;
    arr.clear();
    lld count=0LL;
    for(int i=lef; i<=rig; i++){
        int val=GetStackHeight(i+1);
        arr.push_back(MP(val, i));
        count+=val*1LL;
    }
    if(node_idx!=node_mst){
        PutLL(node_mst, count);
        Send(node_mst);
        Receive(node_mst);
        count=GetLL(node_mst);
    }else{
    ...
    lld right=0LL;
    lld left=0LL;
    for(vector<pii>::iterator it=arr.begin(); it!=arr.end(); it++){
        it->first-=avg+((it->second<mod)?1:0);
        right+=it->first*1LL;
    }
    if(node_idx>0){
        Receive(node_idx-1);
        left=GetLL(node_idx-1);
        right+=left;
    }
    if(node_idx+1<node_num){
        PutLL(node_idx+1, right);
        Send(node_idx+1);
    }
