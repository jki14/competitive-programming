...
int main() {
    ...
    lld node_avg=1LL;
    lld node_cnt=1LL;
    while(node_cnt*node_num<count){
        node_avg++;
        node_cnt<<=1;
    }
    if(node_num>count/node_cnt){
        node_num=count/node_cnt;
        node_mst=node_num;
    }
    vector<pii> arr;
    if(node_idx<node_num){
        lld lef=node_cnt*node_idx;
        lld rig=lef+node_cnt-1LL;
        arr.clear();
        for(lld i=lef; i<=rig; i++){
            arr.push_back(MP(char2int(GetFavoriteMove(i)), (int)i));
        }
        for(int len=1; len*1LL<node_cnt; len<<=1){
            for(int i=0; (i+len)*1LL<node_cnt; i+=(len<<1)){
                if(!(arr[i].first==arr[i+len].first || ((arr[i].first+1)%3)==arr[i+len].first)){
                    pii tmp=arr[i+len];
                    arr[i+len]=arr[i];
                    arr[i]=tmp;
                }
        ...
        PutInt(node_mst, arr[0].first);
        PutInt(node_mst, arr[0].second);
        Send(node_mst);
    }
    if(node_idx==node_mst){
        ...
        int tail=SIZE(arr);
        for(int len=1; len<tail; len<<=1){
            for(int i=0; i+len<tail; i+=(len<<1)){
                if(!(arr[i].first==arr[i+len].first || ((arr[i].first+1)%3)==arr[i+len].first)){
                    pii tmp=arr[i+len];
                    arr[i+len]=arr[i];
                    arr[i]=tmp;
