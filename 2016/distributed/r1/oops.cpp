...
int main() {
    ...
    lld node_avg=n/node_num;
    lld lef=node_avg*node_idx;
    lld rig=(node_idx==node_num-1)?(n-1LL):(lef+node_avg-1LL);
    lld pik=-inf, vly=inf;
    ...
    //cerr<<"node "<<node_idx<<" lef="<<lef<<" rig="<<rig<<" pik="<<pik<<" vly="<<vly<<endl;
    if(node_idx==MASTER_NODE){
        ...
        for(int i=0; i<node_num; i++){
            if(i==node_idx){
                if(pik>-inf)hig.push_back(pik);
                if(vly<inf)low.push_back(vly);
            }else{
                Receive(i);
                lld x=GetLL(i);
                lld y=GetLL(i);
                if(x>-inf)hig.push_back(x);
                if(y<inf)low.push_back(y);
            }
        }
        lld ans=0LL;
        for(vector<lld>::const_iterator itx=hig.begin(); itx!=hig.end(); itx++){
            for(vector<lld>::const_iterator ity=low.begin(); ity!=low.end(); ity++){
                UPDMAX(ans, (*itx)-(*ity));
            }
        }
        cout<<ans<<endl;
    }else{
        PutLL(MASTER_NODE, pik);
        PutLL(MASTER_NODE, vly);
        Send(MASTER_NODE);
    }
    return 0;
}
