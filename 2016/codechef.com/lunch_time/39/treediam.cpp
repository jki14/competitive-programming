...
int n, w[N];
int u[N], v[N];
int nbs[N], nxt[M], dst[M], lev[M], eid;

int fir[N], sec[N], mrk[N], seq[N];

void add_edge(const int u, const int v, const int k){
    nxt[eid]=nbs[u];dst[eid]=v;lev[eid]=k;nbs[u]=eid++;
    nxt[eid]=nbs[v];dst[eid]=u;lev[eid]=k;nbs[v]=eid++;
}

int calc(const int u, const int k){
    int ret=w[u];
    mrk[u]=1;
    fir[u]=sec[u]=0;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(lev[i]<=k || mrk[v])continue;
        int foo=calc(v, k);
        UPDMAX(ret, foo);
        if(fir[v]+w[v]>=fir[u]){
            sec[u]=fir[u];
            fir[u]=fir[v]+w[v];
        }else if(fir[v]+w[v]>sec[u]){
            sec[u]=fir[v]+w[v];
        }
    }
    UPDMAX(ret, fir[u]+sec[u]+w[u]);
    //printf("calc(%d, %d)=%d\n", u, k, ret);
    return ret;
}
