...
void cyc(const int p){
    g[0][p]=0;r[0][p]=++msk;
    for(int k=0; k<n; k++){
        for(int u=0; u<n; u++){
             if(r[k][u]<msk)continue;
             for(int i=nbs[u]; ~i; i=nxt[i]){
                if(r[k+1][dst[i]]<msk){
                    r[k+1][dst[i]]=msk;
                    g[k+1][dst[i]]=inf;
                }
                UPDMIN(g[k+1][dst[i]], g[k][u]+wht[i]);
             }
         }
    }
    for(int i=1; i<=n; i++){
        if(r[i][p]<msk)continue;
        UPDMIN(ans, g[i][p]*1.0/i);
    }
}
...
int main() {
    ...
    while(T--){
        ...
        for(int k=0; k<n; k++){
            for(int u=0; u<n; u++){
                if(f[k][u]>=inf)continue;
                if(u==t)UPDMIN(ans, f[k][u]*1.0/k);
                for(int i=nbs[u]; ~i; i=nxt[i]){
                    UPDMIN(f[k+1][dst[i]], f[k][u]+wht[i]);
                }
            }
        }
        if(f[n][t]<inf)UPDMIN(ans, f[n][t]*1.0/n);
        memset(r, 0xff, sizeof(r));msk=0;
        for(int u=0; u<n; u++){
            if(c[s][u] && c[u][t])cyc(u);
        }
        if(ans==inf)
            printf("-1\n");
        else printf("%.6f\n", ans);
