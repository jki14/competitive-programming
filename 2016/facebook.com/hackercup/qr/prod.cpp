...
void insert(node_t *cur, char *str){
	for(char *p=str; *p; p++){
		int key=*p-'a';
		if(key>=26)while(1);
		if(cur->nxt[key]==NULL){
			cur->nxt[key]=til++;
			if(til-pul>101000)while(1);
			cur->nxt[key]->clear(m);
		}
		cur=cur->nxt[key];
	}
	cur->count++;
	if(cur->count>1)while(1);
}

void core_entry(node_t *cur){
	for(int i=cur->count; i>=1; i--)cur->cost[i]=i;
	cur->cap=cur->count;
	for(int i=0; i<26; i++){
		if(cur->nxt[i]!=NULL)
			core_entry(cur->nxt[i]);
		else continue;
		cur->cap+=cur->nxt[i]->cap;
		for(int j=cur->cap; j>=1; j--){
			for(int k=1; k<=cur->nxt[i]->cap && k<=j; k++){
				UPDMIN(cur->cost[j], cur->cost[j-k]+cur->nxt[i]->cost[k]+2);
			}
		}
	}
}
