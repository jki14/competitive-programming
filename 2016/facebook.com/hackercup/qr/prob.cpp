...
int main() {
	...
	for(int _T=1; _T<=T; _T++){
		...
		for(int p=0; p<n; p++){
			for(int i=0; i<3; i++){
				for(int j=0; j<3; j++){
					if(f[p][i][j]==inf)continue;
					// 00
					if(!(i==0 && c[0][p]=='X') && !(j==0 && c[1][p]=='X')){
						UPDMIN(f[p+1][c[0][p]!='X'?(i^(i&1)):1][c[1][p]!='X'?(j^(j&1)):1], f[p][i][j]);
					}
					// 10
					if(c[0][p]!='X' && !(j==0 && c[1][p]=='X')){
						UPDMIN(f[p+1][2][c[1][p]!='X'?j:1], f[p][i][j]+1);
					}
					// 01
					if(!(i==0 && c[0][p]=='X') && c[1][p]!='X'){
						UPDMIN(f[p+1][c[0][p]!='X'?i:1][2], f[p][i][j]+1);
					}
					// 11
					if(c[0][p]!='X' && c[1][p]!='X'){
						UPDMIN(f[p+1][2][2], f[p][i][j]+2);
					}
				}
			}
		}
