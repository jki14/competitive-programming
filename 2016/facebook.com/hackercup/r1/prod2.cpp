...
const int rnk[17] = { inf,  1,  2,inf,  3,inf,inf,inf,  5,inf,inf,inf,inf,inf,inf,inf,  9};

int n, g[N][N];
int nbs[N], nxt[E], dst[E], enm;
int vst[N], pnt[N], bit[N], msk;
int a[N], b[N];
...
int find(const int u){
	...
	for(int i=nbs[u]; ~i; i=nxt[i]){
		const int v=dst[i];
		if(!~pnt[v] || (vst[pnt[v]]!=msk && find(pnt[v]))){
			pnt[v]=u;
			return 1;
		}
	}
...
int next_combination(const int x){
	const int x_lowbit = x & -x;
	const int fund = x + x_lowbit;
	const int sup = ((fund ^ x) >> 2) / x_lowbit;
	return fund | sup;
}

int beat(const int lhs, const int rhs){
	lef.clear();rig.clear();
	for(int i=0; i<n; i++){
		if(lhs&bit[i]){
			lef.push_back(i);
	...
	for(int i=0; i<m; i++){
		for(int j=0; j<m; j++){
			if(g[lef[i]][rig[j]])add_edge(i, j);
		}
	}
	//printf("beat(%x, %x) ", lhs, rhs);
	for(int i=0; i<m; i++){
		msk++;
		if(!find(i)){
			//printf("failed\n");
...
void core_entry(const int s, const int m){
	//printf("core_entry(%x, %d)\n", s, m);
	for(int i=0; i<n; i++){
		if(s&bit[i]){
			UPDMIN(a[i], rnk[m]);
		}
	...
	if(m==1)return;
	for(int k=bit[m>>1]-1; k<bit[m]; k=next_combination(k)){
		int k2=0, cur=1;
		for(int i=1; i<=k; i<<=1){
			while(!(cur&s))cur<<=1;
			if(k&i)k2|=cur;
