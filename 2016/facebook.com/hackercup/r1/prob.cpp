...
class object_t{
	public:
	lld til, det;
	object_t(const lld lhs=0LL, const lld rhs=0LL){
		this->til=lhs;
		this->det=rhs;
	}
	object_t & next(){
	...
	bool operator < (const object_t &rhs) const{
		if(this->til!=rhs.til)return this->til>rhs.til;
		return this->det>rhs.det;
	}
};

priority_queue<object_t> pq, dq;
...
int main() {
	...
	for(int _T=1; _T<=T; _T++){
		printf("Case #%d: ", _T);
		scanf("%d%d%d%lld", &n, &m, &p, &d);
		while(!pq.empty())pq.pop();
		for(int i=0; i<m; i++){
			int buf;
			scanf("%d", &buf);
			pq.push(object_t(buf*1LL, buf*1LL));
		}
		...
		for(int i=0; i<n; i++){
			object_t cur=pq.top();
			pq.pop();
			while(!dq.empty() && dq.top().til<=cur.til+d)dq.pop();
			object_t nxt(cur.til+d*1LL, d*1LL);
			if(SIZE(dq)==p){
				nxt=dq.top();
				dq.pop();
			}
