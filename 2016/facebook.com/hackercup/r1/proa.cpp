...
int main() {
	...
	for(int _T=1; _T<=T; _T++){
		...
		memset(f, 0x3f, sizeof(f));f[0][3]=0;
		for(int i=0; i<n; i++){
			for(int j=0; j<4; j++){
				if(f[i][j]>=inf)continue;
				for(int k=0; k<=6; k++){
					if(j+k<4){
						if(i!=0 && a[i-1]+k>100)continue;
						if(j+k<3){
							if(i==0 && k>=a[i])continue;
							if(i!=0 && a[i-1]+k>=a[i])continue;
							if(i!=0 && a[i]-a[i-1]>10*(k+1))continue;
							UPDMIN(f[i+1][j+k+1], f[i][j]+k);
						}else{
							UPDMIN(f[i+1][0], f[i][j]+k);
						}
					}else{
						if(i!=0 && a[i-1]+(3-j)>100)continue;
						int k2=k-(3-j);
						if(k2>3 || k2>=a[i])continue;
						UPDMIN(f[i+1][k2], f[i][j]+k);
					}
				}
			}
		}
		ans=inf;
		for(int j=0; j<4; j++)UPDMIN(ans, f[n][j]+(3-j));
		printf("%d\n", ans);
	}
	return 0;
}
