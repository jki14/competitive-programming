...
long double core_entry(const lld cap){
	const lld com=cap/s[n];
	const lld til=cap%s[n];
	long double buf=0.0L;
	for(int i=1; i<=n; i++){
		buf+=(s[i]-s[i-1])*(s[i]-s[i-1])*0.5L*com;
		if(til<s[i-1])continue;
		if(til>=s[i]){
			buf+=(s[i]-s[i-1])*(s[i]-s[i-1])*0.5L;
		}else{
			buf+=(til-s[i-1])*(til-s[i-1])*0.5L;
		}
	}
	return buf;
}
