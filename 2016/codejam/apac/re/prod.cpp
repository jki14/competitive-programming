...
int n, p, a[N];
int low[N][N], hig[N][N];
int pl[N][N], ph[N][N], sl[N][N], sh[N][N];
int f[N][N];

int runBA(){
    int foo=f[0][n-1];
    for(int i=0; i<n; i++){
        if(i==0 || (low[0][i-1]==0 && low[0][i-1]-hig[0][i-1]==1-i)){
            int f0=(i!=0)?f[0][i-1]:0;
            for(int j=i; j<n; j++){
                if(low[i][j]-hig[i][j]!=i-j)continue;
                int f1=f0+1;
                int z=n-1;
                if(hig[i][j]!=n-1){
                    int p=MIN(sl[n-1][hig[i][j]+1], sh[n-1][n-1]);
                    if(!~p)continue;
                    f1+=f[p][n-1];
                    z=p-1;
                }
                int x=j+1;
                int y=ph[x][low[i][j]-1];
                if(!~y)continue;
                f1+=1;
                if(y+1<=z){
                    if(low[y+1][z]-hig[y+1][z]!=y+1-z)continue;
                    f1+=f[x][y];
                }else{
                    if(y>z)continue;
                }
        ...
        if(i==0 || (low[0][i-1]==0 && low[0][i-1]-hig[0][i-1]==1-i)){
            ...
            for(int j=i; j<n; j++){
                ...
                if(x<=y-1){
                    ...
                    f1+=1;
                    if(b+1<=c){
                        if(low[b+1][c]-hig[b+1][c]!=b+1-c)continue;
                        f1+=f[a][b];
                    }
                }else continue;
                if(y<=z){
...
int runCAB(){
    int foo=f[0][n-1];
    for(int j=0; j<n; j++){
        if(j==n-1 || (hig[j+1][n-1]==n-1 && low[j+1][n-1]-hig[j+1][n-1]==j-n+2)){
            int f0=(j!=n-1)?f[j+1][n-1]:0;
            for(int i=0; i<=j; i++){
                if(low[i][j]-hig[i][j]!=i-j)continue;
                //printf("\ni=%d j=%d\n", i, j);
                int f1=f0+1;
                ...
                int x=0;
                if(low[i][j]!=0){
                    int p=MAX(ph[0][low[i][j]-1], pl[0][0]);
                    if(!~p)continue;
                    f1+=f[0][p];
                    x=p+1;
                }
                //printf("x=%d\n", x);
                int y=sh[z][(j!=n-1)?(low[j+1][n-1]-1):(n-1)];
                if(!~y)continue;
                //printf("y=%d\n", y);
                if(x<=y-1){
                    int a=x;
                    ...
                    f1+=1;
                    if(a<=b-1){
                        if(low[a][b-1]-hig[a][b-1]!=a-b+1)continue;
                        f1+=f[b][c];
                    }
                }else continue;
                if(y<=z){
...
int main(){
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        memset(hig, 0xc0, sizeof(hig));
        for(int k=0; k<n; k++){
            for(int i=0; i+k<n; i++){
                if(!k){
                    low[i][i]=hig[i][i]=a[i];
                }else{
                    low[i][i+k]=MIN(low[i][i+k-1], a[i+k]);
                    hig[i][i+k]=MAX(hig[i][i+k-1], a[i+k]);
                }
            }
        }
        ...
        for(int k=0; k<n; k++){
            for(int i=0; i+k<n; i++){
                int j=i+k;
                if(low[i][j]-hig[i][j]==i-j){
                    if(!~pl[i][low[i][j]])pl[i][low[i][j]]=j;
                    if(!~ph[i][hig[i][j]])ph[i][hig[i][j]]=j;
                    if(!~sl[j][low[i][j]])sl[j][low[i][j]]=i;
                    if(!~sh[j][hig[i][j]])sh[j][hig[i][j]]=i;
                    f[i][j]=1;
                    UPDMAX(f[i][j], 1+f[pl[i][low[i][j]]+1][j]);
                }
            }
        }
        if(p==2){
