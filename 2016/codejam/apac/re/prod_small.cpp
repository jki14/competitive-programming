...
int main() {
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        for(int i=0; i<n; i++){
            ...
            for(int k=1; i+k<n; k++){
                int j=i+k;
                low[i][j]=MIN(low[i][j-1], a[j]);
                hig[i][j]=MAX(hig[i][j-1], a[j]);
            }
        }
        ...
        for(int k=0; k<n; k++){
            for(int i=0; i+k<n; i++){
                int j=i+k;
                if(hig[i][j]-low[i][j]==j-i){
                    f[i][j]=1;
                    if(!~g[i] || hig[i][g[i]]<hig[i][j])g[i]=j;
                    if(!~r[i] || low[i][r[i]]>low[i][j])r[i]=j;
                    if(r[i]!=j){
                        UPDMAX(f[i][j], f[i][r[i]]+f[r[i]+1][j]);
                    }
                    red[low[i][j]][hig[i][j]]=i;
                }else{
                    f[i][j]=-inf;
                }
            }
        ...
        for(int i=0; i<n; i++){
            ...
            for(int j=i; j<n; j++){
                ...
                int bw=0;
                if(hig[i][j]!=n-1){
                    rig=red[hig[i][j]+1][n-1]-1;
                    if(rig<mid)continue;
                    bw=f[rig+1][n-1];
                }
                //printf("%d->%d(%d->%d)%d->%d(%d->%d)%d->%d\n", 0, i-1, i, j, lef, mid, mid+1, rig, rig+1, n-1);
                if(mid<rig){
                    if(f[mid+1][rig]<0)continue;
                    if(low[lef][mid]==hig[mid+1][rig]+1 && low[mid+1][rig]==(i>0?hig[0][i-1]+1:0) && low[i][j]==hig[lef][mid]+1){
                        //printf("update");
                        UPDMAX(ans, f[lef][mid]+fw+bw+2);
                    }
                }else{
                    if(low[lef][rig]==(i>0?hig[0][i-1]+1:0)){
                        UPDMAX(ans, fw+bw+2);
                    }
                }
            }
