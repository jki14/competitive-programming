...
class solver_c{
	private:
	int n, ans;
	vector<int> arr, vst, grp, cnt, buf;
	deque<int> que;
	vector<pii> f;
	void make_que(const int u){
		vst[u]=1;
		if(!vst[arr[u]])
			make_que(arr[u]);
		que.push_back(u);
	}
	void make_grp(const int u, const int p){
		grp[u]=p;
		if(!~grp[arr[u]])
			make_grp(arr[u], p);
	}
	void calc(const int u){
		if(cnt[grp[u]]>2){
			...
			if(!~f[arr[u]].first)
				calc(arr[u]);
			f[u].first=f[arr[u]].first;
			f[u].second=f[arr[u]].second+1;
		}
	}
	public:
	int case_id;
	...
	void core_entry(FILE *fhook){
		while(!que.empty())que.pop_back();
		vst.resize(n);for(vector<int>::iterator it=vst.begin(); it!=vst.end(); it++)*it=0;
		grp.resize(n);for(vector<int>::iterator it=grp.begin(); it!=grp.end(); it++)*it=-1;
		cnt.resize(n);for(vector<int>::iterator it=cnt.begin(); it!=cnt.end(); it++)*it=0;
		buf.resize(n);for(vector<int>::iterator it=buf.begin(); it!=buf.end(); it++)*it=0;
		f.resize(n);for(vector<pii>::iterator it=f.begin(); it!=f.end(); it++)it->first=-1;
		for(int u=0; u<n; u++){
			if(vst[u])continue;
			make_que(u);
		}
		...
		//printf("ans=%d\n", ans);
		for(int u=0; u<n; u++){
			if(~f[u].first)continue;
			calc(u);
		}
		for(int u=0; u<n; u++)
			UPDMAX(buf[f[u].first], f[u].second);
		int tmp=0;
		for(vector<int>::iterator it=buf.begin(); it!=buf.end(); it++)tmp+=*it;
		for(int u=0; u<n; u++){
			if(u==grp[u] || cnt[grp[u]]!=2)continue;
			if(buf[u] && buf[grp[u]])
				tmp-=2;
		}
		UPDMAX(ans, tmp);
		fprintf(fhook, "Case #%d: %d\n", case_id, ans);
	}
};
...
void *multithread_handler(void *key){
	manager_t *manager=(manager_t*)key;
	solver_c *solver= new solver_c();
	int e;
	char fname[fname_len];
	while(1){
		if((e=pthread_mutex_lock(&manager->mutex))!=0){
			fprintf(stderr, "pthread_mutex_lock() failed with %d\n", e);
		}
		manager->case_cur++;
		if(manager->case_cur>manager->case_num){
			if((e=pthread_mutex_unlock(&manager->mutex))!=0){
				fprintf(stderr, "pthread_mutex_unlock() failed with %d\n", e);
			}
			break;
		}
		solver->case_id=manager->case_cur;
		solver->input();
		if((e=pthread_mutex_unlock(&manager->mutex))!=0){
			fprintf(stderr, "pthread_mutex_unlock() failed with %d\n", e);
		}
		sprintf(fname, "multithread_output_case_%d.mtout", solver->case_id);
		FILE *fhook=fopen(fname, "w");
		solver->core_entry(fhook);
		fclose(fhook);
	}
	delete solver;
	pthread_exit(NULL);
...
int main(){
	...
	for(i=0; i<thread_num; i++){
		if((e=pthread_create(&pthread[i], NULL, multithread_handler, (void*)&manager))!=0){
			fprintf(stderr, "pthread_create() failed with %d\n", e);
		}
	}
	for(i=0; i<thread_num; i++){
		if((e=pthread_join(pthread[i], NULL))!=0){
			fprintf(stderr, "pthread_join() failed with %d\n", e);
		}
	}
	for(i=1; i<=manager.case_num; i++){
		sprintf(fname, "multithread_output_case_%d.mtout", i);
		FILE *fhook=fopen(fname, "r");
		while(fgets(buff, buff_size, fhook)!=NULL){
			fputs(buff, stdout);
		}
		fclose(fhook);
		unlink(fname);
