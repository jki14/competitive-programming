...
void core_entry(){
	int pos=0;
	for(int i=0; i<10; i++){
		while(cnt[tab[i].first-'A']){
			ans[pos++]='0'+tab[i].second.first;
			for(string::const_iterator it=tab[i].second.second.begin(); it!=tab[i].second.second.end(); it++){
				cnt[*it-'A']--;
			}
		}
	}
