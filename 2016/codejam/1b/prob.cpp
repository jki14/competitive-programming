...
int n, pot, til[L], top[L];
char sc[L], sj[L];
char rc[L], rj[L];
lld ten[L], coder, jammer, pain, scr;

char fmt[B];

inline void update(const lld c, const lld j){
	if(llabs(c-j)<llabs(coder-jammer)){
		coder=c;
		jammer=j;
	}else if(llabs(c-j)==llabs(coder-jammer)){
		if(c<coder || (c==coder && j<jammer)){
			coder=c;
			jammer=j;
		}
	}
...
inline void update(char *rc, char *rj){
	lld c, j;
	reverse(rc, rc+n);
	reverse(rj, rj+n);
	sscanf(rc, "%lld", &c);
...
void lazy_flush(const int p, char *rc, char *rj){
	for(int i=p; i<n; i++){
		rc[i]=sc[i];rj[i]=sj[i];
		if(sc[i]!='?' && sj[i]!='?')continue;
		if(sc[i]=='?'){
			if(sj[i]=='?'){
				rc[i]=rj[i]='0';
			}else rc[i]=rj[i];
...
int main(){
	...
	for(int _T=1; _T<=T; _T++){
		...
		pain=0LL;
		for(int i=0; i<n; i++){
			if(sc[i]!='?' && sj[i]!='?' && sc[i]!=sj[i]){
				pot=i;
				pain+=(sc[i]-sj[i])*ten[i];
			}
		}
		//REDUCE
		rc[n]=rj[n]=0;
		...
		for(int i=0; i<=pot; i++){
			...
			if(sc[i]!='?' && sj[i]!='?')continue;
			lld bst=inf;
			for(int x=(sc[i]=='?'?0:(sc[i]-'0')); x<=(sc[i]=='?'?9:(sc[i]-'0')); x++){
				for(int y=(sj[i]=='?'?0:(sj[i]-'0')); y<=(sj[i]=='?'?9:(sj[i]-'0')); y++){
					lld cur=(x-y)*ten[i];
					if(llabs(scr+cur)<llabs(bst)){
						rc[i]=x+'0';
						rj[i]=y+'0';
						bst=scr+cur;
					}
		...
		for(int i=0; i<=pot; i++){
			...
			for(int x=(sc[i]=='?'?0:(sc[i]-'0')); x<=(sc[i]=='?'?9:(sc[i]-'0')); x++){
				for(int y=(sj[i]=='?'?0:(sj[i]-'0')); y<=(sj[i]=='?'?9:(sj[i]-'0')); y++){
					lld cur=(x-y)*ten[i];
					if(bst==inf || llabs(scr+cur)>llabs(bst)){
						rc[i]=x+'0';
						rj[i]=y+'0';
						bst=scr+cur;
					}
		...
		for(int i=pot+1; i<n; i++){
			...
			for(int x=(sc[i]=='?'?0:(sc[i]-'0')); x<=(sc[i]=='?'?9:(sc[i]-'0')); x++){
				for(int y=(sj[i]=='?'?0:(sj[i]-'0')); y<=(sj[i]=='?'?9:(sj[i]-'0')); y++){
					if((scr>0 && x-y==-1) || (scr<0 && x-y==1)){
						rc[i]=x+'0';
						rj[i]=y+'0';
						lazy_flush(i+1, rc, rj);
						update(rc, rj);
					}
				}
			}
