...
int n, m, k, cnt;
int g[N][N], pnt[N], chk[N];

map<string, int> lef, rig;

inline int gidx(map<string, int> &dic, const string &s){
	map<string, int>::const_iterator it=dic.find(s);
	if(it!=dic.end()){
		return it->second;
	}else dic.insert(make_pair(s, SIZE(dic)));
	return dic[s];
}
