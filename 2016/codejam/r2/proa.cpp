...
vector<int> go(const int k, const int n){
    vector<int> buf;
    if(!n){
        buf.push_back(k);
        return buf;
    }
    vector<int> lef=go(k, n-1);
    vector<int> rig=go((k+1)%3, n-1);
    if(compare(lef, rig)<0){
        for(vector<int>::const_iterator it=lef.begin(); it!=lef.end(); it++){
            buf.push_back(*it);
        }
        for(vector<int>::const_iterator it=rig.begin(); it!=rig.end(); it++){
            buf.push_back(*it);
        }
    }else{
        for(vector<int>::const_iterator it=rig.begin(); it!=rig.end(); it++){
...
int check(const vector<int> &ans, const vector<int> &a){
    vector<int> s(3, 0);
...
int main() {
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        vector<int> ans;
        for(int i=0; i<3; i++){
            vector<int> buf=go(i, n);
            if(!check(buf, a))continue;
            if(ans.empty() || compare(buf, ans)<0)
                ans=buf;
        }
        if(ans.empty()){
            printf("IMPOSSIBLE\n");
