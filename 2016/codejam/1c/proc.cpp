...
int main() {
	...
	for(int _T=1; _T<=T; _T++){
		...
		for(int i=0; i<MIN(a[0], m); i++)
			c[0][0][i]=1;
		for(int i=1; i<MIN(MAX(a[0], m), a[1]); i++){
			for(int j=0; j<a[0]; j++){
				int p=((j-1)+a[0])%a[0];
				c[0][i][j]=c[0][i-1][p];
			}
		}			
		for(int i=1; i<MIN(MAX(a[1], m), a[2]); i++){
			for(int x=0; x<a[1]; x++){
				for(int y=0; y<a[0]; y++){
					int p=((x-1)+a[1])%a[1];
					//int q=((y-1)+a[0])%a[0];
		...
		/*if(ans!=MIN(m, a[2])*a[0]*a[1]){
			fprintf(stderr, "Case #%d: %d(%d)\n", _T, ans, MIN(m, a[2])*a[0]*a[1]);
		}*/
		...
		for(int i=0; i<a[2]; i++){
			for(int j=0; j<a[1]; j++){
				for(int k=0; k<a[0]; k++){
					if(!c[i][j][k])continue;
					printf("%d %d %d\n", k+1, j+1, i+1);
					xy[k*10+j]++;if(xy[k*10+j]>m)printf("Er!\n");
					xz[k*10+i]++;if(xz[k*10+i]>m)printf("Er!\n");
					yz[j*10+i]++;if(yz[j*10+i]>m)printf("Er!\n");
				}
			}
		}
	}
