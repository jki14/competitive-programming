...
void build(node_t **node, const int lef, const int rig){
    *node=tail++;
    (*node)->lef=lef;
    (*node)->rig=rig;
    if(lef==rig){
        ...
        (*node)->gcd=GCD((*node)->chd[0]->gcd, (*node)->chd[1]->gcd);
    }
}

int query(node_t *node, const int lef, const int rig){
    if(rig<node->lef || lef>node->rig)return 0;
    if(lef<=node->lef && node->rig<=rig){
        return node->gcd;
    }else{
        int res=0;
        if(lef<=node->chd[0]->rig){
            res=query(node->chd[0], lef, rig);
        }
        if(rig>=node->chd[1]->lef){
            if(res==0){
                res=query(node->chd[1], lef, rig);
            }else res=GCD(res, query(node->chd[1], lef, rig));
        }
        return res;
    }
}
...
int main() {
    ...
    for(int _T=1; _T<=T; _T++){
        ...
        build(&root, 0, n-1);
        cnt.clear();
        deque<pair<int, lld> > sp, sq;
        for(int i=n-1; ~i; i--){
            sp.clear();
            sp.push_back(MP(a[i], 1LL));
            for(;!sq.empty();sq.pop_back()){
                int cur=GCD(sq.back().first, a[i]);
                if(cur==sp.back().first){
                    sp.back().second+=sq.back().second;
                }else sp.push_back(MP(cur, sq.back().second));
            }
            for(deque<pair<int, lld> >::iterator it=sp.begin(); it!=sp.end(); it++){
