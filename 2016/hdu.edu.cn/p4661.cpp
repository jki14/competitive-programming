...
void first_entry(const int u, const int anc){
	f[u]=1;c[u]=0;
...
void second_entry(const int u, const int anc){
	g[u]=f[u];
	if(~anc){
		int buf=g[anc], cnt=n-1;
		cnt-=c[u]+1;
		MOD_MUL(&buf, INVERSEE(f[u], MOD));
		MOD_MUL(&buf, q[cnt+c[u]+1]);
		MOD_MUL(&buf, p[cnt]);
		MOD_MUL(&buf, p[c[u]+1]);
		MOD_MUL(&g[u], buf);
		MOD_MUL(&g[u], p[c[u]+cnt+1]);
		MOD_MUL(&g[u], q[c[u]]);
		MOD_MUL(&g[u], q[cnt+1]);
	}
	//printf("g[%d]=%d\n", u, g[u]);
...
int main(){
	for(int i=p[0]=1; i<N; i++)p[i]=MOD_MUL(p[i-1], i);
	for(int i=0; i<N; i++)q[i]=INVERSEE(p[i], MOD);
	int T;scanf("%d", &T);
	while(T--){
		scanf("%d", &n);
