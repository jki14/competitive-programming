...
int n, m, a[S];;
char sa[N], sb[N];
complex_t<double> x[S], y[S];

int main(){
	while(scanf("%s%s", sa, sb)!=EOF){
		reverse(sa, sa+strlen(sa));
		reverse(sb, sb+strlen(sb));
		m=strlen(sa)+strlen(sb);
		for(n=1; n<=m;)n<<=1;
		for(m=0; sa[m]; m++){
			x[m].r=(sa[m]-'0')*1.0;
			x[m].i=0.0;
		}
		for(;m<n; m++)x[m].r=x[m].i=0.0;
		for(m=0; sb[m]; m++){
			y[m].r=(sb[m]-'0')*1.0;
			y[m].i=0.0;
		}
		for(;m<n; m++)y[m].r=y[m].i=0.0;
		fast_fourier_trans(x, n, 1);
		fast_fourier_trans(y, n ,1);
		for(int i=0; i<n; i++)x[i]=x[i]*y[i];
		fast_fourier_trans(x, n, 0);
		memset(a, 0, sizeof(a));
		for(int i=0; i<n; i++){
			a[i]+=(int)(x[i].r+0.5);
			a[i+1]+=a[i]/10;
