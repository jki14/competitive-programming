...
int nbs[C], nxt[E], dst[E], buf[C], vst[C], enm;

void add_edge(const int u, const int v){
	nxt[enm]=nbs[u];nbs[u]=enm;dst[enm++]=v;
}

int find_entry(const int u, const int msk){
	for(int i=nbs[u]; ~i; i=nxt[i]){
		int v=dst[i];
		if(vst[v]==msk)continue;
		vst[v]=msk;
		if(!~buf[v] || find_entry(buf[v], msk)){
			buf[v]=u;
			return 1;
		}
	}
...
int judge_entry(const int cap){
	//memset(nbs, 0xff, sizeof(nbs));enm=0;
	for(int i=0; i<n; i++)nbs[i]=-1;enm=0;
	for(int i=0; i<n; i++){
		for(int j=0; j<n; j++){
			if(rnk[i]<rnk[j] && g[i][j]<=cap){
				//printf("add_edge %d->%d\n", i, j);
				add_edge(i, j);
			}
		}
...
int core_entry(){
	...
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(judge_entry(pul[mid])){
			if(mid==0 || !judge_entry(pul[mid-1]))return pul[mid];
			rig=mid-1;
		}else lef=mid+1;
	}
	while(1);
...
int main() {
	...
	while(T--){
		...
		for(int u=0; u<s; u++){
			...
			for(int v=u+1; v<s; v++){
				f[u][v]=f[v][u]=1e7;
				//if(u>=n && v>=n+m && u-n==v-n-m)continue;
				point_t<double> p=(u<n)?a[u]:((u<n+m)?b[u-n].p:b[u-n-m].q);
				point_t<double> q=(v<n)?a[v]:((v<n+m)?b[v-n].p:b[v-n-m].q);
				segment_t<double> cur(p ,q);
				int flag=1;
				for(int i=0; i<m; i++){
					if(cur.intersection(b[i], 1)){
						flag=0;
						break;
					}
				}
				if(flag){
					f[u][v]=f[v][u]=cur.length();
				}
			}
		}
		for(int k=0; k<s; k++){
		...
		for(int i=til=0; i<n; i++){
			for(int j=0; j<n; j++){
				g[i][j]=round(f[i][j]*100.0);
				if(g[i][j]<0)while(1);
				pul[til++]=g[i][j];
			}
		}
		sort(pul, pul+til);
		til=unique(pul, pul+til)-pul;
		printf("%.2f\n", core_entry()*0.01);
	}
	return 0;
}
