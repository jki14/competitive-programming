...
static inline int cmp(const point_t<double> &x, const point_t<double> &y){
	if(fsign((x-a[0])^(y-a[0])))
		return fsign((x-a[0])^(y-a[0]))>0;
	else return a[0].dist2(x)<a[0].dist2(y);
}

void core_entry(){
	sort(a+1, a+n, cmp);
	for(int i=2; i<n; i++){
		if(fsign((a[i]-a[0])^(a[1]-a[0]))){
			printf("%d %d %d\n", a[0].idx, a[1].idx, a[i].idx);
			return;
		}
	}
	while(1);
