...
class hash_t{
	public:
	lld hig, low;
	hash_t(const lld lhs=0LL, const lld rhs=0LL){
		this->hig=lhs;
		this->low=rhs;
	}
	bool operator < (const hash_t &rhs) const{
		if(this->hig!=rhs.hig)return this->hig<rhs.hig;
	...
	while(scanf("%d%d", &n, &k)!=EOF){
		...
		for(int i=0; i<m; i++){
			...
			for(int j=0; j<len; j++){
				...
				vst[idx]=1;
				if(idx>=50){
					fresh.hig |= 1LL<<(idx-50);
				}else fresh.low |= 1LL<<idx;
			}
			UPDMAX(hig, sum);
			UPDMIN(low, sum);
			buk.insert(fresh);
		...
		if(SIZE(buk)<k){
			...
			if(til>=len){
				...
				UPDMIN(low, sum);
				sum=0;
				for(int i=0; i<len; i++)sum+=b[til-i-1];
				UPDMAX(hig, sum);
			}
		}
		printf("%.9f %.9f\n", low*1.0/len, hig*1.0/len);
	}
	return 0;
}
