    ...
    while(cin>>n>>m){
        ...
        for(int i=0; i<m; i++){
            ...
            if(lef!=rig){
                ...
                base+=r[i].w*1LL;
                root=n+i;
                chd[root].push_back(lef);
                chd[root].push_back(rig);
            }else{
                edge[r[i].u].push_back(i);
                edge[r[i].v].push_back(i);
            }
        }
        ...
        for(int i=0; i<m; i++){
            lld foo=base-r[r[i].d].w+r[i].w-budget/r[i].c;
            if(ans>foo){
                ans=foo;
                ace=i;
            }
