...
int main() {
    while(scanf("%d", &n)!=EOF){
        ...
        for(int i=0; i<n; i++){
            ...
            it=s.find(key);
            if(it!=s.end()){
                int foo=MIN(a[i][0], a[i][1]);
                UPDMIN(foo, a[i][2]+it->second.second);
                if(foo>cur){
                    cur=foo;
                    ans.resize(2);
            ...
            it=s.find(key);
            if(it!=s.end()){
                if(it->second.second<a[i][2]){
                    it->second.first=i+1;
                    it->second.second=a[i][2];
                }
            }else s.insert(MP(key, MP(i+1, a[i][2])));
            key=MP(a[i][0], a[i][2]);
            it=s.find(key);
            if(it!=s.end()){
                if(it->second.second<a[i][1]){
                    it->second.first=i+1;
                    it->second.second=a[i][1];
                }
            }else s.insert(MP(key, MP(i+1, a[i][1])));
            key=MP(a[i][1], a[i][2]);
            it=s.find(key);
            if(it!=s.end()){
                if(it->second.second<a[i][0]){
                    it->second.first=i+1;
                    it->second.second=a[i][0];
                }
            }else s.insert(MP(key, MP(i+1, a[i][0])));
        }
        printf("%d\n", SIZE(ans));
        if(SIZE(ans)==1){
            printf("%d\n", ans[0]);
