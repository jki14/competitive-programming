...
class road_t{
    ...
    int w, c, d;
    int u, v, idx;
    bool operator < (const road_t &rhs) const{
        return this->w<rhs.w;
    }
} r[M];
...
class cool_t{
    public:
    map<int, int, greater<int> > foo;
    cool_t(){
        foo.clear();
        foo.insert(MP(-inf, -inf));
    }
    void set(const int flag, const int value){
        foo[flag]=value;
    }
    pii get(const int flag){
        map<int, int, greater<int> >::iterator it=foo.lower_bound(flag);
...
inline pii find(const int u, const int flag){
    pii foo=f[u].get(flag);
    if(foo.second!=u){
        pii patch=find(foo.second, flag);
        f[u].set(MAX(foo.first, patch.first), patch.second);
    }
    return f[u].get(flag);
}

inline int linked(const int u, const int v, const int flag){
    pii lef=find(u, flag);
    pii rig=find(v, flag);
    return lef.second==rig.second;
}

inline int keyedge(const int u, const int v, const int r){
    int lef=0, rig=r;
    while(lef<=rig){
        int mid=(lef+rig)>>1;
        if(!linked(u, v, mid)){
            if(mid==rig || linked(u, v, mid+1))return mid;
            lef=mid+1;
        }else{
            rig=mid-1;
        }
...
int main() {
    ...
    while(cin>>n>>m){
        ...
        sort(r, r+m);mst=0LL;til=0;
        for(int i=0; i<m; i++){
            pii lef=find(r[i].u, i);
            pii rig=find(r[i].v, i);
            if(lef.second!=rig.second){
                f[rig.second].set(i, lef.second);
                r[i].d=i;
                mst+=r[i].w*1LL;
                que[++til]=i;
            }else{
                r[i].d=keyedge(r[i].u, r[i].v, i);
            }
        }
        ans=0x3f3f3f3f3f3f3f3fLL;
        for(int i=0; i<m; i++){
            lld foo=mst+r[i].w-r[r[i].d].w-budget/r[i].c;
            if(ans>foo){
                ans=foo;
                ace=i;
            }
        ...
        cout<<ans<<endl;
        cout<<r[ace].idx<<" "<<r[ace].w-budget/r[ace].c<<endl;
        for(int i=1; i<n; i++){
            if(que[i]==r[ace].d)continue;
            cout<<r[que[i]].idx<<" "<<r[que[i]].w<<endl;
        }
    }
    return 0;
}
