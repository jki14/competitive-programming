...
vector<pair<int, char> > pic;

int n, m, a[N], b[N];

int core_entry(int p, int q){
    /*printf("p=%d b[%d]=%d\n", p, q, b[q]);
    for(int i=0; i<n; i++)
        printf(" %d", a[i]);
    printf("\n");*/
    ...
    for(; p<n && cnt<b[q]; p++){
        ...
        if(cnt==b[q]){
            if(single==0){
                return core_entry(p+1, q+1);
            }
            if(pos==p-single){
                while(pos<p && a[pos]==a[pos+1]){
                    pos++;
                }
            }
            //printf("pos=%d, p=%d\n", pos, p);
            if(pos>p-single && a[pos]>a[pos-1]){
                pic.push_back(MP(pos+1, 'L'));
                a[pos-1]+=a[pos];
                for(int i=pos; i<n-1; i++){
                    a[i]=a[i+1];
                }
                n--;
                return core_entry(p-single, q);
            }else if(pos<p && a[pos]>a[pos+1]){
                pic.push_back(MP(pos+1, 'R'));
                a[pos]+=a[pos+1];
