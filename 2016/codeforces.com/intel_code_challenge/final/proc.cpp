...
struct nod_t{
    int x, y;
    nod_t(){ this->x=this->y=0; }
    nod_t(const int x, const int y){
        this->x=x;
        this->y=y;
    }
    bool operator < (const nod_t &rhs) const{
...
struct ray_t{
    nod_t p, d;
    ray_t(){}
    ray_t(const int px, const int py, const int dx, const int dy){
        this->p.x=px;
        this->p.y=py;
        this->d.x=dx;
        this->d.y=dy;
    }
    ray_t(const nod_t &lhs, const int dx, const int dy){
        this->p=lhs;
        this->d.x=dx;
        this->d.y=dy;
    }
    ...
    lld next(){
        ...
        this->p.x+=this->d.x*k;
        this->p.y+=this->d.y*k;
        if(this->p.x==0 || this->p.x==n){
            this->d.x*=-1;
            cnt++;
        }
        if(this->p.y==0 || this->p.y==m){
            this->d.y*=-1;
            cnt++;
        }
        if(cnt>1)
    ...
    bool operator < (const ray_t &rhs) const{
        if(this->p<rhs.p || rhs.p<this->p)return this->p<rhs.p;
        return this->d<rhs.d;
    }
};

set<ray_t> uni;
map<ray_t, vector<int> > rut;
map<ray_t, vector<int> >::iterator it;

inline lld dist(const nod_t &lhs, const nod_t &rhs){ return MIN(abs(lhs.x-rhs.x), abs(lhs.y-rhs.y))*1LL; }
...
int main() {
    ...
    while(cin>>n>>m>>k){
        ...
        for(int i=0; i<k; i++){
            ...
            pnt[i].y=y;
            for(int r=0; r<4; r++){
                ray_t tmp(x, y, dx[r], dy[r]);
                ray_t cur(tmp.tail(), -dx[r], -dy[r]);
                //printf("%d,%d : %d,%d -> %d,%d\n", x, y, cur.p.x, cur.p.y, cur.d.x, cur.d.y);
                it=rut.find(cur);
                if(it!=rut.end()){
                    it->second.push_back(i);
                }else{
                    rut.insert(MP(cur, vector<int>(1, i)));
                }
            }
        }
        uni.clear();
        ray_t cur(0, 0, 1, 1);
        lld tim=0LL;
        while(uni.find(cur)==uni.end()){
            uni.insert(cur);
            //printf("r %d, %d -> %d, %d\n", cur.p.x, cur.p.y, cur.d.x, cur.d.y);
            it=rut.find(cur);
            if(it!=rut.end()){
                for(vector<int>::iterator it2=it->second.begin(); it2!=it->second.end(); it2++){
                    UPDMIN(ans[*it2], dist(cur.p, pnt[*it2])+tim);
                }
            }
            tim+=cur.next();
            if(tim<0)break;
