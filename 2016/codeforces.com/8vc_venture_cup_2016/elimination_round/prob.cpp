...
class state_t{
	public:
	int c[3];
	state_t(const int x=0, const int y=0, const int z=0){
		this->c[0]=x;
		this->c[1]=y;
		this->c[2]=z;
	}
	bool operator < (const state_t &rhs) const{
		if(this->c[0]!=rhs.c[0])return this->c[0]>rhs.c[0];
		if(this->c[1]!=rhs.c[1])return this->c[1]>rhs.c[1];
		return this->c[2]>rhs.c[2];
	}
	int count() const{
		int buf=0;
		for(int i=0; i<3; i++)buf+=this->c[i];
		return buf;
	}
};
...
int main() {
	while(scanf("%d", &n)!=EOF){
		...
		for(int i=1; i<n; i++){
			cur=prv;
			f[cur].clear();
			for(set<state_t>::const_iterator it=f[prv].begin(); it!=f[prv].end(); it++){
				for(int x=0; x<3; x++){
					for(int y=0; y<3; y++){
						if(x==y && it->c[x]>=2){
							state_t tmp=*it;
		...
		for(set<state_t>::const_iterator it=f[cur].begin(); it!=f[cur].end(); it++){
			if(it->c[0]){
				putchar('B');
			}else if(it->c[1]){
				putchar('G');
