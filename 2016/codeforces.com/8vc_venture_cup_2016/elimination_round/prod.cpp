...
complex_t<long double> fir[S], sec[S];
...
int main(){
	...
	while(scanf("%d", &n)!=EOF){
		...
		fast_fourier_trans(fir, til, 1);
		fast_fourier_trans(sec, til, 1);
		for(int i=0; i<til; i++){
			fir[i] = fir[i]*fir[i]*sec[i];
		}
		fast_fourier_trans(fir, til, 0);
		ans=0.0L;
		for(int i=0; i<til; i++){
			cnt[i]=(lld)(fir[i].r+0.5L);
			if(i<OFFSET*3){
				ans+=cnt[i]/buf;
			}
		}
