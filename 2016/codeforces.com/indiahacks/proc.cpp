...
inline int suit(const int pos, const int val){
	if(pos&1){
		//peak
		if((pos-1>=0 && val<=a[pos-1]) || (pos+1<n && val<=a[pos+1]))return 0;
	}else{
		//valley
		if((pos-1>=0 && val>=a[pos-1]) || (pos+1<n && val>=a[pos+1]))return 0;
	}
...
inline void two_check(const int i, const int j){
	if(i>j){
		two_check(j, i);
	}else{
		if(i+1==j){
			swap(a[i], a[j]);
			if(suit(i, a[i]) && suit(j, a[j])){
				ans.insert(MP(i, j));
			}
			swap(a[i], a[j]);
		}else{		
			if(suit(i, a[j]) && suit(j, a[i])){
				ans.insert(MP(i, j));
			}
		}
	}
...
inline void one_check(const int i){
	for(int j=0; j<n; j++){
		if(i==j)continue;
		two_check(i, j);
	}
...
void kp_entry(const int cnt, int pnt, const int msk){
	if(msk==(1<<m)-1){
		int x, y, tmp=pnt;
		if(cnt==1){
			for(x=0; tmp!=1; tmp>>=1)x++;
			one_check(bp[x]);
		}else{
			for(y=0; tmp!=1; tmp>>=1){
				if(tmp&1)x=y;
	...
	if(cnt==0){
		for(int i=0; i<m; i++){
			int cov=1<<i;
			if(i>0 && bp[i-1]==bp[i]-1)cov|=1<<(i-1);
			if(i<m-1 && bp[i+1]==bp[i]+1)cov|=1<<(i+1);
			kp_entry(cnt+1, pnt|(1<<i), msk|cov);
		}
	}
	if(cnt==1){
		int k, tmp=pnt;
...
int main() {
	while(scanf("%d", &n)!=EOF){
		...
		bp.clear();
		for(int i=0; i<n; i++){
			if(!suit(i, a[i]))bp.push_back(i);
		}
		m=SIZE(bp);
		ans.clear();
		if(m<=6)kp_entry(0, 0, 0);
		printf("%d\n", SIZE(ans));
