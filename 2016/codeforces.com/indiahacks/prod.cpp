...
int enough(const long double x){
	if(fsign(x)<=0)return 1;
	for(int i=1; i<num; i++){
		ec[i]=ef[i]=0;
		if(eu[i] && ev[i]){
			if(ee[i]){
				lld tmp=(lld)(ee[i]*1.0L/x);
				if(tmp>foo)
					ec[i]=foo;
				else ec[i]=tmp;
			}
...
long double bs_entry(){
	long double low=0.0L, hig=50000000.0L;
	while(fsign(low-hig)<=0){
		long double mid=(low+hig)*0.5L;
		//printf("low=%.9Lf hig=%.9Lf mid=%.9Lf\n", low, hig, mid);
		if(enough(mid)){
			if(fsign(mid-hig)>=0 || !enough(mid+eps))
				return mid;
			low=mid+eps;
		}else hig=mid-eps;
	}
...
int main() {
	while(scanf("%d%d%d", &n, &m, &foo)!=EOF){
		...
		AddEdge(0, 1, foo);
		double buf=(double)(bs_entry()*foo);
		printf("%.9f\n", buf);
	}
	return 0;
}
