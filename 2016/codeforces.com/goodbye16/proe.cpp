...
const char grow[6]="2017?";
const char kill[6]="???66";
...
class foo_t{
    ...
    foo_t(const char chr){
        this->set(chr);
    }
    int* operator [](const int row){
        return data[row];
    }
    const foo_t operator ^(const foo_t &rhs) const{
        foo_t res;
        memset(res.data, 0x3f, sizeof(res.data));
        for(int i=0; i<5; i++){
            for(int j=i; j<5; j++){
                for(int k=i; k<=j; k++){
                    UPDMIN(res.data[i][j], this->data[i][k]+rhs.data[k][j]);
                }
            }
        }
        return res;
    ...
    void clear(){
        memset(this->data, 0x3f, sizeof(this->data));
        for(int i=0; i<5; i++)this->data[i][i]=0;
    }
    void set(const char chr){
        memset(this->data, 0x3f, sizeof(this->data));
        for(int i=0; i<5; i++){
            //keep
            data[i][i]=(chr==kill[i] || chr==grow[i])?1:0;
            //grow
            if(i+1<5){
                data[i][i+1]=(chr==grow[i])?0:inf;
            }
        }
    }
};
...
void build(segtree_t **node, const int low, const int hig){
    *node=tail++;
    (*node)->low=low;
    (*node)->hig=hig;
    if(low==hig){
        ...
        (*node)->foo=(*node)->lef->foo^(*node)->rig->foo;
    }
}

foo_t query(segtree_t *node, const int low, const int hig){
    if(hig<node->low || low>node->hig){
        foo_t foo;
        return foo;
    }
    if(low<=node->low && node->hig<=hig){
        ...
        return query(node->lef, low, hig)^query(node->rig, low, hig);
    }
}
...
int main() {
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<m; i++){
            int low, hig;
            scanf("%d%d", &low, &hig);
            int cache=query(root, low, hig)[0][4];
            if(cache!=inf){
                printf("%d\n", cache);
            }else printf("-1\n");
        }
