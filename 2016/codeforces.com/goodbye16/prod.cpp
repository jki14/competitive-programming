...
int n, a[31], ans;
char f[310][310][31][6][8];
char g[310][310];

void run(const int x, const int y, const int k, const int l, const int r){
    if(f[x][y][k][l][r])return;
    f[x][y][k][l][r]=1;
    //if(!g[x][y])printf("%d, %d\n", x-63, y-63);
    g[x][y]=1;
    if(l>0){
        run(x+dx[r], y+dy[r], k, l-1, r);
    }else{
        if(k+1<n){
            int lef=(r+1+8)%8;
            run(x+dx[lef], y+dy[lef], k+1, a[k+1]-1, lef);
            int rig=(r-1+8)%8;
            run(x+dx[rig], y+dy[rig], k+1, a[k+1]-1, rig);
        }
    }
}
