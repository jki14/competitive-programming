...
string s;
multimap<int,pis> a;
multimap<int,pis>::iterator it,jt;

const int mdays[13]={ 0,31,29,31,30,31,30,31,31,30,31,30,31 };

int sec(){
	int mo,da,hh,mm,ss,ret=0;
	sscanf(s.c_str(),"2012-%d-%d %d:%d:%d",&mo,&da,&hh,&mm,&ss);
	for(int i=1;i<mo;i++)ret+=mdays[i];
	ret=ret+da;
	ret=ret*24+hh;
	ret=ret*60+mm;
	ret=ret*60+ss;
...
int main(){a.clear();
	...
	while(gets(c)){
		if((int)strlen(c)<K)break;
		s.clear();
		for(int i=0;i<K;i++)s.push_back(c[i]);
		a.insert(_mkpr(sec(),_mkpr(0,s)));
	}
	for(it=a.begin();it!=a.end();it++)(*it).second.first=++p;
	for(it=a.begin();it!=a.end();it++){
		int cur=(*it).first-n;
		jt=a.upper_bound(cur);
		if(jt==a.end())continue;
		if((*it).second.first-(*jt).second.first+1>=m){
