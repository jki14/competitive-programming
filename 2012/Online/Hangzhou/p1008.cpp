...
int core(const int &u,const int &v,const int &k){
	int lef=1,rig=v-u+1;
	if(que(u,v,lef)>k)return 0;
	if(que(u,v,rig)<=k)return rig;rig--;
	while(lef<=rig){
		int mid=(lef+rig)>>1;
		if(que(u,v,mid)<=k){
			if(que(u,v,mid+1)>k)return mid;
			lef=mid+1;
		}else rig=mid-1;
	}
	return -1;
