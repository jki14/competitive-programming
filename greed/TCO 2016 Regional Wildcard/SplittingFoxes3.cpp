...
// CUT begin
ifstream data("SplittingFoxes3.sample");
...
bool do_test(int n, int k, vector<int> a, vector<int> b, vector<int> x, vector<int> __expected) {
    time_t startClock = clock();
    SplittingFoxes3 *instance = new SplittingFoxes3();
    vector<int> __result = instance->restore(n, k, a, b, x);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(n, k, a, b, x, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1504922573;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "SplittingFoxes3 (1000 Points)" << endl << endl;
    }
