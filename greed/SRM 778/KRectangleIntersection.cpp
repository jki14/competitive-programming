...
struct segment_t {
  int x, y;
  segment_t(int const _x, int const _y) {
    x = _x;
    y = _y;
  }
  bool operator < (segment_t const& rhs) const {
...
class KRectangleIntersection {
  ...
  long long maxIntersection(vector<int> xl, vector<int> yl, vector<int> xh, vector<int> yh, int k) {
    ...
    segs.reserve(n);
    for (int i = 0; i < n; ++i) {
      bars.emplace_back(yl[i]);
      bars.emplace_back(yh[i]);
    }
    sort(bars.begin(), bars.end());
    unique(bars.begin(), bars.end());
    for (int low : bars) {
      for (int hig : bars) {
        if (low >= hig) continue;
        segs.clear();
        for (int i = 0; i < n; ++i) {
          if (yl[i] > low || yh[i] < hig) continue;
          segs.emplace_back(xl[i], xh[i]);
        }
        sort(segs.begin(), segs.end());
        while (!pq.empty()) pq.pop();
        for (auto const& el : segs) {
          pq.push(el.y);
          while (pq.top() <= el.x) pq.pop();
          while (pq.size() > static_cast<size_t>(k)) pq.pop();
          if (pq.size() == static_cast<size_t>(k)) {
            foo = max((pq.top() - el.x) * 1LL * (hig - low), foo);
          }
        }
      }
    }
