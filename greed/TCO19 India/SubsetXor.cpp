...
// CUT begin
ifstream data("SubsetXor.sample");
...
bool do_test(vector<long long> A, long long __expected) {
    time_t startClock = clock();
    SubsetXor *instance = new SubsetXor();
    long long __result = instance->computeSmallest(A);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "SubsetXor (1000 Points)" << endl << endl;
    }
