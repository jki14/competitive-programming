...
lld const inf = 0x3f3f3f3f3f3f3f3fLL;

lld execute(lld const r, lld const c, lld const x, lld const y) {
...
class ChocolateBreaking {
  public:
  lld minDiff(lld r, lld c, lld p) {
    lld foo = inf;
    for (lld x = 1; p / x >= x; ++x) {
      if (p % x != 0) continue;
      lld const y = p / x;
      UPDMIN(foo, execute(r, c, x, y));
      UPDMIN(foo, execute(r, c, y, x));
    }
    if (foo == inf) {
      foo = -1;
    }
...
// CUT begin
ifstream data("ChocolateBreaking.sample");
...
bool do_test(int rows, int cols, long long pieces, long long __expected) {
    time_t startClock = clock();
    ChocolateBreaking *instance = new ChocolateBreaking();
    long long __result = instance->minDiff(rows, cols, pieces);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(rows, cols, pieces, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1564574160;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "ChocolateBreaking (500 Points)" << endl << endl;
    }
