...
// CUT begin
ifstream data("PreInPost.sample");
...
bool do_test(vector<string> s, vector<int> a1, vector<int> a2, string e1, string e2, vector<int> __expected) {
    time_t startClock = clock();
    PreInPost *instance = new PreInPost();
    vector<int> __result = instance->findMissing(s, a1, a2, e1, e2);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(s, a1, a2, e1, e2, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1496200572;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "PreInPost (1000 Points)" << endl << endl;
    }
