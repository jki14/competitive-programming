...
class MaximumPenalty {
  public:
  vector<int> schedule(vector<int> w, vector<int> c_) {
    n = w.size();
...
// CUT begin
ifstream data("MaximumPenalty.sample");
...
bool do_test(vector<int> L, vector<int> P, vector<int> __expected) {
    time_t startClock = clock();
    MaximumPenalty *instance = new MaximumPenalty();
    vector<int> __result = instance->schedule(L, P);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(L, P, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1613237990;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "MaximumPenalty (550 Points)" << endl << endl;
    }
