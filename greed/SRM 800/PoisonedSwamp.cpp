...
struct xyz_t {
  ...
  xyz_t(int const x_, int const y_, int const z_) {
    x = x_;
    y = y_;
    z = z_;
  }
...
class PoisonedSwamp {
  ...
  string cross(vector<string> a) {
    ...
    for (; !seq.empty(); seq.pop()) {
      int const x = seq.front().x;
      int const y = seq.front().y;
      int const z = seq.front().z;
      int const d = f[x][y][z];
      for (int i = 0; i < 4; ++i) {
        int const x_ = x + dx[i];
        int const y_ = y + dy[i];
        if (x_ < 0 || x_ >= h) continue;
        if (y_ < 0 || y_ >= w) continue;
        if (a[x_][y_] == '#') continue;
        int z_ = z;
        if (a[x_][y_] != '.') {
          if (a[x_][y_] == 'S') {
        ...
        if (z_ >= 10) continue;
        if (x_ == h - 1 && y_ == w - 1) return "possible";
        if (f[x_][y_][z_] > d + 1) {
          f[x_][y_][z_] = d + 1;
          seq.emplace(x_, y_, z_);
        }
      }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "PoisonedSwamp (250 Points)" << endl << endl;
    }
