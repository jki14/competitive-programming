...
class DivisibleSetDiv1 {
    ...
    string isPossible(vector<int> b) {
        ...
        for(int i=head; i<S; i++){
            if(check(i, 0, 0, i))return "Possible";
        }
        return "Impossible";
    }
    private:
    int check(const int r, const int p, const int f, const int s){
        int flag=0, cur=r;
        if(p==n-1){
            flag=(r*a[p]>=s && r>f)?1:0;
        }else{
            cur=s/a[p]+(((s%a[p])!=0)?1:0);
            UPDMAX(cur, f+1);
            if(cur>r){
                flag=0;
            }else{
                flag=check(r-cur, p+1, cur, s);
            }
        }
        return flag;
    }
