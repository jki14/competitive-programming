...
class XorOrderDiv1 {
    ...
    int get(int m, int n, int a0, int b) {
        for(int i=0; i<n; i++){
            a[i]=MOD_ADD(a0, MOD_MUL(i, b, (1<<m)), (1<<m));
        }
        sort(a, a+n);
        memset(f, 0, sizeof(f));
        memset(g, 0, sizeof(g));
        for(int k=m-1; ~k; k--){
            for(int i=0; i<n; ){
                int til=i;
                while(til<n && (a[til]>>(k+1))==(a[i]>>(k+1)))til++;
                int bar=i;
                while(bar<n && (a[bar]>>k)==(a[i]>>k))bar++;
                for(int p=i; p<bar; p++){
                    g[p]+=til-bar;
                    MOD_ADD(&f[p], MOD_MUL(til-bar, g[p]));
                }
                for(int p=bar; p<til; p++){
                    g[p]+=bar-i;
                    MOD_ADD(&f[p], MOD_MUL(bar-i, g[p]));
                }
                i=til;
            }
        }
