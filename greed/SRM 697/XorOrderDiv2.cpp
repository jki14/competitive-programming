...
class XorOrderDiv2 {
    ...
    vector<long long> getExpectation(int m, vector<int> a) {
        ...
        for(int i=0; i<n; i++){
            ...
            for(int j=m-1; j>=0; j--){
                int cil=i;
                while(cil<n && (arr[cil].first>>(j+1))==(arr[i].first>>(j+1)))cil++;
                int pos=i;
                while(pos<cil && (arr[pos].first>>j)==(arr[i].first>>j))pos++;
                lld cnt=cil-pos;
                /*cil=pos=i;
                while(cil>=0 && (arr[cil].first>>(j+1))==(arr[i].first>>(j+1)))cil--;
                while(pos>cil && (arr[pos].first>>j)==(arr[i].first>>j))pos--;
                cnt+=pos-cil;*/
                //printf("cnt=%lld\n", cnt);
                ans+=cnt*cnt*(1ll<<(m-1));
