...
class PreviousOccurrence {
  ...
  int findValue(int x, int y) {
    int cur = 0;
    for (int i = 0; i < 10000000; ++i) f[i] = -1;
    for (int pos = 0; pos < 10000000; ++pos) {
      if (cur == y) {
        return pos;
      }
      int nxt = ~f[cur] ? pos - f[cur] : x;
      f[cur] = pos;
      cur = nxt;
    }
    return -1;
...
// CUT begin
ifstream data("PreviousOccurrence.sample");
...
bool do_test(int defaultValue, int query, int __expected) {
    time_t startClock = clock();
    PreviousOccurrence *instance = new PreviousOccurrence();
    int __result = instance->findValue(defaultValue, query);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(defaultValue, query, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1580973776;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "PreviousOccurrence (250 Points)" << endl << endl;
    }
