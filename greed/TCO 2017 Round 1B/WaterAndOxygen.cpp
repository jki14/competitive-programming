...
class WaterAndOxygen {
    public:
    double maxDays(int rx, int ry, int cx, int cy) {
        double ax=rx*1.0/cx;
        double ay=ry*1.0/cy;
        double ans=MIN(ax, ay);
        if(ax>ay){
            double det=(ax-ay)*cx;
            double trn=det/((cx*1.0)/(cy*2.0)+1.0);
            double tx=rx*1.0-trn;
            double ty=ry*1.0+trn*0.5;
            ans=MIN(tx*1.0/cx, ty*1.0/cy);
        }
        return ans;
    }
};
