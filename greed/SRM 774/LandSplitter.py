...
def do_test(N, A, B, __expected):
    startTime = time.time()
    instance = LandSplitter()
    exception = None
    try:
        __result = instance.cheapest(N, A, B);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("LandSplitter (250 Points)\n\n")
    ...
    with open("LandSplitter.sample", "r") as f:
        while True:
            label = f.readline()
