...
inline double drate(const double c, const double m){
    return 1.0-((m-c)/m)*0.5;
}

inline int check(const double dmg){
    MATX<double> a(2), b(2);
    a[0][0]=c;a[0][1]=dmg*0.5;
    a[1][0]=0.0;a[1][1]=0.0;
    b[0][0]=1.0-dmg*0.5/m;b[0][1]=0.0;
    b[1][0]=-1.0;b[1][1]=1.0;
    a=a*(b.pow(n));
...
double core(){
    double lef=0.0, rig=c/drate(c, m), mid=-1.0;
    while(rig-lef>eps){
        mid=(lef+rig)*0.5;
        if(check(mid)){
            rig=mid;
...
double solution2(){
/* solution 2
 * damage per attack (1-((M-C)/2M)) * D => ((M+C)/2M)*D
 * let y = D/2M then damage per attack is (M+C)*y
 * let r = 1-y, the health point lose rate then (M+C)*r
 * (M+C)*(r^n) >= M
 * r^n >= M/(M+C)
 */
    double r=pow(m/(m+c), 1.0/n);
    double y=1-r;
    return 2.0*m*y;
}
