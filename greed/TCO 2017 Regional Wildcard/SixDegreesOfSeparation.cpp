...
int n, m;
int nbs[2100], nxt[4100], dst[4100], eid;
int anc[2100], low[2100];
int f[2100][8], g[2100][8];
int edg[2100][2100];
...
void process(const int u, const int a){
    anc[u]=a;
    ...
    for(int i=nbs[u]; ~i; i=nxt[i]){
        ...
        if(anc[v]!=u)continue;
        for(int y=2; y>=0; y--){
            if(!~low[v] || f[v][low[v]]>f[v][o+y])low[v]=o+y;
        }
        f[u][o-2]+=f[v][low[v]];
        g[v][o-2]=low[v];
    }
    ...
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(anc[v]!=u)continue;
        if(!~key || f[key][o+1]-f[key][low[key]]>f[v][o+1]-f[v][low[v]]){
            key=v;
        }
        g[v][o+0]=g[v][o-2];
    }
    if(~key){
        f[u][o+0]=f[u][o-2]+f[key][o+1]-f[key][low[key]];
        g[key][o+0]=o+1;
    }
    //-1
    f[u][o-1]=0;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(anc[v]!=u)continue;
        if(!~low[v] || f[v][low[v]]>f[v][o-2])low[v]=o-2;
        f[u][o-1]+=f[v][low[v]];
        g[v][o-1]=low[v];
    }
    //1
    ...
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(anc[v]!=u)continue;
        if(!~key || f[key][o+2]-f[key][low[key]]>f[v][o+2]-f[v][low[v]]){
            key=v;
        }
        g[v][o+1]=g[v][o-1];
    }
    if(~key){
        f[u][o+1]=f[u][o-1]+f[key][o+2]-f[key][low[key]];
        g[key][o+1]=o+2;
    }
    //2
    f[u][o+2]=1;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(anc[v]!=u)continue;
        if(!~low[v] || f[v][low[v]]>f[v][o-1])low[v]=o-1;
        f[u][o+2]+=f[v][low[v]];
        g[v][o+2]=low[v];
    }
    //fprintf(stderr, "anc[%d]=%d\n", u, anc[u]);
    for(int x=-2; x<=2; x++){
        //fprintf(stderr, "f[%d][%d]=%d\n", u, x, f[u][o+x]);
    }
}

void handle(const int u, const int x){
    if(x==o+2)crx.push_back(u);
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(anc[v]!=u)continue;
        handle(v, g[v][x]);
...
void petr0(const int u, const int a, const int d){
    anc[u]=a;
    cnt[d%3]++;
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
...
void petr1(const int u, const int d, const int k){
    if(d%3==k)crx.push_back(u);
    for(int i=nbs[u]; ~i; i=nxt[i]){
        int v=dst[i];
        if(anc[v]!=u)continue;
        petr1(v, d+1, k);
...
class SixDegreesOfSeparation {
    ...
    vector<int> getAdditionalEdges(int N, vector<int> A, vector<int> B) {
        ...
        crx.clear();
        int k=0;
        for(int i=1; i<3; i++)if(cnt[k]>cnt[i])k=i;
        petr1(0, 0, k);
        /*memset(anc, 0xff, sizeof(anc));
        ...
        for(int x=0; x<=2; x++){
            if(f[0][o+x]-1<=n/3){
                crx.clear();
                handle(0, o+x);
                //fprintf(stderr, "crx.size=%d, f=%d\n", SIZE(crx), f[0][o+x]);
                assert(SIZE(crx)==f[0][o+x]);
                break;
            }
            if(x==2)assert(0);
        }*/
        fprintf(stderr, "%d, %d, %d : crx.size=%d\n", cnt[0], cnt[1], cnt[2], SIZE(crx));
        foo.clear();
        for(int i=1; i<SIZE(crx); i++){
            if(edg[crx[0]][crx[i]])continue;
            foo.push_back(crx[0]);
            foo.push_back(crx[i]);
        }
        return foo;
