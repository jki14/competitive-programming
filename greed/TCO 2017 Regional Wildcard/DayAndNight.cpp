...
// CUT begin
ifstream data("DayAndNight.sample");
...
bool do_test(vector<int> a, vector<int> b, vector<int> day, vector<int> night, int dayToNight, int nightToDay, int __expected) {
    time_t startClock = clock();
    DayAndNight *instance = new DayAndNight();
    int __result = instance->minimalCost(a, b, day, night, dayToNight, nightToDay);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        from_stream(night);
        int dayToNight;
        from_stream(dayToNight);
        int nightToDay;
        from_stream(nightToDay);
        next_line();
        int __answer;
        ...
        if ( do_test(a, b, day, night, dayToNight, nightToDay, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1509873974;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "DayAndNight (900 Points)" << endl << endl;
    }
