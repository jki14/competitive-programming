...
// CUT begin
ifstream data("ColorfulParentheses.sample");
...
bool do_test(vector<int> color, long long __expected) {
    time_t startClock = clock();
    ColorfulParentheses *instance = new ColorfulParentheses();
    long long __result = instance->count(color);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(color, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1487870837;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "ColorfulParentheses (800 Points)" << endl << endl;
    }
