...
int n, m, ans;
int cur[L], til;
int route[L][4], flag[L], fail[L], num;
int f[N][L];

void insert(const int len, const int k){
    int p=0;
    for(int i=0; i<len; i++){
        //printf("%d", cur[i]);
        if(!route[p][cur[i]]){
...
class Softmatch {
    public:
    int count(string seq, vector<string> pat) {
        n=LENGTH(seq);
        m=SIZE(pat);
        //clear();
        ...
        for(int i=0; i<n; i++){
            const int rem=n-i;
            for(int k=0; k<m; k++){
                const int len=LENGTH(pat[k]);
                if(rem<len)continue;
                til=0;
                for(int j=0; j<len; j++){
                    if(seq[i+j]=='a'){
                        if(pat[k][j]=='0' || pat[k][j]=='1'){
                            cur[til++]=0;
                        }else{
                            cur[til++]=1;
                        }
                    }else{
                        if(pat[k][j]=='0' || pat[k][j]=='2'){
                            cur[til++]=2;
                        }else{
                            cur[til++]=3;
                        }
        ...
        for(int i=0; i<n; i++){
            ...
            for(int j=0; j<=num; j++){
                ...
                for(int c=0; c<2; c++){
                    ...
                    UPDMAX(f[i+1][p], f[i][j]+__builtin_popcount(foo));
                }
            }
        }
        ans=0;
...
// CUT begin
ifstream data("Softmatch.sample");
...
bool do_test(string S, vector<string> patterns, int __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(S, patterns, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1487694262;
