...
inline Iter next2(Iter const& foo) {
  Iter bar(foo);
  return ++bar;
}

template<typename Iter>
inline Iter prev2(Iter const& foo) {
  Iter bar(foo);
  return --bar;
}
...
class LineOff {
  ...
  int movesToDo(string points) {
    list<char> a(points.begin(), points.end());
    int foo = 0;
    auto lhs = a.begin();
    while (true) {
      if (lhs == a.end()) break;
      ...
      if (rhs == a.end()) break;
      if (*lhs == *rhs) {
        auto tmp = (lhs != a.begin()) ? prev2(lhs) : next2(rhs);
        a.erase(rhs);
        a.erase(lhs);
        lhs = tmp;
        ++foo;
