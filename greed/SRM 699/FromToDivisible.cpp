...
int nbs[V], nxt[M], dst[M], wht[M], eid;

inline void add_edge(const int u, const int v, const int w){
    //if(eid>10000)while(1);
...
class FromToDivisible {
    ...
    int shortest(int N, int S, int T, vector<int> a, vector<int> b) {
        ...
        v=s[T]+1;
        for(map<int, int>::iterator it=s.begin(); it!=s.end(); it++){
            if(S%it->first==0)add_edge(u, it->second, 0);
            if(T%it->first==0)add_edge(it->second+1, v, 0);
            add_edge(it->second+1, it->second, 0);
        }
        for(map<int, int>::iterator itx=s.begin(); itx!=s.end(); itx++){
            for(map<int, int>::iterator ity=itx; ity!=s.end(); ity++){
                if(itx==ity)continue;
                if(LCM(itx->first*1LL, ity->first*1LL)>n*1LL)continue;
                add_edge(itx->second+1, ity->second, 0);
                add_edge(ity->second+1, itx->second, 0);
            }
        }
        for(int i=0; i<m; i++){
            add_edge(s[a[i]], s[b[i]]+1, 1);
        }
        vector<int> dis(pid, inf);
        vector<int> vst(pid, 0);
        queue<int> que;
        dis[u]=0;
        vst[u]=1;
        que.push(u);
