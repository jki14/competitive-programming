...
class BuffingPixie {
    ...
    int fastestVictory(int HP, int normalAttack, int buffedAttack) {
        if(normalAttack*2>buffedAttack){
            return ceil(HP*1.0/normalAttack);
        }else return MIN(ceil(HP*1.0/buffedAttack)*2, ceil((HP-normalAttack)*1.0/buffedAttack)*2+1);
    }
};

// CUT begin
ifstream data("BuffingPixie.sample");
...
bool do_test(int HP, int normalAttack, int buffedAttack, int __expected) {
    time_t startClock = clock();
    BuffingPixie *instance = new BuffingPixie();
    int __result = instance->fastestVictory(HP, normalAttack, buffedAttack);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        from_stream(HP);
        int normalAttack;
        from_stream(normalAttack);
        int buffedAttack;
        from_stream(buffedAttack);
        next_line();
        int __answer;
        ...
        if ( do_test(HP, normalAttack, buffedAttack, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1473273769;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "BuffingPixie (250 Points)" << endl << endl;
    }
