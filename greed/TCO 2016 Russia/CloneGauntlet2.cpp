...
class CloneGauntlet2 {
    ...
    int minClones(vector<int> parent, vector<int> connection) {
        ...
        int n=SIZE(parent)+1;
        for(int u=n-1; u!=0; u=parent[u-1]){
            key[u]=1;
        }
        int ans=0, til=0;
        for(int u=0; u!=n-1;){
...
// CUT begin
ifstream data("CloneGauntlet2.sample");
...
bool do_test(vector<int> parent, vector<int> connection, int __expected) {
    time_t startClock = clock();
    CloneGauntlet2 *instance = new CloneGauntlet2();
    int __result = instance->minClones(parent, connection);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(parent, connection, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1473273690;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "CloneGauntlet2 (500 Points)" << endl << endl;
    }
