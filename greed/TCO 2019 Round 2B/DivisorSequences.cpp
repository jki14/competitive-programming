...
class DivisorSequences {
  ...
  static int process(int const n) {
    auto const it = f.find(n);
    if (it != f.end()) return it->second;
    int bar = 1;
    for (int x = 2; x <= n / x; ++x) {
      if (n % x != 0) continue;
      if (n / x <= 2) continue;
      int const len = process(n / x - 1);
      if (bar < len + 1) {
        bar = len + 1;
      }
    }
    f.insert(make_pair(n, bar));
...
unordered_map<int, int> DivisorSequences::f{{0, 0}};
