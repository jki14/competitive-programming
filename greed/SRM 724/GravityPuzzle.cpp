...
class GravityPuzzle {
    ...
    int count(vector<string> board) {
        int h=SIZE(board), w=SIZE(board[0]);
        int cnt=0, noh=0, nov=0;
        //count
        for(int i=0; i<h; i++){
            for(int j=0; j<w; j++){
                cnt+=(board[i][j]=='#')?1:0;
        ...
        if(cnt==0 || cnt==h*w)return 1;
        //noh-test
        for(int i=0; i<h; i++){
            lld raw=0LL;
            for(int j=0; j<w; j++){
                if(board[i][j]=='#')raw|=1LL<<j;
            }
            if(!raw)continue;
            if(__builtin__popcountll(raw+LOWBIT(raw))==1 && (board[i][0]=='#' || board[i][w-1]=='#')){
        ...
        for(int j=0; j<w; j++){
            lld raw=0LL;
            for(int i=0; i<h; i++){
                if(board[i][j]=='#')raw|=1LL<<i;
            }
            if(!raw)continue;
            if(__builtin__popcountll(raw+LOWBIT(raw))==1 && (board[0][j]=='#' || board[h-1][j]=='#')){
