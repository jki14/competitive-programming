...
struct seg_t {
  lld s, e;
  seg_t(int const lhs, int const rhs)
      : s(lhs), e(rhs) {
  }
  lld len() const {
    return e - s;
...
class OneHandRoadPainting {
  public:
  lld fastest(vector<int> s_, vector<int> e_, int d_) {
    n = SIZE(s_);
    m = d_;
    a.clear();
    a.reserve(n);
    for (int i = 0; i < n; ++i) {
      a.emplace_back(seg_t(s_[i], e_[i]));
    }
    lld foo = 0LL;
    for (int k = n - 1; k >= 0; --k) {
      //fprintf(stderr, "k = %d\n", k);
      lld const len = a[k].len();
      lld const r = (len + m - 1) / m;
      lld rem = m * r - len;
      foo += (a[k].e + a[k].e - m * (r - 1)) * r;
      while (k >= 1 && rem > 0) {
        if (rem >= a[k - 1].len()) {
          rem -= a[k - 1].len();
          --k;
