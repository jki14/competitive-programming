...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> class point_t{
        ...
        bool operator < (const point_t &rhs) const{
            if(this->y-rhs.y!=0)
                return this->y-rhs.y>0;
            return this->x-rhs.x<0;
        }
...
point_t<int> land[N], overhead[N];

int run(const int x, const int a, const int y, const int b){
    if(~f[x][a][y][b]){
        //printf("f %d %d %d %d = %d\n", x, a, y, b, f[x][a][y][b]);
        return f[x][a][y][b];
    }
    ...
    if(y-x-1){
        foo=0;
        for(int i=x+1; i<y; i++){
            MOD_ADD(&foo, MOD_MUL(run(x, a, i, top), run(i, top, y, b)));
        }
    }else{
        foo=1;
    }
...
class CoastGuard {
    public:
    int count(vector<int> d, vector<int> _x, vector<int> _y) {
        n=SIZE(d);
        sort(d.begin(), d.end());
        for(int i=0; i<n; i++){
            land[i+1].x=d[i];
        ...
        for(int x=0; x<=n; x++)for(int y=x+1; y<=n; y++){
            for(int a=0; a<=n; a++)for(int b=0; b<=n; b++){
                ...
                int remain=y-x-1;
                for(int i=1; i<n; i++){
                    if(overhead[i].y>overhead[a].y || overhead[i].y>overhead[b].y)continue;
                    if(!segment_t<int>(land[x], overhead[a]).contain(overhead[i])
                    && !segment_t<int>(land[y], overhead[b]).contain(overhead[i])
                    && overhead[i].cross(land[x], overhead[a])<=0 
                    && overhead[i].cross(land[y], overhead[b])>=0){
                        //printf("find %d (%d, %d)\n", i, overhead[i].x, overhead[i].y);
                        remain--;
                        if(!~g[x][a][y][b]){
                            g[x][a][y][b]=i;
                        }
