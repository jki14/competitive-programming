...
class CardCounter {
    private:
    double dealerTurn(vector<int> &deck, const pii dealer, const pii player){
        int dealerCurrent=dealer.first;
        if(dealer.second && dealerCurrent+10<=21){
            dealerCurrent+=10;
        }
        if(dealerCurrent<17){
            //greedy keep
            double foo=0.0;
            int total=accumulate(deck.begin(), deck.end(), 0);
            for(int i=0; i<10; i++){
                if(!deck[i])continue;
                deck[i]--;
                foo+=(deck[i]+1)*1.0/total*dealerTurn(deck, MP(dealer.first+i+1, dealer.second+(i==0?1:0)), player);
                deck[i]++;
            }
            return foo;
        }else{
            //greedy pass
            int playerCurrent=player.first;
            if(player.second && playerCurrent+10<=21){
                playerCurrent+=10;
            }
            if(playerCurrent<=21 && (dealerCurrent>21 || playerCurrent>dealerCurrent)){
                return 1.0;
            }else{
                return 0.0;
            }
    ...
    double playerTurn(vector<int> &deck, const pii dealer, const pii player){
        if(player.first>21)return 0.0;
        //get passScore
        double passScore=dealerTurn(deck, dealer, player);
        //get keepScore
        ...
        for(int i=0; i<10; i++){
            if(!deck[i])continue;
            deck[i]--;
            keepScore+=(deck[i]+1)*1.0/total*playerTurn(deck, dealer, MP(player.first+i+1, player.second+(i==0?1:0)));
            deck[i]++;
        }
        //get decision
        return MAX(passScore, keepScore);
    }
    public:
    double winningChance(vector<int> deck, int dealer, vector<int> player) {
        pii tmp(player[0]+player[1], 0);
        if(player[0]==1 || player[1]==1)tmp.second=1;
        return playerTurn(deck, MP(dealer, (dealer==1)?1:0), tmp);
    }
};
