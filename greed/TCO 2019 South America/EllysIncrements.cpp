...
class EllysIncrements {
  ...
  int getMin(vector<int> a) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      for (int j = 0; j <= m; ++j) {
        if (f[i][lowbar] - lowbar > f[i][j] - j) {
          lowbar = j;
        }
        if (!ps.count(a[i] + j)) continue;
        UPDMIN(f[i + 1][j], f[i][lowbar] + j - lowbar);
        while (pq.top().second < j) {
          pq.pop();
        }
        UPDMIN(f[i + 1][j], -pq.top().first);
...
// CUT begin
ifstream data("EllysIncrements.sample");
...
bool do_test(vector<int> A, int __expected) {
    ...
    EllysIncrements *instance = new EllysIncrements();
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "EllysIncrements (500 Points)" << endl << endl;
    }
