...
inline int comr(const int x, const int y){
    //printf("comr[%d][%d]=%d\n", y, x, c[y+x-1][x]);
    return c[y+x-1][x];
}
...
class OrderedProduct {
    ...
    int count(vector<int> a) {
        ...
        int pre=0;
        for(int i=0; i<n; i++){
            for(int len=0; len<=pre; len++){
                for(int nxt=len; nxt<=len+a[i]; nxt++){
                    int foo=f[i][len];
                    //printf("foo=%d\n", foo);
                    MOD_MUL(&foo, comr(nxt-len, len+1));
                    //printf("foo=%d\n", foo);
                    MOD_MUL(&foo, comr(a[i]-(nxt-len), nxt));
                    //printf("foo=%d\n", foo);
                    MOD_ADD(&f[i+1][nxt], foo);
                    //printf("f[%d][%d]=%d\n", i+1, nxt, f[i+1][nxt]);
                }
            }
            pre+=a[i];
