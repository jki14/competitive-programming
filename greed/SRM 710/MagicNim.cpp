...
class MagicNim {
    ...
    string findWinner(vector<int> arr) {
        ...
        if(foo==0 || goo<=1)return "Alice";
        if(goo<=3){
            if(!(foo>>1) || (foo&1))return "Alice";
        }else{
            if(foo>>1)return "Alice";
        }
        return "Bob";
    }
};
...
bool do_test(vector<int> arr, string __expected) {
    time_t startClock = clock();
    MagicNim *instance = new MagicNim();
    string __result = instance->findWinner(arr);
