...
void build(const int u, const int p, const int r, const int d){
    if(u<=r){
        path[k].u=u;
        path[k].v=r;
        path[k].w=d;
...
class JumpDistancesOnTree {
    ...
    string isPossible(vector<int> p, vector<int> S) {
        ...
        for(int i=0; i<k; i++){
            if(v[path[i].w]==0)continue;
            if(find(path[i].u)==find(path[i].v))continue;
            col[find(path[i].v)]=find(path[i].u);
        }
        for(int i=0; i<k; i++){
            if(v[path[i].w]==0)continue;
            if(find(path[i].u)!=find(0))continue;
            if(find(path[i].v)!=find(0))continue;
            v[path[i].w]=0;
        }
        for(int i=0; i<=n; i++){
            if(v[i]){
