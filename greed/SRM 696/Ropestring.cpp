...
class Ropestring {
    ...
    string makerope(string s) {
        multiset<int, greater<int> > even, odd;
        int dot=0;
        for(int i=0; i<LENGTH(s); i++){
            ...
            int len=1;
            while(i+1<LENGTH(s) && s[i+1]=='-'){
                i++;
                len++;
            }
            if(len&1){
                odd.insert(len);
            }else even.insert(len);
        }
        string foo;
        for(multiset<int, greater<int> >::iterator it=even.begin(); it!=even.end(); it++){
            if(!foo.empty()){
                foo.push_back('.');
                dot--;
            }
            for(int i=0; i<*it; i++)foo.push_back('-');
        }
        for(multiset<int, greater<int> >:: iterator it=odd.begin(); it!=odd.end(); it++){
            if(!foo.empty()){
                foo.push_back('.');
                dot--;
            }
