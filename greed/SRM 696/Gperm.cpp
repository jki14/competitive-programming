...
//HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> inline int UPDMIN(T &x, const T y){ if(x>y){x=y;return 1;}else return 0; }
    template<class T> inline int UPDMAX(T &x, const T y){ if(x<y){x=y;return 1;}else return 0; }
...
class Gperm {
    ...
    int countfee(vector<int> x, vector<int> y) {
        m=SIZE(x);
        for(int i=0; i<N; i++)foo[i].reset();
        for(int i=0; i<m; i++){
            foo[x[i]].set(i);
            foo[y[i]].set(i);
        }
        for(int i=0; i<N; i++)foo[i].flip();
        memset(f[cur], 0x3f, sizeof(f[cur]));
        f[cur][(1<<m)-1]=0;
        while(true){
            int flag=1;
            cur=pre;
            memcpy(f[cur], f[pre], sizeof(f[cur]));
            for(int r=0; r<(1<<m); r++){
                if(f[pre][r]>=inf)continue;
                bitset<M> bs(r);
                for(int i=0; i<m; i++){
                    if(!bs.test(i))continue;
                    if(UPDMIN(f[cur][(bs&foo[x[i]]).to_ulong()], (int)(f[pre][r]+bs.count())) 
                    || UPDMIN(f[cur][(bs&foo[y[i]]).to_ulong()], (int)(f[pre][r]+bs.count()))){
                        flag=0;
                    }
                    //printf("f[%s(%lu)]=%d\n", (bs&foo[x[i]]).to_string().c_str(), (bs&foo[x[i]]).to_ulong(), f[cur][(bs&foo[x[i]]).to_ulong()]);
                    //printf("f[%s(%lu)]=%d\n", (bs&foo[y[i]]).to_string().c_str(), (bs&foo[y[i]]).to_ulong(), f[cur][(bs&foo[x[i]]).to_ulong()]);
                }
            }
            if(flag)break;
        }
