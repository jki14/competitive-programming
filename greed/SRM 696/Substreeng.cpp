...
// CUT begin
ifstream data("Substreeng.sample");
...
bool do_test(vector<int> par, string ch, string pat, int __expected) {
    time_t startClock = clock();
    Substreeng *instance = new Substreeng();
    int __result = instance->count(par, ch, pat);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(par, ch, pat, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1470878853;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "Substreeng (900 Points)" << endl << endl;
    }
