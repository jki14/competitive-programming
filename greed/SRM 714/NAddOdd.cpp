...
struct problem_t{
    ...
    int k;
    int e;
    bool operator < (const problem_t &rhs) const{
        if(this->lef!=rhs.lef)return this->lef<rhs.lef;
        if(this->rig!=rhs.rig)return this->rig<rhs.rig;
        if(this->k!=rhs.k)return this->k<rhs.k;
        return this->e<rhs.e;
    }
    problem_t(const lld &_lef, const lld &_rig, const int _k, const int _e){
        this->lef=_lef;
        this->rig=_rig;
        this->k=_k;
        this->e=_e;
...
class NAddOdd {
    private:
    map<problem_t, lld> cache;
    lld process(lld lef, lld rig, const int k, const int e){
        UPDMAX(lef, k*1LL+1LL);
        if(lef>rig)return 0LL;
        problem_t problem(lef, rig, k, e);
        map<problem_t, lld>::iterator it=cache.find(problem);
        if(it!=cache.end())return it->second;
        lld foo=0LL;
        if(e==0){
            foo+=rig/2LL-(lef-1LL)/2LL;
            if(foo>0LL){
                foo+=process(lef>>1, rig>>1, k, 0);
                foo+=process(lef>>1, rig>>1, k ,1);
            }
        }else{
            foo+=(rig+1LL)/2LL-lef/2LL;
            if(foo>0LL){
                foo+=process(lef+k*1LL, rig+k*1LL, k, 0);
            }
        }
        cache.insert(MP(problem, foo));
        fprintf(stderr, "%lld->%lld (%d) %d = %lld\n", lef, rig, k ,e, foo);
        return foo;
    }
    public:
    NAddOdd(){
    ...
    long long solve(long long L, long long R, int K) {
        return process(L, R, K, 0)+process(L, R, K, 1);
    }
};
