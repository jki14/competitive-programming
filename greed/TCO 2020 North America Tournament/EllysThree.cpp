...
class EllysThree {
  public:
  int getMin(int n, int m, vector<string> const& b) {
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < m; ++j) {
        a[i][j] = b[i][j] - '0';
      }
...
// CUT begin
ifstream data("EllysThree.sample");
...
bool do_test(int N, int M, vector<string> matrix, int __expected) {
    ...
    EllysThree *instance = new EllysThree();
    int __result = instance->getMin(N, M, matrix);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "EllysThree (600 Points)" << endl << endl;
    }
