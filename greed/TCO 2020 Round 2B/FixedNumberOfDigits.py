...
class FixedNumberOfDigits:
    def sum(self, start, step, numberOfDigits):
        while numberOfDigits > 0:
            lef, rig = 1, 1
            curlen = len(str(start))
            while len(str(start + step * rig)) == curlen:
                rig *= 2
            while lef < rig:
                mid = (lef + rig) / 2
                if len(str(start + step * mid)) == curlen:
                    lef = mid + 1
                else:
                    rig = mid
            if curlen * lef < numberOfDigits:
                numberOfDigits -= curlen * lef
                start += step * lef
            else:
                k = (numberOfDigits - 1) / curlen
                numberOfDigits -= k * curlen
                start += step * k
                return int(str(start)[:numberOfDigits])
        return 0
