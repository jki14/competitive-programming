...
// CUT begin
ifstream data("BrightLampsRemake.sample");
...
bool do_test(string init, vector<string> buttons, vector<long long> __expected) {
    time_t startClock = clock();
    BrightLampsRemake *instance = new BrightLampsRemake();
    vector<long long> __result = instance->maxAndCount(init, buttons);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(init, buttons, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1488113284;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "BrightLampsRemake (1000 Points)" << endl << endl;
    }
