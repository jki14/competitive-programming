...
int rate(const lld cur){
    int foo=0;
    for(lld i=1LL; i<=cur; i<<=1){
        if(cur&i)foo++;
    }
    return foo;
}
...
void roll(lld cur){
    ...
    for(; !que.empty(); que.pop()){
        ...
        for(int j=0; j<m; j++){
            ...
            if(rate(nxt)<rate(cur) && f.find(nxt)==f.end()){
                f.insert(nxt);
                que.push(nxt);
            }
        }
...
class MovingTokens {
    public:
    int move(int _n, int _m, vector<int> moves) {
        for(int i=0; i<55; i++)bit[i]=1LL<<i;
        n=_n; m=_m;
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++){
                a[i][j]=moves[j*n+i];
