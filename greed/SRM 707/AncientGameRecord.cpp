...
// CUT begin
ifstream data("AncientGameRecord.sample");
...
bool do_test(int n, int m, vector<int> x, vector<int> y, string d, int __expected) {
    time_t startClock = clock();
    AncientGameRecord *instance = new AncientGameRecord();
    int __result = instance->minimalRemove(n, m, x, y, d);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(n, m, x, y, d, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1488113469;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "AncientGameRecord (1000 Points)" << endl << endl;
    }
