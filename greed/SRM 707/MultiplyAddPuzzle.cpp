...
// CUT begin
ifstream data("MultiplyAddPuzzle.sample");
...
bool do_test(long long s, long long t, long long a, long long b, long long __expected) {
    time_t startClock = clock();
    MultiplyAddPuzzle *instance = new MultiplyAddPuzzle();
    long long __result = instance->minimalSteps(s, t, a, b);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(s, t, a, b, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1488113405;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "MultiplyAddPuzzle (450 Points)" << endl << endl;
    }
