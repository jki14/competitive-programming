...
class KthStringAgain {
    public:
    string getKth(string s, lld k) {
        memcpy(a, s.c_str(), sizeof(char)*s.length());
        memset(foo, 0, sizeof(foo));
        n=LENGTH(s);k--;
        for(int i=0; i<n; i++){
            for(foo[i]='a'; foo[i]<='z'; foo[i]++){
                lld cnt=count();
                if(cnt<=k){
                    k-=cnt;
                }else{
...
// CUT begin
ifstream data("KthStringAgain.sample");
...
bool do_test(string s, long long k, string __expected) {
    time_t startClock = clock();
    KthStringAgain *instance = new KthStringAgain();
    string __result = instance->getKth(s, k);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "KthStringAgain (600 Points)" << endl << endl;
    }
