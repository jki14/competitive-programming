...
static constexpr lld inf = 0x3f3f3f3f3f3f3f3fLL;

int n;
lld a[1024], b[1024];
lld c[1024];

std::pair<lld, int> calc(){
  lld det = 0LL;
/*
  while (true) {
    std::pair<lld, int> pos{inf, 0};
    std::pair<lld, int> neg{-inf, 0};
    int zeros = 0;
    for (int i = 0; i < n; ++i) {
      lld const bar = b[i] - a[i];
      if (bar > 0) {
        pos.first = min(bar, pos.first);
        ++pos.second;
      } if (bar < 0) {
        neg.first = max(bar, neg.first);
        ++neg.second;
      } else {
        ++zeros;
      }
    }
    if (neg.second + zeros - pos.second <= 0) {
      det -= pos.first;
      for (int i = 0; i < n; ++i) {
        b[i] -= pos.first;
      }
    ...
    if (pos.second + zeros - neg.second < 0) {
      det -= neg.first;
      for (int i = 0; i < n; ++i) {
        b[i] -= neg.first;
      }
...
*/
  ...
  return std::make_pair(score, static_cast<int>(det));
}

class Parabola {
  public:
  vector<int> estimate(vector<int> const& yvals) {
    n = yvals.size();
    for (int i = 0; i < n; ++i) {
      a[i] = yvals[i];
    }
