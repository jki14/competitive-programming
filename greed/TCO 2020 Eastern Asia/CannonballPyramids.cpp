...
class CannonballPyramids {
  ...
  vector<int> build(int B) {
    ...
    bar.push_back(1LL);
    for (lld i = 2LL; ; ++i) {
      bar.push_back(bar.back() + i * i);
      if (bar.back() > 1000000000LL) break;
    }
    // fprintf(stderr, "%lu", bar.size());
    vector<int> foo;
    lld cur = B;
    while (cur) {
      int k = lower_bound(bar.begin(), bar.end(), cur) - bar.begin();
      if (bar[k] == cur) {
        foo.push_back(k + 1);
        break;
      } else {
