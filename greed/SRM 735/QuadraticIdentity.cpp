...
class QuadraticIdentity {
    ...
    vector<long long> getFixedPoints(long long m) {
        ...
        sort(r.begin(), r.end());
        while (SIZE(r)>500) {
            for (int i = 0; i * 2 < SIZE(r); ++i) {
                r[i] = r[i * 2];
            }
            r.erase(r.begin() + (SIZE(r) >> 1), r.end());
        }
        return r;
    }
};
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "QuadraticIdentity (1000 Points)" << endl << endl;
    }
