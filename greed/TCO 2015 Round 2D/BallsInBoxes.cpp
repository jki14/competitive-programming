...
lld bitlen(const lld x){
    lld len=0LL;
    for(lld y=1LL; y<x; y<<=1)len++;
    return len;
}
...
class BallsInBoxes {
    ...
    long long maxTurns(long long N, long long K) {
        lld cand=N-K+1LL;
        lld brut=(cand-K)/K;
        return brut+bitlen(cand-brut*K);
    }
};

// CUT begin
ifstream data("BallsInBoxes.sample");
...
bool do_test(long long N, long long K, long long __expected) {
    time_t startClock = clock();
    BallsInBoxes *instance = new BallsInBoxes();
    long long __result = instance->maxTurns(N, K);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "BallsInBoxes (500 Points)" << endl << endl;
    }
