...
void solution2(int const k, int const h, int const s, double const p) {
  if (h < 3) {
    for (int i = 0; i < n; ++i) {
      if (v[i] == c[i]) continue;
      ++v[i];
      solution2(k + 1, h + 1, s + i, p * (c[i] - v[i] + 1) / (r - k));
      --v[i];
    }
  } else {
    if (s == m) {
...
void solution(int const k, double const p) {
  if (k < 3) {
    int& i = a[k];
    for (i = 0; i < n; ++i) {
      if (v[i] == c[i]) continue;
      ++v[i];
      solution(k + 1, p * (c[i] - v[i] + 1) / (r - k));
      --v[i];
    }
  } if (k == 3) {
    bar = 0.0;
...
class CardDrawGame {
  ...
  double winChance(int __m, vector<int> __c) {
    c = std::move(__c);
    n = c.size();
    m = __m;
    r = std::accumulate(c.begin(), c.end(), 0);
...
// CUT begin
ifstream data("CardDrawGame.sample");
...
bool do_test(int target, vector<int> count, double __expected) {
    time_t startClock = clock();
    CardDrawGame *instance = new CardDrawGame();
    double __result = instance->winChance(target, count);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(target, count, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1579192647;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "CardDrawGame (500 Points)" << endl << endl;
    }
