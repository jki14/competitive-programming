...
class EllysDifferentPrimes {
  private:
  inline int check(int const prime) {
    static char buf[32];
    sprintf(buf, "%d", prime);
    static int cnt[16];
    for (int i = 0; i < 10; ++i) {
  ...
  vector<int> cook(vector<int> const& full) {
    vector<int> fill;
    fill.reserve(full.size());
    for (auto const prime : full) {
      if (check(prime)) {
        fill.push_back(prime);
      }
    }
    return fill;
  }
  ...
  int getClosest(int n) {
    static auto const tablefull = joshu::primetable(51000000);
    static auto const table = cook(tablefull);
    for (size_t i = 0; i < table.size(); ++i) {
      if (table[i + 1] < n) continue;
      if (n - table[i] <= table[i + 1] - n) {
        return table[i];
      } else {
        return table[i + 1];
      }
