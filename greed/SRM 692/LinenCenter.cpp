...
//HEAD_OF_JKI'S_HEADER_
    ...
    //KMP
    vector<int> kmp_build(const string &s){
        const int m=LENGTH(s);
        vector<int> r(m+1, -1);
        int j=-1;
        for(int i=0; i<m;){
            while(~j && s[i]!=s[j])j=r[j];
            r[++i]=++j;
        }
        return r;
    }
...
int f[R][L], g[B][R], h[B][R], cur=0;

class LinenCenter {
    private:
    int bad_check(int k, const string &s, const vector<int> &r){
        const int m=LENGTH(s);
        for(int i=0; i<m-1; i++){
            while(~k && s[i]!=s[k])k=r[k];
            k++;
            if(k>=m)return 1;
        }
        return 0;
    ...
    int countStrings(string s, int n, int k) {
        const int m=LENGTH(s);
        vector<int> r=kmp_build(s);
        vector<int> bad(m, 0);
        for(int i=0; i<m; i++){
            bad[i]=bad_check(i, s, r);
            //printf("bad[%d]=%d\n", i, bad[i]);
        }
        memset(f, 0, sizeof(f));
        f[0][0]=1;
        ...
        for(int p=1; p<=k; p<<=1){
            ...
            if(k&p){
                for(int i=0; i<=n; i++){
                    h[cur][i]=0;
                    for(int j=0; j<=i; j++){
                        MOD_ADD(&h[cur][i], MOD_MUL(h[pre][j], g[pre][i-j]));
                    }
                }
            }else{
                for(int i=0; i<=n; i++)
            ...
            for(int i=0; i<=n; i++){
                g[cur][i]=0;
                for(int j=0; j<=i; j++){
                    MOD_ADD(&g[cur][i], MOD_MUL(g[pre][j], g[pre][i-j]));
                }
            }
            /*printf("\ng:{");
            for(int i=0; i<=n; i++){
        ...
        for(int i=0; i<=n; i++){
            h[cur][i]=0;
            for(int j=0; j<=i; j++){
                MOD_ADD(&h[cur][i], MOD_MUL(h[pre][j], d[i-j]));
            }
        }
        int ans=0;
        for(int i=0; i<=n; i++){
