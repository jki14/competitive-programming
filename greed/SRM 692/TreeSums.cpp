...
int nbs[V], dst[V], nxt[V], anc[V], eid;
lld wht[V], pln[V];
int que[V], cnt[V], cor[V], lef, rig;
lld foo[V], opt[V], dis[V]; 

class TreeSums {
    private:
    void add_edge(const int u, const int v, const lld w){
        nxt[eid]=nbs[u];dst[eid]=v;wht[eid]=w;nbs[u]=eid++;
        anc[v]=u; pln[v]=w;
    }
    public:
    lld findSums(int n, int cur, int rc, int rd) {
        //input
        memset(nbs, 0xff, sizeof(nbs));eid=0;
        for(int v=1; v<n; v++){
            cur=(cur*1LL*rc+rd*1LL)%1000000000LL;
            int u=cur%v;
            cur=(cur*1LL*rc+rd*1LL)%1000000000LL;
            lld w=cur*1LL%1000000LL;
            add_edge(u, v, w);
        }
        //calculate
        lld ans=0LL;
        ...
        for(int k=rig-1; ~k; k--){
            ...
            for(int i=nbs[u]; ~i; i=nxt[i]){
                ...
                cnt[u]+=cnt[v];
                foo[u]+=foo[v]+cnt[v]*pln[v];
            }
            if(~p){
                //printf("opt[%d]=%lld\n", u, opt[u]);
                opt[u]=foo[u]-foo[p]-cnt[p]*pln[p]+opt[p]+(cnt[u]-cnt[p])*(dis[p]+pln[p]);
                //printf("opt[%d]=%lld\n", u, opt[u]);
                dis[u]=dis[p]+pln[p];
                int v=cor[p];
                while(v!=u && cnt[v]<=(cnt[u]-cnt[v])){
                    opt[u]+=(cnt[v]-(cnt[u]-cnt[v]))*pln[v];
                    dis[u]-=pln[v];
                    v=anc[v];
                }
                cor[u]=v;
