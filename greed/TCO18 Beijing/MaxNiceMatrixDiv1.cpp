...
class MaxNiceMatrixDiv1 {
  ...
  vector<int> getMaxMatrix(int a, int b, int c) {
    ...
    // => k <= d * 2 / c + 1
    int const k = d * 2 / c + 1;
    vector<vector<int>> foo(c, vector<int>(k, 0));
    for (int i = 0; i < c - 1; ++i) {
      foo[i][0] = (i & 1) ? (k - 1) : 0;
      int const det = (i & 1) ? -1 : 1;
      for (int j = 1; j < k; ++j) {
        foo[i][j] = foo[i][j - 1] + det;
      }
    }
    if (c & 1) {
      foo[c - 2][0] = k - 2;
      for (int j = 1; j < k; ++j) {
        foo[c - 2][j] = foo[c - 2][j - 1] - 2;
        if (foo[c - 2][j] < 0) {
          foo[c - 2][j] = k - 1;
        }
      }
