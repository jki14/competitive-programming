...
int main(){
	while(scanf("%lld%lld%d",&A,&B,&s)!=EOF){
		sprintf(a,"%lld",A);int _n=strlen(a);
		for(int i=_n-1;i>=0;i--)a[n-_n+i]=a[i];
		for(int i=n-_n-1;i>=0;i--)a[i]='0';
		
		memset(f,0x3f,sizeof(f));cnt=a[0]-'0';
		for(int i=a[0]-'0'+1;i<=2;i++)f[0][i]=(i-a[0]+'0')*pow(10LL,n-1);
		for(int k=1;k<n;k++){
			for(int r=0;r<=k*9+2;r++){
				for(int i=0;i<=9;i++){
					lld det=((lld)(i-a[k]+'0'))*pow(10LL,n-1-k);
					if(r-i>=0 &&f[k-1][r-i]<inf)_updmin(f[k][r],f[k-1][r-i]+det);
					if(i>(a[k]-'0') && r-i==cnt)_updmin(f[k][r],det);
				}
						}
			cnt+=a[k]-'0';
		}
