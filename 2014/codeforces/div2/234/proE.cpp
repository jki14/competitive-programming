...
int prvFresh(const int &k, const int &i, const int &lef, const int &rig, const int &p, int &prv){
	if(p<lef || p>rig)return 0;
	if(c[k][i]){
		if(lef<prv){
			prv=lef;
			return 1;
	...
	if(lef!=rig){int mid=(lef+rig)>>1;
		if(prvFresh(k,L,lef,  mid,p,prv))return 1;
		if(prvFresh(k,R,mid+1,rig,p,prv))return 1;
	}
	return 0;
}

int nxtFresh(const int &k, const int &i, const int &lef, const int &rig, const int &p, int &nxt){
	if(p<lef || p>rig)return 0;
	if(c[k][i]){
		if(rig>nxt){
			nxt=rig;
	...
	if(lef!=rig){int mid=(lef+rig)>>1;
		if(nxtFresh(k,L,lef,  mid,p,nxt))return 1;
		if(nxtFresh(k,R,mid+1,rig,p,nxt))return 1;
	}
	return 0;
}

void remove(const int &k, const int &i, const int &lef, const int &rig, const int &p, int &prv, int &nxt){
	if(p<lef || p>rig)return;
	if(c[k][i]){
		_updmin(prv,lef);
		_updmax(nxt,rig);
		if(lef!=rig)
	...
	if(lef!=rig){int mid=(lef+rig)>>1;
		remove(k,L,lef,  mid,p,prv,nxt);
		remove(k,R,mid+1,rig,p,prv,nxt);
	}
}

void insert(const int &k, const int &i, const int &lef, const int &rig, const int &p, int &prv, int &nxt){
	if(p<lef || p>rig)return;
	if(lef!=rig){int mid=(lef+rig)>>1;
		insert(k,L,lef,  mid,p,prv,nxt);
		insert(k,R,mid+1,rig,p,prv,nxt);
		if(c[k][L] && c[k][R]){
			c[k][i]=1;
			_updmin(prv,lef);
			_updmax(nxt,rig);
...
inline lld sum(const int &x){ return ((lld)x)*(((lld)x)+1LL)/2LL; }

void update(const int &p, const int &x){
	if(a[p]==x)return;
	for(int i=0;i<B;i++){
		if((a[p]&bit[i])==(x&bit[i]))continue;
		if(a[p]&bit[i]){
			int prv=p,nxt=p;
			remove(i,0,1,n,p,prv,nxt);//printf("_prv=%d _nxt=%d\n",prv,nxt);
			while(prvFresh(i,0,1,n,prv-1,prv));
			while(nxtFresh(i,0,1,n,nxt+1,nxt));
			//printf("remove p=%d prv=%d nxt=%d, val=%d\n",p,prv,nxt,bit[i]);
			prv=p-prv;nxt=nxt-p;
			ans+=((lld)bit[i])*(sum(prv)+sum(nxt)-sum(prv+nxt+1));
		}else{
			int prv=p,nxt=p;
			insert(i,0,1,n,p,prv,nxt);//printf("_prv=%d _nxt=%d\n",prv,nxt);
			while(prvFresh(i,0,1,n,prv-1,prv));
			while(nxtFresh(i,0,1,n,nxt+1,nxt));
			//printf("insert p=%d prv=%d nxt=%d, val=%d\n",p,prv,nxt,bit[i]);
			prv=p-prv;nxt=nxt-p;
			ans+=((lld)bit[i])*(sum(prv+nxt+1)-sum(prv)-sum(nxt));
...
int main(){
	for(int i=0;i<B;i++)bit[i]=_ni(i);
	while(scanf("%d%d",&n,&m)!=EOF){ans=0LL;
		memset(a,0,sizeof(a));
		memset(c,0,sizeof(c));
		for(int i=1;i<=n;i++){
			int x;scanf("%d",&x);
