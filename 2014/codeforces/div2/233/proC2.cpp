...
lld neg(const lld &x){
	lld per=b/x,lst=b%(lld)x;
	return _sqr(per+1LL)*(lst)+_sqr(per)*(x-lst);
}

void output(const int &x){
	lld per=b/(lld)x,lst=b%(lld)x;
	for(int i=0;i<lst;i++){
		if(i){ printf("o");a--; }
		for(lld j=0LL;j<=per;j++)printf("x");
	}
	for(int i=lst;i<x;i++){
		if(i || x==1){
			if(i<x-1){
...
int main(){
	while(cin>>a>>b){
		...
		cur=a*a-b*b;key=1;
		for(int i=2;i<=m;i++){
			lld val=_sqr(a-(lld)i+2LL)+((lld)i-2LL)-neg((lld)i);
			//printf("val(%d)=%lld\n",i,val);
			if(val>cur){
				cur=val;key=i;
			}
