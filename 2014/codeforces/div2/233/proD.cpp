...
double core(const int &r, const int &c){
	if(r>n || c>n)return 0.0;
	if(r==n && c==n)return 0.0;
	if(v[r][c])return f[r][c];
	v[r][c]=1;f[r][c]=_sqr((double)n);
	f[r][c]+=core(r+1,c+1)*((double)n-(double)r)*((double)n-(double)c);
	f[r][c]+=core(r+1,c)*((double)n-(double)r)*((double)c);
	f[r][c]+=core(r,c+1)*((double)r)*((double)n-(double)c);
	f[r][c]/=(double)(n*n-r*c);
	//printf("f[%d][%d]=%.6f\n",r,c,f[r][c]);
	return f[r][c];
}
