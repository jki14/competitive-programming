...
int _a,_b;
lld a,b,h,k,ans1,ans2;
	
char s[L];

int main(){
	while(scanf("%d%d",&_a,&_b)!=EOF){
		//if(_a>=_b){
		a=_a;b=_b;h=b/2LL;
		ans1=a*a-b*b+(a>0LL?2LL*h*(b-h):0LL);
			
		k=max(b/(a+1LL),1LL);int p=0;
		lld _k=k,sum=0LL;ans2=0LL;
		while(b!=0LL){b--;
			s[p++]='x';
			_k--;sum++;
			if(_k==0LL && a>0LL){a--;
		...
		if(ans1>=ans2){
			cout<<ans1<<endl;
			for(int i=0;i<h;i++)printf("x");
			for(int i=0;i<_a;i++)printf("o");
			for(int i=h;i<_b;i++)printf("x");
			printf("\n");
		}else{
			cout<<ans2<<endl;
			printf("%s\n",s);
