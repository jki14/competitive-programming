...
int n, a[N][N];
int eu[S], ev[S], ew[S], id[S], es;
int nxt[M], nbs[N], dst[M], wht[M], enm;

int counter, f[N];

int cmp(const int x, const int y){ return ew[x]<ew[y]; }
...
int check(const int u, const int f, const int r, const lld d){
	if(d!=(lld)a[r][u])return 0;
	counter++;
	for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
		if(v==f)continue;
		if(!check(v, u, r, d+(lld)wht[i]))return 0;
	}
	return 1;
}
...
int tree(){
	...
	for(int i=0;i<n;i++)for(int j=i+1;j<n;j++){
		eu[es]=i;ev[es]=j;ew[es]=a[i][j];id[es]=es;
		es++;
	}
	sort(id, id+es, cmp);
	...
	for(int _i=0;k<n-1;_i++){int i=id[_i];
		if(find(eu[i])==find(ev[i]))continue;
		k++;
		addedge(eu[i], ev[i], ew[i]);
		f[find(eu[i])]=find(ev[i]);
	}
	for(int i=0;i<n;i++){
		counter=0;
		if(check(i, -1, i, 0)==0 || counter!=n)return 0;
	}
	return 1;
}

int Core(){
	for(int i=0;i<n;i++)if(a[i][i]!=0)return 0;
	for(int i=0;i<n;i++)for(int j=i+1;j<n;j++)
		if(a[i][j]!=a[j][i] || a[i][j]==0)return 0;
	return tree();
}
