...
int dfsTree(const int &u){
	mrk[u]=1;f[u]=vst[u]?1:0;
	if(vst[u])vnm--;
...
int visit(const int &u){
	vst[u]^=1; res[ans++]=u;
	return vst[u]? 1:-1;
}

int once(const int &u, const int &v){
	
	//printf("once %d(%d)->%d(%d)  :: ",u+1,vst[u],v+1,vst[v]);

	int ret = visit(v);f[v]+=ret; 
	ret += visit(u);
	//printf("%d f[%d]=%d\n",ret,v+1,f[v]);
	return ret;
}

int core(const int &u, const int &nomore){
	int d = visit(u);int bch = 0;
	for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
		if(!use[i])continue;
		if(f[v])bch++;
	}
	for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
		...
		if(nomore && !bch){
			...
			d+=visit(u);
			while(f[v]){
				//printf("f[%d]=%d vst[4]=%d vst[6]=%d vst[9]=%d\n",v,f[v],vst[4],vst[6],vst[9]);
				d+=once(u,v);
			}
		}
	}
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		int rut=-1;vnm=0;memset(vst,0,sizeof(vst));
		for(int i=0;i<n;i++){
			scanf("%d",&vst[i]);
			if(vst[i]){ rut=i;vnm++; }
		}
		...
		memset(f,0,sizeof(f));
		dfsTree(rut);//printf("f[6]=%d\n",f[6]);
		//for(int i=0;i<n;i++)printf("f[%d]=%d vst=%d\n",i+1,f[i],vst[i]);
		if(vnm){ printf("-1\n");continue; }
		ans=0;core(rut, 1);
		printf("%d\n",ans);
		for(int i=0;i<ans;i++){
