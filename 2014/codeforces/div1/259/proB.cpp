...
int prm[V],pnm,sb[V];
int f[N][S], g[N][S], c[N][S];

int n,a[N],ans[N],key,bit[B];

queue<int> que[N];

void init(){
	pnm=0;
	for(int i=2;i<V;i++){int flag=1;
		for(int p=0;p<pnm;p++){
			if((i%prm[p])==0){
				flag=0;break;
			}
	...
	//printf("pnm=%d\n",pnm);
	for(int i=0;i<=pnm;i++)bit[i]=_ni(i);
	for(int i=0;i<V;i++){sb[i]=0;
		for(int p=0;p<pnm;p++)
			if((i%prm[p])==0)sb[i]|=bit[p];
	}
}
...
int main(){
	...
	while(scanf("%d",&n)!=EOF){
		...
		memset(f,0x3f,sizeof(f));f[0][0]=0;que[0].push(0);
		for(int i=0;i<n;i++){
			for(;!que[i].empty();que[i].pop()){int j=que[i].front();
				for(int k=1;k<V;k++){
					if(j&sb[k])continue;
					int val=f[i][j]+abs(k-a[i]);
					int sta=j|sb[k];
					if(val<f[i+1][sta]){
						if(i+1<n && f[i+1][sta]==inf)que[i+1].push(sta);
						f[i+1][sta]=val;
						g[i+1][sta]=j;
						c[i+1][sta]=k;
					}
		...
		key=0;for(int i=1;i<S;i++)
			if(f[n][key]>f[n][i])key=i;
		for(int i=n;i;i--){
			ans[i-1]=c[i][key];
			key=g[i][key];
		}
