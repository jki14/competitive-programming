...
void init(){
	mp.clear();
	for(int i=0;i<s;i++)mp.insert(_mkpr(a[i],0));
	int msk=0;for(it=mp.begin();it!=mp.end();it++)(*it).second=++msk;
	for(int i=0;i<s;i++){ a[i]=mp[a[i]];
		//printf("a[%d]=%d\n",i,a[i]);
	}
}

lld sum(const int &x,const int &msk){
	lld ret=0LL;
...
void add1(const int &x,const int &msk){
	for(int i=x;i<=s;i+=_lowbit(i)){
		if(d[i]!=msk)c[i]=0LL;
		d[i]=msk;c[i]++;
	}
}
...
void build(){
	memset(d,0xff,sizeof(d));int msk=0;
	for(int k=0;k<=n;k++){h[k]=0LL;
		int len=bit[k];
		for(int head=0;head<s;head+=len){int tail=head+len;
			for(int i=head;i<tail;i++){
				if(d[a[i]]!=msk){ e[a[i]]=0LL;d[a[i]]=msk; }
				if(e[a[i]]>0LL){
					lld prv=(e[a[i]]*(e[a[i]]-1LL))>>1;
					e[a[i]]++;
					lld nxt=(e[a[i]]*(e[a[i]]-1LL))>>1;
					h[k]+=nxt-prv;
				}else e[a[i]]++;
			}msk++;
		}
	...
	for(int k=0;k<=n;k++){f[k]=0LL;
		int len=bit[k];
		for(int head=0;head<s;head+=len){int tail=head+len;
			int half=head+(len>>1);
			int plef=head,prig=half,p=0;
			while(plef<half && prig<tail){
				int lef,rig;lef=rig=0;
				int key=min(a[plef],a[prig]);
				while(plef<half && a[plef]==key){ b[p++]=a[plef++];lef++; }
				while(prig<tail && a[prig]==key){ b[p++]=a[prig++];rig++; }
				f[k]+=((lld)rig)*((lld)(half-plef));
				//g[k]+=((lld)lef)*((lld)(tail-prig));
			}
			while(plef<half)b[p++]=a[plef++];
			while(prig<tail)b[p++]=a[prig++];
			for(int i=head;i<tail;i++)a[i]=b[i-head];
		}
	}
	ans=0LL;for(int i=0;i<=n;i++)ans+=f[i];
	for(int i=0;i<=n;i++)g[i]=((((lld)bit[i])*(((lld)bit[i])-1LL))>>1)*((lld)bit[n-i]);
	for(int i=n;i>=1;i--)g[i]-=g[i-1];
		//for(int i=0;i<=n;i++)printf("%d: f=%lld g=%lld h=%lld\n",i,f[i],g[i],h[i]);
}

int main(){
	for(int i=0;i<=n_max;i++)bit[i]=_ni(i);
	while(scanf("%d",&n)!=EOF){s=bit[n];
		...
		for(int i=0;i<m;i++){
			...
			for(int j=1;j<=q;j++){
				lld nxt=g[j]-h[j]-f[j];
				//printf("f[%d]=%lld g[%d]=%lld nxt=%lld ans=%lld\n",j,f[j],j,g[j],nxt,ans);
				ans+=nxt-f[j];
				f[j]=nxt;
			}
			//cout<<ans<<endl;
