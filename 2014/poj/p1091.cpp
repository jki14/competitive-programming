...
void core(const int &x, const int &y, const int &k, const int &v){
	if(x==k){
		ans+=pow((lld)(m/v),n)*((k&1)?-1LL:1LL);
		//printf("v=%d k=%d ans=%lld\n",v,k,ans);
		return;
	}
	for(int i=y;i<cnt;i++)core(x+1,i+1,k,v*fac[i]);
}
...
int main(){
	while(scanf("%d%d",&n,&m)!=EOF){
		...
		//for(int i=0;i<cnt;i++)printf("fac[%d]=%d\n",i,fac[i]);

		for(int k=1;k<=cnt;k++)core(0,0,k,1);
