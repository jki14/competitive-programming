...
int count(const lld &x){
	int ret=0;
	for(lld k=1LL;k<=x;k<<=1)
		if(x&k)ret++;
	return ret;
}
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		...
		s.clear();memset(vst,0,sizeof(vst));
		for(int j=0;j<n;j++){ b[j]=c[0]^a[j];s.insert(_mkpr(b[j],j));vst[j][j]=1; }
		for(int i=1;i<n;i++){
			//printf("i=%d\n",i);
			t.clear();
			for(int j=0;j<n;j++){ 
			...
			for(it=s.begin();it!=s.end();){
				if(t.find((*it).first)==t.end()){
					//printf("erase %lld\n",(*it).first);
					map<lld,int>::iterator jt=it;
					it++;s.erase(jt);
				}else it++;
			}
			//printf("size(s)=%d size(t)=%d\n",_size(s),_size(t));
		}
		ans=inf;for(it=s.begin();it!=s.end();it++){
			_updmin(ans,count((*it).first));
		}
