...
void _amd_(lld &x, const lld &y){ x=_amd(x,y); }
	...
	for(int _T=1;_T<=T;_T++){
		...
		for(int i=0;i<3;i++){
			for(int j=0;j<n;j++){
				if(!f[i][j])continue;
				for(int k=j+1;k<=n;k++)
					_amd_(f[i+1][k],f[i][j]*(lld)(n-k+1));
			}
		}
		ans=0LL;for(int i=1;i<=n;i++)_amd_(ans,f[3][i]*6LL);
		for(int i=1;i<=n;i++)_amd_(ans,(lld)((n-i+1)*(n-i+1)*(n-i+1)));
		//printf("%lld\n",ans);
		cout<<ans<<endl;
	}
	return 0;
