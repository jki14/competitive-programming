...
int check(const lld x){
	sprintf(a,"%lld",x);int len=strlen(a);
	vector<char> s;s.clear();
	for(int i=0;i<len;i++){
		if(s.empty() || s[_size(s)-1]!=a[i])
			s.push_back(a[i]);
	}
	for(int i=0;i<_size(s);i++)
		if(s[i]!=s[_size(s)-1-i])return 0;
	return 1;
}
...
int main(){
	for(int i=1;i<=n;i++){
		...
		if(!check(x)){
			printf("%d:%lld check failed\n",i,x);
			continue;
		}
		for(lld y=i-1; y>x; y--){
			if(check(y)){
				printf("%d:%lld should be %lld\n",i,x,y);
				break;
			}
		}
	}
