...
#define tail prx[_size(prx)-1]
#define next prx[_size(prx)-2]

const int def=0xc0c0c0c0;

int n;
lld num,ans,ans1,ans2;
char a[L], b[L];

int core1(const int &k){
	if(a[k]=='0')return 0;
	vector<char> prx;prx.clear();
	for(int i=0;i<k;i++)
		if(prx.empty() || tail!=a[i])prx.push_back(a[i]);
	
	memcpy(b,a,sizeof(char)*L);
	b[k]=a[k]-1;if(prx.empty() || tail!=)

	return ans1>dec;
}

int core2(const int &k){
	if(a[k]=='0')return 0;
	vector<char> prx;prx.clear();
	for(int i=0;i<k;i++)
		if(prx.empty() || tail!=a[i])prx.push_back(a[i]);
	if(_size(prx)<2)return 0;
	prx.pop_back();
	
	if(n-k<_size(prx))return 0;
	if(tail>a[k]-1)return 0;
	memcpy(b,a,sizeof(char)*L);

	int p=k;	
	while(!prx.empty()){
		b[p++]=tail;
		while(_size(prx)>1 && tail>next && n-p>_size(prx)-1 && p<n)b[p++]=tail;
		prx.pop_back();
	}
	for(;p<n;p++)b[p]=b[p-1];

	b[n]=0;sscanf(b,"%lld",&ans1);
	return ans2>dec;
}

int t10(){
	lld key=1LL;for(int i=1;i<n;i++)key*=10LL;
	if(num%key!=0)return 0;
	if(num/key==1){
		printf("%lld\n",num-1LL);
	}else{
		b[0]=a[0]-1;
		for(int i=1;i+1<n;i++)b[i]='9';
		b[n-1]=b[0];
		b[n]=0;sscanf(b,"%lld",&ans);
		printf("%lld\n",ans);
	}
	return 1;
}
...
int main(){
	...
	while(T--){
		...
		ans1=ans2=def;
		for(int i=n-1;i>=1;i--)if(core1(i))break;
		for(int i=n-1;i>=1;i--)if(core2(i))break;
