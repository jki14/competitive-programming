...
int main(){
	...
	while(T--){
		...
		if(n==1){
			...
			ans=0;int k=a[0]-a[1];
			for(int i=0;2+i<n;i++){
				/*if(i>a[2+i])_addMd(ans, 1);
				if(a[1]-i<=0)_addMd(ans, 1);*/
				int flag=0;
				{
					for(int j=i;j<=a[2+i];j++){
						if(j-i+1>a[2+i])break;
						if(a[1]-j<=0)break;
						_addMd(ans, a[1]-j);
						flag=1;
						//printf("ans=%d i=%d j=%d r=%d k=%d\n",ans,i,j,r,k);
					}	
				}
				if(!flag)_addMd(ans, 1);
				for(int r=1;r<=k;r++){
					for(int j=0;j<a[2+i]-i-r;j++){
					//for(int j=i+r;j<=a[2+i];j++){
						if(j>a[2+i])break;
						if(a[1]-j-i-r<=0)break;
						_addMd(ans, a[1]-a[2+i]+j);
						flag=1;
						//printf("ans=%d i=%d j=%d r=%d k=%d\n",ans,i,j,r,k);
					}	
				}
