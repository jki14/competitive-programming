...
int n, m;
int nbs[N], nxt[E], dst[E], idx[E], enm;
int path[N], anc[N], f[N], g[N];
int qt[M], qu[M], qv[M], qd[M], qr[M];
lld vw[N], ew[N], vd[N], ed[N];
vector<int> lca[N];
queue<int> que;
...
inline int find(const int u){ if(u!=f[u])f[u]=find(f[u]);return f[u]; }
...
void build(const int u, const int e){
	path[u]=e;
	lca[u].clear();
...
void tarjan(const int u){
	...
	mk[u]=1;
	for(int _i=0; _i<_size(lca[u]); _i++){int i=lca[u][_i];
		int v=(qu[i]==u)?qv[i]:qu[i];
		if(mk[v]){
			qr[i]=g[find(f[v])];
			//printf("LCA(%d,%d)=%d\n",u,v,qr[i]);
		}
...
int main(){
	...
	for(int _T=1; _T<=T; _T++){
		...
		build(ROOT, n);anc[ROOT]=n;

		for(int i=0; i<m ;i++){
			scanf("%s%d%d%d", cs, &qu[i], &qv[i], &qd[i]);
			qt[i]=(cs[3]=='1')?1:0;
			qu[i]--;qv[i]--;
			lca[qu[i]].push_back(i);
			lca[qv[i]].push_back(i);
		}
