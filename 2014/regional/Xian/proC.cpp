...
lld Core(const int head, const int tail){
	lld count=0;sign++;
	...
	if(count<=1LL)return count*count;
	int p=head;
	lld lef=0LL, cap=count>>1;
	for(sign++;lef<=cap;p++){
		if(v[a[p]]!=sign){
			v[a[p]]=sign;
			lef++;
	...
	//printf("%d--%d-->%d\n",head,p,tail);
	lld ret=min(count*count, Core(head, p)+Core(p, tail));
	//printf("%d->%d %lld\n",head,tail,ret);
	return ret;
}

int main(){
	memset(v, 0, sizeof(v));sign=0;
	while(scanf("%d",&n)!=EOF){
		map<int, int>s;s.clear();
		for(int i=0;i<n;i++){
			scanf("%d",&a[i]);
			s.insert(_mkpr(a[i], 0));
		}
		int idx=0;
