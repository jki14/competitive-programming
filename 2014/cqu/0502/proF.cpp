...
int core(const int &root){
	++msk;int ret=dis[root]=0;vst[root]=msk;
	while(!que.empty())que.pop();que.push(root);
	for(;!que.empty();que.pop()){
		int u=que.front();
		_updmax(ret,dis[u]);
		for(int i=nbs[u];i;i=nxt[i]){int v=dst[i];
			if(vst[v]==msk)continue;
			vst[v]=msk;dis[v]=dis[u]+1;
			que.push(v);
		}
	}
	return ret;
		...
		for(int i=0;i<n;i++){
			for(int j=0;j<m;j++){
				for(int k=0;k<B;k++){
					...
					if(x>=n || y>=m)continue;
					int u=g[i][j],v=g[x][y];
					if(c[u]!=c[v])AddEdge(find(u),find(v));
				}
			}
		}
		//CORE
