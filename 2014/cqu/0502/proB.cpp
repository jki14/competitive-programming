...
void core(){
	for(;!que.empty();que.pop()){
		int msk=que.front().first,pos=count(msk);
		int val=que.front().second,cnt=f[msk][val];
		for(int i=0;i<n;i++){
			if(msk&bit[i])continue;
			int nxtMsk=msk|bit[i];
			int nxtVal=min(val+c[pos][i],m);
			if(!vst[nxtMsk][nxtVal]){
				vst[nxtMsk][nxtVal]=1;
				  f[nxtMsk][nxtVal]=0;
				if(pos+1<n)que.push(_mkpr(nxtMsk,nxtVal));
			}
			f[nxtMsk][nxtVal]+=cnt;
		}
	}
...
int main(){
	...
	while(T--){
		...
		que.push(_mkpr(0,0));
		core();
		int msk=bit[n]-1,x,y;x=y=0;
		for(int i=0;i<=m;i++){
			if(!vst[msk][i])continue;
			if(i>=m)y+=f[msk][i];
			x+=f[msk][i];
		}
		if(!y)
