...
int main(){
	...
	while(T--){
		...
		for(lev=0;sum<num;){
			...
			if(!_size(ans[lev]))break;
			dst[0]-=det[0];dst[1]-=det[1];
			lev++;
		}
		if(sum<num){
			printf("No solution\n");
			...
			for(int i=lev-1;i>=0;i--){
				for(int j=0;j<_size(ans[i]);j++){
					if(!fst)printf(" ");fst=0;
					int k=ans[i][j];
					if(!typ[k])
						printf("C%d",pos[k]);
					else printf("R%d",pos[k]);
				}
				//printf("|");
			}
			printf("\n");
