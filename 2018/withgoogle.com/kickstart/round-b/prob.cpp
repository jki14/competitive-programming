...
inline int check(const int p, const int tail){
    for(const pii &row : condi[p]){
        if(__builtin_popcount(tail&mask[row.first])!=row.second)
            return 0;
    }
    return 1;
}
...
__int128 count(){
    unordered_map<int, __int128> valid[2];
    valid[0].clear();
    valid[0].insert(MP(0, (__int128)1));
    valid[1].clear();
    for(int i=0; i<n; i++){
        valid[(i&1)^1].clear();
        for(const pair<int, __int128> &arc : valid[i&1]){
            for(int k=0; k<2; k++){
                if(foo[i] && foo[i]-'0'!=k)continue;
                int next=((arc.first<<1)|k)&mask[len[i+1]];
                if(check(i+1, next)){
                    unordered_map<int, __int128>::iterator it=valid[(i&1)^1].find(next);
                    if(it!=valid[(i&1)^1].end()){
                        it->second+=arc.second;
                    }else{
                        valid[(i&1)^1].insert(MP(next, arc.second));
                    }
                }
            }
        }
    ...
    __int128 cnt=0;
    for(const pair<int, __int128> &arc : valid[n&1]){
        cnt+=arc.second;
    }
    return cnt;
}

void process(const int p, const lld kth){
    if(p==n)return;
    foo[p]='0';
    __int128 cnt=count();
    //fprintf(stderr, "prefix = %s, cnt = %lld\n", foo, (lld)cnt);
    if(cnt>=kth){
        process(p+1, kth);
    }else{
        foo[p]='1';
...
char* process(){
    ...
    for(int i=0; i<k; i++){
        int x, y, z;
        scanf("%d%d%d", &x, &y, &z);
        condi[y].push_back(MP(y-x+1, z));
        UPDMAX(len[y], y-x+1);
    }
    for(int i=n-1; ~i; i--){
        UPDMAX(len[i], len[i+1]-1);
    }
    for(int i=0; i<=n; i++){
        foo[i]=0;
    }
...
int main(){
    for(int i=0; i<=16; i++)mask[i]=(1<<i)-1;
    int c;scanf("%d", &c);
    for(int i=0; i<c; i++){
        printf("Case #%d: %s\n", i+1, process());
        fflush(stdout);
