...
struct dot_t{
    int x, y, z;
    dot_t(const int lhs=0, const int rhs=0, const int idx=0){
        this->x=lhs;
        this->y=rhs;
        this->z=idx;
    }
    bool operator < (const dot_t &rhs) const{
        if(this->x!=rhs.x){
            return this->x<rhs.x;
...
lld process(){
    int v0, h0, _a, _b, _c, _d, _e, _f;
    scanf("%d%d%d%d%d%d%d%d%d%d", &n, &v0, &h0, &_a, &_b, &_c, &_d, &_e, &_f, &m);
    s[0]=dot_t(v0, h0, 0);
    for(int i=1; i<n; i++){
        s[i]=dot_t((_a*1LL*s[i-1].x+_b*1LL*s[i-1].y+_c*1LL)%m, (_d*1LL*s[i-1].x+_e*1LL*s[i-1].y+_f*1LL)%m, i);
    }
    for(int i=0; i<n; i++){
        s[i].x++;
        s[i].y++;
    ...
    for(int i=0, k=0; i<n; i++){
        ...
        g[s[i].z][2]=cnt(s[i].y-1);
        g[s[i].z][1]=cnt(m)-cnt(s[i].y);
    }
    memset(f, 0, sizeof(f));
    for(int i=n-1, k=n-1; ~i; i--){
        while(s[k].x>s[i].x){
            add(s[k].y);
            k--;
        }
        g[s[i].z][3]=cnt(s[i].y-1);
        g[s[i].z][0]=cnt(m)-cnt(s[i].y);
    }
    ans=n*1LL*(n-1)*(n-2)/6;
    for(int i=0; i<n; i++){
        ans-=g[i][0]*g[i][2];
