...
int MagicPlay(const int a, const int k) {
    if (a != 1) {
        return MOD_MUL(MOD_MUL(a, MOD_POW(a, k) - 1), INVERSEE(a - 1));
    } else {
        return k;
    }
}

int KickstartAlarm(const vector<int> &a, const int k) {
    const int n = a.size();
    int foo = 0;
    int prefix = 0;
    for (int i = 0; i < n; ++i) {
        MOD_ADD(&prefix, MagicPlay(i + 1, k));
        MOD_ADD(&foo, MOD_MUL(MOD_MUL(prefix, a[i]), n - i));
    }
    return MOD_STD(foo);
}
...
int main() {
    ...
    for (int i = 0; i < c; ++i) {
        ...
        vector<int> y(n, y1);
        for (int j = 1; j < n; ++ j) {
            x[j] = MOD_ADD(MOD_ADD(MOD_MUL(x[j - 1], C, F), MOD_MUL(y[j - 1], D, F)), E1, F);
            y[j] = MOD_ADD(MOD_ADD(MOD_MUL(x[j - 1], D, F), MOD_MUL(y[j - 1], C, F)), E2, F);
        }
        vector<int> a(n, 0);
        for (int j = 0; j < n; ++j) {
            a[j] = MOD_ADD(x[j], y[j], F);
        }
        cout << "Case #" << i + 1 << ": " << KickstartAlarm(a, k) << endl;
    }
    return 0;
}
