...
int compare(const Point &lhs, const Point &rhs){
    if(lhs.x != rhs.x){
        return lhs.x < rhs.x;
    }else{
        return lhs.y > rhs.y;
...
int main() {
    ...
    for (int case_id = 1; case_id <= case_num; ++case_id) {
        int n, k;
        scanf("%d%d", &n, &k);
        vector<Point> towers(n, Point());
        vector<Point> ballos(k, Point());
        int a, b, c, m;
        scanf("%d%d%d%d%d%d", &towers[0].x, &towers[1].x, &a, &b, &c, &m);
        for(int i = 2; i < n; ++i){
            towers[i].x = (towers[i - 1].x * 1LL * a + towers[i - 2].x * 1LL * b + c) % m + 1;
        }
        scanf("%d%d%d%d%d%d", &towers[0].y, &towers[1].y, &a, &b, &c, &m);
        for(int i = 2; i < n; ++i){
            towers[i].y = (towers[i - 1].y * 1LL * a + towers[i - 2].y * 1LL * b + c) % m + 1;
        }
        scanf("%d%d%d%d%d%d", &ballos[0].x, &ballos[1].x, &a, &b, &c, &m);
        for(int i = 2; i < k; ++i){
            ballos[i].x = (ballos[i - 1].x * 1LL * a + ballos[i - 2].x * 1LL * b + c) % m + 1;
        }
        scanf("%d%d%d%d%d%d", &ballos[0].y, &ballos[1].y, &a, &b, &c, &m);
        for(int i = 2; i < k; ++i){
            ballos[i].y = (ballos[i - 1].y * 1LL * a + ballos[i - 2].y * 1LL * b + c) % m + 1;
        }
        int foo = 0;
        //left to right
        sort(towers.begin(), towers.end(), compare);
        sort(ballos.begin(), ballos.end(), compare);
        for(int i = 0, j = 0, x = 0, y = 0; i < n || j < k; ){
            if(i < n && (j >= k || towers[i].x <= ballos[j].x)){
                y -= towers[i].x - x;
                x = towers[i].x;
                UPDMAX(y, towers[i].y);
                i++;
        ...
        //right to left
        for(int i = n - 1, j = k - 1, x = inf, y = 0; i >= 0 || j >= 0; ){
            if(i >= 0 && (j < 0 || towers[i].x >= ballos[j].x)){
                y -= x - towers[i].x;
                x = towers[i].x;
                UPDMAX(y, towers[i].y);
                //fprintf(stderr, "<- (%d, %d) x = %d, y = %d\n", towers[i].x, towers[i].y, x, y);
                i--;
            }else{
                y -= x - ballos[j].x;
                x = ballos[j].x;
