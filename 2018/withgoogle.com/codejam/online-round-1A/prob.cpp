...
int r, b, c;
lld m[1024], s[1024], p[1024];
lld h[1024];

int check(const lld cap){
    for(int i=0; i<c; i++){
        h[i]=MAX(MIN((cap-p[i])/s[i], m[i]), 0LL);
    }
    sort(h, h+c, greater<lld>());
    lld sum=0LL;
    for(int i=0; i<r; i++){
        sum+=h[i];
    }
    return (sum>=b*1LL)?1:0;
}

lld bsearch(){
    lld lef=1LL, rig=2000000000000000000LL;
    while(lef<=rig){
        lld mid=(lef+rig)>>1;
        if(check(mid)){
            if(mid==lef || !check(mid-1LL))return mid;
            rig=mid-1LL;
        }else{
            lef=mid+1LL;
        }
