...
struct position_t{
    int v, x, y;
    position_t(const int _v, const int _x, const int _y){
        this->v=_v;
        this->x=_x;
        this->y=_y;
    }
...
void process(){
    ...
    for(int x=1; x<14; x++){
        for(int y=1; y<14; y++){
            d[x][y]=9;
            s.insert(position_t(d[x][y], x, y));
        }
    }
    while(true){
        fprintf(stdout, "%d %d\n", s.begin()->x+1, s.begin()->y+1);
        fflush(stdout);
        int x, y;
        ...
        if(!b[x][y]){
            ...
            for(int i=0; i<9; i++){
                ...
                if(c<=0 || c>=14)continue;
                set<position_t>::iterator it=s.find(position_t(d[r][c], r, c));
                s.erase(it);
                d[r][c]--;
                if(d[r][c]){
                    s.insert(position_t(d[r][c], r, c));
                }
            }
        }
    }
