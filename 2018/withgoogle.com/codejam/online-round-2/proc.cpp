...
class bipartite_matching{
    ...
    int find(const pair<int, unordered_set<int>> &node, unordered_map<int, int> &partner, unordered_set<int> &visited){
        visited.insert(node.first);
        for(const int rightNodeIdx : node.second){
            unordered_map<int, int>::iterator it=partner.find(rightNodeIdx);
            if(it==partner.end() || (!visited.count(it->second) && this->find(*this->left.find(it->second), partner, visited))){
                partner[rightNodeIdx]=node.first;
                return 1;
            }
        }
        return 0;
    ...
    void add_edge(const int u, const int v){
        unordered_map<int, unordered_set<int>>::iterator it=this->left.find(u);
        if(it==this->left.end()){
            this->left.insert(make_pair(u, unordered_set<int>()));
            it=this->left.find(u);
    ...
    int count(){
        ...
        for(const pair<int, unordered_set<int>> &leftNode : this->left){
            visited.clear();
            if(this->find(leftNode, partner, visited))foo++;
        }
        return foo;
    }
};
