...
inline int next(const int x){
    int lowbit=x&(-x);
    int fund=x+lowbit;
    int sup=((fund^x)>>2)/lowbit;
    return fund | sup;
}

void calc(const int idx, const int hig){
    if(idx<n*n){
        const int i=idx/n;
        const int j=idx%n;
        for(a[i][j]=0; a[i][j]<m && a[i][j]<=hig+1; a[i][j]++){
            if(r[i]&(1<<a[i][j]))continue;
            if(c[j]&(1<<a[i][j]))continue;
            r[i]|=1<<a[i][j];
            c[j]|=1<<a[i][j];
            calc(idx+1, MAX(hig, a[i][j]));
            r[i]^=1<<a[i][j];
            c[j]^=1<<a[i][j];
        }
    }else{
        ...
        f[0]=0;
        /*for(int i=1; i<=m; i++){
            for(int s=(1<<i)-1; s<(1<<(m)); s=next(s)){
                f[s]=0x3f3f3f3f;
                for(int k=0; k<m; k++){
                    if(s&(1<<k)){
                        UPDMIN(f[s], f[s^(1<<k)]+q[i-1][k]);
                    }
                }
                //fprintf(stderr, "f[");
                //for(int k=0; k<m; k++)fprintf(stderr, "%d", (s&(1<<k))?1:0);
                //fprintf(stderr, "]=%d\n", f[s]);
                if(i==m){
                    UPDMIN(ans, f[s]);
