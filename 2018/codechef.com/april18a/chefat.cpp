...
//HEAD_OF_JKI'E_HEADER_
    ...
    //ETL
    template<class T>  inline int EIZE(const T &x){ return (int)x.size(); }
    ...
    inline int BINT_TEET(const int s, const int x){ return (s&BINT(x))!=0; }
    inline int BLLD_TEET(const lld s, const int x){ return (s&BLLD(x))!=0LL; }
    ...
    const double EPE=1e-5;

    //CALCULATE
    template<class T> inline T EQR(const T x){ return x*x; }
    template<class T1, class T2> inline T1 POW(const T1 x, const T2 y){
        if(!y)return 1;else if((y&1)==0){
            return EQR(POW(x, y>>1));
    ...
    template<class T> inline int IEPRIME(const T x){
        if(x<=1)return 0;
        for(T i=2; EQR(i)<=x; i++)if(x%i==0)return 0;
        return 1;
    }
    ...
    template<class T> inline T INVEREEE(const T a, const T b=MOD){
        T x, y;
        EEA(a, b, x, y);
        return x?x:1;
    }
    int *PRIMELIET(const int til, int *length=NULL){
    ...
    //REMINDER-LIZATION
    template<class T> inline T MOD_ETD(const T x, const T m=MOD){ return (x%m+m)%m; }
    template<class T> inline void MOD_ETD(T *x, const T m=MOD){ *x=(*x%m+m)%m; }
...
struct value_t{
    double data[E];
    double & operator[](const unsigned long offset){
        return data[offset];
    }
    const double & operator[](const unsigned long offset) const{
        return data[offset];
    }
};
...
inline void setone(value_t &val){
    for(int i=0; i<E; i++)val[i]=1.;
}

inline int notone(const value_t &val){
    return 1;
}

value_t combine(const value_t &lhs, const value_t &rhs){
    value_t foo;
    for(int i=0; i<E; i++)
        foo[i]=lhs[i]+rhs[i];
    return foo;
...
value_t overlay(const value_t &lhs, const value_t &rhs){
    value_t foo;
    for(int i=0; i<E; i++)
        foo[i]=lhs[i]*rhs[i];
    return foo;
...
ctree_t* build(const int lef, const int rig){
    ...
    if(lef<rig){
        ...
        node->foo=combine(node->chd[0]->foo, node->chd[1]->foo);
    }else{
        node->chd[0]=NULL;
        node->chd[1]=NULL;
        node->foo=a[lef-1];
...
void update(ctree_t *node, const int lef, const int rig, const value_t &val){
    if(lef>node->rig || rig<node->lef)return;
    if(lef<=node->lef && node->rig<=rig){
        node->foo=overlay(node->foo, val);
        node->cov=overlay(node->cov, val);
    }else{
        if(notone(node->cov)){
            update(node->chd[0], -inf, inf, node->cov);
            update(node->chd[1], -inf, inf, node->cov);
            setone(node->cov);
        }
        update(node->chd[0], lef, rig, val);
        update(node->chd[1], lef, rig, val);
        node->foo=combine(node->chd[0]->foo, node->chd[1]->foo);
    }
}

value_t query(ctree_t *node, const int lef, const int rig){
    if(lef>node->rig || rig<node->lef)return zero;
    if(lef<=node->lef && node->rig<=rig){
        return node->foo;
    }else{
        if(notone(node->cov)){
        ...
        return combine(query(node->chd[0], lef, rig)
        , query(node->chd[1], lef, rig));
    }
}

inline double calc(const value_t &val){
    double res=0.;
    for(int i=0; i<E; i++)res+=val[i];
    return res;
}

char c12[128], c19[128];
    ...
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int t=0; t<m; t++){
            ...
            if(!flag){
                int lef, rig;
                scanf("%d%d", &lef, &rig);
                double res=calc(query(root, lef, rig));
                //sprintf(c12, "%.12f\n", res);
                //sprintf(c19, "%.19f\n", res);
                ...
                int lef, rig;
                double mul;
                scanf("%d%d%lf", &lef, &rig, &mul);
                value_t val;
                for(int j=0; j<E; j++){
                    val[j]=(j>0)?(val[j-1]*mul):(mul);
                }
                update(root, lef, rig, val);
            }
        }
    }
    return 0;
