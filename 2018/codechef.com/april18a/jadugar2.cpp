...
inline int query(const int n, const int a, const int b, unordered_map<int, int> &cache){
    unordered_map<int, int>::iterator it=cache.find(n);
    if(it!=cache.end())return it->second;
    int foo=MOD_MUL(query(n-1, a, b, cache), a);
    int goo=0;
    for(int i=1; i<n; i++){
        MOD_ADD(&goo, MOD_MUL(query(i, a, b, cache), query(n-i, a, b, cache)));
    }
    MOD_ADD(&foo, MOD_MUL(goo, b));
    cache.insert(MP(n, foo));
    return foo;
}
...
int main(){
    ...
    while(scanf("%d%d%d%d", &n, &k, &a, &b)!=EOF){
        if(n<=5000){
            ...
            deque<pii> det;
            det.push_back(MP(1, 1));
            int cur=k;
            int sub=0;
            int neg=1;
            for(int i=2; i<=n; i++){
                //fprintf(stderr, "%d\n", MOD_STD(cur));
                //fprintf(stderr, "cur=%d sub=%d neg=%d\n", MOD_STD(cur), MOD_STD(sub), MOD_STD(neg));
                while(!det.empty() && det.front().first<=((i-1)>>1)){
                    neg=MOD_ADD(neg, -det.front().second);
                    det.pop_front();
                }
                int prv=cur;
                sub=MOD_ADD(MOD_ADD(sub, MOD_MUL(cur, cur)), -MOD_MUL(neg, neg));
                if((i&1)==0){
                    MOD_ADD(&sub, MOD_MUL(det.front().second, det.front().second));
                }
                cur=MOD_ADD(MOD_MUL(cur, a), MOD_MUL(sub, b));
                MOD_ADD(&neg, MOD_ADD(cur, -prv));
                det.push_back(MP(i, MOD_ADD(cur, -prv)));
            }
            MOD_STD(&cur);
            printf("%d\n", cur);
        }
