...
int p[1024];
int a[1024];
int nbs[1024], dst[1024], nxt[1024];
int f[1024], score, attempt;
int ru[1024], rv[1024];
int is_prime[20000001], prime[10000001];
...
int calc(const int u) {
    ...
    if (u != p[0]) {
        foo += is_prime[f[u]];
        foo += is_prime[sum - f[u]];
    }
    return foo;
}

void rec(const int u) {
    for (int i = nbs[u]; ~i; i = nxt[i]) {
        rec(dst[i]);
        ru[m] = u;
        rv[m] = dst[i];
...
int main() {
    ...
    for (int i = 2; i <= 20000000; ++i) is_prime[i] = 1;
    int len=0;
    for (int i = 2; i <= 20000000; ++i) {
        if (is_prime[i]) prime[len++] = i;
        for (int j = 0; j < len && prime[j] <= 20000000 / i; ++j) {
    ...
    while (scanf("%d", &n)!=EOF) {
        ...
        while (true) {
            clock_t tac = clock();
            if ((tac - tic) * 1.0 / CLOCKS_PER_SEC > 1.8) break;
            attempt++;
            for (int i = 0; i < n; ++i) {
                int pos = i + rand() % (n - i);
                swap(p[i], p[pos]);
        ...
        //fprintf(stderr, "attempt = %d, score = %d\n", attempt, score);
        for (int i = 0; i < m; ++i) {
            printf("%d %d\n", ru[i] + 1, rv[i] + 1);
        }
    }
