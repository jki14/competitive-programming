...
inline int bylef(const request_t &lhs, const request_t &rhs) {
    return lhs.lef > rhs.lef;
}

inline int byidx(const request_t &lhs, const request_t &rhs) {
    return lhs.idx < rhs.idx;
}

void solution(const vector<string> &a, vector<request_t> &r) {
    int n = a.size();
    int m = r.size();
    for (int i = 0; i < m; ++i) {
        r[i].idx = i;
    }
    sort(r.begin(), r.end(), bylef);
    tail = pool;
    tail->next[0] = nullptr;
    tail->next[1] = nullptr;
    tail->flag = n;
    root = tail++;
    for (int i = 0, b = n; i < m; ++i) {
        while (b > r[i].lef) {
            b--;
            node_t *node = root;
        ...
        node_t *node = root;
        for (const char c : r[i].bs) {
            int k = (c == '0') ? 1 : 0;
            if (node->next[k] != nullptr && node->next[k]->flag <= r[i].rig) {
                node = node->next[k];
            } else {
                node = node->next[k^1];
            }
...
int main() {
    ...
    vector<request_t> r(m, request_t());
    for (int i = 0; i < m; ++i) {
        cin >> r[i].lef >> r[i].rig >> r[i].bs;
        r[i].lef--;
        r[i].rig--;
    }
    solution(a, r);
