...
struct cont_t{
    int length;
    int tail;
    cont_t(const int lhs = 0, const int rhs = 0) {
        this->length = lhs;
        this->tail = rhs;
    }
    bool operator < (const cont_t &rhs) const{
    ...
    while (scanf("%d%d%d", &n, &q, &m)!=EOF) {
        ...
        for (int i = 0; i < q; ++i) {
            if (req[i] == '!') {
                ...
                if (a[head]) {
                    int index = find(head);
                    s.erase(cont_t(b[index], index));
                    b[index]--;
                    if (b[index]) {
                        s.insert(cont_t(b[index], index));
                    }
                    b[head] = 1;
                    f[head] = head;
                }
                if (a[tail] && a[head]) {
                    s.erase(cont_t(b[tail], tail));
                    b[head] += b[tail];
                    b[tail] = 0;
                    f[tail] = head;
                    s.insert(cont_t(b[head], head));
                } else if (a[head]) {
                    s.insert(cont_t(b[head], head));
                }
                tail = head;
            } else {
                int foo = s.begin()->length;
                printf("%d\n", MIN(foo, m));
            }
        }
    }
