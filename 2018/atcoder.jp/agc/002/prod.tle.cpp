...
int n, m;
int lef[100010], rig[100010];
int f[100010], c[100010], v[100010];
int ff[320][100010];
int fc[320][100010];
int pos[320];
...
inline int find(const int u, int *f, int *f0,int *c, int *c0, int *v, const int flag){
    if(v[u]!=flag){
        v[u]=flag;
        f[u]=f0[u];
        c[u]=c0[u];
    ...
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<m; i++){
            ...
            for(int j=0; ; j++){
                ...
                if(cnt>=z){
                    if(j>0){
                        for(int p=pos[j-1]+1; ; p++){
                            int g=find(lef[p], f, ff[j-1], c, fc[j-1], v, i);
                            int h=find(rig[p], f, ff[j-1], c, fc[j-1], v, i);
                            if(g!=h){
                                f[g]=h;
                                c[h]+=c[g];
                            }
                            int cnt=c[find(x, f, ff[j-1], c, fc[j-1], v, i)];
                            if(find(x, f, ff[j-1], c, fc[j-1], v, i)!=find(y, f, ff[j-1], c, fc[j-1], v, i)){
                                cnt+=c[find(y, f, ff[j-1], c, fc[j-1], v, i)];
                            }
                            if(cnt>=z){
                                printf("%d\n", p+1);
                                break;
