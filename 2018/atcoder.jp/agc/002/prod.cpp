...
struct task_t{
    int lef, rig, head, tail;
    task_t(const int argv0, const int argv1, const int argv2, const int argv3){
        this->lef=argv0;
        this->rig=argv1;
        this->head=argv2;
        this->tail=argv3;
...
void process(const int lef, const int rig, const int head, const int tail){
    if(head>tail)return;
    /*fprintf(stderr, "[%d, %d]:\n", lef+1, rig+1);
    for(int i=head; i<=tail; i++){
        fprintf(stderr, "%d(%d) ", r[i].p, r[i].c);
    }
    fprintf(stderr, "\n");*/
    if(lef==rig){
        for(int i=head; i<=tail; i++){
    ...
    while(last+1<=mid){
        last++;
        //fprintf(stderr, "link %d->%d (%d)\n", e[last].v, e[last].u, e[last].p+1);
        c[find(e[last].v)]+=c[find(e[last].u)];
        f[find(e[last].u)]=find(e[last].v);
    }
    for(int i=head; i<=tail; i++){
        int cnt=c[find(r[i].x)];
        if(find(r[i].y)!=find(r[i].x)){
            cnt+=c[find(r[i].y)];
        }
        r[i].c=(cnt>=r[i].z)?1:0;
    }
    ...
    int q=tail;
    while(true){
        while(p<=tail && r[p].c==1)p++;
        while(q>=head && r[q].c==0)q--;
        if(p<q){
            swap(r[p], r[q]);
        }else{
            break;
    ...
    tq.push(task_t(lef, mid, head, q));
    tq.push(task_t(mid+1, rig, p, tail));
}
...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        q=0;
        for(int i=0; i<m; i++){
            scanf("%d%d", &e[q].u, &e[q].v);
            e[q].u--;
            e[q].v--;
            if(find(e[q].u)!=find(e[q].v)){
                e[q].p=i;
                f[find(e[q].u)]=find(e[q].v);
                q++;
            }
        }
        m=q;
        ...
        tq.push(task_t(0, m-1, 0, q-1));
        last=m;
        for(; !tq.empty(); tq.pop()){
            process(tq.front().lef, tq.front().rig, tq.front().head, tq.front().tail);
        }
        for(int i=0; i<q; i++){
            printf("%d\n", ans[i]);
        }
