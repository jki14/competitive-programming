...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        for(int i=0; i<m; i++){
            ...
            int y=a[(i+1)%m];
            int z=a[(i-1+m)%m];
            //double a2=p[y].dist2(p[z]);
            //double b2=p[x].dist2(p[y]);
            //double c2=p[x].dist2(p[z]);
            //ans[p[x].idx]=PI-acos((b2+c2-a2)/(sqrt(b2)*sqrt(c2)*2.0));
            double ty=atan2(p[y].y-p[x].y, p[y].x-p[x].x);
            double tz=atan2(p[z].y-p[x].y, p[z].x-p[x].x);
            while(tz<ty-eps)tz+=2*PI;
            ans[p[x].idx]=PI-(tz-ty);
            sum+=ans[p[x].idx];
        }
        for(int i=0; i<n; i++){
            ans[i]/=sum;
