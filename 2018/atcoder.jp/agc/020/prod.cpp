...
inline int check(int a, int b, const int k, int p){
    if(p%(k+1)==k)p++;
    a-=(p+1)/(k+1)*k;
    a-=(p+1)%(k+1);
    b-=(p+1)/(k+1);
    return a>=0 && b>=0 && (b*1LL<=(a+1)*1LL*k);
}
...
int find(const int a, const int b, const int k){
    ...
    while(lef<=rig){
        ...
        if(!check(a, b, k, mid)){
            if(mid==lef || check(a, b, k, mid-1))return mid;
            rig=mid-1;
        }else{
            lef=mid+1;
        }
...
int main(){
    ...
    if(scanf("%d", &n)!=EOF){
        for(int k=0; k<n; k++){
            ...
            int cap=(MAX(a, b)+MIN(a, b))/(MIN(a, b)+1);
            int key=find(a, b, cap);
            //fprintf(stderr, "cap=%d, key=%d\n", cap, key);
            for(int i=c-1; i<d && i<key; i++){
                if(i%(cap+1)==cap){
                    putchar('B');
                }else{
                    putchar('A');
            ...
            b-=key/(cap+1);
            int sec=key+b-a*cap;
            //fprintf(stderr, "cap=%d, key=%d, sec=%d\n", cap, key, sec);
            for(int i=MAX(c-1, key); i<d && i<sec; i++){
                putchar('B');
            }
            for(int i=MAX(c-1, sec); i<d; i++){
                if((i-sec)%(cap+1)==0){
                    putchar('A');
                }else{
                    putchar('B');
