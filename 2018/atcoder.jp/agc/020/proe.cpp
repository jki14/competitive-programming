...
static const int MOD=998244353; //1000000000000000003LL
...
map<__int128, int> f[128];

char a[128];

inline __int128 submask(const __int128 mask, const int offset, const int len){
    return (mask>>offset) & ((((__int128)1)<<len)-((__int128)1));
}

int process(const int len, const __int128 val){
    map<__int128, int>::iterator it=f[len].find(val);
    if(it!=f[len].end())return it->second;
    int foo=MOD_MUL(process(len-1, val>>1), (int)(val&1)+1);
    for(int la=1; la+la<=len; la++){
        __int128 sw=submask(val, 0, la);
        for(int k=2; la*k<=len; k++){
            int pad=k*la;
            sw&=submask(val, pad-la, la);
            MOD_ADD(&foo, MOD_MUL(process(len-pad, val>>pad), process(la, sw)));
        }
    }
    f[len].insert(MP(val, foo));
    return foo;
}

int main(){
    while(scanf("%s", a)!=EOF){
        for(int i=0; i<128; i++)f[i].clear();
        f[0].insert(MP((__int128)0, 1));
        f[1].insert(MP((__int128)0, 1));
        f[1].insert(MP((__int128)1, 2));
        int len=0;
        __int128 val=0;
        for(int i=0; a[i]; i++){
            len++;
