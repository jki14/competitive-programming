...
struct pos_t {
    int x, y;
    pos_t(const int lhs = 0, const int rhs = 0) {
        this->x = lhs;
        this->y = rhs;
    }
};

void solution(const vector<pos_t> &e) {
    const int n = e.size();
    const int k = ((e[0].x ^ e[0].y) & 1);
    for (int i = 1; i < n; ++i) {
        if (k != ((e[i].x ^ e[i].y) & 1)) {
            printf("-1\n");
            return;
    ...
    vector<int> foo(31, 0);
    for (int i = 0; i < n; ++i) {
        int wd = e[i].x, hd = e[i].y;
        if (!k) wd--;
        for (int j = 30; j >= 0; --j) {
            int ds = 1 << j;
            for (int t = 0; t < 4; ++t) {
                if (llabs((wd - dx[t] * ds) * 1LL) + llabs((hd - dy[t] * ds) * 1LL) < (1 << j)) {
                    wd -= dx[t] * ds;
                    hd -= dy[t] * ds;
                    foo[j] = t;
                    break;
        ...
        if (!k) putchar(ori[0]);
        for (int j = 0; j <= 30; ++j) putchar(ori[foo[j]]);
        putchar('\n');
    }
}
