...
inline void add(int *a, const int p){
    for(int i=p; i<=m; i+=LOWBIT(i))a[i]++;
}

inline int cnt(int *a, const int p){
    int foo=0;
...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        f[0][0]=0;
        for(int i=1; i<=m; i++){
            for(int x=max(i-n, 0); x<=i && x<=n; x++){
                const int y=i-x;
                if(x>0){
                    //place c=0
                    int cost=cnt(b[0][x], p[0][x]);
                    cost+=cnt(b[1][y], p[0][x]);
                    //fprintf(stderr, "p[0][%d]=%d\n", x, p[0][x]);
                    /*for(int j=0; j<=m; j++){
                        fprintf(stderr, " %d", b[0][x][j]);
                    }
                    fprintf(stderr, "\n");
                    for(int j=0; j<=m; j++){
                        fprintf(stderr, " %d", b[1][y][j]);
                    }
                    fprintf(stderr, "\n");*/
                    //fprintf(stderr, "cost=%d\n", cost);
                    UPDMIN(f[i][x], f[i-1][x-1]+cost);
                }
                if(y>0){
                    //place c=1
                    int cost=cnt(b[1][y], p[1][y]);
                    cost+=cnt(b[0][x], p[1][y]);
                    UPDMIN(f[i][x], f[i-1][x]+cost);
                }
            }
        }
        printf("%d\n", f[m][n]);
