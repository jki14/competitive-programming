...
inline int test(const int x, const int y){
    for(int i=0; i<w; i++){
        if(a[x][i]!=a[y][b[i]])return 0;
        if(a[y][i]!=a[x][b[i]])return 0;
    }
    return 1;
}
...
int bmatch(const int remain, const int self){
    if(remain==0){
        assert(!self);
        return solve();
    }else if(pcnt[remain]==1){
        assert(self);
        b[pcnt[remain-1]]=pcnt[remain-1];
        return bmatch(0, 0);
    }else{
        int x=LOWBIT(remain);
        if(self){
            b[pcnt[x-1]]=pcnt[x-1];
            if(bmatch(remain^x, 0))return 1;
        }
        for(int bar=remain^x; bar; ){
            int y=LOWBIT(bar);
            bar^=y;
            b[pcnt[x-1]]=pcnt[y-1];
            b[pcnt[y-1]]=pcnt[x-1];
            if(bmatch(remain^x^y, self))return 1;
        }
    }
    return 0;
}
...
int main(){
    for(int i=0; i<16384; i++)pcnt[i]=__builtin_popcount(i);
