...
struct state_t{
    int x, y;
    lld dist;
    state_t(const int x, const int y, const lld dist){
        this->x = x;
        this->y = y;
        this->dist = dist;
    }
...
int main(){
    while(scanf("%d%d%d", &n, &m, &k)!=EOF){
        ...
        set<state_t> sq;
        sq.insert(start);
        f[start.x][start.y] = 0LL;
        for(; !sq.empty(); sq.erase(sq.begin())){
            const state_t &current = *sq.begin();
            //fprintf(stderr, "f[%d][%d] = %lld\n", current.x, current.y, current.dist);
            if(c[current.x][current.y] == 'G'){
                foo = current.dist;
            }
            const int p = current.dist % k; 
            for(int t = 0; t < 4; ++t){
                const int x = current.x + dx[t];
                const int y = current.y + dy[t];
                if(c[x][y] == '#')continue;
                if(b[p][t] == inf)continue;
                if(f[x][y] > current.dist + b[p][t]){
                    if(f[x][y] != inf){
                        sq.erase(state_t(x, y, f[x][y]));
                    }
                    f[x][y] = current.dist + b[p][t];
                    sq.insert(state_t(x, y, f[x][y]));
                }
            }
        }
        if(foo != inf){
