...
int calc(const int k, const int r){
    if(~f[k][r])return f[k][r];
    int &foo = f[k][r];
    if (k > 0) {
        int free = r + h[k];
        foo = 0;
        ...
        for(int i = 0; i * k <= free; ++i){
            MOD_ADD(&foo, MOD_MUL(calc(k - 1, free - i * k), selections));
            MOD_MUL(&selections, g[free - i * k][k]);
            MOD_MUL(&selections, INVERSEE(i + 1));
        }
    } else {
        foo = (r == 0) ? 1 : 0;
    }
...
int main(){
    for(int i = 0; i <= 1000; ++i){
        g[i][0] = g[i][i] = 1;
        for(int j = 1; j < i; ++j){
            g[i][j] = MOD_ADD(g[i - 1][j], g[i - 1][j - 1]);
        }
    }
    while(scanf("%d", &n)!=EOF){
        for(int i = 0; i < n; ++i){
