...
struct tail_t{
    int pos;
    tail_t *prv;
} tailpool[210000], *taillast;

tail_t *endas0[210000], *endas1[210000];
int cnt0, cnt1;
...
int process(){
    ...
    for(int i=0; a[i]; i++){
        if(a[i]=='0'){
            if(cnt1){
                taillast->pos=i;
                taillast->prv=endas1[--cnt1];
                endas0[cnt0++]=taillast++;
            }else{
                taillast->pos=i;
                taillast->prv=NULL;
                endas0[cnt0++]=taillast++;
            ...
            if(cnt0){
                taillast->pos=i;
                taillast->prv=endas0[--cnt0];
                endas1[cnt1++]=taillast++;
            }else{
                return 1;
            }
        }
...
int main(){
    while(scanf("%s", a)!=EOF){
        ...
        if(process()){
            ...
            stack<int> foo;
            while(cnt0){
                taillast=endas0[--cnt0];
                for(; taillast!=NULL; taillast=taillast->prv){
                    foo.push(taillast->pos+1);
                }
                printf("%d", SIZE(foo));
                for(; !foo.empty(); foo.pop()){
                    printf(" ");
                    printf("%d", foo.top());
                }
                printf("\n");
