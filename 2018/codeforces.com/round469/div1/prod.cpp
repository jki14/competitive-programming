...
int n, d;
lld b, a[110000], s[110000];

int check(const int len){
    if(len+len>=n)return 1;
    int lef=len+1;
    int rig=n-len;
    int t=len+1;
    while(lef<rig){
        int rb=MIN(lef*1LL+t*1LL*d, n*1LL);
        int lb=MAX(rig*1LL-t*1LL*d, 1LL);
        if(s[rb]-s[0]<(t-len)*1LL*b)return 0;
        if(s[n]-s[lb-1]<(t-len)*1LL*b)return 0;
        if(lb<=rb && s[n]<(t-len)*2LL*b)return 0;
        lef++;
        rig--;
        t++;
    }
    if(lef==rig){
        if(s[n]<(t-len)*2LL*b-b)return 0;
    }
    return 1;
}
