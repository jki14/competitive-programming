...
struct prob_t{
    int a, t, id;
    bool operator < (const prob_t &rhs) const{
        if(this->t!=rhs.t)return this->t<rhs.t;
        if(this->a!=rhs.a)return this->a<rhs.a;
        return this->id<rhs.id;
    }
} pb[210000], *pbs[210000], **last;

inline int cmp(const prob_t *lhs, const prob_t *rhs){
    return lhs->a<rhs->a;
}
...
void insert(node_t *node, const int pos){
    if(node->lef==node->rig){
        node->cnt+=1;
        node->sum+=pb[pos].t;
    }else{
        if(pos<=node->chd[0]->rig){
            insert(node->chd[0], pos);
        }else{
            insert(node->chd[1], pos);
        }
...
int cost(node_t *node, const int req){
    if(req<=0)return 0;
    if(req>node->cnt)return inf;
    if(req==node->cnt){
        return node->sum;
    }else{
        if(req>node->chd[0]->cnt){
            return cost(node->chd[0], node->chd[0]->cnt)+cost(node->chd[1], req-node->chd[0]->cnt);
        }else{
            return cost(node->chd[0], req);
        }
    }
}
...
int main(){
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        build(&root, 0, n-1);
        for(int k=n; ~k; k--){
            while(last>=pbs && (*last)->a>=k){
                insert(root, (int)((*last)-pb));
                last--;
            }
            if(cost(root, k)<=m){
                printf("%d\n%d\n", k, k);
