...
int check(const double threshold) {
    double low = a[0] * 1.0 - threshold;
    double hig = a[0] * 1.0 + threshold;
    for (int i = 1; i < n; ++i) {
        low = low + b[i - 1] * 1.0;
        hig = hig + c[i - 1] * 1.0;
        UPDMAX(low, a[i] * 1.0 - threshold);
        UPDMIN(hig, a[i] * 1.0 + threshold);
        if (low > hig + eps) return 0;
    }
    return 1;
}

double binary_search(double rig) {
    double lef = 0.0;
    while (lef < rig - eps) {
...
int main(){
    ...
    for (int caseId = 1; caseId <= caseNum; ++caseId) {
        ...
        scanf("%d%d", &n, &m);
        int w, x, y, z;
        scanf("%d%d%d%d%d%d", &a[0], &a[1], &w, &x, &y, &z);
        for (int i = 2; i < n; ++i) {
            a[i] = (w * 1LL * a[i - 2] + x * 1LL * a[i - 1] + y * 1LL) % z;
        }
        memset(b, 0xc0, sizeof(b));
        memset(c, 0x3f, sizeof(c));
        for (int k = 0; k < m; ++k) {
            int x, y, u, d;
            scanf("%d%d%d%d", &x, &y, &u, &d);
            if (x < y) {
        ...
        int cur = a[0];
        for (int i = 1; i < n; ++i) {
            int low = MAX(cur + b[i - 1], 0);
            int hig = cur + c[i - 1];
            cur = a[i];
            if (cur < low) {
                UPDMAX(rig, (low - cur) * 1.0);
                cur = low;
            } else if (cur > hig) {
                UPDMAX(rig, (cur - hig) * 1.0);
                cur = hig;
            }
        }
        printf("%.14f\n", binary_search(rig));
