...
void dfs(const int u, int &k, const int depth){
    r[u].first = k;
    d[u] = depth;
    a[k++] = u;
...
inline void update(stnode_t *node){
    if(node->chd[0]->val >= node->chd[1]->val){
        node->pos = node->chd[0]->pos;
        node->val = node->chd[0]->val;
    }else{
        node->pos = node->chd[1]->pos;
        node->val = node->chd[1]->val;
    }
}

stnode_t* build(const int lef, const int rig){
    stnode_t* node = last++;
    node->lef = lef;
    node->rig = rig;
    if(lef == rig){
...
pii getmax(stnode_t *node, const int lef, const int rig){
    if(lef <= node->lef && node->rig <= rig){
        return MP(node->val, node->pos);
    }else{
        pii foo(-1, 0);
        if(lef <= node->chd[0]->rig){
            pii tmp = getmax(node->chd[0], lef, rig);
            if(tmp.first > foo.first){
                foo = tmp;
            }
        }
        if(rig >= node->chd[1]->lef){
            pii tmp = getmax(node->chd[1], lef, rig);
            if(tmp.first > foo.first){
                foo = tmp;
            }
        }
...
void remove(stnode_t *node, const int pos){
...
int main(){
    ...
    for(int case_id = 1; case_id <= case_num; ++ case_id){
        ...
        sort(c, c + m, cmp);
        for(int i = 0; i <m; ++i){
            auto foo = getmax(root, r[c[i]].first, r[c[i]].second);
            if(foo.first > 0){
                ans += foo.first * 1LL;
                remove(root, foo.second);
            }
        }
        printf("Case #%d: %lld\n", case_id, ans);
        fflush(stdout);
