...
struct stnode_t{
    ...
    int lef, rig;
    stnode_t *chd[2];
} pool[110000], *root, *tail;

int n, a[51000], g[51000];
lld s[51000];
lld ans;
list<int> gkp, mkp;
...
void setcov(stnode_t *node, const int lef, const int rig, const lld cov){
    if(lef<=node->lef && node->rig<=rig){
        node->cov=cov;
        node->foo=node->val+cov;
    }else{
        if(node->cov!=inf){
            setcov(node->chd[0], node->lef, node->rig, node->cov);
            setcov(node->chd[1], node->lef, node->rig, node->cov);
            node->cov=inf;
        }
        if(lef<=node->chd[0]->rig)setcov(node->chd[0], lef, rig, cov);
        if(rig>=node->chd[1]->lef)setcov(node->chd[1], lef, rig, cov);
        node->foo=MIN(node->chd[0]->foo, node->chd[1]->foo);
    }
}

lld getmin(stnode_t *node, const int lef, const int rig){
    if(lef<=node->lef && node->rig<=rig){
        return node->foo;
    }else{
        if(node->cov!=inf){
        ...
        lld foo=inf;
        if(lef<=node->chd[0]->rig)UPDMIN(foo, getmin(node->chd[0], lef, rig));
        if(rig>=node->chd[1]->lef)UPDMIN(foo, getmin(node->chd[1], lef, rig));
        return foo;
    }
}

void process(const int p){
    for(const int rig : gkp){
        g[rig]=GCD(g[rig], a[p]); 
    }
    g[p+1]=abs(a[p]);
    gkp.push_back(p+1);
    for(list<int>::iterator it=gkp.begin(); it!=gkp.end(); ){
        if(next(it)!=gkp.end() && g[*it]==g[*next(it)]){
            it=gkp.erase(it);
        }else{
            it++;
        }
    ...
    int lef=p;
    while(!mkp.empty() && a[mkp.back()]<=a[p]){
        lef=mkp.back();
        mkp.pop_back();
    }
    a[lef]=a[p];
    mkp.push_back(lef);
    //fprintf(stderr, "setcov(root, %d, %d, %lld)\n", lef, p, a[p]*1LL);
    setcov(root, lef, p, a[p]*1LL);
    lld lhs=s[p+1];
    lef=0;
    for(const int rig : gkp){
        lld rhs=getmin(root, lef, rig-1);
        //fprintf(stderr, "[%d, %d] lhs=%lld rhs=%lld gcd=%d\n", lef, rig-1, lhs, rhs, g[rig]);
        UPDMAX(ans, (lhs-rhs)*g[rig]);
        lef=rig;
    }
}
