...
int slot(const int len, const int cnt){
    int foo=1;
    if(cnt){
        int var=(len>>1)+1;
        int fee=cnt>>1;
        /*for(int i=0; i<fee; i++){
            MOD_MUL(&foo, var+fee-1-i);
            MOD_MUL(&foo, INVERSEE(i+1));
        }*/
        MOD_MUL(&foo, p[var+fee-1]);
        MOD_MUL(&foo, INVERSEE(p[var-1]));
        MOD_MUL(&foo, INVERSEE(p[fee]));
    }
    return foo;
}
...
int main(){
    p[0]=1;
    for(int i=1; i<110000; i++)p[i]=MOD_MUL(p[i-1], i);
    while(scanf("%s", s+1)!=EOF){
        ...
        for(int k=0; k<n; k++){
            int l, r;
            scanf("%d%d", &l, &r);
            for(int i=0; i<26; i++)a[i]=c[r][i]-c[l-1][i];
            int len=0, foo=1, odd=0;
            for(int i=0; i<26; i++){
                if(a[i]&1){
                    //odd
                    MOD_MUL(&foo, slot(len, a[i]-1));
                    len+=a[i]-1;
                    odd++;
                }else{
                    //even
                    MOD_MUL(&foo, slot(len, a[i]));
                    len+=a[i];
                }
            }
            if(odd){
