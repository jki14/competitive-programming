...
/*struct keypoint_t{
    ...
    int lef, rig;
    keypoint_t(){
        this->que=(int*)malloc(sizeof(int)*512);
        this->clear();
    }
    ~keypoint_t(){
        free(this->que);
    ...
    int empty(){
        return this->lef==this->rig;
    }
    int front(){
        return this->que[this->lef];
    }
    int back(){
        return this->que[this->rig-1];
    }
    ...
    int upper_bound(const int x){
        ...
        while(lef<=rig){
            ...
            if(this->que[mid]>x){
                if(mid==lef || this->que[mid-1]<=x)return this->que[mid];
                rig=mid-1;
            }else{
                lef=mid+1;
            }
...
void process(){
    ...
    for(int i=0; i<=n+1; i++){
        sr[i][0]=0LL;
        for(int j=1; j<=m+1; j++){
            sr[i][j]=sr[i][j-1]+a[i][j]*1LL;
        }
    }
    for(int j=0; j<=m+1; j++){
        sc[0][j]=0LL;
        for(int i=1; i<=n+1; i++){
            sc[i][j]=sc[i-1][j]+a[i][j]*1LL;
        }
    }
    //process - part
    for(int x=1; x<=n; x++){
        ...
        for(int y=x; y<=n ;y++){
            ...
            for(int z=1; z<=m; z++){
                ...
                stc[z]+=sr[y][z];
                //update - ver
                while(!coq[z].empty() && y-coq[z].front()>k)coq[z].pop_front();
                while(!coq[z].empty() && sc[coq[z].back()][z]<=sc[y][z])coq[z].pop_back();
                coq[z].push_back(y);
                UPDMIN(ver[z], sc[y][z]-sc[coq[z].front()][z]);
                //upgrade
                if(stc[prv]+ver[fix]>=stc[alt]+ver[z]){
                    prv=alt;
                    fix=z;
                }
                if(stc[alt]>=stc[z]){
                ...
                if(x!=1 || y!=n || z!=m)
                    UPDMAX(ans, stc[z]-stc[prv]-ver[fix]);
            }
        }
    }
    //process - full
    for(int x=1; x<=1; x++){
        ...
        lld patch=inf;
        for(int z=1; z<=m; z++)UPDMIN(patch, ver[z]);
        lld lowest=stc[1];
        for(int z=2; z<=m; z++){
            UPDMIN(patch, lowest+MIN(ver[z], 0LL));
            UPDMIN(lowest, stc[z]);
        }
        UPDMAX(ans, stc[m]-patch);
    }
}
