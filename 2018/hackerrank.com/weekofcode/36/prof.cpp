...
struct node_t{
    ...
    node_t *left;
    node_t*prev[20];
} pool[2100000], *last[1100000], *tail;

const lld inf=0x3f3f3f3f3f3f3f3fLL;

int n;
int x[1100000], y[1100000], z[1100000];
int h[1100000], c[1100000], l[1100000];
lld f[1100000];
int g[1100000];

void pushback(node_t **hook, node_t *prev, const int h, const int c){
    node_t *node=*hook=tail++;
    //fprintf(stderr, "+(%d, %d) @ %d\n", h, c, (int)(node-pool));
    node->h=h;
    node->c=c;
    node->left=NULL;
    for(int i=0; i<20; i++)node->prev[i]=NULL;
    while(prev!=NULL && prev->c>=node->c)prev=prev->prev[0];
    while(prev!=NULL && prev->prev[0]!=NULL){
        node_t *bench=prev->prev[0];
        if((h-bench->h)*1LL*(c-prev->c)<=(h-prev->h)*1LL*(c-bench->c)){
            prev=bench;
        }else{
            break;
        }
    ...
    for(int i=0; prev!=NULL; i++){
        if(i==0){
            memcpy(tail, prev, sizeof(node_t));
            prev=tail++;
            prev->left=node;
        }
        node->prev[i]=prev;
...
inline int right(node_t *node, const int hx, const int cx){
    node_t *left=node->left;
    assert(left!=NULL);
    //X = (node->h - prev->h) / (node->c - prev->c)
    if((left->c-node->c)*1LL*cx>=(left->h-node->h)*1LL*hx){
        //fprintf(stderr, "%d/%d >= %d/%d (%d, %d)\n", cx, hx, left->h-node->h, left->c-node->c, node->h, node->c);
        return 1;
    }else{
        //fprintf(stderr, "%d/%d < %d/%d (%d, %d)\n", cx, hx, left->h-node->h, left->c-node->c, node->h, node->c);
        return 0;
...
pii find(node_t *node, const int hx, const int cx){
    //x-pos = cx/hx
    for(int i=19; ~i; i--){
        if(node->prev[i]==NULL)continue;
        if(right(node->prev[i], hx, cx)){
            node=node->prev[i];
        }
    }
    return MP(node->h, node->c);
}
...
int main(){
    if(scanf("%d", &n)!=EOF){
        ...
        c[1]=y[1];
        l[1]=z[1];
        pushback(&last[1], last[0], h[1], c[1]);
        f[1]=g[1]=0;
        //fprintf(stderr, "i=%d, h=%d, c=%d, l=%d, f=%lld, g=%d\n", 1, h[1], c[1], l[1], f[1], g[1]);
        for(int i=2; i<=n; i++){
            h[i]=x[i]^g[i-1];
            c[i]=y[i]^g[i-1];
            l[i]=z[i]^g[i-1];
            pushback(&last[i], last[i-1], h[i], c[i]);
            /*for(node_t *node=last[i]; node!=NULL; node=node->prev[0]){
                fprintf(stderr, "-(%d, %d)\n", node->h, node->c);
            }
            fprintf(stderr, "\n");*/
            f[i]=-inf;
            /*for(int j=1; j<=i-l[i]; j++){
                UPDMAX(f[i], h[j]*1LL*c[i]-h[i]*1LL*c[j]);
            }*/
            pii foo=find(last[i-l[i]], c[i], h[i]);
            //fprintf(stderr, "\n");
            /*if(f[i]>foo.first*1LL*c[i]-foo.second*1LL*h[i]){
                for(node_t *node=last[i-l[i]]; node!=NULL; node=node->prev[0]){
                    fprintf(stderr, "-(%d, %d)\n", node->h, node->c);
                }
                fprintf(stderr, "[%d, %d] ", c[i], h[i]);
                for(int j=1; j<=i-l[i]; j++){
                    if(f[i]==(h[j]*1LL*c[i]-h[i]*1LL*c[j])){
                        fprintf(stderr, "<%d, %d, %lld>", h[j], c[j], f[i]);
                    }
                }
                fprintf(stderr, " (%d, %d, %lld)\n", foo.first, foo.second, foo.first*1LL*c[i]-foo.second*1LL*h[i]);
                assert(0);
            }*/
            f[i]=foo.first*1LL*c[i]-foo.second*1LL*h[i];
            MOD_ADD(&fsum, (int)(f[i]%1000000007LL));
            MOD_STD(&fsum);
            g[i]=fsum;
            //fprintf(stderr, "i=%d, h=%d, c=%d, l=%d, f=%lld, g=%d\n", i, h[i], c[i], l[i], f[i], g[i]);
        }
        printf("%d\n", g[n]);
    }
    return 0;
