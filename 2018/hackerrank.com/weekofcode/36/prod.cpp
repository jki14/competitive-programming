...
struct stnode_t{
    stnode_t *chd[2];
    pair<lld, lld> lowcase, higcase;
    lld foo, cov;
    int lef, rig;
} pool[210000], *root, *tail;

int n, m;
int h[110000], p[110000];

map<int, int> hm;
int hr[110000];

inline pair<lld, lld> lowcase(const pair<lld, lld> lhs, const pair<lld, lld> rhs){
...
inline pair<lld, lld> higcase(const pair<lld, lld> lhs, const pair<lld, lld> rhs){
    if(rhs.second+rhs.first<lhs.second+lhs.first){
        return rhs;
    }else{
        return lhs;
    }
...
inline void update(stnode_t *node){
    node->lowcase=lowcase(node->chd[0]->lowcase, node->chd[1]->lowcase);
    node->higcase=higcase(node->chd[0]->higcase, node->chd[1]->higcase);
    node->foo=MIN(node->chd[0]->foo, node->chd[1]->foo);
}
...
void build(stnode_t **hook, const int lef, const int rig){
    ...
    node->cov=-inf;
    if(lef==rig){
        node->lowcase=MP(hr[lef]*1LL, inf);
        node->higcase=MP(hr[lef]*1LL, inf);
        node->foo=inf;
    }else{
...
void cover(stnode_t *node, const int lef, const int rig, const lld val){
    if(node==root){
        //fprintf(stderr, "cover %d->%d %lld\n", lef, rig, val);
    }
    if(lef<=node->lef && node->rig<=rig){
        node->lowcase=(MP(hr[node->rig]*1LL, val));
        node->higcase=(MP(hr[node->lef]*1LL, val));
        node->foo=val;
        node->cov=val;
    }else{
        if(node->cov!=-inf){
            //fprintf(stderr, "zerg\n");
            cover(node->chd[0], node->lef, node->rig, node->cov);
            cover(node->chd[1], node->lef, node->rig, node->cov);
            node->cov=-inf;
        }
        if(lef<=node->chd[0]->rig){
            cover(node->chd[0], lef, rig, val);
        }
        if(rig>=node->chd[1]->lef){
            cover(node->chd[1], lef, rig, val);
        }
        update(node);
    }
}

pair<lld, lld> lowqr(stnode_t *node, const int pos){
    if(node->rig<=pos){
        return node->lowcase;
    }else{
        if(node->cov!=-inf){
            return MP(pos, node->cov);
        }
        if(pos<=node->chd[0]->rig){
            return lowqr(node->chd[0], pos);
        }else{
            return lowcase(node->chd[0]->lowcase, lowqr(node->chd[1], pos));
        }
    }
}

pair<lld, lld> higqr(stnode_t *node, const int pos){
    if(node->lef>=pos){
        return node->higcase;
    }else{
        if(node->cov!=-inf){
        ...
        if(pos>=node->chd[1]->lef){
            return higqr(node->chd[1], pos);
        }else{
            return higcase(node->chd[1]->higcase, higqr(node->chd[0], pos));
        }
    }
}

lld query(stnode_t *node, const int pos){
    if(node->lef==node->rig){
        ...
        if(pos<=node->chd[0]->rig){
            return query(node->chd[0], pos);
        }else{
            return query(node->chd[1], pos);
        }
    }
}

void draw(stnode_t *node){
    if(node->lef!=node->rig){
       draw(node->chd[0]);
       draw(node->chd[1]);
    }
    fprintf(stderr, "[%d, %d](%lld), ", node->lef, node->rig, node->foo);
    if(node==root){
        fprintf(stderr, "\n");
    }
}
...
int main(){
    while(scanf("%d", &n)!=EOF){
        ...
        for(map<int, int>::iterator it=hm.begin(); it!=hm.end(); it++){
            hr[it->second=++m]=it->first;
            //fprintf(stderr, "hm[%d]=%d, hr[%d]=%d\n", it->first, it->second, it->second, hr[it->second]);
        }
        for(int i=0; i<n-1; i++){
            scanf("%d", &p[i]);
        }
        ...
        build(&root, 1, m);
        //draw(root);
        cover(root, hm[h[n]], hm[h[n]], 0LL);
        //draw(root);
        for(int i=0; i<n-1; i++){
            int offset=hm[h[i]];
            lld value=inf;
            pair<lld, lld> pll;
            pll=lowqr(root, offset);
            if(pll.second<inf){
                UPDMIN(value, pll.second+h[i]*1LL-pll.first+p[i]*1LL);
            }
            pll=higqr(root, offset);
            if(pll.second<inf){
                UPDMIN(value, pll.second+pll.first-h[i]*1LL+p[i]*1LL);
            }
            if(offset>1){
                cover(root, 1, offset-1, inf);
            }
            lld prev=query(root, offset);
            //fprintf(stderr, "at %d: offset=%d value=%lld prev=%lld\n", i, offset, value, prev);
            if(value<prev){
                cover(root, offset, offset, value);
            }
        }
        printf("%lld\n", root->foo+n*1LL);
    }
