...
//[HEAD_OF_JKI'S_HEADER_
    ...
    template<class T> class maxflow_c{
        ...
        void clean(){
            for(mf_edge_t *edge=this->edge_pool; edge!=this->edge_tail; edge++){
                edge->cur=0;
            }
        }
    ...
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=0; i<m; i++){
            ...
            if(k==2){
                ...
                u--;
                v--;
                maxflow.add_edge(u, v, f*2LL);
                maxflow.add_edge(v, u, f*2LL);
            }else{
                int v[3];
                ...
                for(int i=0; i<3; i++){
                    for(int j=0; j<3; j++){
                        if(i==j)continue;
                        maxflow.add_edge(v[i], v[j], f*1LL);
                    }
                }
            }
        }
        ...
        for(int u=0; u<n; u++){
            for(int v=u+1; v<n; v++){
                maxflow.clean();
                UPDMAX(ans, foo-(maxflow.isap(u, v)>>1));
            }
        }
        printf("%lld\n", ans);
    }
