...
class treap_t{
    private:
    treap_node_t *root;
    void rotate(treap_node_t **hook, const int rig){
        treap_node_t *parent=*hook;
        treap_node_t *node=parent->chd[rig^1];
        node->sup=parent->sup;
        parent->chd[rig^1]=node->chd[rig];
        parent->cnt=1;
        if(parent->chd[0]!=NULL)parent->cnt+=parent->chd[0]->cnt;
        if(parent->chd[1]!=NULL)parent->cnt+=parent->chd[1]->cnt;
        parent->sup=node;
        node->chd[rig]=parent;
        node->cnt=1;
        if(node->chd[0]!=NULL)node->cnt+=node->chd[0]->cnt;
        if(node->chd[1]!=NULL)node->cnt+=node->chd[1]->cnt;
        *hook=node;
    }
    void insert(treap_node_t **hook, treap_node_t *parent, const int key){
        if(*hook!=NULL){
            treap_node_t *node=*hook;
            //modify
            node->cnt++;
            if(node->key==key){
                //do no-thing
                ...
                if(key<node->key){
                    this->insert(&node->chd[0], node, key);
                }else{
                    this->insert(&node->chd[1], node, key);
                }
            }
        }else{
            //creat
            treap_node_t *node=tail++;
            node->key=key;
            node->wht=rand();
            node->cnt=1;
            node->sup=parent;
            ...
            *hook=node;
            //adjust
            while(true){
                treap_node_t *p=node->sup;
                if(p==NULL || node->wht<=p->wht)break;
                treap_node_t *s=p->sup;
                if(s!=NULL){
                    for(int j=0; j<2; j++){
                        if(s->chd[j]==p){
                            for(int i=0; i<2; i++){
                                if(p->chd[i]==node){
                                    this->rotate(&s->chd[j], i^1);
                                    break;
                                }
                            }
                            break;
    ...
    int count(treap_node_t *node, const int cap){
        if(node==NULL)return 0;
        //fprintf(stderr, "key=%d, wht=%d, cnt=%d\n", node->key, node->wht, node->cnt);
        //fprintf(stderr, "cap=%d\n", cap);
        if(cap<node->key){
            return this->count(node->chd[0], cap);
        }else{
            int buf=1;
            if(node->chd[0]!=NULL)buf+=node->chd[0]->cnt;
            return this->count(node->chd[1], cap)+buf;
        }
    }
    public:
    treap_t(){
    ...
    void insert(const int key){
        this->insert(&this->root, NULL, key);
    }
    int count(const int cap){
        //this->draw(this->root);
        //fprintf(stderr, "count %d = %d\n", cap, this->count(this->root, cap));
        return this->count(this->root, cap);
    }
    void draw(treap_node_t *node){
        if(node==NULL)return;
        this->draw(node->chd[0]);
        fprintf(stderr, "key=%d, wht=%d, cnt=%d\n", node->key, node->wht, node->cnt);
        this->draw(node->chd[1]);
    }
};*/

class solver_t{
    private:
    //vector<vector<treap_t>> low;
    vector<vector<int>> low;
    vector<int> hig;
    public:
    solver_t(){
        low.resize(NANA);
        for(int x=0; x<NANA; x++){
            low[x].resize(x+1);
        }
        hig.resize(100001);
        this->clear();
    ...
    void putlow(const int k, const int b){
        int x=k-1;
        int y=b%k;
        //low[x][y].insert(b);
        low[x][y]++;
    }
    void puthig(const int k, const int b){
        for(int i=b; i<=100000; i+=k){
            hig[i]++;
        }
    ...
    void put(const int k, const int b){
        if(k<=NANA){
            this->putlow(k, b);
        }else{
            this->puthig(k, b);
