...
int main() {
  ...
  while (cin >> n >> m) {
    ...
    /*
    z = 1;
    pq.push(make_pair(-z, z));
    ++z;
    while (true) {
      lld const p = -pq.top().first;
      lld const lhs = pq.top().second;
      lld const rhs = p / lhs;
      pq.pop();
      if (p >= m) {
        cout << p << endl;
      ...
      if (z <= n && lhs * (rhs + 1) > z * z) {
        pq.push(make_pair(-z * z, z));
        ++z;
      }
      if (rhs + 1 > n) continue;
      pq.push(make_pair(-lhs * (rhs + 1), lhs));
    }
    */
    z = static_cast<lld>(sqrt(m)) + 1;
    z = z * z;
    for (lld i = 1LL; i <= n && i * i <= m; ++i) {
      lld const rhs = (m + i - 1) / i;
      if (rhs <= n) {
        z = min(i * rhs, z);
      }
    }
